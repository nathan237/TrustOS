...
- **ðŸ¦€ 100% Rust, `no_std`** â€” zero C, zero binary blobs, `unsafe` only at MMIO boundaries.
- **ðŸ”¬ Hardware X-Ray** â€” every PCI device, MSR, ACPI table, GPU register readable at boot. No Linux, no Mesa, no abstraction.
- **ðŸŒ Remote-first** â€” framebuffer streamed over UDP, shell over UDP, kernel logs over UDP. Debug headless boards from any laptop.
- **ðŸ› ï¸ Built solo** â€” one developer, one vision, no committee.
...
## Highlight reel â€” v0.11.0 (April 2026)
...
From-scratch AMD driver in pure `no_std` Rust on an RX 580X (Polaris 10). Ring buffer in GART, firmware responsive, RPTR/WPTR advancing. Root cause of the 14-iteration debug saga: the **Graphics Memory Controller** (L1/L2 TLB, system aperture, VM flat mode) was uninitialized â€” the SDMA firmware couldn't resolve ring addresses. Not the registers, not the PCIe link â€” the memory subsystem.

â†’ Full devlog: [`docs/devlog/gpu_amd_sdma_milestone.md`](docs/devlog/gpu_amd_sdma_milestone.md)

### Other wins this cycle

- **Intel HDA audio** â€” complete `no_std` driver, working sound on ThinkPad T61 / AD1984 codec
- **Ring 3 userland** â€” protected user-mode processes, 85-check conformance audit
- **Hardware diagnostics** â€” 6 new modules: `pciraw`, `regdiff`, `ioscan`, `regwatch`, `aer`, `timing`
- **ThinkPad EC** â€” fan/thermal/battery readout + CPU frequency scaling via MSR
- **CoreMark** â€” 25,000 iter/sec on bare metal Intel G4400 ([`docs/BENCHMARK.md`](docs/BENCHMARK.md))
- **Security audit** â€” preemptive cross-OS vulnerability sweep

â†’ Full history: [`CHANGELOG.md`](CHANGELOG.md)
...
| Project | Lang | `no_std` kernel | Bare-metal GPU bring-up | Remote UDP shell | Solo dev |
|---|---|---|---|---|---|
| **TrustOS** | Rust | âœ… | âœ… AMD Polaris from scratch | âœ… | âœ… |
| Redox OS | Rust | partial (microkernel) | âŒ (uses Linux drivers via patch) | âŒ | âŒ (team) |
| Theseus | Rust | âœ… | âŒ | âŒ | research |
| SerenityOS | C++ | âŒ | partial | âŒ | âŒ (team) |
| Hubris | Rust | âœ… | embedded only | âŒ | âŒ (Oxide) |
| seL4 | C | âœ… | âŒ | âŒ | âŒ |
...
| **CPU** | All CPUID leaves, MSRs, family/model/stepping, AVX/SSE/AES feature flags |
| **SMBIOS / DMI** | Board, BIOS, DIMM (size/speed/type), chassis, serials |
| **ACPI** | RSDP â†’ XSDT walk, MADT/FADT/HPET/MCFG/SSDT, raw hex |
| **AMD GPU** | MMIO, GRBM, SDMA state, GMC/VM, VRAM, doorbells |
| **NVIDIA GPU** | PMC/PBUS regs, GPU identity |
| **Intel HDA** | Codec tree, widgets, amp caps, pin config |
| **ThinkPad EC** | Fans, thermals, battery, EC regs |
| **Memory map** | Full UEFI map: type, range, page count |
| **Boot timing** | TSC per-subsystem checkpoints (Âµs precision) |
| **Network / Storage** | NIC enum + MAC, NVMe/AHCI BAR decode |
...
- **UDP 7779** â€” framebuffer screencap (chunked SCRN protocol)
- **UDP 7777** â€” remote shell
- **UDP 6666** â€” netconsole (kernel debug output)
...
### USB
1. Grab an ISO from [**Releases**](https://github.com/nathan237/TrustOS/releases).
2. Flash with [**Rufus**](https://rufus.ie/) â€” **DD Image mode**.
3. Boot via F12 / DEL. Works on UEFI **and** Legacy BIOS.
...
```
TrustOS/
â”œâ”€â”€ kernel/src/        the kernel (drivers, netstack, hwdiag, shell, â€¦)
â”œâ”€â”€ userland/          Ring 3 processes + syscall interface
â”œâ”€â”€ boot/              Limine boot helpers
â”œâ”€â”€ crates/            shared crates
â”œâ”€â”€ sdk/               developer SDK
â”œâ”€â”€ tools/             build helpers, source translator
â”œâ”€â”€ scripts/           remote_screen.py, pxe_server.py, â€¦
â”œâ”€â”€ docs/              architecture, devlogs, benchmarks, images
â”œâ”€â”€ firmware/          OVMF
â””â”€â”€ limine/            bootloader (submodule)
```
...
Patches welcome. Read [`CONTRIBUTING.md`](CONTRIBUTING.md) first â€” kernel rules are strict (`no unwrap()`, `no println!`, `no_std` everywhere).

## Author

Built and maintained by **[Nathan](https://github.com/nathan237)** â€” solo.
If TrustOS is useful or interesting to you, [become a sponsor](https://github.com/sponsors/nathan237).
