...
def main():
    ...
    parser.add_argument("--payloads", default=None,
                        help="Path to payload directory (default: test_payloads/)")
    ...
    # Payload directory (overridable)
    global PAYLOAD_DIR, RESULTS_FILE
    payload_dir = Path(args.payloads) if args.payloads else PAYLOAD_DIR
    PAYLOAD_DIR = payload_dir
    RESULTS_FILE = payload_dir.parent / f"dfu_test_results_{payload_dir.name}.json"
    
    # Load manifest
    manifest_path = payload_dir / "test_manifest.json"
    if not manifest_path.exists():
        log(f"ERROR: test_manifest.json not found in {payload_dir}.", "ERROR")
        return
    
    manifest = json.loads(manifest_path.read_text())
    all_cases = manifest["cases"]
    log(f"Loaded {len(all_cases)} test cases from {payload_dir.name}")
