...
fn generate_click() -> Vec<i16> {
    crate::drivers::hda::generate_sine(CLICK_FREQ_HZ, CLICK_DURATION_MS, 20000)
}
...
fn play_click() {
    ...
    if !click.is_empty() {
        ...
        let _ = crate::drivers::hda::write_samples_and_play(&click, CLICK_DURATION_MS + 5);
    }
}
...
fn finish_session() {
    ...
    crate::serial_println!("[SYNC]   Std deviation:    {} ms", stddev_ms);
    crate::serial_println!("[SYNC]   Human reaction:  -{} ms (subtracted)", human_reaction_us / 1000);
    crate::serial_println!("[SYNC]   → Computed A/V offset: {} ms", offset_ms);
