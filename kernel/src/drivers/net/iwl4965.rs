...
impl RxQueue {
    fn new() -> Self {
        ...
        for _ in 0..RX_QUEUE_SIZE {
            ...
            let phys = crate::memory::virt_to_phys(virt)
                .unwrap_or(virt.wrapping_sub(crate::memory::hhdm_offset()));
...
impl Iwl4965 {
        ...
        let fw_data = {
            ...
            match guard.as_ref() {
                ...
                None => {
                    ...
                    match crate::ramfs::with_fs(|fs| fs.read_file("/firmware/iwlwifi-4965-2.ucode").map(|d| d.to_vec())) {
                        Ok(d) => d,
                ...
                Ok(()) => {
                    ...
                    for i in 0..50u32 {
                        ...
                        if i == 25 {
                            ...
                            crate::serial_println!("[IWL4965] Init cal progress: INT={:#X} rxpkts so far", int_val);
                        }
                    }
    ...
    fn bsm_set_dram_addrs(&self, inst: &[u8], data: &[u8]) -> Result<(), &'static str> {
        ...
        if !inst.is_empty() {
            ...
            let phys = crate::memory::virt_to_phys(virt).unwrap_or(virt.wrapping_sub(hhdm));
    ...
    fn init_queues(&mut self) -> Result<(), &'static str> {
        ...
        let stts_phys = crate::memory::virt_to_phys(stts_virt)
    ...
    fn send_hcmd(&mut self, cmd_id: u8, data: &[u8]) -> Result<(), &'static str> {
        ...
        let buf_phys = crate::memory::virt_to_phys(buf_virt)
            .unwrap_or(buf_virt.wrapping_sub(crate::memory::hhdm_offset()));
    ...
    fn poll_rx(&mut self) {
        ...
        crate::serial_println!("[IWL4965] poll_rx: hw_write={} read={} — new packets!", hw_write, read);
...
pub fn debug_read_csr(offset: u32) -> Option<u32> {
    ...
    for dev in &devices {
        if dev.vendor_id == INTEL_VENDOR && IWL_SUPPORTED_IDS.contains(&dev.device_id) {
            ...
            let virt = match crate::memory::map_mmio(phys, 0x2000) {
                Ok(v) => v as usize,
                Err(_) => return None,
            };
            unsafe {
...
pub fn debug_load_firmware() -> Result<(), &'static str> {
    ...
    match driver.start() {
        Ok(()) => {
            ...
            crate::println!("    MAC_CLK={}", if gp & CSR_GP_CNTRL_REG_FLAG_MAC_CLOCK_READY != 0 { "ready" } else { "DEAD" });
            Ok(())
        }
        Err(e) => {
            crate::println!();
