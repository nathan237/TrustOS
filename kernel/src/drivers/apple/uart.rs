...
// UCON bits  
const UCON_RX_IRQ: u32     = 1 << 0;    // RX interrupt or polling mode
const UCON_TX_IRQ: u32     = 1 << 2;    // TX interrupt or polling mode
const UCON_RX_ERR_IRQ: u32 = 1 << 6;    // RX error interrupt
const UCON_RX_TIMEOUT: u32 = 1 << 7;    // RX timeout enable
const UCON_TXMODE_IRQ: u32 = 0x01 << 2; // TX mode: interrupt
const UCON_RXMODE_IRQ: u32 = 0x01 << 0; // RX mode: interrupt

// UFCON bits
const UFCON_FIFO_EN: u32   = 1 << 0;    // Enable FIFO
const UFCON_RX_RESET: u32  = 1 << 1;    // Reset RX FIFO
const UFCON_TX_RESET: u32  = 1 << 2;    // Reset TX FIFO
// RX trigger level: 1/4 FIFO
const UFCON_RX_TRIG_8: u32 = 0x01 << 4;
// TX trigger level: empty
const UFCON_TX_TRIG_0: u32 = 0x00 << 6;

// UTRSTAT bits
const UTRSTAT_RX_READY: u32    = 1 << 0;  // RX buffer has data
const UTRSTAT_TX_EMPTY: u32    = 1 << 1;  // TX buffer empty
const UTRSTAT_TX_DONE: u32     = 1 << 2;  // TX shifter empty (fully sent)

// UFSTAT bits (Apple-specific FIFO depths may differ)
const UFSTAT_TX_FULL: u32  = 1 << 14;     // TX FIFO full
const UFSTAT_RX_FULL: u32  = 1 << 6;      // RX FIFO full (varies by HW)
...
static ANY_UART_INIT: AtomicBool = AtomicBool::new(false);
...
pub unsafe fn init(index: u8, base: u64, ref_clock: u32, baud: u32) -> Result<(), &'static str> {
    ...
    // Reset FIFOs
    uart_write32(base, UFCON, UFCON_FIFO_EN | UFCON_RX_RESET | UFCON_TX_RESET);
    
    // Wait for FIFO reset to complete (few cycles)
    for _ in 0..100 {
        let stat = uart_read32(base, UFCON);
        if stat & (UFCON_RX_RESET | UFCON_TX_RESET) == 0 {
            break;
        }
    }
    ...
    uart_write32(base, UCON, 
        UCON_RXMODE_IRQ | UCON_TXMODE_IRQ | UCON_RX_ERR_IRQ | UCON_RX_TIMEOUT);
    
    // Configure FIFO: enable, RX trigger at 8 bytes, TX trigger at empty
    uart_write32(base, UFCON, UFCON_FIFO_EN | UFCON_RX_TRIG_8 | UFCON_TX_TRIG_0);
    ...
    let ubrdiv_val = (divisor_16 / 16) - 1;
    let ufrac_val = divisor_16 - (ubrdiv_val + 1) * 16;
    
    uart_write32(base, UBRDIV, ubrdiv_val);
    uart_write32(base, UFRACVAL, ufrac_val);
    
    crate::serial_println!("[APPLE-UART] UART{}: UBRDIV={}, UFRAC={}", 
        index, ubrdiv_val, ufrac_val);
    ...
    ANY_UART_INIT.store(true, Ordering::SeqCst);
...
pub fn write_byte(index: u8, byte: u8) {
    ...
    let uart = match guard[index as usize].as_mut() {
        Some(u) => u,
        None => return,
    };
    ...
    unsafe {
        ...
        while uart_read32(uart.base, UFSTAT) & UFSTAT_TX_FULL != 0 {
            timeout -= 1;
            if timeout == 0 {
...
pub fn tx_done(index: u8) -> bool {
    let guard = UARTS.lock();
    match guard[index as usize].as_ref() {
        Some(uart) => unsafe {
            uart_read32(uart.base, UTRSTAT) & UTRSTAT_TX_DONE != 0
        },
        None => true,
    }
}
...
pub fn is_initialized() -> bool {
    ANY_UART_INIT.load(Ordering::SeqCst)
}
