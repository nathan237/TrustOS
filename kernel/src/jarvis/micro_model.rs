...
pub const MICRO_VOCAB: usize = 256;    // byte-level (same as full)
pub const MICRO_D_MODEL: usize = 64;
pub const MICRO_N_HEADS: usize = 2;
pub const MICRO_D_K: usize = MICRO_D_MODEL / MICRO_N_HEADS; // 32
pub const MICRO_D_FF: usize = 128;     // 2× d_model
pub const MICRO_N_LAYERS: usize = 1;
pub const MICRO_MAX_SEQ: usize = 64;
pub const MICRO_RMS_EPS: f32 = 1e-5;
...
impl MicroWeights {
    pub fn param_count(&self) -> usize {
        ...
        + MICRO_MAX_SEQ * MICRO_D_MODEL          // pos_embed
        + MICRO_N_LAYERS * (
    ...
    pub fn deserialize(data: &[f32]) -> Option<Self> {
        ...
        let pos_embed = slice_vec(data, &mut pos, MICRO_MAX_SEQ * MICRO_D_MODEL)?;
    ...
    pub fn new_random() -> Self {
        ...
        MicroWeights {
            ...
            pos_embed: random_vec(MICRO_MAX_SEQ * MICRO_D_MODEL, 0.02, &mut seed),
            layers,
...
impl MicroEngine {
    pub fn new() -> Self {
        MicroEngine {
            cache_k: (0..MICRO_N_LAYERS).map(|_| Vec::with_capacity(MICRO_MAX_SEQ * MICRO_D_MODEL)).collect(),
            cache_v: (0..MICRO_N_LAYERS).map(|_| Vec::with_capacity(MICRO_MAX_SEQ * MICRO_D_MODEL)).collect(),
            cache_len: 0,
            ...
            buf_attn: vec![0.0; MICRO_MAX_SEQ],
    ...
    pub fn generate(&mut self, model: &MicroWeights, prompt: &[u8], max_tokens: usize) -> Vec<u8> {
        self.clear_cache();
        let max = max_tokens.min(MICRO_MAX_SEQ);
        let mut output = Vec::with_capacity(max);

        for &token in prompt.iter().take(MICRO_MAX_SEQ - 1) {
    ...
    pub fn classify(&mut self, model: &MicroWeights, input: &[u8]) -> u8 {
        self.clear_cache();
        for &token in input.iter().take(MICRO_MAX_SEQ) {
            self.forward_one(model, token);
        }
    ...
    pub fn compute_loss(&mut self, model: &MicroWeights, tokens: &[u8]) -> f32 {
        self.clear_cache();
        let seq_len = tokens.len().min(MICRO_MAX_SEQ);
    ...
    fn forward_one(&mut self, model: &MicroWeights, token: u8) {
        ...
        for l in 0..MICRO_N_LAYERS {
            ...
            for h in 0..MICRO_N_HEADS {
                ...
                softmax(&mut self.buf_attn[..n_pos]);
                for t in 0..n_pos {
...
fn check_fs() -> bool {
    crate::ramfs::with_fs(|fs| fs.exists("/"))
}
...
pub fn check_full_brain_file() -> bool {
    crate::ramfs::with_fs(|fs| fs.exists("/jarvis/weights.bin"))
}
