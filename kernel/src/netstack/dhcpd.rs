...
static LEASES: Mutex<[Lease; MAX_LEASES]> = Mutex::new([Lease::empty(); MAX_LEASES]);
...
mod option {
    ...
    pub const PXE_VENDOR_CLASS: u8 = 60;
...
fn find_or_allocate_ip(mac: &[u8; 6]) -> Option<[u8; 4]> {
    ...
    for offset in 0..pool_size.min(MAX_LEASES) {
        let candidate_ip = [
            cfg.pool_base[0],
            cfg.pool_base[1],
            cfg.pool_base[2],
...
fn send_response(response_type: u8, xid: &[u8; 4], client_mac: &[u8; 6], client_ip: &[u8; 4], pxe: bool) {
    ...
    packet.extend_from_slice(&[option::MSG_TYPE, 1, response_type]);
    ...
    let response_name = match response_type {
