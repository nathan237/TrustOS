...
/// TFTP opcodes
mod opcode {
    pub const RRQ: u16 = 1;   // Read request
    pub const WRQ: u16 = 2;   // Write request (we don't support)
    pub const DATA: u16 = 3;  // Data packet
    pub const ACK: u16 = 4;   // Acknowledgment
    pub const ERROR: u16 = 5; // Error
}
...
pub fn handle_request_packet(data: &[u8], src_ip: [u8; 4], src_port: u16) {
    ...
    match opcode {
        ...
        _ => {
            send_error(src_ip, src_port, 69, error_code::ILLEGAL_OP, "Invalid opcode");
        }
    }
}
...
pub fn handle_transfer_packet(data: &[u8], src_ip: [u8; 4], src_port: u16, local_port: u16) {
    ...
    if opcode == opcode::ACK {
        ...
        handle_ack(src_ip, src_port, local_port, block);
    }
}
...
fn handle_read_request(data: &[u8], client_ip: [u8; 4], client_port: u16) {
    ...
    let filename_end = match payload.iter().position(|&b| b == 0) {
        Some(pos) => pos,
        None => {
            send_error(client_ip, client_port, 69, error_code::ILLEGAL_OP, "Bad request");
            return;
        }
    };

    let filename = match core::str::from_utf8(&payload[..filename_end]) {
        Ok(s) => s,
        Err(_) => {
    ...
    // Look up file
    let files = VIRTUAL_FILES.lock();
    let file_data = match files.get(clean_name) {
        Some(data) => *data,
        None => {
...
fn handle_ack(client_ip: [u8; 4], client_port: u16, local_port: u16, block: u16) {
    let mut sessions = SESSIONS.lock();

    let session = match sessions.get_mut(&local_port) {
        Some(s) => s,
        None => return,
    };
...
fn send_error(dest_ip: [u8; 4], dest_port: u16, src_port: u16, code: u16, msg: &str) {
    ...
    let mut packet = Vec::with_capacity(5 + msg_bytes.len());
