...
pub fn run_all_tests() -> (usize, usize, String) {
    ...
    fn dummy_builtin(_id: u8, _argc: usize, _argv: *const i64) -> i64 { 0 }
...
pub fn smoke_test() -> bool {
    ...
    match super::native::compile_native(source) {
        Ok(prog) => {
            match unsafe { super::native::execute_native(&prog, dummy_builtin) } {
                Ok(42) => true,
                _ => false,
            }
        }
...
pub fn bench_fibonacci() -> (i64, u64) {
    ...
    // Compile (not timed)
    let prog = match super::native::compile_native(source) {
        Ok(p) => p,
        Err(_) => return (0, 0),
    };
