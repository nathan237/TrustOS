    ...
    fn decode_one(&mut self, block: &mut TranslatedBlock) -> bool {
        ...
        match opcode {
            ...
            0x39 => {
                ...
                let rv_reg = x86_to_rv(reg);
                // CMP doesn't store result, only sets flags
                block.emit(RvInst::CmpFlags { rs1: rv_rm, rs2: rv_reg });
                false
            }
            ...
            0x3B => {
                ...
                let rv_rm = x86_to_rv(rm);
                let rv_reg = x86_to_rv(reg);
                block.emit(RvInst::CmpFlags { rs1: rv_reg, rs2: rv_rm });
                false
            }
            ...
            0x85 => {
                ...
                block.emit(RvInst::And { rd: Reg::X5, rs1: rv_rm, rs2: rv_reg });
            ...
            0xFF => {
                ...
                match op_ext {
                    ...
                    2 => { // CALL r/m
                        ...
                        block.emit(RvInst::Addi { rd: Reg::X2, rs1: Reg::X2, imm: -8 });
                        let next_addr = self.base_addr + self.offset as u64;
                        block.emit(RvInst::Li { rd: Reg::X5, imm: next_addr as i64 });
            ...
            0x70..=0x7F => {
                ...
                let cond = x86_cc_to_flag(opcode & 0x0F);
                block.emit(RvInst::BranchCond { cond, offset: target });
            ...
            0xE8 => {
                ...
                // Push return address
                block.emit(RvInst::Addi { rd: Reg::X2, rs1: Reg::X2, imm: -8 });
                let ret_addr = self.base_addr + self.offset as u64;
                block.emit(RvInst::Li { rd: Reg::X5, imm: ret_addr as i64 });
                block.emit(RvInst::Sd { rs2: Reg::X5, rs1: Reg::X2, offset: 0 });
                block.emit(RvInst::Call { offset: target });
                block.successors.push(target as u64);
                block.successors.push(ret_addr);
                true
            }
                ...
                if self.offset < self.code.len() {
                    ...
                    match byte2 {
                        ...
                        0x80..=0x8F => {
                            let rel = self.read_i32() as i64;
                            let target = self.base_addr as i64 + self.offset as i64 + rel;
                            let cond = x86_cc_to_flag(byte2 & 0x0F);
                            block.emit(RvInst::BranchCond { cond, offset: target });
                            let fallthrough = self.base_addr + self.offset as u64;
                            block.successors.push(target as u64);
                            block.successors.push(fallthrough);
