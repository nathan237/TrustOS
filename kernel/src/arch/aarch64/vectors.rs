...
extern "C" fn el1_sync_handler(tf: &TrapFrame) {
    let esr = unsafe { super::cpu::read_esr_el1() };
    let far = unsafe { super::cpu::read_far_el1() };
    let ec = (esr >> 26) & 0x3F;
...
#[no_mangle]
extern "C" fn el1_irq_handler(_tf: &TrapFrame) {
...
extern "C" fn el0_sync_handler(tf: &TrapFrame) {
    ...
    match ec {
        0x15 => {
            ...
            let _syscall_num = tf.regs[8];
            let _arg0 = tf.regs[0];
            // TODO: dispatch to syscall handler
            crate::serial_println!("[SYSCALL] SVC from user: num={}", _syscall_num);
        }
        0x20 => {
...
#[no_mangle]
extern "C" fn el0_irq_handler(_tf: &TrapFrame) {
