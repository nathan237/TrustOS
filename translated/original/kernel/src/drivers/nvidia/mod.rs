...
unsafe fn gpu_engine_init(mmio: u64) -> bool {
...
unsafe fn setup_2d_surface(mmio: u64, fb_phys: u64, width: u32, height: u32, pitch: u32) {
    crate::serial_println!("[NVIDIA] Setting up 2D surface: {}x{} pitch={} fb_phys={:#X}",
        width, height, pitch, fb_phys);
...
pub fn accel_fill_rect(x: u32, y: u32, w: u32, h: u32, color: u32) {
    ...
    unsafe {
        ...
        for row in y..y_end {
            ...
            let row_ptr = base.offset(row_offset);
...
pub fn accel_copy_rect(src_x: u32, src_y: u32, dst_x: u32, dst_y: u32, w: u32, h: u32) {
    let vram = VRAM_BASE.load(Ordering::Relaxed);
    if vram == 0 {
        return;
    }
...
pub fn init() {
    ...
    let mmio_phys = match dev.bar_address(bar::MMIO) {
    ...
    crate::serial_println!("[NVIDIA] BAR0 (MMIO): phys={:#010X} size={}MB", mmio_phys, mmio_size / (1024*1024));
    
    let mmio_virt = match memory::map_mmio(mmio_phys, mmio_size) {
        Ok(v) => v,
        Err(e) => {
    ...
    let vram_phys = dev.bar_address(bar::VRAM).unwrap_or(0);
    ...
    let (chipset_id, stepping) = unsafe { read_gpu_identity(mmio_virt) };
    
    // Step 6: Read VRAM size
    let vram_size = unsafe { read_vram_size(mmio_virt) };
    
    // Step 7: Read PCIe link info
    let (pcie_gen, pcie_width) = read_pcie_link(&dev);
    
    // Step 8: Initialize GPU engines
    let engines_ok = unsafe { gpu_engine_init(mmio_virt) };
    if !engines_ok {
...
/// Get GPU info
pub fn get_info() -> Option<NvGpuInfo> {
