...
pub const MAX_DRONES: usize = 128;
pub const MAX_EDGES: usize = 256;
pub const NUM_FORMATIONS: usize = 6;
...
pub struct DroneSwarmState {
    ...
    drone_x: [f32; MAX_DRONES],
    drone_y: [f32; MAX_DRONES],
    drone_z: [f32; MAX_DRONES],

    // Target positions (current formation endpoints)
    target_x: [f32; MAX_DRONES],
    target_y: [f32; MAX_DRONES],
    target_z: [f32; MAX_DRONES],

    // 2D projected screen positions (recomputed each frame)
    proj_x: [f32; MAX_DRONES],
    proj_y: [f32; MAX_DRONES],
    proj_depth: [f32; MAX_DRONES],
...
impl DroneSwarmState {
    pub const fn new() -> Self {
        Self {
            ...
            screen_w: 0.0,
            screen_h: 0.0,
            drone_x: [0.0; MAX_DRONES],
            drone_y: [0.0; MAX_DRONES],
            drone_z: [0.0; MAX_DRONES],
            target_x: [0.0; MAX_DRONES],
            target_y: [0.0; MAX_DRONES],
            target_z: [0.0; MAX_DRONES],
            proj_x: [0.0; MAX_DRONES],
            proj_y: [0.0; MAX_DRONES],
            proj_depth: [0.0; MAX_DRONES],
            num_drones: 0,
            edge_a: [0; MAX_EDGES],
            edge_b: [0; MAX_EDGES],
            num_edges: 0,
...
fn build_formation(s: &mut DroneSwarmState, idx: u8) {
    ...
    match idx % NUM_FORMATIONS as u8 {
        0 => build_helix(s),
        1 => build_tesseract(s),
        2 => build_atom(s),
        3 => build_galaxy(s),
...
fn build_atom(s: &mut DroneSwarmState) {
    ...
    let nuc_r = 0.08f32;
    let nuc_base = nd;
    let nuc_pos: [(f32, f32, f32); 4] = [
        (nuc_r, 0.0, 0.0),
        (-nuc_r, 0.0, 0.0),
        (0.0, nuc_r, 0.0),
        (0.0, -nuc_r, 0.0),
    ];
    for &(nx, ny, nz) in nuc_pos.iter() {
        s.target_x[nd] = nx;
        s.target_y[nd] = ny;
        s.target_z[nd] = nz;
        nd += 1;
...
fn build_galaxy(s: &mut DroneSwarmState) {
    ...
    for arm in 0..NUM_ARMS {
        let offset = (arm as f32 / NUM_ARMS as f32) * 6.2831853;

        for i in 0..ARM_N {
            let t = i as f32 / (ARM_N - 1) as f32;
            let r = base_r + t * (max_r - base_r);
    ...
    for iz in 0..NZ {
        for iy in 0..NY {
            for ix in 0..NX {
                ...
                if ix + 1 < NX && ne < MAX_EDGES {
                    s.edge_a[ne] = idx as u8;
                    s.edge_b[ne] = (idx + 1) as u8;
                    ne += 1;
                }
                if iy + 1 < NY && ne < MAX_EDGES {
                    s.edge_a[ne] = idx as u8;
                    s.edge_b[ne] = (idx + NX) as u8;
                    ne += 1;
                }
                if iz + 1 < NZ && ne < MAX_EDGES {
                    s.edge_a[ne] = idx as u8;
...
pub fn update(s: &mut DroneSwarmState) {
    ...
    if s.state_timer >= HOLD_FRAMES {
        let old_num = s.num_drones;
        s.formation_idx = (s.formation_idx + 1) % NUM_FORMATIONS as u8;
        build_formation(s, s.formation_idx);
        // New drones beyond old count: start from center so they "fly out"
        for i in old_num..s.num_drones {
            s.drone_x[i] = 0.0;
            s.drone_y[i] = 0.0;
            s.drone_z[i] = 0.0;
        }
...
fn render_glow(s: &mut DroneSwarmState) {
    ...
    for e in 0..s.num_edges {
        ...
        let avg_depth = (s.proj_depth[a] + s.proj_depth[b]) * 0.5;
        let depth_bright = (1.8 / avg_depth).min(1.5);
...
fn choreo_color(time: f32, formation: u8) -> (i16, i16, i16) {
    ...
    let (br, bg, bb) = match formation % NUM_FORMATIONS as u8 {
