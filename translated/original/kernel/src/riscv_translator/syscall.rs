    ...
    match syscall {
        UnifiedSyscall::Write => {
            ...
            if fd == 1 || fd == 2 {
                ...
                for i in 0..count {
                    if let Ok(b) = mem.read_u8(buf_addr + i as u64) {
        ...
        UnifiedSyscall::Mmap => {
            ...
            let alloc_addr = if addr != 0 { addr } else { 0x4000_0000 + mem.total_allocated as u64 };
            if len > 0 && len <= 64 * 1024 * 1024 {
                mem.map(alloc_addr, len);
                (alloc_addr as i64, false)
            } else {
                (-12i64, false) // ENOMEM
            }
        }
