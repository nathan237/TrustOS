...
pub fn set_audio_data(beat: f32, bass: f32, sub_bass: f32, mid: f32,
    ...
    FRAME_NUM.store(frame, Ordering::Relaxed);
}
...
pub fn get_frame_num() -> u32 { FRAME_NUM.load(Ordering::Relaxed) }
...
pub fn add_effect(name: &str, source: &str) -> Result<(), &'static str> {
    ...
    if reg.effects.len() >= MAX_EFFECTS {
...
pub fn run_frame() -> bool {
    ...
    if let Some(src) = source {
        ...
        match crate::trustlang::run(&src) {
            Ok(_) => {}
            Err(e) => {
                let frame = FRAME_NUM.load(Ordering::Relaxed);
