...
impl BeatStudio {
    ...
    fn anthem_init(&mut self) {
        ...
        for t in self.tracks.iter_mut() {
            t.num_steps = 32;
            for s in 0..MAX_STEPS { t.steps[s] = BeatStep::off(); }
        }
    ...
    fn anthem_drop(&mut self) {
        ...
        for i in 0..32 {
            let vel = match i % 4 { 0 => 80, 2 => 100, 1 => 35, _ => 50 };
            self.tracks[2].steps[i] = BeatStep::on(vel);
        }
...
impl MatrixState {
    ...
    fn draw_rain(&self) {
        ...
        let col_width = self.fb_w / self.num_cols as u32;

        for (col_idx, col) in self.columns.iter().enumerate() {
            let x = (col_idx as u32 * col_width) + col_width / 2;
...
fn compute_energy(audio: &[i16], start: usize, end: usize) -> u32 {
    ...
    if e <= s { return 0; }
    let slice = &audio[s..e];
    let sum_abs: u64 = slice.iter().map(|v| v.unsigned_abs() as u64).sum();
    let avg = (sum_abs / slice.len().max(1) as u64) as u32;
    (avg * 100 / 8000).min(100)
}
...
fn draw_glow_waveform(
    ...
    let num_pts: usize = 256;
    let x_step = (wave_w / num_pts as u32).max(1);
...
fn draw_cyber_hud(
    ...
    crate::framebuffer::fill_rect_alpha(tx.saturating_sub(12), 10, tw + 24, 36, 0x000000, 160);
    crate::graphics::scaling::draw_text_at_scale(tx as i32, 14, title, 0x00FFCC, scale);
    ...
    let sec_s = format!("{}/8 L{}/{}", sec_idx + 1, loop_num + 1, total_loops);
    let sw = sec_s.len() as u32 * 8 + 16;
    let sx = fb_w.saturating_sub(sw + 8);
    crate::framebuffer::fill_rect_alpha(sx, 8, sw, 20, 0x000000, 140);
    crate::framebuffer::draw_text(&sec_s, sx + 8, 12, 0x00AA88);
...
pub fn launch_narrated_showcase() -> Result<(), &'static str> {
    ...
    if wait_ms_interruptible(6500) { showcase_cleanup(); return Ok(()); }
    ...
    crate::framebuffer::swap_buffers();
    if wait_ms_interruptible(5500) { showcase_cleanup(); return Ok(()); }
...
pub fn launch_anthem_showcase() -> Result<(), &'static str> {
    ...
    crate::framebuffer::swap_buffers();
    if wait_ms_interruptible(6000) { showcase_cleanup(); return Ok(()); }
    ...
    for (sec_idx, sec) in sections.iter().enumerate() {
        ...
        crate::framebuffer::swap_buffers();
        if wait_ms_interruptible(4500) { showcase_cleanup(); return Ok(()); }
...
pub fn launch_trap_showcase() -> Result<(), &'static str> {
    ...
    for &(track_idx, name, desc, loops, accent) in &layers {
        // Un-mute this layer
        ...
        crate::framebuffer::swap_buffers();
        if wait_ms_interruptible(1800) { showcase_cleanup(); return Ok(()); }
    ...
    crate::framebuffer::swap_buffers();
    if wait_ms_interruptible(2500) { showcase_cleanup(); return Ok(()); }
    ...
    let sec_loops: [u32; 8] = [3, 5, 1, 4, 1, 3, 3, 1];
    ...
    for f in 0..30u32 {
        ...
        let msg_w = msg.len() as u32 * 16 + 32;
        let mx = (fb_w.saturating_sub(msg_w)) / 2;
        let my = fb_h / 2 - 16;
        crate::framebuffer::fill_rect_alpha(mx.saturating_sub(8), my.saturating_sub(8), msg_w + 16, 48, 0x000000, 180);
        crate::graphics::scaling::draw_text_at_scale(mx as i32, my as i32, msg, 0x00FFCC, 2);
        crate::framebuffer::swap_buffers();
        if wait_ms_interruptible(80) { showcase_cleanup(); return Ok(()); }
    }
    ...
    let mut sec_boundaries: Vec<(usize, usize)> = Vec::new(); // (sec_idx, global_step)
    ...
    for sec in 0..8usize {
        ...
        for _lp in 0..sec_loops[sec] {
            sec_boundaries.push((sec, global_step_count));
    ...
    for g in 0..total_global_steps {
        ...
        for (bi, &(si, gs)) in sec_boundaries.iter().enumerate() {
            if gs <= g {
                cur_sec = si;
                // Count how many boundaries share this section up to bi
                cur_loop = sec_boundaries[..=bi].iter().filter(|&&(s, _)| s == si).count() as u32 - 1;
            }
        }
        ...
        draw_cyber_hud(fb_w, fb_h, sec_names[cur_sec], cur_sec,
            cur_loop, sec_loops[cur_sec], s, total_steps, studio.bpm);
