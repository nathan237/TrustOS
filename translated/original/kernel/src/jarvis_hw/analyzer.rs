...
pub fn analyze_hardware(profile: &HardwareProfile) -> Vec<HwInsight> {
...
fn analyze_compute(insights: &mut Vec<HwInsight>, profile: &HardwareProfile) {
    // SIMD capabilities
    if profile.has_avx512 {
        insights.push(HwInsight {
...
fn analyze_memory(insights: &mut Vec<HwInsight>, profile: &HardwareProfile) {
...
fn analyze_storage(insights: &mut Vec<HwInsight>, profile: &HardwareProfile) {
...
fn analyze_network(insights: &mut Vec<HwInsight>, profile: &HardwareProfile) {
...
fn analyze_security(insights: &mut Vec<HwInsight>, profile: &HardwareProfile) {
...
fn analyze_architecture(insights: &mut Vec<HwInsight>, profile: &HardwareProfile) {
    match profile.arch {
        "x86_64" => {
            insights.push(HwInsight {
                category: InsightCategory::Resource,
...
pub fn generate_plan(profile: &HardwareProfile) -> ExecutionPlan {
    ...
    let max_gen = if ram_mb >= 2048 { 256 }
...
pub fn record_inference(elapsed_us: u64) {
    ...
    if count > 10 {
        let avg = TOTAL_INFERENCE_US.load(Ordering::Relaxed) / count;
        if elapsed_us > avg * 3 {
...
pub fn format_insights(insights: &[HwInsight]) -> String {
