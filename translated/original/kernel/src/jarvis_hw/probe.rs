...
pub struct HardwareProfile {
    ...
    pub encryption_detected: Vec<EncryptionInfo>,
...
fn probe_cpu(profile: &mut HardwareProfile) {
    #[cfg(target_arch = "x86_64")]
    {
        ...
        profile.cpu_vendor = match caps.vendor {
        ...
        // Brand string
        let brand_len = caps.brand_string.iter()
            .position(|&b| b == 0)
            .unwrap_or(48);
        if let Ok(s) = core::str::from_utf8(&caps.brand_string[..brand_len]) {
...
fn probe_storage(profile: &mut HardwareProfile) {
    ...
    if crate::drivers::ahci::is_initialized() {
        for port in crate::drivers::ahci::list_devices() {
            ...
            profile.storage_devices.push(StorageInfo {
...
fn probe_partitions_and_encryption(profile: &mut HardwareProfile) {
    ...
    if crate::drivers::ahci::is_initialized() {
        ...
        for port_num in 0..port_count {
            if crate::drivers::ahci::get_port_info(port_num).is_some() {
...
fn detect_encryption_header(buf: &[u8], disk_name: &str, part_num: Option<u8>, profile: &mut HardwareProfile) {
        ...
        && buf[3] == b'S' && buf[4] == 0xBA && buf[5] == 0xBE
    {
        ...
        let enc_type = if version == 2 { EncryptionType::Luks2 } else { EncryptionType::Luks1 };
        let detail = format!("LUKS v{} detected at sector 0", version);

        // Avoid duplicates
        if !profile.encryption_detected.iter().any(|e| e.disk_name == disk_name && e.encryption_type == enc_type) {
            profile.encryption_detected.push(EncryptionInfo {
...
fn compute_scores(profile: &mut HardwareProfile) {
    ...
    let freq_factor = (profile.tsc_freq_hz as f32 / 5_000_000_000.0).min(1.0);
    let gpu_bonus = if profile.has_gpu { 0.3 } else { 0.0 };

    profile.compute_score = ((core_factor * 0.4 + freq_factor * 0.3 + simd_mult / 4.0 * 0.3) + gpu_bonus).min(1.0);
