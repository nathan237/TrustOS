...
pub fn answer_query(query: &str, profile: &HardwareProfile) -> QueryResult {
        ...
        || q.contains("how much") || q.contains("how many")
    {
        ...
        if q.contains("ram") || q.contains("memory") || q.contains("memoire") {
            return query_memory_info(profile);
        }

        // CPU info
        if q.contains("cpu") || q.contains("processor") || q.contains("processeur") {
            return query_cpu_info(profile);
        }
        ...
        if q.contains("disk") || q.contains("disque") || q.contains("storage")
            || q.contains("stockage")
        {
            return query_storage_info(profile);
        }

        // Encryption info
        if q.contains("encrypt") || q.contains("chiffr") || q.contains("crypt") {
            return query_encryption_info(profile);
        }

        // PCI info
        if q.contains("pci") || q.contains("device") || q.contains("bus") {
            return query_pci_info(profile);
        }
    }
...
fn query_memory_info(profile: &HardwareProfile) -> QueryResult {
    QueryResult {
...
fn query_cpu_info(profile: &HardwareProfile) -> QueryResult {
...
fn query_storage_info(profile: &HardwareProfile) -> QueryResult {
    let mut details = Vec::new();
...
fn query_encryption_info(profile: &HardwareProfile) -> QueryResult {
    if profile.encryption_detected.is_empty() {
        return QueryResult {
...
fn query_pci_info(profile: &HardwareProfile) -> QueryResult {
    QueryResult {
...
pub fn format_query_result(result: &QueryResult) -> String {
    ...
    let conf_icon = if result.confidence >= 0.9 { "\x01G[CERTAIN]\x01W" }
