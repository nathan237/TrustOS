...
pub fn init_double_buffer() {
    ...
    *BACKBUFFER.lock() = Some(buffer);
    ...
    let mut prev_buf = alloc::vec::Vec::new();
    if prev_buf.try_reserve_exact(size).is_ok() {
        prev_buf.resize(size, 0u32);
        *PREV_FRAME.lock() = Some(prev_buf.into_boxed_slice());
...
pub fn fill_rect_alpha(x: u32, y: u32, w: u32, h: u32, color: u32, alpha: u32) {
    ...
    if !ptr.is_null() {
        let stride = FRAME_BB_STRIDE.load(Ordering::Relaxed) as usize;
        for py in y1..y2 {
            let row_ptr = unsafe { ptr.add(py as usize * stride + x1 as usize) };
            let row_w = (x2 - x1) as usize;
            #[cfg(target_arch = "x86_64")]
            unsafe { crate::graphics::simd::blend_fill_row_sse2(row_ptr, row_w, color, alpha); }
            #[cfg(not(target_arch = "x86_64"))]
            for px in x1..x2 {
