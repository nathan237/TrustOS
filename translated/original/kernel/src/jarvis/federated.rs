...
const MIN_GRADIENTS_FOR_ROUND: usize = 1;

/// Maximum pending gradient buffers (prevent OOM)
const MAX_PENDING_GRADIENTS: usize = 16;
...
const BASE_SYNC_INTERVAL_MS: u64 = 30_000;

/// Minimum sync interval (aggressive learning phase)
const MIN_SYNC_INTERVAL_MS: u64 = 5_000;

/// Maximum sync interval (converged, save bandwidth)
const MAX_SYNC_INTERVAL_MS: u64 = 120_000;
...
static LAST_AVG_LOSS: Mutex<f32> = Mutex::new(0.0);

/// Server-side momentum buffer (accumulated gradient direction)
static SERVER_MOMENTUM_BUF: Mutex<Vec<f32>> = Mutex::new(Vec::new());
...
static ADAPTIVE_INTERVAL_MS: AtomicU64 = AtomicU64::new(30_000);
...
pub fn enable() {
    ...
    LAST_SYNC_MS.store(crate::time::uptime_ms(), Ordering::SeqCst);
...
pub fn stats() -> String {
    ...
    let loss = *LAST_AVG_LOSS.lock();
    let interval = ADAPTIVE_INTERVAL_MS.load(Ordering::SeqCst);
...
pub fn receive_gradient_bytes(raw_bytes: &[u8]) {
    ...
    if raw_bytes.len() >= 4 && &raw_bytes[0..4] == b"JCMP" {
        if let Some(compressed) = super::compression::deserialize_compressed(raw_bytes) {
            ...
            if inbox.len() < MAX_PENDING_GRADIENTS {
...
pub fn poll() {
    ...
    let last_sync = LAST_SYNC_MS.load(Ordering::SeqCst);
    let interval = ADAPTIVE_INTERVAL_MS.load(Ordering::SeqCst);
    ...
    LAST_SYNC_MS.store(now, Ordering::SeqCst);
...
fn leader_aggregate() {
    ...
    if total_count < MIN_GRADIENTS_FOR_ROUND {
        return;
    }
    ...
    for grad in &raw_gradients {
        ...
        let weight = peer_step_weights.get(grad_idx).copied().unwrap_or(1).max(1) as f64;
    ...
    {
        let mut momentum = SERVER_MOMENTUM_BUF.lock();
...
fn worker_send_gradients() {
    ...
    // Get leader info
    let leader = match super::mesh::get_leader() {
        Some(l) => l,
...
fn compute_local_gradient() -> Vec<f32> {
    let model = super::MODEL.lock();
    let model_ref = match model.as_ref() {
        Some(m) => m,
        None => return Vec::new(),
    };
...
fn update_adaptive_interval() {
    ...
    let new_interval = if loss > HIGH_LOSS_THRESHOLD {
        ...
        let range = MAX_SYNC_INTERVAL_MS as f32 - MIN_SYNC_INTERVAL_MS as f32;
        (MIN_SYNC_INTERVAL_MS as f32 + range * (1.0 - t)) as u64
    };

    let old = ADAPTIVE_INTERVAL_MS.swap(new_interval, Ordering::SeqCst);
...
pub fn current_interval_ms() -> u64 {
    ADAPTIVE_INTERVAL_MS.load(Ordering::SeqCst)
}
