...
const ELECTION_TIMEOUT_MS: u64 = 20_000;

/// Leader heartbeat interval (ms)
const LEADER_HEARTBEAT_MS: u64 = 5000;
...
static LAST_ELECTION_MS: AtomicU64 = AtomicU64::new(0);
...
pub fn init() {
    ...
    LAST_ELECTION_MS.store(0, Ordering::SeqCst);
...
pub fn poll() {
    ...
    if IS_LEADER.load(Ordering::SeqCst) {
        ...
        if now.wrapping_sub(last_sent) >= LEADER_HEARTBEAT_MS {
        ...
        let last_election = LAST_ELECTION_MS.load(Ordering::SeqCst);

        let timed_out = now.wrapping_sub(last_hb) >= ELECTION_TIMEOUT_MS;
        let cooldown_ok = now.wrapping_sub(last_election) >= ELECTION_COOLDOWN_MS;
...
fn start_election() {
    ...
    LAST_ELECTION_MS.store(crate::time::uptime_ms(), Ordering::SeqCst);
