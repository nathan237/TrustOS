...
const MAX_PAYLOAD_SIZE: usize = 32 * 1024 * 1024; // 32 MB max (for full model weights)
const CONNECT_TIMEOUT_MS: u32 = 30000;
...
fn build_request(req_id: u32, cmd: Command, payload: &[u8]) -> Vec<u8> {
...
fn build_response(req_id: u32, status: Status, payload: &[u8]) -> Vec<u8> {
    let mut pkt = Vec::with_capacity(HEADER_SIZE + payload.len());
    pkt.extend_from_slice(MAGIC);
    pkt.extend_from_slice(&req_id.to_be_bytes());
...
fn build_response_header(req_id: u32, status: Status, payload_len: u32) -> Vec<u8> {
...
fn parse_header(data: &[u8]) -> Option<(u32, u8, u32)> {
    ...
    let req_id = u32::from_be_bytes([data[4], data[5], data[6], data[7]]);
    let cmd = data[8];
...
pub fn call(dest_ip: [u8; 4], dest_port: u16, cmd: Command, payload: &[u8]) -> Result<(Status, Vec<u8>), &'static str> {
    ...
    if !crate::netstack::tcp::wait_for_established(dest_ip, dest_port, src_port, CONNECT_TIMEOUT_MS) {
        return Err("RPC connect timeout");
    }
    // Send request
    let request = build_request(req_id, cmd, payload);
    ...
    let timeout_ms = CONNECT_TIMEOUT_MS as u64 * 4; // 120s for large transfers
    loop {
        crate::netstack::poll();
        ...
        if response_buf.len() >= HEADER_SIZE {
            if let Some((_, _, plen)) = parse_header(&response_buf) {
                let total = HEADER_SIZE + plen as usize;
                if response_buf.len() >= total {
                    // Parse response
                    let status = match response_buf[8] {
                        0 => Status::Ok,
                        2 => Status::Busy,
                        _ => Status::Error,
                    };
                    let resp_payload = response_buf[HEADER_SIZE..total].to_vec();
...
fn handle_connection(src_port: u16, remote_ip: [u8; 4], remote_port: u16) {
    ...
    loop {
        ...
        if crate::time::uptime_ms().wrapping_sub(start) > CONNECT_TIMEOUT_MS as u64 {
    ...
    // Parse request
    let (req_id, cmd_byte, payload_len) = match parse_header(&buf) {
        Some(v) => v,
        None => {
            let _ = crate::netstack::tcp::send_fin(remote_ip, remote_port, src_port);
            return;
    ...
    let cmd = match Command::from_byte(cmd_byte) {
        Some(c) => c,
        None => {
            let resp = build_response(req_id, Status::Error, b"Unknown command");
    ...
    let total_resp_len = HEADER_SIZE + response_payload.len();

    if response_payload.len() <= 65536 {
        // Small payload: build complete response
        let resp = build_response(req_id, status, &response_payload);
        let _ = crate::netstack::tcp::send_data(remote_ip, remote_port, src_port, &resp);
    } else {
        // Large payload: send header then payload separately to halve memory usage
        let resp_header = build_response_header(req_id, status, response_payload.len() as u32);
        let _ = crate::netstack::tcp::send_data(remote_ip, remote_port, src_port, &resp_header);
        crate::netstack::poll();
    ...
    let flush_iters = if total_resp_len > 1_000_000 { 100 } else { 20 };
