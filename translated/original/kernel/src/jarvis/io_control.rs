...
static LAST_AUDIT_MS: AtomicU64 = AtomicU64::new(0);
...
pub fn full_audit() -> IoAudit {
    ...
    // Cache result
    LAST_AUDIT_MS.store(crate::time::uptime_ms(), Ordering::SeqCst);
...
pub fn control_score(audit: &IoAudit) -> u8 {
    ...
    ((score * 100) / max_score).min(100) as u8
}
...
pub fn poll() {
    ...
    let last = LAST_AUDIT_MS.load(Ordering::SeqCst);
