...
fn memory_sequences(p: &HardwareProfile, s: &mut Vec<String>) {
    ...
    let heap_kb = p.heap_size_bytes / 1024;
    let used_kb = p.heap_used_bytes / 1024;
    let free_kb = p.heap_free_bytes / 1024;
    s.push(format!("Q: Heap status? A: {} KB total, {} KB used, {} KB free",
        heap_kb, used_kb, free_kb));
...
fn storage_sequences(p: &HardwareProfile, s: &mut Vec<String>) {
    ...
    for dev in &p.storage_devices {
        let cap_mb = dev.capacity_bytes / (1024 * 1024);
        ...
        s.push(format!("Storage: {} ({}) — {} MB", dev.name, kind, cap_mb));
    }
...
fn encryption_sequences(p: &HardwareProfile, s: &mut Vec<String>) {
    ...
    for enc in &p.encryption_detected {
        let enc_type = match enc.encryption_type {
        ...
        s.push(format!("Encryption: {} on {} — {}", enc_type, enc.disk_name, enc.detail));
    }
