...
#[cfg(target_arch = "x86_64")]
#[inline]
pub unsafe fn blend_fill_row_sse2(dst: *mut u32, count: usize, color: u32, alpha: u32) {
    if count == 0 { return; }
    if alpha == 0 { return; }
    if alpha >= 255 {
    ...
    let src_a_lo = _mm_mullo_epi16(s_lo, alpha_vec);
    let src_a_hi = _mm_mullo_epi16(s_hi, alpha_vec);
    ...
    while remaining >= 4 {
        ...
        let dst_mul_lo = _mm_mullo_epi16(d_lo, inv_a_vec);
        let sum_lo = _mm_add_epi16(_mm_add_epi16(src_a_lo, dst_mul_lo), round_vec);
        let res_lo = _mm_srli_epi16(sum_lo, 8);

        // Pixels 2-3
        let d_hi = _mm_unpackhi_epi8(d, zero);
        let dst_mul_hi = _mm_mullo_epi16(d_hi, inv_a_vec);
        let sum_hi = _mm_add_epi16(_mm_add_epi16(src_a_hi, dst_mul_hi), round_vec);
        let res_hi = _mm_srli_epi16(sum_hi, 8);
