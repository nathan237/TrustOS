...
unsafe { ptr.add(used_offset) as *mut virtio::VirtqUsed };
    ...
    pub fn setup_rx_buffers(&mut self) -> Result<(), &'static str> {
        ...
        for _ in 0..number_buffers {
            ...
            let buffer_ptr = &*buffer as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const ReceiveBuffer;
...
unsafe {
            ...
            let buffer_ptr = (crate::memory::hhdm_offset() + desc.addr) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const ReceiveBuffer;
