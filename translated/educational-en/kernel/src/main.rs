...
#![no_std]
// #![no_main] — custom entry point instead of fn main().
#![no_main]
...
#[used]
#[link_section = ".text"]
static KMAIN_REFERENCE: // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
match entry.entry_type {
            ...
            let type_code = // Pattern matching — Rust's exhaustive branching construct.
match entry.entry_type {
...
unsafe { JARVIS_BRAIN_MODULE = Some(data); }
    ...
    if let Some(brain_data) = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
