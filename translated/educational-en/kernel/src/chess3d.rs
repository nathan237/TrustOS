...
pub fn tick(&mut self) {
    ...
    fn render_pieces(&self, buffer: &mut [u32], w: usize, h: usize) {
        ...
        for pr in &pieces {
            ...
            let absolute_type = if pr.piece < 0 { -pr.piece } else { pr.piece };

            let mesh = get_piece_mesh(absolute_type);
    ...
    fn render_piece_mesh(&self, buffer: &mut [u32], w: usize, h: usize,
        ...
        if let Some(ref faces) = mesh.faces {
            ...
            let light_directory = V3 { x: -0.3, y: -0.8, z: -0.5 };
            let light_length = fast_sqrt(light_directory.x * light_directory.x + light_directory.y * light_directory.y + light_directory.z * light_directory.z);
            let light = V3 { x: light_directory.x / light_length, y: light_directory.y / light_length, z: light_directory.z / light_length };
    ...
    fn render_hud(&self, buffer: &mut [u32], w: usize, h: usize) {
        ...
        let phase_text = // Pattern matching — Rust's exhaustive branching construct.
match self.chess.phase {
        ...
        if !phase_text.is_empty() {
            let phase_color = // Pattern matching — Rust's exhaustive branching construct.
match self.chess.phase {
...
fn cone_capability(base: usize, apex: usize, n: usize) -> (Vec<(usize, usize, usize)>, Vec<(usize, usize)>) {
    let mut faces = Vec::new();
    let mut edges = Vec::new();
    for i in 0..n {
        let j = (i + 1) % n;
...
fn mesh_pawn() -> PieceMesh {
    ...
    let (f, e) = cone_capability(b6, apex, n); faces.extend(f); edges.extend(e);

    // Bottom cap
    let bottom_center = verts.len();
    verts.push(V3 { x: 0.0, y: 0.0, z: 0.0 });
    let (f, e) = cone_capability(b0, bottom_center, n); faces.extend(f); edges.extend(e);
...
fn mesh_knight() -> PieceMesh {
    ...
    let (f, e) = connect_rings(b3, b4, n); faces.extend(f); edges.extend(e);
    let (f, e) = connect_rings(b4, b5, n); faces.extend(f); edges.extend(e);
    let (f, e) = cone_capability(b5, ear, n); faces.extend(f); edges.extend(e);

    let bottom = verts.len();
    verts.push(V3 { x: 0.0, y: 0.0, z: 0.0 });
    let (f, e) = cone_capability(b0, bottom, n); faces.extend(f); edges.extend(e);
...
fn mesh_rook() -> PieceMesh {
    ...
    let (f, e) = connect_rings(b3, b4, n); faces.extend(f); edges.extend(e);
    let (f, e) = connect_rings(b4, b5, n); faces.extend(f); edges.extend(e);
    let (f, e) = cone_capability(b5, top_center, n); faces.extend(f); edges.extend(e);
...
fn mesh_king() -> PieceMesh {
    ...
    let (f, e) = cone_capability(b6, top_ring_center, n); faces.extend(f); edges.extend(e);
