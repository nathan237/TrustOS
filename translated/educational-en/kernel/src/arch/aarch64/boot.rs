...
const MAIR_DEVICE_INDEX: u64 = 4; // AttrIndx[2:0] = 4 → bits[4:2]
...
unsafe fn setup_mmio_identity_map(kernel_virt_base: u64, kernel_physical_base: u64) {
    ...
    let l0_physical = virt_to_physical(&raw // Compile-time constant — evaluated at compilation, zero runtime cost.
const TTBR0_L0 as u64);
    let l1_physical = virt_to_physical(&raw // Compile-time constant — evaluated at compilation, zero runtime cost.
