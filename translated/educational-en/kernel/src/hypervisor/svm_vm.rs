...
impl SvmVirtualMachine {
    ...
    pub fn initialize(&mut self) -> Result<()> {
        ...
        crate::serial_println!("[SVM-VM {}] IOPM allocated at HPA=0x{:X}", self.id, iopm_physical);
        ...
        crate::serial_println!("[SVM-VM {}] MSRPM allocated at HPA=0x{:X}", self.id, msrpm_physical);
        ...
        if self.features.npt {
            ...
            if let Err(e) = npt.map_range(
                ...
                guest_memory_physical,             // Host physical address
...
match exit {
            ...
            SvmExitCode::IoioIn | SvmExitCode::IoioOut => {
                ...
                let directory = if matches!(exit, SvmExitCode::IoioIn) { "IN" } else { "OUT" };
            ...
            SvmExitCode::MsrRead | SvmExitCode::MsrWrite => {
                ...
                let msr_directory = if is_write { "WRMSR" } else { "RDMSR" };
                crate::lab_mode::trace_bus::emit_vm_exit(
                    self.id, msr_directory, guest_rip,
            ...
            SvmExitCode::Rdtsc => {
                ...
                let tsc = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
unsafe { core::arch::x86_64::_rdtsc() };
            ...
            SvmExitCode::ExceptionPF => {
                ...
                let error_code = exit_info1 as u32;
                let fault_address = exit_info2;
                if self.stats.vmexits < 200 {
                    crate::serial_println!("[SVM-VM {}] #PF at 0x{:X} (err=0x{:X}) RIP=0x{:X}", 
                        self.id, fault_address, error_code, guest_rip);
                }
                let vmcb = self.vmcb.as_mut().ok_or(HypervisorError::VmcbNotLoaded)?;
                vmcb.write_state(state_offsets::CR2, fault_address);
        ...
        if let Some(dec) = decoded {
            if !dec.is_write {
                if let Some(register_index) = dec.register {
                    ...
                    mmio::write_guest_register(&mut self.guest_regs, register_index, value as u64);
                    return;
                }
            }
        }
...
const LAPIC_SVR: u32 = 0x0F0;
                // Compile-time constant — evaluated at compilation, zero runtime cost.
const LAPIC_INTERRUPT_HANDLER_BASE: u32 = 0x100;
                // Compile-time constant — evaluated at compilation, zero runtime cost.
...
match offset {
                ...
                LAPIC_TIMER_LVT => {
                    ...
                    if self.stats.vmexits < 1000 {
                        ...
                        let mode_str = // Pattern matching — Rust's exhaustive branching construct.
match mode {
                            0 => "one-shot",
                            1 => "periodic",
                            2 => "TSC-deadline",
            ...
            // Read
            let value: u32 = // Pattern matching — Rust's exhaustive branching construct.
match offset {
                ...
                LAPIC_INTERRUPT_HANDLER_BASE..=0x170 => 0,
...
match self.lapic.dcr & 0xB {
    ...
    fn check_hpet_interrupts(&mut self) -> Option<u64> {
        ...
        for (i, &(fired, interrupt_request_route)) in timer_status.iter().enumerate() {
            if !fired {
                continue;
            }
    ...
    fn handle_io(&mut self, info1: u64) {
        ...
        let _size = // Pattern matching — Rust's exhaustive branching construct.
match (info1 >> 4) & 0x7 {
            0 => 1, // Byte
            1 => 2, // Word
            2 => 4, // Dword
...
match port {
                ...
                0x2F8..=0x2FF => // Pattern matching — Rust's exhaustive branching construct.
match port & 0x7 {
                    5 => 0x60, // LSR: TX empty
                    _ => 0,
                },
                ...
                0x40 | 0x41 | 0x42 => {
                    ...
                    if pit_character.latched {
                        ...
                        let simulated = pit_character.count.wrapping_sub(
...
match pit_character.access {
                        1 => {
                            // Lobyte only
                            pit_character.reload = (pit_character.reload & 0xFF00) | v as u16;
                        ...
                        3 => {
                            // Lo/hi sequence
                            if pit_character.write_hi_pending {
                                pit_character.reload = (pit_character.reload & 0x00FF) | ((v as u16) << 8);
                                pit_character.count = pit_character.reload;
                                ...
                                pit_character.reload = (pit_character.reload & 0xFF00) | v as u16;
                                pit_character.write_hi_pending = true;
                            }
                        }
                        _ => {}
                    }
...
const IA32_SYSENTER_EIP: u32 = 0x0176;
                // Compile-time constant — evaluated at compilation, zero runtime cost.
const IA32_MCG_CAPABILITY: u32 = 0x0179;
                // Compile-time constant — evaluated at compilation, zero runtime cost.
...
const MSR_SFMASK: u32 = 0xC000_0084;
                // Compile-time constant — evaluated at compilation, zero runtime cost.
const MSR_FILESYSTEM_BASE: u32 = 0xC000_0100;
                // Compile-time constant — evaluated at compilation, zero runtime cost.
...
match msr {
                ...
                MSR_FILESYSTEM_BASE => {
                    let vmcb = self.vmcb.as_mut().unwrap();
                    vmcb.write_state(state_offsets::FILESYSTEM_BASE, value);
                }
                MSR_GS_BASE => {
                    let vmcb = self.vmcb.as_mut().unwrap();
            ...
            // RDMSR
            let value: u64 = // Pattern matching — Rust's exhaustive branching construct.
match msr {
                ...
                MSR_FILESYSTEM_BASE => {
                    let vmcb = self.vmcb.as_ref().unwrap();
                    vmcb.read_state(state_offsets::FILESYSTEM_BASE)
                }
                MSR_GS_BASE => {
                    let vmcb = self.vmcb.as_ref().unwrap();
    ...
    fn handle_vmmcall(&mut self) -> bool {
        ...
        let (result, should_continue): (i64, bool) = // Pattern matching — Rust's exhaustive branching construct.
match function {
            // Exit VM
            0x00 => {
                self.state = SvmVmState::Stopped;
...
mod tests {
    ...
    fn test_lapic_divider_decode() {
        ...
        for &(dcr, expected) in &map {
            let d = // Pattern matching — Rust's exhaustive branching construct.
match dcr & 0xB {
                0x0 => 2u64, 0x1 => 4, 0x2 => 8, 0x3 => 16,
