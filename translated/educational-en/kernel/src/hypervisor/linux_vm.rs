...
const BOOT_PARAMS_ADDRESS: u64 = 0x10000;
...
const CMDLINE_ADDRESS: u64 = 0x20000;
    ...
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const CMDLINE_MAXIMUM_SIZE: usize = 2048;
...
const KERNEL_LOAD_ADDRESS: u64 = 0x100000;
    ...
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const INITIALIZE_STACK: u64 = 0x8000;
...
const GDT_ADDRESS: u64 = 0x1000;
    ...
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const INITRAMFS_ADDRESS: u64 = 0x1000000;  // 16 MB
...
const HDRS_MAGIC: u32 = 0x53726448;
    ...
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const MINIMUM_PROTOCOL_VERSION: u16 = 0x0200;
...
const CAN_USE_HEAP: u8 = 0x80;
    ...
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const LOADER_TYPE: u8 = 0xFF;  // Undefined/other
...
static NEXT_ID: AtomicU64 = AtomicU64::new(0x10000);
    ...
    pub fn setup_boot_params(&mut self, initramfs_address: u64, initramfs_size: u32) -> Result<()> {
        ...
        self.guest_memory[cmdline_address + cmdline_length] = 0; // Null terminate
    ...
    fn setup_e820_map(&mut self, boot_params_addr: usize) -> Result<()> {
        ...
        let main_memory_start = 0x100000u64;
        let main_memory_size = (self.guest_memory.len() as u64).saturating_sub(main_memory_start);
        self.write_e820_entry(e820_offset, 3, main_memory_start, main_memory_size, boot_proto::E820_RAM);
        entry_count += 1;
    ...
    fn setup_gdt(&mut self) -> Result<u64> {
        let gdt_address = boot_proto::GDT_ADDRESS as usize;
        ...
        // Null descriptor
        self.guest_memory[gdt_address..gdt_address + 8].copy_from_slice(&[0u8; 8]);
        
        // Code segment: 0x00CF9A000000FFFF
        let code_seg: u64 = 0x00CF9A000000FFFF;
        self.guest_memory[gdt_address + 8..gdt_address + 16].copy_from_slice(&code_seg.to_le_bytes());
        
        // Data segment: 0x00CF92000000FFFF
        let data_seg: u64 = 0x00CF92000000FFFF;
        self.guest_memory[gdt_address + 16..gdt_address + 24].copy_from_slice(&data_seg.to_le_bytes());
        
        // Return GDT pointer (limit:base)
        Ok(boot_proto::GDT_ADDRESS)
    }
    ...
    pub fn boot(&mut self, bzimage: &[u8], initramfs: &[u8]) -> Result<()> {
        ...
        // Load initramfs
        let initramfs_address = self.load_initramfs(initramfs)?;
        
        // Setup boot parameters
        self.setup_boot_params(initramfs_address, initramfs.len() as u32)?;
        ...
        crate::serial_println!("[LINUX-VM {}] GDT at: 0x{:X}", self.id, gdt_address);
        crate::serial_println!("[LINUX-VM {}] Boot params: 0x{:X}", self.id, boot_proto::BOOT_PARAMS_ADDRESS);
    ...
    fn boot_with_vmx(&mut self, entry_point: u64) -> Result<()> {
        ...
        crate::serial_println!("[LINUX-VM {}] Entry: 0x{:X}, Boot params: 0x{:X}", 
            self.id, entry_point, boot_proto::BOOT_PARAMS_ADDRESS);
        ...
        match vm.start(entry_point, boot_proto::INITIALIZE_STACK) {
            Ok(()) => {
    ...
    fn boot_with_svm(&mut self, entry_point: u64) -> Result<()> {
        ...
        let boot_result = svm_vm::with_vm(vm_id, |vm| -> Result<()> {
            ...
            vm.setup_protected_mode_for_linux(
                entry_point, 
                boot_proto::INITIALIZE_STACK,
                boot_proto::BOOT_PARAMS_ADDRESS
            )?;
