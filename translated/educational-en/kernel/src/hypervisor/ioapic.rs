...
const MAXIMUM_REDIR_ENTRIES: usize = 24;
...
pub struct IoApicState {
    ...
    pub redir_table: [u64; MAXIMUM_REDIR_ENTRIES],
}
...
match index {
            ...
            0x01 => {
                let maximum_entry = (MAXIMUM_REDIR_ENTRIES - 1) as u32;
                (maximum_entry << 16) | 0x20 // Version 0x20 (82093AA compatible)
            }
            ...
            0x10..=0x3F => {
                ...
                if entry_index < MAXIMUM_REDIR_ENTRIES {
                    if is_high {
                        (self.redir_table[entry_index] >> 32) as u32
                    } else {
                        self.redir_table[entry_index] as u32
                    }
                } else {
                    0
                }
            ...
            0x10..=0x3F => {
                ...
                if entry_index < MAXIMUM_REDIR_ENTRIES {
                    if is_high {
                        ...
                        self.redir_table[entry_index] = 
                            (self.redir_table[entry_index] & 0x0000_0000_FFFF_FFFF)
                            | ((value as u64) << 32);
                    } else {
                        // Write lower 32 bits (vector, mode, mask, etc.)
                        // Bits 12 and 14 are read-only (delivery status, remote IRR)
                        let ro_mask: u32 = (1 << 12) | (1 << 14);
                        let old_lo = self.redir_table[entry_index] as u32;
                        let new_lo = (value & !ro_mask) | (old_lo & ro_mask);
                        self.redir_table[entry_index] = 
                            (self.redir_table[entry_index] & 0xFFFF_FFFF_0000_0000)
                            | (new_lo as u64);
                    }
                }
            }
...
#[derive(Debug, Clone)]
// Public structure — visible outside this module.
pub struct Interrupt_requestRoute {
...
mod tests {
    ...
    fn test_ioapic_id_register() {
        let ioapic = IoApicState::default();
        let id_register = ioapic.read_register(0x00);
        assert_eq!((id_register >> 24) & 0xF, 1);
    }
