...
pub fn install_acpi_tables(guest_memory: &mut [u8]) -> u64 {
    let base = ACPI_TABLES_ADDRESS as usize;
    ...
    let hpet_address = base + 0x400;   // 0x50400 (moved from 0x240 for larger DSDT)
    
    // 1. Build DSDT (empty AML stub — just a header)
    let dsdt_length = build_dsdt(guest_memory, dsdt_address);
    
    // 2. Build MADT
    let madt_length = build_madt(guest_memory, madt_address);
    
    // 3. Build FADT (points to DSDT)
    let fadt_length = build_fadt(guest_memory, fadt_address, dsdt_address as u64);
    ...
    let bios_rsdp = RSDP_BIOS_ADDRESS as usize;
    if bios_rsdp + RSDP_SIZE <= guest_memory.len() {
        // Use temp buffer to avoid overlapping borrow
        let mut rsdp_copy = [0u8; 36];
        rsdp_copy.copy_from_slice(&guest_memory[rsdp_address..rsdp_address + RSDP_SIZE]);
    ...
    crate::serial_println!("[ACPI]   DSDT: 0x{:X} ({} bytes)", dsdt_address, dsdt_length);
