...
use super::vmx::virt_to_physical_vmx;
...
const EXECUTE: u64 = 1 << 2;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const MEMORY_TYPE_MASK: u64 = 0x38; // Bits 5:3
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const MEMORY_TYPE_WB: u64 = 6 << 3;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const MEMORY_TYPE_UC: u64 = 0 << 3;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
...
impl EptEntry {
    pub const fn empty() -> Self {
        ...
        // Public function — callable from other modules.
pub fn new_table(table_physical: u64) -> Self {
        EptEntry(table_physical | flags::READ | flags::WRITE | flags::EXECUTE)
    }
    
        // Public function — callable from other modules.
pub fn new_page(page_physical: u64, rwx: bool) -> Self {
        let mut entry = page_physical | flags::MEMORY_TYPE_WB;
        if rwx {
        ...
        // Public function — callable from other modules.
pub fn new_large_page(page_physical: u64) -> Self {
        EptEntry(page_physical | flags::READ | flags::WRITE | flags::EXECUTE 
...
impl EptManager {
    ...
    pub fn ept_pointer(&self) -> EptPointer {
        let pml4_virt = self.pml4.as_ref() as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const EptTable as u64;
    ...
    fn setup_identity_mapping(&mut self, size: usize) -> Result<()> {
        ...
        for pdpt_index in 0..pdpts_needed {
            ...
            let start_page = pdpt_index * 512;
            for pd_index in 0..512 {
                let page_number = start_page + pd_index;
                if page_number >= pages_2mb {
                    break;
                }
            ...
            let pd_virt = pd.as_ref() as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const EptTable as u64;
            let pd_physical = virt_to_physical_vmx(pd_virt);
            self.pds.push(pd);
            
            // Create PDPT entry pointing to PD (physical address!)
            let mut pdpt = Box::new(EptTable::new());
            pdpt.entries[0] = EptEntry::new_table(pd_physical);
            
            let pdpt_virt = pdpt.as_ref() as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const EptTable as u64;
            let pdpt_physical = virt_to_physical_vmx(pdpt_virt);
            self.pdpts.push(pdpt);
            
            // PML4 entry pointing to PDPT (physical address!)
            self.pml4.entries[pdpt_index] = EptEntry::new_table(pdpt_physical);
        }
...
const EptTable as u64;
            let pdpt_physical = virt_to_physical_vmx(pdpt_virt);
            self.pml4.entries[pml4_index] = EptEntry::new_table(pdpt_physical);
            self.pdpts.push(pdpt);
        }
        ...
        let pdpt_table = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { &mut *(pdpt_table_virt as *mut EptTable) };
        
        if !pdpt_table.entries[pdpt_index].is_present() {
            let pd = Box::new(EptTable::new());
            let pd_virt = pd.as_ref() as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const EptTable as u64;
            let pd_physical = virt_to_physical_vmx(pd_virt);
            pdpt_table.entries[pdpt_index] = EptEntry::new_table(pd_physical);
            self.pds.push(pd);
        }
        ...
        let pd_table = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { &mut *(pd_table_virt as *mut EptTable) };
        
        // Map as 2MB large page (aligned)
        let aligned_host = host_physical & !0x1FFFFF;
        pd_table.entries[pd_index] = EptEntry::new_large_page(aligned_host);
    ...
    pub fn setup_guest_memory_mapping(&mut self, guest_memory: &[u8]) -> Result<()> {
        ...
        crate::serial_println!("[EPT] Mapping GPA 0x0 -> HPA 0x{:X} ({} MB)",
                              guest_memory_physical, size / (1024 * 1024));
        ...
        for pdpt_index in 0..pdpts_needed {
            ...
            for pd_index in 0..512 {
                ...
                let host_physical = guest_memory_physical + (page_number * 0x200000) as u64;
                pd.entries[pd_index] = EptEntry::new_large_page(host_physical);
            }
