...
type Result<T> = core::result::Result<T, HypervisorError>;
...
pub fn init() -> Result<()> {
    ...
    crate::serial_println!("[HV] CPU Vendor: {:?}", vendor);
        ...
        // Pattern matching — Rust's exhaustive branching construct.
match vendor {
        CpuVendor::Intel => initialize_intel_vmx(),
        CpuVendor::Amd => initialize_amd_svm(),
        CpuVendor::Unknown => {
...
fn initialize_intel_vmx() -> Result<()> {
...
pub fn backend_information() -> alloc::string::String {
        // Pattern matching — Rust's exhaustive branching construct.
match virt_backend() {
