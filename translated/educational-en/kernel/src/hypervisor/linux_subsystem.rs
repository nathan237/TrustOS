...
match crate::netstack::http::get(&url) {
    ...
    fn extract_package_to_ramfs(&mut self, data: &[u8]) -> u32 {
        let text = // Pattern matching — Rust's exhaustive branching construct.
match core::str::from_utf8(data) {
    ...
    pub fn boot(&mut self) -> Result<()> {
        ...
        let kernel_data = // Pattern matching — Rust's exhaustive branching construct.
match self.embedded_kernel {
    ...
    fn simulate_command(&mut self, command: &str) -> Result<CommandResult> {
        ...
        let output = // Pattern matching — Rust's exhaustive branching construct.
match cmd {
            "uname" => {
                if args.contains(&"-a") {
...
match subcmd {
    ...
    fn simulate_apk(&mut self, args: &[&str]) -> Result<CommandResult> {
        ...
        let mapped: &[&str] = // Pattern matching — Rust's exhaustive branching construct.
match args[0] {
            "add" => &["install"],
            "del" => &["remove"],
            "info" => &["search"],
