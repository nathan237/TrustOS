...
pub fn run_all_tests() -> (usize, usize, alloc::vec::Vec<String>) {
    ...
    results.push(test_acpi_madt_lapic_address());
    results.push(test_acpi_madt_has_lapic_entry());
    results.push(test_acpi_madt_has_ioapic_entry());
    results.push(test_acpi_madt_interrupt_request_overrides());
    ...
    results.push(test_pic_eoi_clears_interrupt_handler());
    ...
    results.push(test_ioapic_interrupt_request_routing());
    ...
    results.push(test_pci_config_address_readback());
    ...
    results.push(test_pci_virtio_block_vendor());
    results.push(test_pci_virtio_block_bar0());
    results.push(test_pci_virtio_block_class());
    results.push(test_pci_virtio_block_subsystem());
    
    // ── Phase 11: VirtIO Block State Tests ───────────────────────
    results.push(test_virtio_block_default_capacity());
    results.push(test_virtio_block_io_features());
    results.push(test_virtio_block_status_reset());
    results.push(test_virtio_block_queue_size());
    results.push(test_virtio_block_capacity_readback());
    ...
    results.push(test_virtio_console_interrupt_handler_clear_on_read());
...
fn test_acpi_madt_lapic_address() -> TestResult {
    let mem = setup_acpi_guest_memory();
    let madt = 0x50080;
    // LAPIC address at offset 36 in MADT
    let lapic_address = read_u32(&mem, madt + 36);
    let passed = lapic_address == 0xFEE0_0000;
    TestResult {
        name: "ACPI MADT LAPIC address = 0xFEE00000",
        passed,
        detail: if !passed {
            Some(format!("got 0x{:08X}", lapic_address))
        } else { None },
    }
}
...
fn test_acpi_madt_has_ioapic_entry() -> TestResult {
    ...
    TestResult {
        ...
        detail: if !found_ioapic {
            Some(format!("I/O APIC entry not found (addr=0x{:08X})", ioapic_address))
        } else { None },
    }
}
...
fn test_acpi_fadt_pm_timer_port() -> TestResult {
    ...
    let pm_tmr_length = mem[fadt + 91];
    // TMR_VAL_EXT flag in flags at offset 112 (bit 4)
    let flags = read_u32(&mem, fadt + 112);
    let tmr_ext = (flags >> 4) & 1;
    
    let passed = pm_tmr == 0xB008 && pm_tmr_length == 4 && tmr_ext == 1;
    TestResult {
        name: "ACPI FADT PM timer at 0xB008 (32-bit)",
        passed,
        detail: if !passed {
            Some(format!("port=0x{:X} len={} ext={}", pm_tmr, pm_tmr_length, tmr_ext))
        } else { None },
    }
}
...
fn test_lapic_divider_values() -> TestResult {
    ...
    for &(dcr_value, expected) in &divider_map {
        let divider = // Pattern matching — Rust's exhaustive branching construct.
match dcr_value & 0xB {
...
fn test_cmos_status_registers() -> TestResult {
    let register_a = cmos_register_value(0x0A);
    let register_b = cmos_register_value(0x0B);
    let register_c = cmos_register_value(0x0C);
    let register_d = cmos_register_value(0x0D);
    
    let passed = register_a == 0x26  // divider + rate
        && register_b == 0x02        // 24h mode
        && register_c == 0x00        // no interrupts pending
        && register_d == 0x80;       // battery OK
    TestResult {
        name: "CMOS status registers A-D",
        passed,
        detail: if !passed {
            Some(format!("A=0x{:02X} B=0x{:02X} C=0x{:02X} D=0x{:02X}", register_a, register_b, register_c, register_d))
        } else { None },
    }
}
...
fn test_mmio_decode_mov_write() -> TestResult {
    ...
    let ok = // Pattern matching — Rust's exhaustive branching construct.
match d {
...
fn test_ioapic_version_register() -> TestResult {
    ...
    let maximum_redir = (ver >> 16) & 0xFF;
    let ok = version == 0x20 && maximum_redir == 23;
    TestResult {
        name: "ioapic_version_register",
        passed: ok,
        detail: if !ok { Some(format!("ver=0x{:X} max_redir={}", version, maximum_redir)) } else { None },
    }
}
...
fn test_ioapic_id_readwrite() -> TestResult {
    ...
    let id_register = ioapic.read(0x10);
    let default_id = (id_register >> 24) & 0xF;
    
    // Write a new ID
    ioapic.write(0x00, 0x00);
    ioapic.write(0x10, 0x05_00_00_00); // Set ID to 5
    let id_register = ioapic.read(0x10);
    let new_id = (id_register >> 24) & 0xF;
...
fn test_ioapic_interrupt_request_routing() -> TestResult {
    let mut ioapic = super::ioapic::IoApicState::default();
...
fn test_hpet_gcap_id() -> TestResult {
    ...
    let rev = gcap & 0xFF;
    let number_timers_m1 = (gcap >> 8) & 0x1F;
    let counter_64bit = (gcap >> 13) & 1;
    let period = (gcap >> 32) as u32;
    let ok = rev == 1 && number_timers_m1 == 2 && counter_64bit == 1 && period == 69_841_279;
    TestResult {
        name: "hpet_gcap_id_register",
        passed: ok,
        detail: if !ok { 
            Some(format!("rev={} timers-1={} 64bit={} period={}", rev, number_timers_m1, counter_64bit, period))
        } else { None },
    }
}
...
fn test_hpet_timer_config() -> TestResult {
    ...
    let periodic_capability = (t0_config >> 4) & 1; // Bit 4 = periodic capable
    ...
    let ok = periodic_capability == 1 && int_enabled == 1 && route == 2;
    TestResult {
        name: "hpet_timer_config",
        passed: ok,
        detail: if !ok {
            Some(format!("periodic_cap={} int_en={} route={} raw=0x{:X}", periodic_capability, int_enabled, route, t0_after))
        } else { None },
    }
}
...
fn test_pci_bus_defaults() -> TestResult {
    ...
    let block_exists = bus.device_exists(0, 3, 0);
    let no_dev4 = !bus.device_exists(0, 4, 0);
    let no_bus1 = !bus.device_exists(1, 0, 0);
    let ok = host_exists && isa_exists && console_exists && block_exists && no_dev4 && no_bus1;
    TestResult {
        name: "pci_bus_defaults",
        passed: ok,
        detail: if !ok { Some(format!("host={} isa={} con={} blk={} no4={} nob1={}", host_exists, isa_exists, console_exists, block_exists, no_dev4, no_bus1)) } else { None },
    }
}
...
fn test_pci_config_address_readback() -> TestResult {
    let mut bus = super::pci::PciBus::default();
...
fn test_dsdt_has_aml_content() -> TestResult {
    ...
    // Read DSDT length
    let dsdt_length = read_u32(&mem, dsdt_offset + 4) as usize;
    
    // A real DSDT with AML should be larger than just a 36-byte header
    let ok = dsdt_length > 36;
    TestResult {
        name: "dsdt_has_aml_content",
        passed: ok,
        detail: if !ok { Some(format!("dsdt_len={}", dsdt_length)) } else { None },
    }
}
...
fn test_dsdt_aml_scope_sb() -> TestResult {
    ...
    let aml = &mem[dsdt_offset + 36..dsdt_offset + dsdt_length];
...
fn test_dsdt_checksum_with_aml() -> TestResult {
    ...
    let ok = verify_checksum(&mem, dsdt_offset, dsdt_length);
    TestResult {
        name: "dsdt_checksum_with_aml",
        passed: ok,
        detail: if !ok { Some(format!("checksum failed for {} bytes", dsdt_length)) } else { None },
    }
}
...
fn test_pci_virtio_block_vendor() -> TestResult {
    let bus = super::pci::PciBus::default();
...
fn test_pci_virtio_block_bar0() -> TestResult {
    let bus = super::pci::PciBus::default();
    let bar0 = u32::from_le_bytes([
...
fn test_pci_virtio_block_class() -> TestResult {
    let bus = super::pci::PciBus::default();
...
fn test_pci_virtio_block_subsystem() -> TestResult {
    let bus = super::pci::PciBus::default();
...
fn test_virtio_block_default_capacity() -> TestResult {
...
fn test_virtio_block_io_features() -> TestResult {
...
fn test_virtio_block_status_reset() -> TestResult {
    let mut state = super::virtio_blk::VirtioBlkState::with_capacity(32768);
    // Set some state
...
fn test_virtio_block_queue_size() -> TestResult {
...
fn test_virtio_block_capacity_readback() -> TestResult {
    let mut state = super::virtio_blk::VirtioBlkState::with_capacity(1024 * 1024); // 1MB
    let capability_lo = state.io_read(0x14);
    let capability_hi = state.io_read(0x18);
    let capacity = (capability_hi as u64) << 32 | capability_lo as u64;
