...
const VENDOR_ID: usize      = 0x00; // 16-bit
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const DEVICE_ID: usize      = 0x02; // 16-bit
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const COMMAND: usize        = 0x04; // 16-bit
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const STATUS: usize         = 0x06; // 16-bit
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const REVISION_ID: usize    = 0x08; // 8-bit
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const PROG_IF: usize        = 0x09; // 8-bit
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SUBCLASS: usize       = 0x0A; // 8-bit
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const CLASS_CODE: usize     = 0x0B; // 8-bit
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const CACHE_LINE: usize     = 0x0C; // 8-bit
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const LATENCY_TIMER: usize  = 0x0D; // 8-bit
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const HEADER_TYPE: usize    = 0x0E; // 8-bit
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const BIST: usize           = 0x0F; // 8-bit
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const BAR0: usize           = 0x10; // 32-bit
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const BAR1: usize           = 0x14; // 32-bit
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SUBSYSTEM_VENDOR: usize = 0x2C; // 16-bit
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SUBSYSTEM_ID: usize   = 0x2E; // 16-bit
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const CAPABILITIES: usize   = 0x34; // 8-bit (pointer)
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const INTERRUPT_LINE: usize = 0x3C; // 8-bit
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const INTERRUPT_PIN: usize  = 0x3D; // 8-bit
}
...
match (bus, device, function) {
    ...
    pub fn read_config_data(&self, offset: u8) -> u32 {
        ...
        if let Some(config) = self.get_config_space(bus, device, function) {
            let register_offset = (register as usize) + (offset as usize);
            if register_offset + 4 <= 256 {
                read_config_u32(config, register_offset & 0xFC) // Always DWORD-aligned read
            } else {
                0xFFFF_FFFF
            }
        } else {
        ...
        if let Some(config) = self.get_config_space_mut(bus, device, function) {
            ...
            if register_offset + 4 <= 256 {
                ...
                match register_offset {
                    ...
                    0x10..=0x27 => {
                        write_config_u32(config, register_offset, value);
                    }
