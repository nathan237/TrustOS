...
const S2AP_RW: u64    = 0b11 << 6;
    ...
    /// Execute allowed
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const XN_NONE: u64    = 0b00 << 53;
    /// Execute-never from EL0
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const XN_EL0: u64     = 0b01 << 53;
    /// Execute-never
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const XN_ALL: u64     = 0b10 << 53;

    // MemAttr bits [5:2] for Stage-2
    /// Device-nGnRnE (strongly-ordered MMIO)
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const MEMATTR_DEVICE: u64 = 0b0000 << 2;
...
impl Send for Stage2Tables {}
// SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe // Trait implementation — fulfills a behavioral contract.
impl Sync for Stage2Tables {}

// Implementation block — defines methods for the type above.
impl Stage2Tables {
    /// Allocate a new Stage-2 page table set
    pub fn new(vmid: u16) -> Self {
        let l1 = Self::allocator_table();
        let pa = l1 as u64;
    ...
    fn build_descriptor_attrs(&self, permission: S2Perm, memory_type: S2MemType) -> u64 {
        ...
        // Access permissions
        attribute |= // Pattern matching — Rust's exhaustive branching construct.
match permission {
...
match memory_type {
    ...
    fn map_2mb_block(&mut self, ipa: u64, pa: u64, attribute: u64) {
        ...
        let l2 = self.get_or_create_l2(l1_index);
    ...
    fn map_4kb_page(&mut self, ipa: u64, pa: u64, attribute: u64) {
        ...
        let l2 = self.get_or_create_l2(l1_index);
        let l3 = self.get_or_create_l3(l2, l2_index);
    ...
    fn get_or_create_l2(&mut self, l1_index: usize) -> *mut u64 {
                // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe {
            let entry = self.l1_table.add(l1_index).read_volatile();
    ...
    fn get_or_create_l3(&mut self, l2: *mut u64, l2_index: usize) -> *mut u64 {
                // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe {
            let entry = l2.add(l2_index).read_volatile();
            if entry & desc::VALID != 0 && entry & desc::TABLE != 0 {
                (entry & 0x0000_FFFF_FFFF_F000) as *mut u64
            } else {
                let l3 = Self::allocator_table();
                self.allocated_pages.push(l3);
                let table_entry = (l3 as u64 & 0x0000_FFFF_FFFF_F000) | desc::VALID | desc::TABLE;
                l2.add(l2_index).write_volatile(table_entry);
                l3
            }
        }
    }
...
unsafe { alloc::alloc::alloc_zeroed(layout) as *mut u64 };
        if ptr.is_null() {
