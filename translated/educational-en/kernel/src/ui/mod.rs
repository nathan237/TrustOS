...
pub fn bottom(&self) -> i32 {
    ...
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const ZERO: Rect = Rect::new(0, 0, 0, 0);
}
...
impl EdgeInsets {
    ...
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const ZERO: EdgeInsets = EdgeInsets::all(0);
}
...
impl Label {
        // Public function — callable from other modules.
pub fn new(text: // Implementation block — defines methods for the type above.
...
impl Widget for Label {
    ...
    fn render(&self, surface: &mut GpuSurface, theme: &Theme) {
        ...
        let x = // Pattern matching — Rust's exhaustive branching construct.
match self.align {
...
pub fn new() -> Self {
        ...
        // Public function — callable from other modules.
pub fn with_placeholder(mut self, placeholder: // Implementation block — defines methods for the type above.
impl Into<String>) -> Self {
...
impl Checkbox {
        // Public function — callable from other modules.
pub fn new(label: // Implementation block — defines methods for the type above.
impl Into<String>) -> Self {
        Self {
            id: next_widget_id(),
            bounds: Rect::ZERO,
...
match self.direction {
        ...
        let main_size = // Pattern matching — Rust's exhaustive branching construct.
match self.direction {
            FlexDirection::Row => inner_w,
            FlexDirection::Column => inner_h,
        };
        
        let mut position = // Pattern matching — Rust's exhaustive branching construct.
match self.align {
            FlexAlign::Start => 0,
        ...
        for child in children.iterator_mut() {
            ...
            let (x, y, w, h) = // Pattern matching — Rust's exhaustive branching construct.
match self.direction {
                FlexDirection::Row => {
                    let cross_position = // Pattern matching — Rust's exhaustive branching construct.
...
match self.cross_align {
            ...
            position += // Pattern matching — Rust's exhaustive branching construct.
match self.direction {
