...
unsafe fn write_exact<T: Copy>(&self, value: &T) -> Result<(), UserCopyError> {
        ...
        let slice = slice::from_raw_parts(
            value as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const T as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8,
            size_of::<T>()
        );
...
pub fn copy_from_user(destination: &mut [u8], source_pointer: u64) -> Result<(), UserCopyError> {
    ...
    unsafe {
        let source = source_pointer as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8;
...
unsafe {
        Ok(core::ptr::read(ptr as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const T))
    }
}
