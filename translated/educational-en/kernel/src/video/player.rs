...
loop {
            ...
            if let Some(pixels) = decoder.next_frame() {
                ...
                Self::blit_frame(pixels, vw, vh, ox, oy, software);

                // Draw simple HUD
                Self::draw_hud(frames_played, total, fps as u32, software, sh);
    ...
    fn draw_hud(current: u32, total: u32, fps: u32, software: u32, sh: u32) {
        // Dark bar at bottom
        crate::framebuffer::fill_rect(0, sh - 20, software, 20, 0xCC000000);

        // Progress bar
        let bar_w = software - 20;
...
pub fn render_realtime(effect: &str, width: u16, height: u16, fps: u16) {
    let frame_mouse = if fps > 0 { 1000u64 / fps as u64 } else { 33 };
    ...
    crate::serial_println!("[video] Starting {} render loop ({}x{} centered on {}x{}, backbuffer={})", 
        effect, rw, rh, software, sh, crate::framebuffer::is_double_buffer_enabled());
