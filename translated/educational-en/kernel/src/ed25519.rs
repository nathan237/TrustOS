...
pub fn finalize(&mut self) -> [u8; 64] {
        ...
        for i in (0..16).rev() {
            self.buffer[112 + (15 - i)] = (bit_length >> (i * 8)) as u8;
        }
...
impl ExtPoint {
    ...
    pub fn scalar_mul(&self, scalar: &[u8; 32]) -> Self {
        ...
        for i in 0..256 {
            let byte_index = i / 8;
            let bit_index = i % 8;
            if (scalar[byte_index] >> bit_index) & 1 == 1 {
                result = result.add(&temporary);
            }
            temporary = temporary.double();
        }
        result
    }
    ...
    pub fn decompress(s: &[u8; 32]) -> Option<Self> {
        ...
        if !check_is_zero {
            // Try x * sqrt(-1)
            let negative_check = fe_add(&vx2, &u);
            let negative_bytes = fe_to_bytes(&negative_check);
            let negative_is_zero = negative_bytes.iter().all(|&b| b == 0);
            if !negative_is_zero {
...
pub fn ed25519_verify(message: &[u8], signature: &[u8; 64], public_key: &[u8; 32]) -> bool {
    ...
    let point_a = // Pattern matching — Rust's exhaustive branching construct.
match ExtPoint::decompress(public_key) {
        Some(p) => p,
        None => return false,
    };

    // Decompress R
    let point_r = // Pattern matching — Rust's exhaustive branching construct.
match ExtPoint::decompress(&r_bytes) {
        Some(p) => p,
        None => return false,
    };
