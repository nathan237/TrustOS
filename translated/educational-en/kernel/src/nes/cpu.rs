...
pub fn new() -> Self {
        ...
        // Public function — callable from other modules.
pub fn reset(&mut self, bus: &mut // Implementation block — defines methods for the type above.
impl CpuBus) {
...
impl CpuBus) -> u8 {
    ...
    fn pull16(&mut self, bus: &mut // Implementation block — defines methods for the type above.
impl CpuBus) -> u16 {
    ...
    fn read8(&mut self, bus: &mut // Implementation block — defines methods for the type above.
impl CpuBus) -> u8 {
        let v = bus.cpu_read(self.pc); self.pc = self.pc.wrapping_add(1); v
    }
    fn read16(&mut self, bus: &mut // Implementation block — defines methods for the type above.
impl CpuBus) -> u16 {
...
impl CpuBus, address: u16) -> u16 {
    ...
    fn imm(&mut self, bus: &mut // Implementation block — defines methods for the type above.
impl CpuBus) -> u8 { self.read8(bus) }
    fn zp_r(&mut self, bus: &mut // Implementation block — defines methods for the type above.
impl CpuBus) -> u8 { let a = self.read8(bus) as u16; bus.cpu_read(a) }
    fn zpx_r(&mut self, bus: &mut // Implementation block — defines methods for the type above.
...
impl CpuBus) -> (u8, u32) {
    ...
    fn aby_r(&mut self, bus: &mut // Implementation block — defines methods for the type above.
impl CpuBus) -> (u8, u32) {
        let base = self.read16(bus); let a = base.wrapping_add(self.y as u16);
        let p = if (base & 0xFF00) != (a & 0xFF00) { 1 } else { 0 };
        (bus.cpu_read(a), p)
    }
    fn izx_r(&mut self, bus: &mut // Implementation block — defines methods for the type above.
impl CpuBus) -> u8 {
    ...
    fn izy_r(&mut self, bus: &mut // Implementation block — defines methods for the type above.
impl CpuBus) -> (u8, u32) {
    ...
    fn zp_a(&mut self, bus: &mut // Implementation block — defines methods for the type above.
impl CpuBus) -> u16 { self.read8(bus) as u16 }
    fn zpx_a(&mut self, bus: &mut // Implementation block — defines methods for the type above.
...
impl CpuBus) -> u16 { let b = self.read16(bus); b.wrapping_add(self.x as u16) }
    fn aby_a(&mut self, bus: &mut // Implementation block — defines methods for the type above.
impl CpuBus) -> u16 { let b = self.read16(bus); b.wrapping_add(self.y as u16) }
    fn izx_a(&mut self, bus: &mut // Implementation block — defines methods for the type above.
impl CpuBus) -> u16 {
        let z = self.read8(bus).wrapping_add(self.x);
        let lo = bus.cpu_read(z as u16) as u16;
        let hi = bus.cpu_read(z.wrapping_add(1) as u16) as u16;
    ...
    fn izy_a(&mut self, bus: &mut // Implementation block — defines methods for the type above.
impl CpuBus) -> u16 {
        let z = self.read8(bus);
        let lo = bus.cpu_read(z as u16) as u16;
        let hi = bus.cpu_read(z.wrapping_add(1) as u16) as u16;
    ...
    fn branch(&mut self, bus: &mut // Implementation block — defines methods for the type above.
impl CpuBus, condition: bool) -> u32 {
    ...
    pub fn step(&mut self, bus: &mut // Implementation block — defines methods for the type above.
impl CpuBus) -> u32 {
        ...
        let cycles = // Pattern matching — Rust's exhaustive branching construct.
match opcode {
            // ===== LDA =====
