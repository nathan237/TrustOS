...
pub fn clear() {
    ...
    for y in 0..height {
        let row = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
fn swap_buffers_mmio_diff(addr: *mut u8, width: usize, height: usize, pitch: usize) {
    ...
    let (bb, pf) = // Pattern matching — Rust's exhaustive branching construct.
...
unsafe {
    ...
    for y in 0..height {
        ...
        let bb8 = bb_row.as_ptr() as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u64;
        let pf8 = pf_row.as_ptr() as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u64;
        let pairs = width / 2;
...
pub fn get_backbuffer_pointer() -> Option<*// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32> {
...
/// `w`: width in pixels
/// `h`: height in pixels
pub fn blit_to_framebuffer_parallel(src: *// Compile-time constant — evaluated at compilation, zero runtime cost.
...
const ParallelBlitContext as *mut u8,
    ...
    blit_rows_worker(0, copy_h, &ctx as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const ParallelBlitContext as *mut u8);
}
...
pub fn fill_rect_alpha(x: u32, y: u32, w: u32, h: u32, color: u32, alpha: u32) {
    ...
    if !ptr.is_null() {
        let stride = FRAME_BB_STRIDE.load(Ordering::Relaxed) as usize;
        for py in y1..y2 {
            let row_pointer = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
pub fn print_boot_status(msg: &str, status: BootStatus) {
    let (status_str, color) = // Pattern matching — Rust's exhaustive branching construct.
match status {
