...
unsafe {
    ...
    for y in 0..height {
        ...
        let bb8 = bb_row.as_pointer() as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u64;
        let pf8 = pf_row.as_pointer() as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u64;
        let pairs = width / 2;
...
/// `w`: width in pixels
/// `h`: height in pixels
pub fn blit_to_framebuffer_parallel(source: *// Compile-time constant — evaluated at compilation, zero runtime cost.
...
const ParallelBlitContext as *mut u8,
    ...
    blit_rows_worker(0, copy_h, &context as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const ParallelBlitContext as *mut u8);
}
