...
use super::{draw_lab_text, char_w, char_h,
            COLUMN_TEXT, COLUMN_DIM, COLUMN_ACCENT, COLUMN_GREEN, COLUMN_YELLOW, COLUMN_RED, COLUMN_PURPLE, COLUMN_CYAN};
...
pub fn draw(state: &EditorState, x: i32, y: i32, w: u32, h: u32) {
    ...
    let header_color = if !state.output_focused { COLUMN_ACCENT } else { COLUMN_DIM };
    draw_lab_text(x, y, header, header_color);
    
    // F5 hint
    let hint = "[Ctrl+S] save [Ctrl+R] run";
    let hint_x = x + w as i32 - (hint.len() as i32 * cw) - 2;
    draw_lab_text(hint_x, y, hint, COLUMN_GREEN);
    ...
    for i in scroll..end {
        ...
        draw_lab_text(x, cy, &line_number, COLUMN_DIM);
    ...
    let out_color = if state.output_focused { COLUMN_GREEN } else { COLUMN_DIM };
...
fn draw_highlighted_line(x: i32, y: i32, line: &str, _maximum_w: u32) {
    ...
    while i < len {
        ...
        if character == '"' {
            ...
            draw_lab_text(cx, y, &s, COLUMN_GREEN);
            cx += s.len() as i32 * cw;
            continue;
        }
        ...
        if character.is_ascii_digit() {
            ...
            draw_lab_text(cx, y, &num, COLUMN_CYAN);
