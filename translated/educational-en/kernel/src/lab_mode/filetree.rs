...
use super::{draw_lab_text, char_w, char_h,
            COLUMN_TEXT, COLUMN_DIM, COLUMN_ACCENT, COLUMN_CYAN, COLUMN_GREEN, COLUMN_YELLOW};
...
pub fn new() -> Self {
    ...
    fn add_directory_children(&mut self, path: &str, depth: usize) {
        ...
        let entries = crate::ramfs::with_filesystem(|fs| {
...
pub fn draw(state: &FileTreeState, x: i32, y: i32, w: u32, h: u32) {
    ...
    if state.nodes.is_empty() {
        draw_lab_text(x + 4, list_y, "Empty filesystem", COLUMN_DIM);
        return;
    }
    ...
    for i in scroll..end {
        ...
        let maximum_name_w = w as i32 - (name_x - x) - 10 * cw;
        let maximum_chars = if cw > 0 { (maximum_name_w / cw) as usize } else { 20 };
        let name = if node.name.len() > maximum_chars && maximum_chars > 3 {
            &node.name[..maximum_chars.saturating_sub(1)]
        } else {
            &node.name
        };
        ...
        if !node.is_dir && node.size > 0 {
            ...
            if sx > name_x + (name.len() as i32 * cw) + cw {
                draw_lab_text(sx, cy, &size_str, COLUMN_DIM);
            }
        }
...
match ext {
            ...
            "toml" | "cfg" | "conf" => COLUMN_GREEN,
            "md" | "txt" => COLUMN_TEXT,
            "sh" | "ps1" | "bat" => COLUMN_YELLOW,
