...
use super::{draw_lab_text, draw_progress_bar, char_w, char_h,
            COLUMN_TEXT, COLUMN_DIM, COLUMN_GREEN, COLUMN_YELLOW, COLUMN_RED, COLUMN_ACCENT, COLUMN_PANEL_BG};
...
pub fn draw(state: &HardwareState, x: i32, y: i32, w: u32, h: u32) {
    ...
    let maximum_chars = if cw > 0 { (w as i32 / cw) as usize } else { 30 };
    let mut cy = y;
    
    // ── CPU ──────────────────────────────────
    draw_lab_text(x, cy, "CPU", COLUMN_ACCENT);
    let cpu_label = format!("{}%", state.cpu_pct);
    let cpu_color = if state.cpu_pct > 80 { COLUMN_RED } else if state.cpu_pct > 50 { COLUMN_YELLOW } else { COLUMN_GREEN };
    ...
    // ── Heap ─────────────────────────────────
    draw_lab_text(x, cy, "Heap", COLUMN_ACCENT);
    ...
    let heap_color = if heap_pct > 85 { COLUMN_RED } else if heap_pct > 60 { COLUMN_YELLOW } else { COLUMN_GREEN };
    ...
    for i in start..end {
        ...
        draw_lab_text(x + 4, cy, label, COLUMN_DIM);
