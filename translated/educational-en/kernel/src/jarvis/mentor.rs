...
pub fn poll_serial() {
    ...
    // Convert to string
    let line = // Pattern matching — Rust's exhaustive branching construct.
...
fn process_command(cmd: &str) {
    ...
    if cmd.starts_with("TEACH:") {
        ...
        *BATCH_LOSS.lock() = (0.0, 0);
        ...
        let average = if count > 0 { total_loss / count as f32 } else { 0.0 };
        respond(&format!("OK:Batch ended. {} sequences, avg loss={:.4}", count, average));
    } else {
