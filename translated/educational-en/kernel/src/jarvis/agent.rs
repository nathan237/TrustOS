...
pub enum AgentAction {
    ...
    Generate { prompt: String, maximum_tokens: usize },
...
match target {
        IntrospectTarget::Architecture => {
            ...
            lines.push(format!("Context: {} tokens max", model::MAXIMUM_SEQUENCE));
            ...
            lines.push(format!("Total parameters: ~{}K", parameter_count_estimate() / 1000));
            lines.push(format!("Memory (FP32): ~{} KB", parameter_count_estimate() * 4 / 1024));
        ...
        IntrospectTarget::SourceCode { module } => {
            ...
            let source = crate::ramfs::with_filesystem(|fs| {
...
match module.as_str() {
                    ...
                    "model" => {
                        ...
                        lines.push(format!("// Persistence: /jarvis/weights.bin ({} KB)",
                            parameter_count_estimate() * 4 / 1024));
                    }
                    "inference" => {
...
fn parameter_count_estimate() -> usize {
