...
unsafe fn register_kernel_file(ptr: *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8, size: usize) {
...
fn get_kernel_data() -> Option<&'static [u8]> {
    ...
    kf.as_ref().map(|r| // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { core::slice::from_raw_parts(r.ptr as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8, r.size) })
}
