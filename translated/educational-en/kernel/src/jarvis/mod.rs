...
pub fn generate(prompt: &str, maximum_tokens: usize) -> String {
    ...
    if FULL_BRAIN_LOADED.load(Ordering::Acquire) {
        let model_guard = MODEL.lock();
        let model = // Pattern matching — Rust's exhaustive branching construct.
match model_guard.as_ref() {
        ...
        let engine = // Pattern matching — Rust's exhaustive branching construct.
...
fn micro_generate(prompt: &str, maximum_tokens: usize) -> String {
    ...
    let micro = // Pattern matching — Rust's exhaustive branching construct.
