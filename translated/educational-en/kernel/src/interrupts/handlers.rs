...
pub extern "x86-interrupt" fn apic_keyboard_handler(_stack_frame: InterruptStackFrame) {
    ...
    let status: u8 = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { status_port.read() };
    
    // Check if data is from mouse (bit 5 of status register)
    if status & 0x20 != 0 {
        let mut data_port = Port::<u8>::new(0x60);
        let _: u8 = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { data_port.read() };
    ...
    let scancode: u8 = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { port.read() };
