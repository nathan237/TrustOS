...
static RECEIVE_UDP: Mutex<BTreeMap<u16, VecDeque<UdpDatagram>>> = Mutex::new(BTreeMap::new());
...
pub fn handle_packet(data: &[u8], src_ip: [u8; 4]) {
    ...
    let mut rx = RECEIVE_UDP.lock();
    let queue = rx.entry(dst_port).or_insert_with(VecDeque::new);
    if queue.len() < 32 {
