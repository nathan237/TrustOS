...
const MAXIMUM_FILE_SIZE: usize = 32 * 1024 * 1024;
...
pub fn init(&mut self) {
    ...
    pub fn ls(&self, path: Option<&str>) -> Result<Vec<(String, FileType, usize)>, FsError> {
        let directory_path = // Pattern matching — Rust's exhaustive branching construct.
match path {
        ...
        if let Some(entry) = self.entries.get(&directory_path) {
            if entry.file_type != FileType::Directory {
    ...
    pub fn mkdir(&mut self, path: &str) -> Result<(), FsError> {
        ...
        if self.entries.contains_key(&absolute_path) {
            return Err(FsError::AlreadyExists);
        }
        
        // Get parent directory
        let parent_path = self.parent_path(&absolute_path);
        let name = self.basename(&absolute_path);
        ...
        self.entries.insert(absolute_path, FsEntry::new_directory(&name));
        Ok(())
    }
    ...
    pub fn touch(&mut self, path: &str) -> Result<(), FsError> {
        ...
        if self.entries.contains_key(&absolute_path) {
            ...
            if let Some(entry) = self.entries.get_mut(&absolute_path) {
        ...
        // Create file entry
        self.entries.insert(absolute_path, FsEntry::new_file(&name));
        Ok(())
    }
    ...
    pub fn write_file(&mut self, path: &str, content: &[u8]) -> Result<(), FsError> {
        ...
        if let Some(entry) = self.entries.get_mut(&absolute_path) {
            if entry.file_type == FileType::File {
                if content.len() > MAXIMUM_FILE_SIZE {
    ...
    pub fn append_file(&mut self, path: &str, content: &[u8]) -> Result<(), FsError> {
        ...
        if let Some(entry) = self.entries.get_mut(&absolute_path) {
            if entry.file_type == FileType::File {
                if entry.content.len() + content.len() > MAXIMUM_FILE_SIZE {
                    return Err(FsError::FileTooLarge);
                }
