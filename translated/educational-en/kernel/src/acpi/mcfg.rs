...
unsafe { &*(mcfg_virt as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const SdtHeader) };
...
unsafe { &*(entry_address as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const McfgEntryRaw) };
...
unsafe { core::ptr::read_unaligned(core::ptr::address_of!(raw.base_address)) };
        let seg = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
