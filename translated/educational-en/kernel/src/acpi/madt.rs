...
unsafe { &*(madt_virt as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const SdtHeader) };
    ...
    let madt_header = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { 
        &*((madt_virt + madt_header_offset as u64) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const MadtHeader) 
    };
