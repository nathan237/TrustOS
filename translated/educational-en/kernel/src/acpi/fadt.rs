...
unsafe { core::ptr::read_unaligned(core::ptr::addr_of!(fadt.sci_interrupt)) };
    let smi_cmd = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
unsafe { core::ptr::read_unaligned(core::ptr::addr_of!(fadt.flags)) };
    let reset_reg = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { core::ptr::read_unaligned(core::ptr::addr_of!(fadt.reset_reg)) };
    ...
    let (sleep_ctrl, sleep_status) = if has_extended && header.length >= 276 {
        let ctrl = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
