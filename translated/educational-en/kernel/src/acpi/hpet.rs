...
unsafe { core::ptr::read_unaligned(core::ptr::addr_of!(hpet.event_timer_block_id)) };
    let base_addr = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { core::ptr::read_unaligned(core::ptr::addr_of!(hpet.base_address)) };
    let min_tick = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
unsafe { core::ptr::read_volatile((virt_addr + regs::CAPABILITY_ID) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u64) };
                (cap >> 32) as u32
            }
            Err(e) => {
...
const u64) }
    ...
    pub fn set_enabled(&self, enabled: bool) {
        ...
        let config_addr = self.base_address + hhdm + regs::CONFIG;
                ...
                // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe {
            let mut config = core::ptr::read_volatile(config_addr as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u64);
            if enabled {
                config |= 1; // ENABLE_CNF
            } else {
