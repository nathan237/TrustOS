...
unsafe { &__text_start as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8 as usize };
    let end = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { &__text_end as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8 as usize };
    let size = end.saturating_sub(start);
    
    let text_bytes = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe {
        core::slice::from_raw_parts(start as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8, size)
    };
...
fn hash_rodata_section() -> [u8; 32] {
    let start = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { &__rodata_start as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8 as usize };
    let end = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { &__rodata_end as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8 as usize };
    let size = end.saturating_sub(start);
    
    let rodata_bytes = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe {
        core::slice::from_raw_parts(start as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8, size)
    };
