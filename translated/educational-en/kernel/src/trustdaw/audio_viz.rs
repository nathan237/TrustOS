...
unsafe {
            for y in 0..h {
                let row = addr.add(y * pitch) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32;
                for x in 0..w {
...
unsafe {
    ...
    fn draw_frame(&self, beat: &BeatState) {
        let (fb, w, h) = // Pattern matching — Rust's exhaustive branching construct.
