...
impl BeatStudio {
    ...
    pub fn load_funky_house(&mut self) {
        ...
        // ══════════════════════════════════════════════
        for i in 0..16 {
            let vel = // Pattern matching — Rust's exhaustive branching construct.
match i % 4 {
                0 => 85,
...
match i % 4 { 0 => 80, 2 => 100, 1 => 35, _ => 50 };
    ...
    fn draw_visual_keyboard(&self, x: u32, y: u32, w: u32, h: u32) {
        ...
        for i in 0..number_white {
            ...
            let oct_offset = i / 7;
            let key_in_oct = i % 7;
            let semitone = // Pattern matching — Rust's exhaustive branching construct.
match key_in_oct {
                0 => 0,  // C
                1 => 2,  // D
                2 => 4,  // E
...
match scancode {
                ...
                0x3B => {
                    let ct = studio.cursor_track;
                    studio.tracks[ct].waveform = // Pattern matching — Rust's exhaustive branching construct.
...
pub fn launch_matrix() -> Result<(), &'static str> {
    ...
    for f in 0..30 {
        matrix.tick();
        let intro_message = // Pattern matching — Rust's exhaustive branching construct.
match f {
    ...
    for f in 0..40 {
        matrix.tick();
        let outro_message = // Pattern matching — Rust's exhaustive branching construct.
match f {
            0..=10  => "> DISCONNECTING...",
            11..=25 => "> SIGNAL LOST",
