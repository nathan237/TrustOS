...
struct ProcessPidDirectory {
    pid: u32,
}

// Trait implementation — fulfills a behavioral contract.
impl DirectoryOperations for ProcessPidDirectory {
    ...
    fn readdir(&self) -> VfsResult<Vec<DirectoryEntry>> {
        let mut entries = vec![
            DirectoryEntry { name: String::from("."), ino: pid_directory_ino(self.pid), file_type: FileType::Directory },
...
struct ProcessRootDirectory {
    entries: Vec<ProcessEntry>,
}

// Trait implementation — fulfills a behavioral contract.
impl DirectoryOperations for ProcessRootDirectory {
    fn lookup(&self, name: &str) -> VfsResult<Ino> {
        ...
        if let Ok(pid) = name.parse::<u32>() {
            ...
            if exists {
                return Ok(pid_directory_ino(pid));
            }
        }
        Err(VfsError::NotFound)
    }
...
pub fn new() -> VfsResult<Self> {
        let entries = vec![
            ...
            ProcessEntry { name: String::from("cmdline"), file_type: ProcFileType::Cmdline, ino: 7 },
            ProcessEntry { name: String::from("stat"), file_type: ProcFileType::Stat, ino: 8 },
        ];
        
        Ok(Self { entries })
    }
    
    fn find_entry(&self, ino: Ino) -> Option<&ProcessEntry> {
...
impl FileSystem for ProcFs {
    ...
    fn get_file(&self, ino: Ino) -> VfsResult<Arc<dyn FileOperations>> {
        ...
        if let Some((pid, index)) = is_pid_file_ino(ino) {
            let ft = // Pattern matching — Rust's exhaustive branching construct.
match index {
    ...
    fn get_directory(&self, ino: Ino) -> VfsResult<Arc<dyn DirectoryOperations>> {
        ...
        // Per-PID directory?
        if let Some(pid) = is_pid_directory_ino(ino) {
            return Ok(Arc::new(ProcessPidDirectory { pid }));
        }
    ...
    fn status(&self, ino: Ino) -> VfsResult<Stat> {
        ...
        // PID directory?
        if is_pid_directory_ino(ino).is_some() {
            return Ok(Stat {
                ino,
                file_type: FileType::Directory,
                mode: 0o555,
