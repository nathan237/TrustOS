...
impl IwlTfd {
    ...
    fn number_tbs(&self) -> usize {
        ...
        let ptr = self.__reserved.as_ptr() as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32;
...
unsafe { core::arch::asm!("out dx, al", in("dx") 0x80u16, in("al") 0u8, options(nomem, nostack)); }
    ...
    fn poll_rx(&mut self) {
        ...
        let hardware_write = {
            let rxq = // Pattern matching — Rust's exhaustive branching construct.
match self.rx_queue.as_ref() {
        ...
        let read = // Pattern matching — Rust's exhaustive branching construct.
match self.rx_queue.as_ref() {
            Some(q) => q.read_ptr,
            None => return,
        };
...
pub fn debug_read_csr(offset: u32) -> Option<u32> {
    ...
    for dev in &devices {
        if dev.vendor_id == INTEL_VENDOR && IWL_SUPPORTED_IDS.contains(&dev.device_id) {
            ...
            let virt = // Pattern matching — Rust's exhaustive branching construct.
match crate::memory::map_mmio(phys, 0x2000) {
                ...
                Err(_) => return None,
                        ...
                        // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe {
                let ptr = (virt + offset as usize) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32;
...
fn debug_register_read(base: usize, offset: u32) -> u32 {
        // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { core::ptr::read_volatile((base + offset as usize) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32) }
}
