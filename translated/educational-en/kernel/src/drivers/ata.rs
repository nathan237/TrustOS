...
unsafe { Port::<u8>::new(base + 7).read() };
    ...
    if wait_ready(base).is_err() {
        let lba_mid = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { Port::<u8>::new(base + 4).read() };
        let lba_high = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
unsafe {
    ...
    for i in 0..256 {
        data[i] = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
match channel {
    ...
    for _ in 0..count {
        ...
        for _ in 0..256 {
            let word = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { data_port.read() };
