...
unsafe fn mmio_rd32(base: u64, offset: u32) -> u32 {
    core::ptr::read_volatile((base + offset as u64) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32)
}
...
unsafe fn read_vram_size(mmio: u64) -> u64 {
    ...
    let size_mb = // Pattern matching — Rust's exhaustive branching construct.
match cfg1 & 0xFFF {
        s if s > 0 => s as u64,
        _ => {
...
match memory::map_mmio(vram_physical, map_size) {
    ...
    let (chipset_id, stepping) = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { read_gpu_identity(mmio_virt) };
    
    // Step 6: Read VRAM size
    let vram_size = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
