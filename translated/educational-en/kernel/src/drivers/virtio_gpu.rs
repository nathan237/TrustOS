...
unsafe { core::ptr::read_volatile(self.base.add(offset as usize)) }
    ...
    fn read16(&self, offset: u32) -> u16 {
                // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { core::ptr::read_volatile(self.base.add(offset as usize) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u16) }
    }
...
unsafe fn read_at<T: Copy>(&self, offset: usize) -> T {
        core::ptr::read_volatile(self.virt.add(offset) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const T)
    }
...
loop {
        ...
        let response_type = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
unsafe { dma.write_at(0, &cmd); }
        ...
        let dma = self.dma_buffer.as_ref().unwrap();
        let response: GpuResponseDisplayInformation = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
unsafe { dma.write_at(256, &flush_command); }
        ...
        // Check responses
        let dma = self.dma_buffer.as_ref().unwrap();
        let t_response = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { dma.read_at::<GpuControllerHeader>(512) }.controller_type;
        let f_response = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
unsafe { dma.write_at(0, &cmd); }
    ...
    let dma = gpu.dma_buffer.as_ref()?;
    let information: VirglCapsetInformation = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
