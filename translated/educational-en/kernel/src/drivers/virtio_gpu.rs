...
loop {
        ...
        let resp_type = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
unsafe { dma.write_at(0, &cmd); }
        ...
        let dma = self.dma_buf.as_ref().ok_or("DMA buffer not initialized")?;
        let resp: GpuResponseDisplayInformation = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
unsafe { dma.write_at(0, &cmd); }
    ...
    let dma = gpu.dma_buf.as_ref()?;
    let info: VirglCapsetInformation = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
