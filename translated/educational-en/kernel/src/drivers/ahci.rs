...
const AHCI_MMIO_SIZE: usize = 0x2000;  // 8KB
    ...
    let abar_virt = // Pattern matching — Rust's exhaustive branching construct.
...
unsafe { &mut *(hba.ports.as_mut_ptr().add(i)) };
            ...
            if det == 3 && ipm == 1 {
                let sig = port.sig;
                let device_type = // Pattern matching — Rust's exhaustive branching construct.
match sig {
...
const _ as u64);
                    ...
                    let framebuffer_physical = virt_to_physical(&*mem.fis as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const _ as u64);
