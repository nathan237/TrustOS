...
fn load_sdma(mmio: u64, engine: usize, fw: &[u8]) -> Result<(), &'static str> {
    ...
    let (address_register, data_register, f32_cntl) = // Pattern matching — Rust's exhaustive branching construct.
match engine {
...
pub fn init(mmio_base: u64) {
    ...
    let smu_ver = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
match load_sdma(mmio_base, 1, ucode) {
    ...
    let grbm_post = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { mmio_read32(mmio_base, regs::GRBM_STATUS) };
