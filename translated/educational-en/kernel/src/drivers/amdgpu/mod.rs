...
pub mod family {
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const FAMILY_UNKNOWN:  u32 = 0;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const FAMILY_NV:       u32 = 143; // Navi 10 / RDNA 1
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const FAMILY_NV14:     u32 = 144; // Navi 14 / RDNA 1 (cut-down)
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const FAMILY_AI:       u32 = 141; // Vega 10 / GCN 5
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const FAMILY_RV:       u32 = 142; // Raven Ridge APU
}
...
match self.device_id {
            0x731F => // Pattern matching — Rust's exhaustive branching construct.
match self.revision {
...
unsafe fn mmio_read32(mmio_base: u64, offset: u32) -> u32 {
    ...
    core::ptr::read_volatile(address as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32)
}
...
#[inline]
pub // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe fn mmio_read_indirect(mmio_base: u64, reg: u32) -> u32 {
...
#[inline]
pub // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe fn mmio_write_indirect(mmio_base: u64, reg: u32, value: u32) {
    mmio_write32(mmio_base, regs::MM_INDEX, reg);
...
unsafe {
        ...
        let gc_information = mmio_read32(mmio_base, regs::GC_VERSION);
        crate::serial_println!("[AMDGPU] GC_VERSION raw: {:#010X}", gc_information);
        
        // Read chip identity from BIF/NBIO (should work even pre-init)
        let rlc_gpu_id = mmio_read32(mmio_base, regs::RLC_PAGE_CNTL);
        ...
        let gc_major = (gc_information >> 16) & 0xFF;
        let gc_minor = (gc_information >> 8) & 0xFF;
        ...
        external_rev = gc_information; // Store full GC version as external rev
...
unsafe {
        ...
        let mc_vm_framebuffer_location = mmio_read32(mmio_base, regs::MC_VM_FRAMEBUFFER_LOCATION_BASE);
        let mc_vm_framebuffer_top = mmio_read32(mmio_base, regs::MC_VM_FRAMEBUFFER_LOCATION_TOP);
        crate::serial_println!("[AMDGPU] MC_VM_FB_LOCATION BASE={:#010X} TOP={:#010X}", 
            mc_vm_framebuffer_location, mc_vm_framebuffer_top);
        
        if mc_vm_framebuffer_top > mc_vm_framebuffer_location && mc_vm_framebuffer_location != 0xFFFFFFFF {
            // These are in 1MB units (shifted by 24 bits in the register)
            let base_mb = (mc_vm_framebuffer_location & 0xFFFF) as u64;
            let top_mb = (mc_vm_framebuffer_top & 0xFFFF) as u64;
...
fn vram_type_string(mmio_base: u64) -> &'static str {
        // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe {
        let mc_sequence = mmio_read32(mmio_base, regs::MC_SEQUENCE_MISC0);
        crate::serial_println!("[AMDGPU] MC_SEQ_MISC0: {:#010X}", mc_sequence);
        
        // Bits 31:28 encode memory type on some chips
        match (mc_sequence >> 28) & 0xF {
            1 => "DDR2",
            2 => "DDR3",
            3 => "GDDR3",
            4 => "GDDR4",
...
match probe() {
    ...
    crate::log!("[AMDGPU] BAR2 (VRAM):     phys={:#014X} size={:#X} ({} MB)", 
        vram_physical, vram_ap_size, vram_ap_size / (1024 * 1024));
    crate::log!("[AMDGPU] BAR4 (Doorbell): phys={:#014X} size={:#X} ({} KB)", 
        doorbell_physical, doorbell_size, doorbell_size / 1024);
    ...
    crate::log!("[AMDGPU] Mapping MMIO: {:#X} -> {} pages...", mmio_physical, map_size / 4096);
    
    let mmio_virt = // Pattern matching — Rust's exhaustive branching construct.
match memory::map_mmio(mmio_physical, map_size) {
        Ok(virt) => {
    ...
    let family_name = // Pattern matching — Rust's exhaustive branching construct.
match asic_family {
