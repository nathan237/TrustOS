...
unsafe {
            ...
            let connector_type = // Pattern matching — Rust's exhaustive branching construct.
match transmit_type {
...
pub fn init(mmio_base: u64) {
    ...
    let dcn_ver = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { mmio_read32(mmio_base, dcn::DCN_VERSION) };
    ...
    let dmcub_status = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { mmio_read32(mmio_base, dcn::DMCUB_STATUS) };
    ...
    for connection in &connectors {
        let status_str = // Pattern matching — Rust's exhaustive branching construct.
