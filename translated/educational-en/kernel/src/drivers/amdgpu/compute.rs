...
unsafe { mmio_read32(mmio_base, regs::GRBM_STATUS) };
    let cp_me_cntl = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
loop {
        let current_fence = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { core::ptr::read_volatile(fence_pointer) };
        ...
        if elapsed >= GPU_TIMEOUT_ITERS {
            ...
            let grbm = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { mmio_read32(mmio, regs::GRBM_STATUS) };
            let rptr = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
pub fn verify_results(agent: AgentKind, number_elements: u32, fill_value: u32) -> (u32, u32) {
    ...
    let data_pointer = state.data_virt as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32;
    let mut pass = 0u32;
    let mut fail = 0u32;
