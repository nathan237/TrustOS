...
const u8) };
        let b_state = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { ptr::read_volatile((buffer_virt + 4) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8) };
...
fn dfu_abort(controller: &mut XhciController, slot_id: u8) -> bool {
    {
        let mut rings = super::xhci::SLOT_RINGS.lock();
        let sr = // Pattern matching — Rust's exhaustive branching construct.
match rings.get_mut(slot_id as usize).and_then(|r| r.as_mut()) {
            Some(r) => r,
            None => return false,
        };
