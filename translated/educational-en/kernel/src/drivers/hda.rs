...
const GCAP: u32     = 0x00;  // 16-bit: Global Capabilities
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const VMIN: u32     = 0x02;  // 8-bit: Minor Version
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const VMAJ: u32     = 0x03;  // 8-bit: Major Version
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const OUTPAY: u32   = 0x04;  // 16-bit: Output Payload Capability
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const INPAY: u32    = 0x06;  // 16-bit: Input Payload Capability
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GCTL: u32     = 0x08;  // 32-bit: Global Control
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const WAKEEN: u32   = 0x0C;  // 16-bit: Wake Enable
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const STATESTS: u32 = 0x0E;  // 16-bit: State Change Status
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GSTS: u32     = 0x10;  // 16-bit: Global Status
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const INTCTL: u32   = 0x20;  // 32-bit: Interrupt Control
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const INTSTS: u32   = 0x24;  // 32-bit: Interrupt Status
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const WALCLK: u32   = 0x30;  // 32-bit: Wall Clock Counter
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SSYNC: u32    = 0x38;  // 32-bit: Stream Synchronization

    // CORB registers
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const CORBLBASE: u32 = 0x40;  // 32-bit: CORB Lower Base Address
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const CORBUBASE: u32 = 0x44;  // 32-bit: CORB Upper Base Address
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const CORBWP: u32    = 0x48;  // 16-bit: CORB Write Pointer
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const CORBRP: u32    = 0x4A;  // 16-bit: CORB Read Pointer
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const CORBCTL: u32   = 0x4C;  // 8-bit: CORB Control
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const CORBSTS: u32   = 0x4D;  // 8-bit: CORB Status
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const CORBSIZE: u32  = 0x4E;  // 8-bit: CORB Size

    // RIRB registers
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const RIRBLBASE: u32 = 0x50;  // 32-bit: RIRB Lower Base Address
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const RIRBUBASE: u32 = 0x54;  // 32-bit: RIRB Upper Base Address
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const RIRBWP: u32    = 0x58;  // 16-bit: RIRB Write Pointer
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const RINTCNT: u32   = 0x5A;  // 16-bit: Response Interrupt Count
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const RIRBCTL: u32   = 0x5C;  // 8-bit: RIRB Control
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const RIRBSTS: u32   = 0x5D;  // 8-bit: RIRB Status
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const RIRBSIZE: u32  = 0x5E;  // 8-bit: RIRB Size

    // Immediate Command (alternative to CORB/RIRB)
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const IC: u32  = 0x60;  // 32-bit: Immediate Command
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const IR: u32  = 0x64;  // 32-bit: Immediate Response
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const ICS: u32 = 0x68;  // 16-bit: Immediate Command Status

    // DMA Position Buffer
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const DPLBASE: u32 = 0x70;  // 32-bit: DMA Position Lower Base
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const DPUBASE: u32 = 0x74;  // 32-bit: DMA Position Upper Base
...
mod sd {
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const CTL: u32    = 0x00;  // 24-bit (3 bytes): Stream Control
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const STS: u32    = 0x03;  // 8-bit: Stream Status
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const LPIB: u32   = 0x04;  // 32-bit: Link Position In Buffer
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const CBL: u32    = 0x08;  // 32-bit: Cyclic Buffer Length
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const LVI: u32    = 0x0C;  // 16-bit: Last Valid Index
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const FIFOS: u32  = 0x10;  // 16-bit: FIFO Size
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const FMT: u32    = 0x12;  // 16-bit: Stream Format
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const BDLPL: u32  = 0x18;  // 32-bit: BDL Lower Address
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const BDLPU: u32  = 0x1C;  // 32-bit: BDL Upper Address
}

/// Global Control bits
mod gctl {
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const CRST: u32   = 1 << 0;   // Controller Reset
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const FCNTRL: u32 = 1 << 1;   // Flush Control
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const UNSOL: u32  = 1 << 8;   // Accept Unsolicited Responses
}

/// Stream Control bits
mod sctl {
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SRST: u32 = 1 << 0;     // Stream Reset
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const RUN: u32  = 1 << 1;     // Stream Run (DMA enable)
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const IOCE: u32 = 1 << 2;     // Interrupt On Completion Enable
    // Bits [23:20] = Stream Number (tag)
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const STREAM_TAG_SHIFT: u32 = 20;
}

/// Stream Status bits
mod ssts {
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const BCIS: u8 = 1 << 2;   // Buffer Completion Interrupt Status
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const FIFOE: u8 = 1 << 3;  // FIFO Error
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const DESE: u8 = 1 << 4;   // Descriptor Error
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const FIFORDY: u8 = 1 << 5; // FIFO Ready
}
...
const GET_CONNECTION_LIST: u32        = 0xF02;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GET_CONNECTION_SELECT: u32      = 0xF01;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GET_PIN_CONTROL: u32      = 0xF07;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GET_CONFIG_DEFAULT: u32   = 0xF1C;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GET_EAPD: u32             = 0xF0C;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GET_POWER_STATE: u32      = 0xF05;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GET_CHANNEL_STREAM: u32   = 0xF06;

    // GET verbs (4-bit verb, 16-bit payload) — use with set_verb_16()
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GET_AMP_GAIN: u32         = 0xB00;  // 4-bit! payload: bit15=out, bit13=left, bits3:0=idx
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GET_STREAM_FORMAT: u32    = 0xA00;  // 4-bit! same as SET but returns current value

    // SET verbs (12-bit verb, 8-bit payload)
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SET_CONNECTION_SELECT: u32      = 0x701;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SET_POWER_STATE: u32      = 0x705;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SET_CHANNEL_STREAM: u32   = 0x706;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SET_PIN_CONTROL: u32      = 0x707;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SET_EAPD: u32             = 0x70C;

    // SET/GET verbs (4-bit verb, 16-bit payload) — use with set_verb_16()
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SET_AMP_GAIN_MUTE: u32    = 0x300;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SET_STREAM_FORMAT: u32    = 0x200;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SET_COEF_INDEX: u32       = 0x500;  // Processing Coefficient Index
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SET_PROCESS_COEF: u32        = 0x400;  // Processing Coefficient Data
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GET_COEF_INDEX: u32       = 0xD00;  // Read back coef index
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GET_PROCESS_COEF: u32        = 0xC00;  // Read back coef data

    // GPIO verbs (12-bit verb, 8-bit data) — sent to AFG node (NID 1)
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SET_GPIO_DATA: u32        = 0x715;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SET_GPIO_MASK: u32        = 0x716;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SET_GPIO_DIRECTORY: u32         = 0x717;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GET_GPIO_DATA: u32        = 0xF15;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GET_GPIO_MASK: u32        = 0xF16;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GET_GPIO_DIRECTORY: u32         = 0xF17;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const PARAMETER_GPIO_COUNT: u32    = 0x11;  // GPIO Count parameter

    // Parameters (used with GET_PARAMETER)
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const PARAMETER_VENDOR_ID: u32     = 0x00;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const PARAMETER_REVISION: u32      = 0x02;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const PARAMETER_NODE_COUNT: u32    = 0x04;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const PARAMETER_FN_GROUP_TYPE: u32 = 0x05;
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const PARAMETER_AUDIO_CAPS: u32    = 0x09;  // Audio Widget Capabilities
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const PARAMETER_PCM_RATES: u32     = 0x0A;  // Supported PCM sizes/rates
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const PARAMETER_STREAM_FMTS: u32   = 0x0B;  // Supported stream formats
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const PARAMETER_PIN_CAPS: u32      = 0x0C;  // Pin Capabilities
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const PARAMETER_AMP_IN_CAPS: u32   = 0x0D;  // Input Amp Capabilities
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const PARAMETER_CONNECTION_LIST_LENGTH: u32 = 0x0E;  // Connection List Length
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const PARAMETER_POWER_STATES: u32  = 0x0F;  // Supported Power States
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const PARAMETER_AMP_OUT_CAPS: u32  = 0x12;  // Output Amp Capabilities
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const PARAMETER_VOL_KNOB_CAPS: u32 = 0x13;
}
...
unsafe fn read8(&self, offset: u32) -> u8 {
        core::ptr::read_volatile((self.mmio_base + offset as u64) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8)
    }
...
unsafe fn write32(&self, offset: u32, val: u32) {
    ...
    pub fn init(dev: &crate::pci::PciDevice) -> Result<Self, &'static str> {
        ...
        let mmio_base = bar0_physical + hhdm;
...
unsafe {
    ...
    fn send_verb(&mut self, codec: u8, nid: u16, verb: u32, payload: u32) -> Result<u32, &'static str> {
        ...
        unsafe {
            ...
            let corb_pointer = self.corb_virt as *mut u32;
            core::ptr::write_volatile(corb_pointer.add(new_wp as usize), cmd);
            ...
            for _ in 0..10000 {
                ...
                if rirb_wp != self.rirb_rp {
                    ...
                    let rirb_pointer = self.rirb_virt as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u64;
                    let response = core::ptr::read_volatile(rirb_pointer.add(self.rirb_rp as usize));
        ...
        for &caddr in &codecs {
            ...
            for fg_nid in start_nid..(start_nid + number_nodes) {
                ...
                for nid in w_start..(w_start + w_count) {
                    ...
                    if connection_length > 0 && !long_form {
    ...
    fn find_output_paths(&mut self) {
        ...
        let pins: Vec<(u16, u32, Vec<u16>)> = self.widgets.iter()
            ...
            .filter(|w| {
                ...
                let connection_ok = connectivity == 0 || connectivity == 2; // Jack or Fixed
                (connectivity != 1 && is_output) || connection_ok
            })
    ...
    fn trace_to_dac(&self, nid: u16, visited: &mut Vec<u16>) -> Option<Vec<u16>> {
        ...
        // Try each connection
        for &connection_nid in &widget.connections {
            if let Some(mut path) = self.trace_to_dac(connection_nid, visited) {
                path.insert(0, nid);
                return Some(path);
            }
        }
    ...
    fn setup_output_stream(&mut self) -> Result<(), &'static str> {
        ...
        if needs_quirks {
            ...
            if is_ad198x {
                let _ = self.set_verb_16(codec, 1, verb::SET_PROCESS_COEF, 0x0008);
        ...
        let all_path_information: Vec<(u16, Vec<u16>)> = self.output_paths.iter()
        ...
        for out_path in &all_paths {
            let path_widget_information: Vec<(u16, WidgetType, Vec<u16>)> = out_path.iter()
                .filter_map(|&nid| self.widgets.iter().find(|w| w.nid == nid)
                    .map(|w| (nid, w.widget_type, w.connections.clone())))
                .collect();

            for (nid, _wtype, connections) in &path_widget_information {
        ...
        unsafe {
            ...
            for i in 0..number_frags {
                let entry = &mut *bdl.add(i as usize);
                entry.address = buffer_physical + (i as u64) * (frag_size as u64);
            ...
            self.write8(sd_base + sd::CTL + 2, controller_high as u8);
...
pub fn play_tone(freq_hz: u32, duration_ms: u32) -> Result<(), &'static str> {
    ...
    crate::serial_println!("[HDA] play_tone: LPIB before={} early={} after={} target={}",
        position_before, position_early, position_after, target);
    if position_early == 0 && position_after == 0 {
...
pub fn diag() -> String {
    let mut s = String::new();
    let mut hda = HDA.lock();
    let ctrl = // Pattern matching — Rust's exhaustive branching construct.
match hda.as_mut() {
        Some(c) => c,
        None => {
...
match hda.as_mut() {
    ...
    for (nid, cfg) in &pin_widgets {
        ...
        let connectivity = // Pattern matching — Rust's exhaustive branching construct.
match (*cfg >> 30) & 0x3 {
            0 => "Jack",
            1 => "None",
            2 => "Fixed",
        ...
        s.push_str(&format!("         pin_ctl={:#04X}(out={} hp={}) eapd={:#04X}(on={} has={})\n",
            pin_controller, out_en, hp_en, eapd, eapd_en, has_eapd));
...
pub fn set_volume(level: u8) -> Result<(), &'static str> {
    ...
    *VOLUME.lock() = level;
    ...
    let maximum_gain = ctrl.widgets.iter()
        ...
        .unwrap_or(39);
    let maximum_gain = if maximum_gain == 0 { 39 } else { maximum_gain };
    let gain = ((level as u32) * (maximum_gain as u32) / 100) as u16;
...
pub fn parse_wav(data: &[u8]) -> Result<WavInformation, &'static str> {
    ...
    Ok(WavInformation { channels, sample_rate, bits_per_sample, data_offset, data_size })
}
...
pub fn play_effect(effect: SoundEffect) -> Result<(), &'static str> {
    let tones: Vec<(u32, u32, i16)> = // Pattern matching — Rust's exhaustive branching construct.
match effect {
...
pub fn rest(dur: u8) -> Self {
    ...
    pub fn freq_hz(&self) -> u32 {
        ...
        let base_frequency = SEMI_RATIO[semi] * 440 / 1000;
        
        if octave_offset >= 0 {
            base_frequency << octave_offset as u32
        } else {
            base_frequency >> (-octave_offset) as u32
        }
    }
