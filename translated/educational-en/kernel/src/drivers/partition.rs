...
match type_byte {
    ...
    pub fn from_gpt_guid(guid: &[u8; 16]) -> Self {
        ...
        const MOUSE_BASIC_DATA: [u8; 16] = [
            0xA2, 0xA0, 0xD0, 0xEB, 0xE5, 0xB9, 0x33, 0x44,
            0x87, 0xC0, 0x68, 0xB6, 0xB7, 0x26, 0x99, 0xC7
        ];
        
        // Microsoft Reserved: E3C9E316-0B5C-4DB8-817D-F92DF00215AE
        const MOUSE_RESERVED: [u8; 16] = [
        ...
        else if guid == &MOUSE_BASIC_DATA { Self::MicrosoftBasicData }
        else if guid == &MOUSE_RESERVED { Self::MicrosoftReserved }
        else if guid == &LINUX_FILESYSTEM { Self::LinuxFilesystemGpt }
...
const GptHeader) };
    ...
    let sectors_needed = (number_entries as usize + entries_per_sector - 1) / entries_per_sector;
...
pub fn read_from_ahci(port: u8) -> Result<PartitionTable, &'static str> {
    ...
    let disk_information = ahci::get_port_information(port).ok_or("Port not found")?;
    ...
    let read_fn = |lba: u64, buffer: &mut [u8]| -> Result<(), &'static str> {
        ...
        ahci::read_sectors(port, lba, 1, &mut sector_buffer)?;
        buffer[..SECTOR_SIZE].copy_from_slice(&sector_buffer);
        Ok(())
    };
