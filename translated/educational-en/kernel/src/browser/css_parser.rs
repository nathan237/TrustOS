...
match self.current_char()? {
    ...
    fn parse_attribute_selector(&mut self) -> SelectorPart {
        ...
        if self.consume_char(']') {
            return SelectorPart::Attribute(attribute_name, None);
        }
        ...
        SelectorPart::Attribute(attribute_name, Some(value))
    }
...
match self.current_char()? {
    ...
    fn parse_number_with_unit(&mut self) -> (f32, Option<LengthUnit>) {
        ...
        let unit = // Pattern matching — Rust's exhaustive branching construct.
match unit_str {
