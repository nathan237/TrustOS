...
match subcmd {
        "mmio" => {
            let base = args.get(1)
                .and_then(|s| parse_hex_or_decrypt(s))
                .unwrap_or(0);
            let size = args.get(2)
                .and_then(|s| parse_hex_or_decrypt(s))
                .unwrap_or(0);
        ...
        "irq" => {
            irq::scan_interrupt_request_topology()
        }
        "gpio" => {
        ...
        "dtb" => {
            command_dtb_information()
        }
        "verify" => {
            command_verify_hardware()
        }
        "auto" => {
...
fn parse_hex_or_decrypt(s: &str) -> Option<u64> {
...
unsafe {
            if let Some(parsed) = dtb_parser::parse_dtb(dtb_address as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8) {
...
fn command_verify_hardware() -> String {
...
const u8) {
                ...
                for device in &parsed.devices {
                    ...
                    let ptr = device.register_base as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32;
