...
#[derive(Debug, Clone)]
// Public structure — visible outside this module.
pub struct SimpleFramebufferInformation {
    pub base: u64,
    pub size: u64,
    pub width: u32,
    pub height: u32,
...
pub struct ParsedDtb {
    ...
    pub simplefb: Option<SimpleFramebufferInformation>,
...
unsafe fn be32(ptr: *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8) -> u32 {
...
unsafe fn be64(ptr: *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8) -> u64 {
...
unsafe fn read_cstr(ptr: *// Compile-time constant — evaluated at compilation, zero runtime cost.
...
unsafe fn parse_dtb(dtb_pointer: *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8) -> Option<ParsedDtb> {
    ...
    let maximum_offset = totalsize - off_struct;
    let mut path_stack: Vec<String> = Vec::new();
    let mut current_path = String::new();

    // Per-node property tracking
    let mut cur_compatible = String::new();
    let mut cur_register_base: u64 = 0;
    let mut cur_register_size: u64 = 0;
...
match token {
            ...
            FDT_END_NODE => {
                ...
                let is_device = cur_register_base != 0 || cur_register_size != 0;

                if in_memory_node && (cur_register_base != 0 || cur_register_size != 0) {
                    result.memory.push((cur_register_base, cur_register_size));
                } else if in_reserved_memory && !current_path.ends_with("reserved-memory") && cur_register_base != 0 {
                    result.reserved.push(ReservedRegion {
                        name: path_stack.last().cloned().unwrap_or_default(),
                        base: cur_register_base,
                        size: cur_register_size,
                        no_map: false, // TODO: check for no-map property
                    });
                } else if in_simplefb {
                    if sfb_base == 0 { sfb_base = cur_register_base; }
                    if sfb_size == 0 { sfb_size = cur_register_size; }
                    if sfb_width > 0 && sfb_height > 0 {
                        result.simplefb = Some(SimpleFramebufferInformation {
                            base: sfb_base,
                            size: sfb_size,
                            width: sfb_width,
                            height: sfb_height,
            ...
            FDT_PROP => {
                if offset + 8 > maximum_offset { break; }
                let prop_length = be32(struct_base.add(offset as usize));
                ...
                offset = align4(offset + prop_length);

                // Handle properties
                match prop_name.as_str() {
                    "model" if path_stack.len() <= 1 => {
                        result.model = read_cstr(prop_data, prop_length as usize);
                    }
                    "compatible" => {
                        let compat = read_cstr(prop_data, prop_length as usize);
                        if path_stack.len() <= 1 {
                            // Root compatible — split by NUL
                            let bytes = core::slice::from_raw_parts(prop_data, prop_length as usize);
                    ...
                    "reg" => {
                        ...
                        if prop_length >= 4 {
                            if address_cells == 2 && prop_length >= 8 {
                                cur_register_base = be64(prop_data);
                            } else {
                                cur_register_base = be32(prop_data) as u64;
                            }

                            let size_offset = (address_cells * 4) as usize;
                            if (size_offset + 4) <= prop_length as usize {
                                if size_cells == 2 && (size_offset + 8) <= prop_length as usize {
                                    cur_register_size = be64(prop_data.add(size_offset));
                                } else {
                                    cur_register_size = be32(prop_data.add(size_offset)) as u64;
                                }
                            }
                        }
                        ...
                        // SimpleFB reg
                        if in_simplefb {
                            sfb_base = cur_register_base;
                            sfb_size = cur_register_size;
                        }
                    }
                    "#address-cells" => {
                        if prop_length >= 4 {
                            address_cells = be32(prop_data);
                        }
                    }
                    "#size-cells" => {
                        if prop_length >= 4 {
                    ...
                    "status" => {
                        cur_status = read_cstr(prop_data, prop_length as usize);
                    }
                    "interrupts" | "interrupts-extended" => {
                        let count = prop_length / 4;
                        for i in 0..count {
                    ...
                    "bootargs" if in_chosen => {
                        result.bootargs = read_cstr(prop_data, prop_length as usize);
                    }
                    // SimpleFB properties
                    "width" if in_simplefb => {
                        if prop_length >= 4 { sfb_width = be32(prop_data); }
                    }
                    "height" if in_simplefb => {
                        if prop_length >= 4 { sfb_height = be32(prop_data); }
                    }
                    "stride" if in_simplefb => {
                        if prop_length >= 4 { sfb_stride = be32(prop_data); }
                    }
                    "format" if in_simplefb => {
                        sfb_format = read_cstr(prop_data, prop_length as usize);
                    }
                    _ => {}
                }
            }
...
pub fn format_dtb_report(dtb: &ParsedDtb) -> String {
    ...
    if !dtb.devices.is_empty() {
        ...
        for device in &dtb.devices {
            let status_icon = // Pattern matching — Rust's exhaustive branching construct.
