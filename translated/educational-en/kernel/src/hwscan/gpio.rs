...
fn decode_bcm_gpio(base: u64, num_pins: u32) -> String {
    ...
    for pin in 0..maximum_pins {
        // Function select: GPFSEL0-5 at offset 0x00-0x14
        let fsel_register = (pin / 10) as u64;
        let fsel_bit = ((pin % 10) * 3) as u32;
        
        if let Some(fsel) = safe_read(base + fsel_register * 4) {
            ...
            for &(dpin, dalt, desc) in DEBUG_ALT_FUNCTIONS {
                ...
                let alt_to_func = // Pattern matching — Rust's exhaustive branching construct.
match dalt {
                    0 => 4, // ALT0
                    1 => 5, // ALT1
                    2 => 6, // ALT2
...
fn decode_qcom_gpio(base: u64, num_pins: u32) -> String {
    ...
    for pin in 0..max {
        ...
        if let Some(cfg) = safe_read(configuration_address) {
            let func = (cfg >> 2) & 0xF;
            let directory_out = (cfg >> 9) & 1;
            let pull = cfg & 0x3;
            ...
            let direction = if directory_out == 1 { "OUT" } else { "IN" };
            let pull_str = // Pattern matching — Rust's exhaustive branching construct.
match pull {
                0 => "",
                1 => "PD",
                2 => "KEEPER",
