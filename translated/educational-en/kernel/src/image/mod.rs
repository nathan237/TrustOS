...
impl Image {
    ...
    pub fn draw_scaled(&self, x: i32, y: i32, dest_w: u32, dest_h: u32) {
        ...
        for dy in 0..dest_h {
            ...
            for dx in 0..dest_w {
                ...
                let pixel = self.get_pixel(source_x, source_y);
    ...
    pub fn scale(&self, new_width: u32, new_height: u32) -> Image {
        ...
        for dy in 0..new_height {
            let source_y = (dy as u64 * self.height as u64 / new_height as u64) as u32;
            for dx in 0..new_width {
                let source_x = (dx as u64 * self.width as u64 / new_width as u64) as u32;
                scaled.set_pixel(dx, dy, self.get_pixel(source_x, source_y));
            }
        }
...
pub fn load(path: &str) -> Option<Image> {
    ...
    if lower_path.ends_with(".bmp") {
        ...
        bmp::load_bitmap(path).or_else(|| ppm::load_ppm(path))
    }
}
