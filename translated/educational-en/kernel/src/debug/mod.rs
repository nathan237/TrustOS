...
unsafe {
    ...
    for _ in 0..max_frames {
        ...
        let frame_pointer = rbp as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u64;
        
        // Read return address (at rbp+8) and previous rbp (at rbp)
        let return_value_address = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { core::ptr::read_volatile(frame_pointer.add(1)) };
        let previous_rbp = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
