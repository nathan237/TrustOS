...
pub fn new(sandbox_id: SandboxId, max_files: usize, max_bytes: usize) -> Self {
        ...
        fs.files.insert(String::from("/"), SandboxFile::new_directory("/"));
        fs.files.insert(String::from("/cache"), SandboxFile::new_directory("cache"));
        fs.files.insert(String::from("/cookies"), SandboxFile::new_directory("cookies"));
        fs.files.insert(String::from("/storage"), SandboxFile::new_directory("storage"));
        fs.files.insert(String::from("/downloads"), SandboxFile::new_directory("downloads"));
        fs
    }
...
match self.files.get(&path) {
    ...
    pub fn list(&self, directory_path: &str) -> Result<Vec<(&str, &SandboxFileType, usize)>, FsError> {
        let directory = self.normalize_path(directory_path)?;
        let prefix = if directory == "/" { String::from("/") } else { format!("{}/", directory) };

        let mut entries = Vec::new();
        for (path, file) in &self.files {
            if path == &directory { continue; } // skip the directory itself
    ...
    pub fn local_storage_set(&mut self, domain: &str, key: &str, value: &str) -> Result<(), FsError> {
        let directory = format!("/storage/{}", domain.replace('.', "_"));
        if !self.exists(&directory) {
            self.mkdir(&directory)?;
        }
        let path = format!("{}/{}", directory, key);
        self.write(&path, value.as_bytes(), domain)
    }
    ...
    pub fn clear(&mut self) {
        ...
        self.files.insert(String::from("/"), SandboxFile::new_directory("/"));
        self.files.insert(String::from("/cache"), SandboxFile::new_directory("cache"));
        self.files.insert(String::from("/cookies"), SandboxFile::new_directory("cookies"));
        self.files.insert(String::from("/storage"), SandboxFile::new_directory("storage"));
        self.files.insert(String::from("/downloads"), SandboxFile::new_directory("downloads"));
    }
