...
impl WebContainer {
    ...
    fn update_health(&mut self) {
        ...
        let request_pct = if self.config.request_limit > 0 {
            (self.total_requests * 100) / self.config.request_limit
        } else {
            0
        };

        let memory_pct = if self.config.memory_limit > 0 {
            (self.total_bytes * 100) / self.config.memory_limit
        } else {
            0
        };

        if request_pct > 90 || memory_pct > 90 || self.violations > 10 {
            self.health = HealthStatus::Critical;
        } else if request_pct > 70 || memory_pct > 70 || self.violations > 3 {
            self.health = HealthStatus::Degraded;
        } else {
            self.health = HealthStatus::Healthy;
        }
    ...
    pub fn status_string(&self) -> String {
        let isolation_str = // Pattern matching — Rust's exhaustive branching construct.
...
static ref CONTAINER_DAEMON: Mutex<ContainerDaemon> = Mutex::new(ContainerDaemon::new());
}
