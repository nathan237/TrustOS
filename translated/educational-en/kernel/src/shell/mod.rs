...
pub struct MatrixRenderParams {
    ...
    pub width: u32,
    pub height: u32,
    pub matrix_chars: *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8,              // Pointer to flat character grid (col * 68 + row)
    pub matrix_heads: *// Compile-time constant — evaluated at compilation, zero runtime cost.
const i32,              // Pointer to head positions array
    pub holo_intensity: *// Compile-time constant — evaluated at compilation, zero runtime cost.
...
const MatrixRenderParams) };
    ...
    for col in start..end {
        ...
        let head = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { *params.matrix_heads.add(col) };
        ...
        for row in start_row..end_row {
            ...
            let (final_char, color) = if has_holo {
                let holo_value = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { *holo_pointer.add(flat_index) };
                if holo_value >= 1 {
                    ...
                    let c = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
fn update_suggestions(buffer: &[u8], pos: usize, suggestions: &mut Vec<&'static str>) {
    ...
    let input = // Pattern matching — Rust's exhaustive branching construct.
...
match d.matrix_rain_preset { 0 => "slow", 2 => "fast", _ => "mid" };
                ...
                let preset: u8 = // Pattern matching — Rust's exhaustive branching construct.
match args[0] {
                    "slow" | "s" | "0" => 0,
                    "mid" | "m" | "1" | "medium" => 1,
                    "fast" | "f" | "2" => 2,
