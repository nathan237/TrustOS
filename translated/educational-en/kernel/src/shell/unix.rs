...
pub(super) fn command_sequence(args: &[&str]) {
    ...
    let (first, inc, last) = // Pattern matching — Rust's exhaustive branching construct.
match args.len() {
...
pub(super) fn command_ioport(args: &[&str]) {
    ...
    let port = // Pattern matching — Rust's exhaustive branching construct.
...
pub(super) fn command_cpuid(args: &[&str]) {
    ...
    let leaf = // Pattern matching — Rust's exhaustive branching construct.
...
pub(super) fn command_fontsmooth(args: &[&str]) {
    ...
    if args.is_empty() {
        let current = // Pattern matching — Rust's exhaustive branching construct.
match get_mode() {
...
match args[0] {
        ...
        "policy" => {
            ...
            let chain = // Pattern matching — Rust's exhaustive branching construct.
match Chain::from_str(args[1]) {
                Some(c) => c,
                None => { crate::println_color!(COLOR_RED, "Invalid chain: {}", args[1]); return; }
            };
            let action = // Pattern matching — Rust's exhaustive branching construct.
...
match args[i] {
    ...
    let field_list = // Pattern matching — Rust's exhaustive branching construct.
match fields {
        Some(f) => f,
        None => {
...
pub(super) fn command_diff(args: &[&str]) {
    ...
    let (c1, c2) = // Pattern matching — Rust's exhaustive branching construct.
