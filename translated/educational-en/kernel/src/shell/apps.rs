...
match args.get(1) {
            ...
            let source = // Pattern matching — Rust's exhaustive branching construct.
match crate::ramfs::with_filesystem(|fs| fs.read_file(&path).map(|d| d.to_vec())) {
                Ok(data) => // Pattern matching — Rust's exhaustive branching construct.
...
match alloc::string::String::from_utf8(data) {
            ...
            fn native_builtin(id: u8, argc: usize, argv: *// Compile-time constant — evaluated at compilation, zero runtime cost.
const i64) -> i64 {
...
fn transpile_scan_binaries() {
    ...
    let entries = // Pattern matching — Rust's exhaustive branching construct.
...
match super::network::read_file_bytes(path) {
    ...
    // Analyze it
    let analysis = // Pattern matching — Rust's exhaustive branching construct.
