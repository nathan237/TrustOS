...
loop {
        let bytes_read = // Pattern matching — Rust's exhaustive branching construct.
...
match crate::ramfs::with_filesystem(|fs| fs.read_file(args[0]).map(|c| c.to_vec())) {
            Ok(content) => // Pattern matching — Rust's exhaustive branching construct.
match core::str::from_utf8(&content) {
        ...
        Some(&"status") | None => {
            ...
            if peers.is_empty() {
                ...
                for (i, peer) in peers.iter().enumerate() {
                    let role_icon = // Pattern matching — Rust's exhaustive branching construct.
match peer.role {
