...
const JARVIS_ERROR: u32 = 0xFFFF4444;        // Error red
const MAXIMUM_HISTORY: usize = 32;
...
enum Intent {
    ...
    CpuInformation,            // "cpu info", "processor"
...
match intent {
        Intent::SystemInformation => {
            ...
            let total = used + free;
            let used_keyboard = used / 1024;
            let total_keyboard = total / 1024;
...
match lang {
                Lang::Fr => Action::Respond(format!(
                    ...
                    used_keyboard, total_keyboard, pct, tasks + 2, crate::cpu::smp::cpu_count()
                )),
...
match lang {
        ...
        Intent::NetworkInformation => {
            Action::ShellCommand(String::from("ifconfig"))
        }
        Intent::CpuInformation => {
...
fn explain_topic(topic: &str, lang: Lang) -> Action {
    let explanation = // Pattern matching — Rust's exhaustive branching construct.
match topic {
        t if matches_any(t, &["rust", "langage"]) => // Pattern matching — Rust's exhaustive branching construct.
match lang {
        ...
        t if matches_any(t, &["tls", "ssl", "https", "crypto"]) => // Pattern matching — Rust's exhaustive branching construct.
match lang {
        ...
        t if matches_any(t, &["kernel", "noyau"]) => // Pattern matching — Rust's exhaustive branching construct.
match lang {
        ...
        t if matches_any(t, &["smp", "multicore", "multi-core"]) => // Pattern matching — Rust's exhaustive branching construct.
match lang {
        ...
        t if matches_any(t, &["trustlang", "language", "langage de prog"]) => // Pattern matching — Rust's exhaustive branching construct.
match lang {
        ...
        t if matches_any(t, &["browser", "navigateur", "html"]) => // Pattern matching — Rust's exhaustive branching construct.
match lang {
        ...
        t if matches_any(t, &["compositor", "gui", "desktop", "bureau"]) => // Pattern matching — Rust's exhaustive branching construct.
match lang {
        ...
        t if matches_any(t, &["jarvis", "ia", "ai", "intelligence"]) => // Pattern matching — Rust's exhaustive branching construct.
match lang {
        ...
        _ => // Pattern matching — Rust's exhaustive branching construct.
match lang {
...
fn howto_topic(topic: &str, lang: Lang) -> Action {
    let howto = // Pattern matching — Rust's exhaustive branching construct.
match topic {
        t if matches_any(t, &["compile", "build", "construire"]) => // Pattern matching — Rust's exhaustive branching construct.
match lang {
        ...
        t if matches_any(t, &["file", "fichier", "creer"]) => // Pattern matching — Rust's exhaustive branching construct.
match lang {
            Lang::Fr => "Pour creer un fichier:\n  touch /mon_fichier.txt\n  echo 'contenu' > /mon_fichier.txt",
            Lang::En => "To create a file:\n  touch /my_file.txt\n  echo 'content' > /my_file.txt",
        },
        t if matches_any(t, &["network", "reseau", "internet"]) => // Pattern matching — Rust's exhaustive branching construct.
match lang {
        ...
        t if matches_any(t, &["theme", "personnaliser", "customize"]) => // Pattern matching — Rust's exhaustive branching construct.
match lang {
...
match crate::ramfs::with_filesystem(|fs| fs.read_file(&path).map(|d| d.to_vec())) {
            ...
            "query" | "ask" | "q" => {
                ...
                crate::print!("{}", crate::jarvis_hw::hardware_query(&question));
                return;
            }
...
match crate::jarvis::load_weights() {
        ...
        "pretrain" | "pt" => {
            ...
            crate::println_color!(COLOR_GRAY, "  {} phases, {} sequences total",
                crate::jarvis::corpus::number_phases(), crate::jarvis::corpus::total_sequences());
            crate::println!();
            ...
            let (steps, average_loss, elapsed) = crate::jarvis::pretrain(epochs, 0.001);

            crate::println_color!(JARVIS_BRAIN, "  Training complete!");
            crate::println_color!(COLOR_GRAY, "  {} steps, avg loss={:.3}, {}ms ({:.1}s)",
                steps, average_loss, elapsed, elapsed as f64 / 1000.0);
            crate::println!();
        ...
        "eval" => {
            ...
            if full_eval {
                ...
                let average_loss = crate::jarvis::eval_corpus();
                crate::println!();

                // Per-phase breakdown
                let model_guard_unavailable = !crate::jarvis::is_ready();
                if !model_guard_unavailable {
                    for phase in 0..crate::jarvis::corpus::number_phases() {
                ...
                if average_loss < 4.0 {
                    crate::println_color!(COLOR_GREEN, "{:.3} (learning!)", average_loss);
                } else if average_loss < 5.5 {
                    crate::println_color!(COLOR_YELLOW, "{:.3} (early stage)", average_loss);
                } else {
                    crate::println_color!(COLOR_RED, "{:.3} (random/untrained)", average_loss);
                }
                ...
                let average_loss = crate::jarvis::eval_quick();
                crate::println!();
                crate::print_color!(COLOR_WHITE, "  Average loss: ");
                if average_loss < 4.0 {
                    crate::println_color!(COLOR_GREEN, "{:.3} (learning!)", average_loss);
        ...
        "propagate" | "autoprop" | "spread" => {
            ...
            if !peers.is_empty() {
                ...
                for peer in &peers {
                    let role = // Pattern matching — Rust's exhaustive branching construct.
match peer.role {
        ...
        "swarm" => {
            ...
            crate::println_color!(COLOR_GRAY, "  {} steps, avg loss={:.3}, {:.1}s",
                steps, average_loss, elapsed as f64 / 1000.0);
