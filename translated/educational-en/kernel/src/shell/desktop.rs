...
pub(super) fn command_benchmark(args: &[&str]) {
    use alloc::vec;
...
pub(super) fn command_cosmic(args: &[&str]) {
...
match subcommand {
        "demo" | "test" => {
            ...
            let mut renderer = // Pattern matching — Rust's exhaustive branching construct.
match CosmicRenderer::new(width, height) {
            ...
            // Button with shadow
            let button_rect = Rect::new(300.0, 100.0, 120.0, 40.0);
            renderer.draw_shadow(button_rect, 8.0, 8.0, Color::BLACK.with_alpha(0.4));
            renderer.fill_rounded_rect(button_rect, 8.0, dark::ACCENT);
...
pub(super) fn command_open(args: &[&str]) {
    if args.is_empty() {
    ...
    command_cosmic_v2_with_app(Some(&app));
}
...
fn check_desktop_resources() -> bool {
    ...
    let physical_ram = crate::memory::total_physical_memory();
    let physical_mb = physical_ram / (1024 * 1024);
    if physical_mb > 0 && physical_mb < 256 {
        crate::println_color!(COLOR_RED, "\u{26A0} Insufficient RAM: {} MB detected (minimum: 256 MB)", physical_mb);
        warnings += 1;
    } else if physical_mb > 0 && physical_mb < 512 {
        crate::println_color!(COLOR_YELLOW, "\u{26A0} Low RAM: {} MB detected (recommended: 512 MB+)", physical_mb);
    }
...
pub(super) fn launch_desktop_env(initial_window: Option<(&str, crate::desktop::WindowType, i32, i32, u32, u32)>) {
    ...
    // Show tier info
    let tier_name = // Pattern matching — Rust's exhaustive branching construct.
match d.desktop_tier {
...
pub(super) fn command_signature(args: &[&str]) {
    use crate::signature;
...
pub(super) fn command_security(args: &[&str]) {
        // Pattern matching — Rust's exhaustive branching construct.
match args.first().copied() {
        Some("status") | None => {
        ...
        Some("gate") => {
            ...
            if let Some(subsystem_name) = args.get(1).copied() {
                let subsystem = // Pattern matching — Rust's exhaustive branching construct.
...
pub(super) fn command_demo(args: &[&str]) {
    let lang = // Pattern matching — Rust's exhaustive branching construct.
match args.first().copied() {
        Some("fr") => "fr",
        _ => "en",
    };
...
loop {
    ...
    // -------------------------------------------------------------------
    let (software, sh) = crate::framebuffer::get_dimensions();

    {
        ...
        let draw_rain_step = |buf: &mut [u32], w: usize, h: usize, cols: &mut [u16], speeds: &[u8], frame: u32| {
            ...
            for column_index in 0..cols.len() {
                let x = column_index * 8;
                if x >= w { continue; }
                cols[column_index] = cols[column_index].wrapping_add(speeds[column_index] as u16);
                if cols[column_index] as usize >= h { cols[column_index] = 0; }
                let y = cols[column_index] as usize;
                let c = (((frame as usize + column_index * 13) % 94) + 33) as u8 as char;
                let glyph = crate::framebuffer::font::get_glyph(c);
                for (row, &bits) in glyph.iter().enumerate() {
                    let py = y + row;
                    if py >= h { break; }
...
loop {
    ...
    pause(1);
    super::commands::command_neofetch();
    pause(3);
    ...
    crate::ramfs::with_filesystem(|fs| { let _ = fs.mkdir("/tutorial"); });
    ...
    super::commands::command_cat(&["/tutorial/hello.txt"], None, None);
    pause(2);

    crate::println_color!(COLOR_CYAN, "  $ tree /");
    super::commands::command_tree(&["/"]);
...
match crate::trustlang::run(tl_code) {
    ...
    super::vm::command_ifconfig();
    pause(2);

    crate::println_color!(COLOR_CYAN, "  $ netstat");
    super::vm::command_netstat();
...

    {
        ...
        let rw = software as usize;
        let rh = sh as usize;
        let mut vp_buffer = alloc::vec![0u32; rw * rh];
...
unsafe {
    ...
    command_cosmic_v2_with_app_timed(Some("shell"), 8000);
    crate::framebuffer::clear();
    pause(1);
...
unsafe {
        ...
        let vp_w = 160usize;
        let vp_h = 160usize;
        let mut vp_buffer = alloc::vec![0u32; vp_w * vp_h];
...
pub(super) fn command_showcase(args: &[&str]) {
...
unsafe {
        ...
        let render_scene = |buf: &mut [u32], w: usize, h: usize,
            ...
            let total_chars: usize = lines.iter().map(|(t, _, _)| t.len()).sum();
            let type_mouse = 80u64 * speed / 2; // ms per char typed
            let type_total_mouse = total_chars as u64 * type_mouse;
...
loop {
    ...
    super::commands::command_uname(&["-a"]);
    pause(4);

    crate::println_color!(COLOR_CYAN, "$ free");
    super::commands::command_free();
    pause(4);

    crate::println_color!(COLOR_CYAN, "$ lscpu");
    super::unix::command_lscpu();
    pause(5);
    ...
    super::commands::command_mkdir(&["/demo"]);
    pause(2);
    ...
    super::commands::command_cat(&["/demo/hello.txt"], None, None);
    pause(3);
...
unsafe {
    ...
    command_cosmic_v2_with_app_timed(Some("browser"), effect_duration);
    ...
    // Clean up
    let _ = crate::ramfs::with_filesystem(|fs| { let _ = fs.rm("/demo/hello.txt"); let _ = fs.rm("/demo/showcase.tl"); });
}
...
pub(super) fn command_showcase_jarvis(args: &[&str]) {
    let speed = // Pattern matching — Rust's exhaustive branching construct.
match args.first().copied() {
        Some("fast") => 1u64,
        Some("slow") => 3,
...
loop {
            ...
            for li in 0..node_positions.len() - 1 {
                let source_index = (next_random() % node_positions[li].len() as u64) as usize;
    ...
    if !trimmed.is_empty() {
        ...
        for c in trimmed.chars().take(200) {
            ...
            let char_frequency = crate::cpu::tsc::frequency_hz();
            if char_frequency > 0 {
                let char_cycles = char_frequency / 1000 * 30; // 30ms per char
    ...
    if mesh_active {
        ...
        let role_str = // Pattern matching — Rust's exhaustive branching construct.
match role {
...
pub fn command_showcase3d() {
    ...
    let draw_stats = |buf: &mut [u32], w: usize, h: usize, scene_name: &str, scene_num: usize, fps: u32, elapsed_s: u32, total_s: u32, quality: usize| {
        ...
        let quality_str = // Pattern matching — Rust's exhaustive branching construct.
match quality { 1 => "Full", 2 => "High", 3 => "Med", _ => "Low" };
...
loop {
        ...
        // Left third: Pyramid
        {
            let mut sub_buffer = alloc::vec![0xFF0C1018u32; third * h];
            crate::formula3d::render_filled_mesh(&mut sub_buffer, third, h,
...
pub(super) fn command_cosmic_v2() {
    command_cosmic_v2_with_app(None);
}
...
pub(super) fn command_cosmic_v2_with_app(initial_app: Option<&str>) {
    command_cosmic_v2_with_app_timed(initial_app, 0);
}
...
pub(super) fn command_cosmic_v2_with_app_timed(initial_app: Option<&str>, timeout_ms: u64) {
    ...
    let mut active_mode = // Pattern matching — Rust's exhaustive branching construct.
match initial_app {
    ...
    const HTML_COLOR_ATTRIBUTE: u32 = 0xFF98C379;      // Green - Attribute names
...
#[derive(Clone, Copy, PartialEq)]
        ...
        while i < chars.len() {
            ...
            if chars[i] == '<' {
                ...
                while i < chars.len() && chars[i] != '>' {
                    ...
                    if attribute_start < i {
                        segments.push(HtmlSegment {
                            text: chars[attribute_start..i].iter().collect(),
                            color: HTML_COLOR_ATTRIBUTE,
                        });
                    }
                    ...
                    if i < chars.len() && (chars[i] == '"' || chars[i] == '\'') {
                        ...
                        segments.push(HtmlSegment {
                            text: chars[value_start..i].iter().collect(),
    ...
    browser_lines.push(make_simple_line("|  COLOR LEGEND:                                             |", 0xFFFFFF00, LineType::Welcome));
    {
        ...
        legend2.segments.push(HtmlSegment { text: String::from("attr"), color: HTML_COLOR_ATTRIBUTE });
    ...
    // ---------------------------------------------------------------------------
    {
        ...
        let mut bitmap_data = alloc::vec::Vec::from(bitmap_header);
        ...
        let _ = crate::ramfs::with_filesystem(|fs| {
            fs.write_file("/images/test.bmp", &bitmap_data)
        });
    ...
    let mut image_viewer_information = String::from("No image loaded");
    ...
    const MAXIMUM_VISIBLE_LINES: usize = 18;  // Lines visible in shell window
    ...
    // Pre-load a demo Rust file
    {
        ...
        let _ = crate::ramfs::with_filesystem(|fs| fs.write_file("/demo.rs", sample_code.as_bytes()));
...
const MAXIMUM_HISTORY: usize = 10;
            ...
            match key {
                ...
                10 | 13 => { // Enter - execute command (ASCII LF=10 or CR=13)
                    if !shell_input.is_empty() {
                        let command_raw = shell_input.clone();
                        let cmd = command_raw.trim();  // Trim whitespace for matching
                        crate::serial_println!("[DEBUG] Enter pressed, cmd = '{}' (trimmed: '{}')", command_raw, cmd);
                        shell_output.push(format!("> {}", cmd));
                        
                        // Add to command history
                        command_history.push(String::from(cmd));
                        if command_history.len() > MAXIMUM_HISTORY {
...
loop {
                                        ...
                                        if frames % 60 == 0 {
                                            ...
                                            let elapsed_sector = elapsed as f32 / crate::cpu::tsc::frequency_hz() as f32;
                                            let fps = frames as f32 / elapsed_sector;
                            ...
                            _ if cmd.starts_with("cd ") => {
                                ...
                                match crate::ramfs::with_filesystem(|fs| fs.cd(path)) {
                                    Ok(()) => {
                                        let new_cwd = crate::ramfs::with_filesystem(|fs| String::from(fs.pwd()));
                            ...
                            _ if cmd.starts_with("ls ") => {
                                ...
                                match crate::ramfs::with_filesystem(|fs| fs.ls(Some(path))) {
                                    Ok(items) => {
                                        if items.is_empty() {
                                            shell_output.push(String::from("(empty)"));
                                        } else {
...
match file_type {
                            ...
                            _ if cmd.starts_with("cat ") => {
                                ...
                                // Use ramfs read
                                match crate::ramfs::with_filesystem(|fs| {
                            ...
                            _ if cmd.starts_with("touch ") => {
                                ...
                                // Create empty file
                                match crate::ramfs::with_filesystem(|fs| fs.write_file(path, &[])) {
                                    Ok(()) => {
...
match app.as_str() {
                            ...
                            "free" => {
                                let used = crate::memory::heap::used() / 1024;
                                let total = crate::memory::heap_size() / 1024;
                                let free_keyboard = total - used;
                                shell_output.push(String::from("              total     used     free"));
                                shell_output.push(format!("Mem:     {:>10}  {:>7}  {:>7}", total, used, free_keyboard));
                            },
                            "tree" => {
                                shell_output.push(String::from("."));
                                                                // Pattern matching — Rust's exhaustive branching construct.
...
match file_type {
                            ...
                            _ if cmd.starts_with("nslookup ") || cmd.starts_with("dig ") => {
                                ...
                                let ip_result = // Pattern matching — Rust's exhaustive branching construct.
match host {
                                    "google.com" | "www.google.com" => Some([142, 250, 179, 110]),
                                    "cloudflare.com" | "www.cloudflare.com" => Some([104, 16, 132, 229]),
                                    "github.com" | "www.github.com" => Some([140, 82, 114, 3]),
...
loop {
                        ...
                        scroll_offset = shell_output.len().saturating_sub(MAXIMUM_VISIBLE_LINES);
                    }
                },
                32..=126 => { // Printable characters
        ...
        let clicked = left && !previous_left;
        let released = !left && previous_left;
        previous_left = left;
                ...
                // Dock icon clicks - also reopens window if closed
                    ...
                    for i in 0..8usize {
                        ...
                        if my >= iy && my < iy + icon_size + 16 {
                            active_mode = // Pattern matching — Rust's exhaustive branching construct.
match i {
                                0 => AppMode::Files,
                                1 => AppMode::Shell,
                                2 => AppMode::Network,
...
match active_mode {
            ...
            let maximum_lines = ((h - content_y - 50) / line_height) as usize;
            ...
            if active_mode == AppMode::Browser {
                ...
                let page_maximum_lines = ((h - page_y - 35) / line_height) as usize;
                ...
                let start_index = if browser_lines.len() > page_maximum_lines {
                    browser_lines.len() - page_maximum_lines
                } else {
                    0
                };
                
                // Render each line with colored segments
                for (i, browser_line) in browser_lines.iter().skip(start_index).enumerate() {
                    let y = page_y + (i as u32) * line_height;
                    if y + line_height > h - 35 { break; }
                    
                    // Line number
                    let line_number = format!("{:3}", start_index + i + 1);
                    win.draw_text(&line_number, 14, y, 0xFF858585);
                    
                    // Render segments with their colors
                    let mut x_position = 10u32 + gutter_width + 5;
                    for segment in &browser_line.segments {
                        win.draw_text(&segment.text, x_position, y, segment.color);
                        x_position += (segment.text.len() as u32) * 8;
                    }
                }
                ...
                if let Some(ref img) = image_viewer_data {
                    ...
                    let image_x = center_x - (scaled_w as i32 / 2);
                ...
                let information_str = format!("Zoom: {}%  |  {}", zoom_pct, image_viewer_information);
                win.draw_text(&information_str, 16, content_y + 5, 0xFFCCCCCC);
                ...
                for vi in 0..visible_lines_count {
                    let line_index = editor_state.scroll_y + vi;
                    if line_index >= editor_state.lines.len() { break; }
                    
                    let ly = code_y + (vi as u32 * line_h);
                    if ly + line_h > code_y + code_h { break; }
                    
                    let is_current = line_index == editor_state.cursor_line;
                    ...
                    // Line number
                    let number_str = format!("{:>4} ", line_index + 1);
                    let number_color = if is_current { COLOR_ACTIVE_LINE } else { COLOR_LINE_NUMBER };
                    win.draw_text(&number_str, 2, ly, number_color);
                    
                    // Code with syntax highlighting
                    let line = &editor_state.lines[line_index];
        ...
        if let Some(hist) = compositor.get_layer_mut(history_layer) {
            ...
            if command_history.is_empty() {
                ...
                for (i, cmd) in command_history.iter().rev().take(10).enumerate() {
                    ...
                    let number_str = format!("{:2}.", num);
                    hist.draw_text(&number_str, 6, y + 2, 0xFF666666);
                    
                    // Command (truncate if too long)
                    let display_command = if cmd.len() > 26 {
                        format!("{}...", &cmd[..23])
                    } else {
                        cmd.clone()
                    };
                    hist.draw_text(&display_command, 30, y + 2, 0xFF88FF88);
                }
            }
        }
...
match active_mode {
            ...
            let win_button_x = 220u32;
            if window_visible {
                // Window is open - show active button
                bar.fill_rect(win_button_x, 6, 100, 28, 0xFF002200);
                bar.draw_rect(win_button_x, 6, 100, 28, green_main);
                bar.draw_text(mode_name, win_button_x + 10, 12, green_bright);
                // Active indicator line at bottom
                bar.fill_rect(win_button_x + 20, 32, 60, 3, green_main);
            } else {
                // Window is minimized - show inactive button
                bar.fill_rect(win_button_x, 6, 100, 28, 0xFF0A0A0A);
                bar.draw_rect(win_button_x, 6, 100, 28, green_dim);
                bar.draw_text(mode_name, win_button_x + 10, 12, green_dim);
            }
            
            // Settings button (gear icon)
            let settings_button_x = 340u32;
            let settings_bg = if settings_open { 0xFF002200 } else { 0xFF0A1A0A };
            bar.fill_rect(settings_button_x, 6, 50, 28, settings_bg);
            bar.draw_rect(settings_button_x, 6, 50, 28, green_dim);
            bar.draw_text("[S]", settings_button_x + 10, 12, if settings_open { green_bright } else { green_main });
        ...
        if let Some(menu) = compositor.get_layer_mut(menu_layer) {
            if menu_open {
                ...
                for (i, (name, item)) in menu_items.iter().enumerate() {
                    ...
                    let (color, icon) = // Pattern matching — Rust's exhaustive branching construct.
match item {
                        MenuItem::App(_) => {
...
pub(super) fn command_cosmic_desktop() {
    ...
    const MATRIX_TRAIL_LENGTH: usize = 30; // Longer trails for fuller look
    ...
    let mut previous_mx = 0.0f32;
    let mut previous_my = 0.0f32;
    let mut previous_theme = use_matrix_theme;
    let mut previous_app = 0usize;
    let mut previous_hovered = -1i32;
    ...
    while running {
        ...
        click_this_frame = left_pressed && !previous_left;
        ...
        if let Some(key) = crate::keyboard::try_read_key() {
            if search_active {
                ...
                match key {
                    ...
                    8 => {  // Backspace
                        if search_length > 0 { search_length -= 1; }
                    },
                    13 => { search_active = false; },  // Enter confirms
                    32..=126 => {  // Printable ASCII
                        if search_length < 31 {
                            search_text[search_length] = key;
                            search_length += 1;
                        }
                    },
                    _ => {}
        ...
        previous_mx = mx;
        previous_my = my;
        previous_theme = use_matrix_theme;
        ...
        let minimum_bg_u32 = minimum_bg.to_u32();
        ...
        for col in 0..MATRIX_COLS {
            ...
            for i in 0..MATRIX_TRAIL_LENGTH {
                ...
                let char_index = (char_seed as usize) % MATRIX_CHARS.len();
                let char_str: [u8; 2] = [MATRIX_CHARS[char_index], 0];
                let char_s = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { core::str::from_utf8_unchecked(&char_str[..1]) };
...
match i {
        ...
        let cwd_display = crate::ramfs::with_filesystem(|fs| String::from(fs.pwd()));
        // Draw "root" in red
        ...
        // Cursor block
        let prompt_length = 4 + 1 + 7 + path_part.len();  // root @ trustos :path$
        let cursor_x = content_x + (prompt_length * 8) as u32;
        ...
        let menu_button_x = 8u32;
        let menu_button_w = 24u32;
        fill_rect(menu_button_x + 4, bar_y + 14, 16, 3, green_main);
        fill_rect(menu_button_x + 4, bar_y + 19, 16, 3, green_main);
        
        // Check if menu button clicked
        if click_this_frame && mx >= menu_button_x as f32 && mx < (menu_button_x + menu_button_w) as f32 &&
           my >= bar_y as f32 {
            menu_open = !menu_open;
        }
        ...
        if search_length == 0 {
            ...
            let search_display = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { core::str::from_utf8_unchecked(&search_text[..search_length]) };
