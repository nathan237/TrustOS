...
const FRAME_MOUSE: u64 = 33; // ~30 fps
    ...
    for sy in 0..logo_ph {
        ...
        for sx in 0..logo_pw {
            ...
            if logo_edge(lx, ly) {
                ...
                let column_seed = (lx.wrapping_mul(7919) + 31) % 97;
                let speed = 1 + column_seed % 3;
                let rain_y = (ly + rain_offset * speed + column_seed * 5) % 17;
...
pub(super) fn command_trustos_trailer() {
    ...
    // -------------------------------------------------------------------
    {
        ...
        for frame in 0..total {
            ...
            for (i, &(target, label)) in counters.iter().enumerate().take(items_shown) {
                ...
                let number_str = alloc::format!("{:>7}", current);
                let full_str = alloc::format!("{}{}", number_str, label);
