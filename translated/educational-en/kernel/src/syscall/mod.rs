...
unsafe { *(pointer_address as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u64) };
            if ptr == 0 { break; }
...
const u64) } as usize;
    ...
    let base = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { *(iov_pointer as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u64) };
    let len  = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
fn system_setsockopt(fd: i32, level: i32, optname: i32, optval: u64, optlen: usize) -> i64 {
    ...
    let data = if optlen > 0 {
                // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { core::slice::from_raw_parts(optval as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u8, optlen) }
    } else {
        &[]
    };
