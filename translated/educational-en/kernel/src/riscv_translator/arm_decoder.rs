...
match op0 {
            ...
            0b1000 | 0b1001 => self.decode_dp_imm(inst, inst_address, block),

            // Branches, exception generation, system
            0b1010 | 0b1011 => return self.decode_branch(inst, inst_address, block),
...
match opc2 {
            ...
            0b110 => {
                ...
                let imm_value = decode_bitmask_imm(inst, sf == 1);

                                // Pattern matching — Rust's exhaustive branching construct.
match log_opc {
                    0b00 => block.emit(RvInst::Andi { rd: rv_rd, rs1: rv_rn, imm: imm_value }),
                    0b01 => block.emit(RvInst::Ori { rd: rv_rd, rs1: rv_rn, imm: imm_value }),
                    0b10 => block.emit(RvInst::Xori { rd: rv_rd, rs1: rv_rn, imm: imm_value }),
                    0b11 => { // ANDS
                        block.emit(RvInst::Andi { rd: rv_rd, rs1: rv_rn, imm: imm_value });
                        block.emit(RvInst::CmpFlags { rs1: rv_rd, rs2: Reg::X0 });
                    }
                    _ => {}
                }
