...
pub fn fast_isqrt(x: f32) -> f32 {
    if x <= 0.0 { return 0.0; }
    let xhalf = 0.5 * x;
    let i = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { core::mem::transmute::<f32, u32>(x) };
    let i = 0x5f375a86u32.wrapping_sub(i >> 1);
    let y = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
