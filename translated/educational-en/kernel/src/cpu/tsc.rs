...
fn calibrate_from_cpuid() -> Option<u64> {
    let cpuid_15 = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
unsafe { core::arch::x86_64::__cpuid(0) };
    if cpuid_0.eax >= 0x16 {
        let cpuid_16 = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
