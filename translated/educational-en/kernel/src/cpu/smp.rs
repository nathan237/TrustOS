...
pub fn send_reschedule_ipi(target_cpu: u32) {
    let apic_id = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
pub fn current_cpu_id() -> u32 {
    let cpuid = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
pub fn init() {
    ...
    BSP_ID.store(// SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { PER_CPU[0].apic_id }, Ordering::Release);
    
    crate::serial_println!("[SMP] BSP initialized (APIC ID: {})", // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
unsafe { PER_CPU[i].work_completed };
        crate::println!("║ CPU {:2}: {} APIC {:3}  Work: {:8}  ║", 
            i, ready, // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
pub fn get_stats() -> (u32, u32, u64) {
    ...
    for i in 0..total as usize {
        total_work += // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { PER_CPU[i].work_completed };
    }
