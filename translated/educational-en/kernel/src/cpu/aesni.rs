...
unsafe {
            ...
            // Load initial key
            round_keys[0] = _mm_loadu_si128(key.as_ptr() as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const __m128i);
    ...
    pub fn encrypt_block(&self, plaintext: &[u8; 16]) -> [u8; 16] {
                // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe {
            let mut block = _mm_loadu_si128(plaintext.as_ptr() as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const __m128i);
    ...
    pub fn decrypt_block(&self, ciphertext: &[u8; 16]) -> [u8; 16] {
                // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe {
            let mut block = _mm_loadu_si128(ciphertext.as_ptr() as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const __m128i);
...
unsafe { _mm_loadu_si128(h_bytes.as_ptr() as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const __m128i) };
...
unsafe { _mm_loadu_si128(block.as_ptr() as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const __m128i) };
...
unsafe { _mm_loadu_si128(length_block.as_ptr() as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const __m128i) };
        ghash = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
...
unsafe { _mm_loadu_si128(j0_encrypt.as_ptr() as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const __m128i) };
        ghash = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
