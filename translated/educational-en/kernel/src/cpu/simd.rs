...
const u8, len: usize) {
    ...
    while i + 8 <= len {
        ...
        let data = (src.add(i) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
...
const u8, len: usize) {
    ...
    while i + 8 <= len {
        ...
        let b = (src.add(i) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u64).read_unaligned();
