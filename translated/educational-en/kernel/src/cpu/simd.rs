...
const u8, len: usize) {
    ...
    while i + 8 <= len {
        ...
        let data = (source.add(i) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
...
const u8, len: usize) -> bool {
    let mut i = 0;
    while i + 8 <= len {
        let va = (a.add(i) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u64).read_unaligned();
        let vb = (b.add(i) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u64).read_unaligned();
        if va != vb {
            return false;
        }
...
const u8, len: usize) {
    ...
    while i + 8 <= len {
        ...
        let b = (source.add(i) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u64).read_unaligned();
