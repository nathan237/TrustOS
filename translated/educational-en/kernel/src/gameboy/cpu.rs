...
pub fn hl(&self) -> u16 { (self.h as u16) << 8 | self.l as u16 }
    ...
    // Fetch
    fn fetch8(&mut self, bus: &mut // Implementation block — defines methods for the type above.
impl GbBus) -> u8 {
        let v = bus.read(self.pc); self.pc = self.pc.wrapping_add(1); v
    }
    fn fetch16(&mut self, bus: &mut // Implementation block — defines methods for the type above.
impl GbBus) -> u16 {
...
impl GbBus) -> u32 {
    ...
    fn execute(&mut self, op: u8, bus: &mut // Implementation block — defines methods for the type above.
impl GbBus) -> u32 {
                // Pattern matching — Rust's exhaustive branching construct.
match op {
            // ===== 0x00-0x0F =====
