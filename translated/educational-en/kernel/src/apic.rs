...
unsafe fn ioapic_read(reg: u32) -> u32 {
    let base = IOAPIC_BASE_VIRT.load(Ordering::Relaxed);
    core::ptr::write_volatile(base as *mut u32, reg);
    core::ptr::read_volatile((base + 0x10) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32)
}
...
match crate::acpi::get_information() {
    ...
    let ioapic_virt = // Pattern matching — Rust's exhaustive branching construct.
...
pub fn init() -> bool {
    ...
    let lapic_virt = // Pattern matching — Rust's exhaustive branching construct.
match crate::memory::map_mmio(lapic_physical, 4096) {
    ...
    let id = lapic_id();
    let version = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
