...
pub use capability::{register_dynamic_type, get_dynamic_type_information, list_dynamic_types, dynamic_type_count};
...
pub fn init() {
    ...
    let root_capability = Capability::root();
    CAPABILITIES.lock().insert(root_capability.id, root_capability);
    ...
    isolation::initialize_subsystem_capabilities();
...
pub fn derive(
    ...
    {
        let caps = CAPABILITIES.lock();
        let parent_capability = caps.get(&parent).ok_or(SecurityError::InvalidCapability)?;
        
        // Can only derive with same or fewer rights
        if !parent_capability.has_rights(new_rights) {
            return Err(SecurityError::InsufficientRights);
        }
        cap_type = parent_capability.cap_type;
    }
