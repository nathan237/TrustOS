...
const ROOT: CapabilityId = CapabilityId(0);
}
...
const DELETE: Self = Self(1 << 3);
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const CREATE: Self = Self(1 << 4);
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GRANT: Self = Self(1 << 5);
    /// Control/configure a resource (e.g., ioctl, set priority)
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const CONTROL: Self = Self(1 << 6);
    /// Map/allocate resource into address space
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const MAP: Self = Self(1 << 7);
    /// Receive notifications/signals from resource
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const SIGNAL: Self = Self(1 << 8);
    /// Access resource in privileged/supervisor mode
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const PRIVILEGED: Self = Self(1 << 9);
    
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const ALL: Self = Self(0x3FF); // 10 bits
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const READ_WRITE: Self = Self(0x03); // READ | WRITE
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const READ_EXECUTE: Self = Self(0x05); // READ | EXECUTE
...
#[derive(Debug, Clone)]
// Public structure — visible outside this module.
pub struct DynamicTypeInformation {
...
static DYNAMIC_TYPE_REGISTRY: Mutex<BTreeMap<u32, DynamicTypeInformation>> = Mutex::new(BTreeMap::new());
...
pub fn get_dynamic_type_information(id: u32) -> Option<DynamicTypeInformation> {
    DYNAMIC_TYPE_REGISTRY.lock().get(&id).cloned()
}

/// List all registered dynamic types
pub fn list_dynamic_types() -> Vec<(u32, DynamicTypeInformation)> {
