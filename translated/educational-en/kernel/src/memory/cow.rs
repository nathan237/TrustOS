...
const PageTable) };
    ...
    for pml4_index in 0..256 {
        ...
        let pdpt = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe {
            &*((parent_pml4.entries[pml4_index].phys_addr() + hhdm) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const PageTable)
        };
        ...
        for pdpt_index in 0..ENTRIES_PER_TABLE {
            ...
            let pd = // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe {
                &*((pdpt.entries[pdpt_index].phys_addr() + hhdm) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const PageTable)
            };
