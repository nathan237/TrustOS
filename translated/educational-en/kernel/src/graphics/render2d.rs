...
unsafe fn new(buffer: *mut u32, width: u32, height: u32, stride: u32) -> Self {
...
const BLACK: Color2D = Color2D::rgb(0, 0, 0);
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const WHITE: Color2D = Color2D::rgb(255, 255, 255);
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const RED: Color2D = Color2D::rgb(255, 0, 0);
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GREEN: Color2D = Color2D::rgb(0, 255, 0);
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const BLUE: Color2D = Color2D::rgb(0, 0, 255);
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const YELLOW: Color2D = Color2D::rgb(255, 255, 0);
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const CYAN: Color2D = Color2D::rgb(0, 255, 255);
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const MAGENTA: Color2D = Color2D::rgb(255, 0, 255);
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const GRAY: Color2D = Color2D::rgb(128, 128, 128);
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const DARK_GRAY: Color2D = Color2D::rgb(64, 64, 64);
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const LIGHT_GRAY: Color2D = Color2D::rgb(192, 192, 192);
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const ORANGE: Color2D = Color2D::rgb(255, 165, 0);
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const PURPLE: Color2D = Color2D::rgb(128, 0, 128);
    pub     // Compile-time constant — evaluated at compilation, zero runtime cost.
const TRANSPARENT: Color2D = Color2D::new(0, 0, 0, 0);
}
...
unsafe {
            ...
            #[cfg(not(target_arch = "x86_64"))]
            {
                for i in 0..copy_length {
                                        // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe { *self.target.buffer.add(destination_offset + i) = gradient_row[source_off + i]; }
                }
            }
        }
    }
