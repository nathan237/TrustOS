...
pub fn set_point(&mut self, layer: usize, x: i32, y: i32, intensity: u8) {
    ...
    pub fn draw_sphere(&mut self, cx: f32, cy: f32, cz: f32, radius: f32, intensity: u8) {
        ...
        for layer_index in 0..self.number_layers {
            ...
            if dz.absolute() < radius {
                ...
                let circle_pixel = (circle_radius * self.width as f32 / 2.0) as i32;
    ...
    fn draw_circle_layer(&mut self, layer: usize, cx: i32, cy: i32, radius: i32, intensity: u8) {
        ...
        for dy in -radius..=radius {
            let dx_maximum = sqrt_approx((r_sq - dy * dy) as f32) as i32;
            for dx in -dx_maximum..=dx_maximum {
    ...
    pub fn draw_line_3d(&mut self, p1: &Point3D, p2: &Point3D, intensity: u8) {
        ...
        for step in 0..=steps {
            ...
            if layer_z >= 0.0 && layer_z <= 1.0 {
                ...
                for d in -1..=1 {
                    self.set_point(layer_index, sx + d, sy, depth_intensity);
                    self.set_point(layer_index, sx, sy + d, depth_intensity);
                }
            }
        }
    }
    ...
    pub fn composite(&self, base_color: u32, glow_color: u32) -> Vec<u32> {
        ...
        for (layer_index, layer) in self.layers.iter().enumerate().rev() {
            let depth = self.layer_depths[layer_index];
