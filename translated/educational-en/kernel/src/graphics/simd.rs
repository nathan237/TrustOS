...
#[inline]
pub // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe fn copy_row_sse2(dst: *mut u32, src: *// Compile-time constant — evaluated at compilation, zero runtime cost.
...
#[inline]
pub // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe fn copy_row_sse2_nt(dst: *mut u32, src: *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32, count: usize) {
    if count == 0 { return; }

    let dst8 = dst as *mut u64;
    let src8 = src as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u64;
...
#[inline]
pub // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe fn blend_row_sse2(dst: *mut u32, src: *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32, count: usize) {
    let mut destination_pointer = dst;
    let mut source_pointer = src;
    let mut remaining = count;
...
unsafe fn blit_to_framebuffer_sse2(
    ...
    src: *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32,
    src_width: usize,
    source_height: usize,
    dst_x: usize,
...
#[target_feature(enable = "avx2")]
pub // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe fn copy_row_avx2(dst: *mut u32, src: *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32, count: usize) {
    if count == 0 { return; }
...
#[target_feature(enable = "avx2")]
pub // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe fn blend_row_avx2(dst: *mut u32, src: *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32, count: usize) {
    if count == 0 { return; }
    ...
    while remaining >= 8 {
        let src_px = _mm256_loadu_si256(s as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const __m256i);
...
#[inline]
pub // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe fn copy_row_fast(dst: *mut u32, src: *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32, count: usize) {
    if has_avx2() {
...
#[inline]
pub // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe fn blend_row_fast(dst: *mut u32, src: *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32, count: usize) {
    if has_avx2() {
