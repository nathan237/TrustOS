...
const MAXIMUM_DIRTY_RECTS: usize = 64;
...
pub fn hline(&mut self, x: i32, y: i32, len: u32, color: u32) {
        ...
        let fill_length = (x2 - x1) as usize;
...
unsafe {
        ...
        #[cfg(not(target_arch = "x86_64"))]
        {
            self.data[start..start + fill_length].fill(color);
        }
...
pub fn vline(&mut self, x: i32, y: i32, len: u32, color: u32) {
    ...
    pub fn blit_region(&mut self, source: &FastSurface, 
        ...
        let dx = dx as usize;
        let dy = dy as usize;
        let actual_sx = (sx1 as i32 + source_offset_x) as usize;
...
unsafe {
            ...
            #[cfg(not(target_arch = "x86_64"))]
            {
                ...
                while x + 8 <= copy_w {
                    for i in 0..8 {
                        ...
                        if alpha == 255 {
                            self.data[destination_index] = source_pixel;
                        } else if alpha > 0 {
                            self.data[destination_index] = blend_fast(source_pixel, self.data[destination_index]);
                        }
                    }
                    x += 8;
                }
    ...
    fn fill_corner(&mut self, cx: i32, cy: i32, r: u32, color: u32, corner: Corner) {
        ...
        for dy in 0..=r {
            for dx in 0..=r {
                if dx * dx + dy * dy <= r_sq {
                    let (pixel, py) = // Pattern matching — Rust's exhaustive branching construct.
match corner {
...
pub struct DirtyRegion {
    pub rects: [Rect; MAXIMUM_DIRTY_RECTS],
    pub count: usize,
...
pub fn new(width: u32, height: u32) -> Self {
        Self {
            rects: [Rect::default(); MAXIMUM_DIRTY_RECTS],
            count: 0,
...
pub fn initialize_font_cache(fg_color: u32) {
