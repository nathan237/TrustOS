...
unsafe {
                let src = data.as_ptr().add(source_offset) as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32;
    ...
    pub fn present_to_backbuffer(&self) {
        ...
        if let Some((ptr, bb_width, bb_height, _stride)) = crate::framebuffer::get_backbuffer_information() {
            let pixels = self.width.min(bb_width) * self.height.min(bb_height);
                        ...
                        // SAFETY: Unsafe block — bypasses Rust memory-safety guarantees. Ensure invariants manually.
unsafe {
                let source_u32 = data.as_ptr() as *// Compile-time constant — evaluated at compilation, zero runtime cost.
const u32;
