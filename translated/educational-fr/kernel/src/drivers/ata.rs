...
const READ_SECTORS: u8 = 0x20;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const READ_SECTORS_EXT: u8 = 0x24;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const WRITE_SECTORS: u8 = 0x30;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const WRITE_SECTORS_EXT: u8 = 0x34;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CACHE_FLUSH: u8 = 0xE7;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CACHE_FLUSH_EXT: u8 = 0xEA;
}
...
pub struct IdeDriveInformation {
...
pub struct IdeController {
    pub drives: Vec<IdeDriveInformation>,
...
/// Identify a drive
fn identify_drive(base: u16, _control: u16, slave: bool) -> Option<IdeDriveInformation> {
    ...
    let status = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { Port::<u8>::new(base + 7).read() };
    if status == 0 {
        return None;
    }
    ...
    if wait_ready(base).is_err() {
        let lba_mid = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { Port::<u8>::new(base + 4).read() };
        let lba_high = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { Port::<u8>::new(base + 5).read() };
...
unsafe {
    ...
    for i in 0..256 {
        data[i] = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
match channel {
    ...
    for _ in 0..count {
        ...
        for _ in 0..256 {
            let word = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { data_port.read() };
...
unsafe { data_port.write(word); }
    ...
    // Flush cache
    unsafe {
        let flush_command = if use_lba48 { cmd::CACHE_FLUSH_EXT } else { cmd::CACHE_FLUSH };
        Port::<u8>::new(base + 7).write(flush_command);
    }
    wait_ready(base)?;
...
pub fn list_drives() -> Vec<IdeDriveInformation> {
