...
// PORTSC bits
pub(crate) // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
pub(crate) // État global partagé protégé par un Mutex (verrou d'exclusion mutuelle).
...
pub fn init(bar0: u64) -> bool {
    ...
    // Map MMIO region
    let base_virt = // Correspondance de motifs — branchement exhaustif de Rust.
...
unsafe { &*capability_regs };
    ...
    if xecp != 0 {
        ...
        for _ in 0..32 {
            let ecap_value = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_volatile(ecap_pointer as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32) };
            ...
            if ecap_id == 1 {
                ...
                if bios_owns != 0 {
                    ...
                    let mut ok = false;
                    for i in 0..1000u32 {
                        let v = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_volatile(ecap_pointer as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32) };
                        if (v >> 16) & 1 == 0 {
                            ok = true;
...
unsafe { core::ptr::write_volatile(ctlsts_pointer, 0); }
    ...
    let op = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &mut *operation_regs };
    ...
    let intr = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
unsafe { &mut *port_regs };
        ...
        if (portsc & PORTSC_CCS) != 0 {
            ...
            let speed_str = // Correspondance de motifs — branchement exhaustif de Rust.
match speed {
...
unsafe { &*controller.capability_regs };
    ...
    let array_physical = // Correspondance de motifs — branchement exhaustif de Rust.
...
fn address_device(controller: &mut XhciController, slot_id: u8, port_number: u8, speed: u8) -> bool {
    ...
    let device_context_physical = virt_to_physical(&*device_context as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const _ as u64);
    ...
    let ep0_ring = // Correspondance de motifs — branchement exhaustif de Rust.
match TransferRing::new() {
    ...
    let input_context_physical = // Correspondance de motifs — branchement exhaustif de Rust.
match crate::memory::frame::allocator_frame_zeroed() {
        Some(p) => p,
    ...
    let maximum_packet = // Correspondance de motifs — branchement exhaustif de Rust.
match speed as u32 {
        SPEED_LOW => 8u16,
        SPEED_FULL => 8,
        SPEED_HIGH => 64,
...
fn get_device_descriptor(controller: &mut XhciController, slot_id: u8, device: &mut XhciDevice) -> bool {
    let buffer_physical = // Correspondance de motifs — branchement exhaustif de Rust.
match crate::memory::frame::allocator_frame_zeroed() {
        Some(p) => p,
        None => return false,
    };
    let buffer_virt = physical_to_virt(buffer_physical) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8;
    ...
    if result.is_some() {
                // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
            let _bcd_usb = ptr::read_unaligned(buffer_virt.add(2) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u16);
            ...
            device.vendor_id = ptr::read_unaligned(buffer_virt.add(8) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u16);
            device.product_id = ptr::read_unaligned(buffer_virt.add(10) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u16);
...
const u8;
    ...
    let total_length = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { ptr::read_unaligned(buffer_virt.add(2) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u16) };
    ...
    let config_value = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
unsafe { *buffer_virt.add(offset) } as usize;
        let descriptor_type = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
match descriptor_type {
            USB_DT_INTERFACE if len >= 9 => {
                let iface_number = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { *buffer_virt.add(offset + 2) };
                let iface_class = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { *buffer_virt.add(offset + 5) };
                let iface_subclass = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { *buffer_virt.add(offset + 6) };
                let iface_protocol = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
match iface_protocol { 1 => "keyboard", 2 => "mouse", _ => "other" });
            ...
            USB_DT_ENDPOINT if len >= 7 => {
                let ep_address = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { *buffer_virt.add(offset + 2) };
                let ep_attrs = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { *buffer_virt.add(offset + 3) };
                let ep_maximum_packet = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { ptr::read_unaligned(buffer_virt.add(offset + 4) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u16) };
                let ep_interval = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { *buffer_virt.add(offset + 6) };
...
fn configure_hid_endpoint(
    ...
    let int_ring = // Correspondance de motifs — branchement exhaustif de Rust.
match TransferRing::new() {
        Some(r) => r,
        None => return false,
    };
    ...
    let input_physical = // Correspondance de motifs — branchement exhaustif de Rust.
match crate::memory::frame::allocator_frame_zeroed() {
        Some(p) => p,
        None => return false,
    };
...
unsafe {
        ...
        let xhci_interval = // Correspondance de motifs — branchement exhaustif de Rust.
match speed as u32 {
...
fn configure_bulk_endpoints(
    ...
    let bulk_in_ring = // Correspondance de motifs — branchement exhaustif de Rust.
match TransferRing::new() {
        Some(r) => r,
        None => return false,
    };
    let bulk_out_ring = // Correspondance de motifs — branchement exhaustif de Rust.
match TransferRing::new() {
        Some(r) => r,
        None => return false,
    };
...
unsafe { (*intr).erdp = erdp | (1 << 3); }
                        ...
                        if cc == 1 || cc == 13 { // Success or Short Packet
                            let report = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
                                core::slice::from_raw_parts(buffer_virt as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
