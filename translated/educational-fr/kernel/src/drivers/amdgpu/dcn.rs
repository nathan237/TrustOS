...
use super::{mmio_read32, mmio_write32, mmio_read_indirect, GpuInformation};
use super::regs::dcn;
...
const MODE_640X480_60: DisplayMode = DisplayMode {
...
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MODE_1280X720_60: DisplayMode = DisplayMode {
...
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MODE_1920X1080_60: DisplayMode = DisplayMode {
...
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MODE_2560X1440_60: DisplayMode = DisplayMode {
...
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MODE_3840X2160_60: DisplayMode = DisplayMode {
...
unsafe fn otg_read(mmio: u64, pipe: u8, register_offset: u32) -> u32 {
    let base = dcn::OTG0_BASE + (pipe as u32) * dcn::OTG_PIPE_STRIDE;
    mmio_read32(mmio, base + register_offset)
}

/// Write OTG register for a specific pipe
unsafe fn otg_write(mmio: u64, pipe: u8, register_offset: u32, value: u32) {
    let base = dcn::OTG0_BASE + (pipe as u32) * dcn::OTG_PIPE_STRIDE;
    mmio_write32(mmio, base + register_offset, value);
}

/// Read HUBP register for a specific pipe
unsafe fn hubp_read(mmio: u64, pipe: u8, register_offset: u32) -> u32 {
    let base = dcn::HUBP0_BASE + (pipe as u32) * dcn::HUBP_PIPE_STRIDE;
    mmio_read32(mmio, base + register_offset)
}

/// Write HUBP register for a specific pipe
unsafe fn hubp_write(mmio: u64, pipe: u8, register_offset: u32, value: u32) {
    let base = dcn::HUBP0_BASE + (pipe as u32) * dcn::HUBP_PIPE_STRIDE;
    mmio_write32(mmio, base + register_offset, value);
}
...
unsafe fn dig_read(mmio: u64, dig: u8, register_offset: u32) -> u32 {
    let base = dcn::DIG0_BASE + (dig as u32) * dcn::DIG_STRIDE;
    mmio_read32(mmio, base + register_offset)
}

/// Read HPD register
unsafe fn hpd_read(mmio: u64, hpd: u8, register_offset: u32) -> u32 {
...
unsafe {
            ...
            let transmit_type = (dig_status >> 16) & 0xF;
            let connector_type = // Correspondance de motifs — branchement exhaustif de Rust.
match transmit_type {
...
unsafe {
        ...
        let dpcd_data = mmio_read32(mmio, aux_base + dcn::AUX_DPHY_TRANSMIT_REF_CONTROL_OFFSET);
...
unsafe {
        ...
        let h_software = h_sync_end.saturating_sub(h_sync_start);
        let h_bp = ht.saturating_sub(h_sync_end);
        
        let v_fp = v_sync_start.saturating_sub(v_active);
        let v_software = v_sync_end.saturating_sub(v_sync_start);
        ...
        Some(DisplayMode {
            h_active, h_front_porch: h_fp, h_sync_width: h_software, h_back_porch: h_bp,
            v_active, v_front_porch: v_fp, v_sync_width: v_software, v_back_porch: v_bp,
            pixel_clock_khz,
...
unsafe {
        ...
        // Set surface size
        let size_register = (surface.height << 16) | surface.width;
        hubp_write(mmio, pipe, dcn::HUBP_SURFACE_SIZE_OFFSET, size_register);
        ...
        crate::serial_println!("[DCN] HUBP{} configured: addr={:#010X}:{:#010X} pitch={} fmt={:?}",
            pipe, address_hi, address_lo, surface.pitch, surface.format);
    }
}
...
pub fn init(mmio_base: u64) {
    ...
    let dcn_ver = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { mmio_read32(mmio_base, dcn::DCN_VERSION) };
    ...
    let dmcub_status = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { mmio_read32(mmio_base, dcn::DMCUB_STATUS) };
    ...
    for connection in &connectors {
        let status_str = // Correspondance de motifs — branchement exhaustif de Rust.
match connection.status {
    ...
    for pipe in 0..6u8 {
                // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
            let surf_address_hi = hubp_read(mmio_base, pipe, dcn::HUBP_SURFACE_ADDRESS_HIGH_OFFSET);
            let surf_address_lo = hubp_read(mmio_base, pipe, dcn::HUBP_SURFACE_ADDRESS_OFFSET);
    ...
    for connection in &connectors {
        if connection.status == ConnectorStatus::Connected {
            if let Some(ref mode) = connection.current_mode {
                crate::log!("[DCN]   Output {}: {} {}x{}@{}Hz", 
                    connection.index, connection.connector_type.name(),
...
match connection.status {
            ...
            let mut line = format!("  Connector {}: {} [{}]", 
                connection.index, connection.connector_type.name(), status);
