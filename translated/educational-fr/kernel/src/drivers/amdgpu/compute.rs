...
fn pm4_set_sh_register(register_offset: u32, value: u32) -> [u32; 3] {
    [
        pm4_type3_header(regs::PM4_SET_SH_REGISTER, 2),
        (register_offset - regs::SH_REGISTER_BASE) >> 2, // Register index (dword offset from base)
        value,
    ]
}

/// Build SET_SH_REG packet for two consecutive registers
fn pm4_set_sh_reg2(register_offset: u32, val0: u32, val1: u32) -> [u32; 4] {
    [
        pm4_type3_header(regs::PM4_SET_SH_REGISTER, 3),
        (register_offset - regs::SH_REGISTER_BASE) >> 2,
        val0,
        val1,
    ]
}
...
fn pm4_release_memory(fence_gpu_address: u64, fence_value: u64) -> [u32; 7] {
    [
        pm4_type3_header(regs::PM4_RELEASE_MEMORY, 6),
        ...
        (fence_gpu_address & 0xFFFFFFFF) as u32,          // address low
        ((fence_gpu_address >> 32) & 0xFFFF) as u32,      // address high
...
fn build_buffer_descriptor(gpu_addr: u64, number_elements: u32, stride: u32) -> [u32; 4] {
    ...
    [base_lo, dw1, number_elements, dw3]
}
...
match alloc::alloc::Layout::from_size_align(RING_SIZE_BYTES, 4096) {
    ...
    let ring_virt = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { alloc::alloc::alloc_zeroed(ring_layout) } as u64;
    ...
    let data_layout = // Correspondance de motifs — branchement exhaustif de Rust.
match alloc::alloc::Layout::from_size_align(DATA_BUFFER_SIZE, 4096) {
    ...
    let data_virt = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { alloc::alloc::alloc_zeroed(data_layout) } as u64;
    ...
    let code_layout = // Correspondance de motifs — branchement exhaustif de Rust.
match alloc::alloc::Layout::from_size_align(4096, 256) {
        Ok(l) => l,
        Err(_) => { crate::log!("[GPU-COMPUTE] ERROR: invalid code layout"); return; }
    };
    let code_virt = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { alloc::alloc::alloc_zeroed(code_layout) } as u64;
    ...
    let grbm_status = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { mmio_read32(mmio_base, regs::GRBM_STATUS) };
    let cp_me_cntl = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { mmio_read32(mmio_base, regs::CP_ME_CNTL) };
...
pub fn dispatch(agent: AgentKind, number_elements: u32, fill_value: u32) -> Result<u64, &'static str> {
...
unsafe { core::ptr::write_volatile(data_pointer.add(i as usize), 0); }
        ...
        AgentKind::MemCopy => {
            ...
            for i in 0..half {
                                // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::write_volatile(data_pointer.add(i as usize), 0xA0A0_0000 + i); }
            }
            for i in half..number_elements {
                                // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::write_volatile(data_pointer.add(i as usize), 0); }
            }
        }
    ...
    // Clear fence
    let fence_pointer = (state.data_virt + FENCE_OFFSET as u64) as *mut u64;
        // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::write_volatile(fence_pointer, 0); }
    
    // Step 2: Upload shader code to code buffer
    let shader = agent.shader_code();
    let code_pointer = state.code_virt as *mut u32;
    for (i, &insn) in shader.iter().enumerate() {
                // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::write_volatile(code_pointer.add(i), insn); }
    }
    ...
    let pgm_packet = pm4_set_sh_reg2(
        regs::COMPUTE_PGM_LO,
        (shader_address_256 & 0xFFFFFFFF) as u32,
        ((shader_address_256 >> 32) & 0xFFFF) as u32,
    );
    ring_write(&mut state, &pgm_packet);
    
    // SET_SH_REG: COMPUTE_PGM_RSRC1
    let rsrc1_packet = pm4_set_sh_register(regs::COMPUTE_PGM_RSRC1, pgm_rsrc1);
    ring_write(&mut state, &rsrc1_packet);
    
    // SET_SH_REG: COMPUTE_PGM_RSRC2
    let rsrc2_packet = pm4_set_sh_register(regs::COMPUTE_PGM_RSRC2, pgm_rsrc2);
    ring_write(&mut state, &rsrc2_packet);
    
    // SET_SH_REG: COMPUTE_NUM_THREAD_X/Y/Z (workgroup size = 64 threads, RDNA wavefront)
    let thread_x_packet = pm4_set_sh_register(regs::COMPUTE_NUMBER_THREAD_X, 64);
    ring_write(&mut state, &thread_x_packet);
    let thread_y_packet = pm4_set_sh_register(regs::COMPUTE_NUMBER_THREAD_Y, 1);
    ring_write(&mut state, &thread_y_packet);
    let thread_z_packet = pm4_set_sh_register(regs::COMPUTE_NUMBER_THREAD_Z, 1);
    ring_write(&mut state, &thread_z_packet);
    ...
    let number_groups = (number_elements + 63) / 64;
    let dispatch_packet = pm4_dispatch_direct(number_groups, 1, 1);
...
loop {
        let current_fence = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_volatile(fence_pointer) };
...
unsafe { mmio_read32(mmio, regs::GRBM_STATUS) };
            let rptr = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
pub fn verify_results(agent: AgentKind, number_elements: u32, fill_value: u32) -> (u32, u32) {
    let state = COMPUTE.lock();
    let data_pointer = state.data_virt as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32;
    let mut pass = 0u32;
    let mut fail = 0u32;
    ...
    for i in 0..check_count {
        let actual = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_volatile(data_pointer.add(i as usize)) };
        let expected = // Correspondance de motifs — branchement exhaustif de Rust.
match agent {
...
pub fn read_data(index: u32) -> Option<u32> {
    ...
    let ptr = state.data_virt as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32;
