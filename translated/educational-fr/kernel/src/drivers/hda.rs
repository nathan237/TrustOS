...
unsafe fn read8(&self, offset: u32) -> u8 {
        core::ptr::read_volatile((self.mmio_base + offset as u64) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8)
    }
        ...
        unsafe {
            ...
            for _ in 0..10000 {
                ...
                if rirb_wp != self.rirb_rp {
                    ...
                    let rirb_pointer = self.rirb_virt as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u64;
...
pub fn play_effect(effect: SoundEffect) -> Result<(), &'static str> {
    let tones: Vec<(u32, u32, i16)> = // Correspondance de motifs — branchement exhaustif de Rust.
match effect {
...
fn parse_note_name(bytes: &[u8]) -> (u8, &[u8]) {
    ...
    let base = // Correspondance de motifs — branchement exhaustif de Rust.
match bytes[0] {
        b'C' | b'c' => 0,
        b'D' | b'd' => 2,
        b'E' | b'e' => 4,
