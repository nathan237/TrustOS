...
pub fn write_byte(index: u8, byte: u8) {
    ...
    let uart = // Correspondance de motifs — branchement exhaustif de Rust.
...
match guard[index as usize].as_ref() {
        Some(uart) => // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
