...
unsafe { core::mem::zeroed() }),
            cmd_tables: core::array::from_fn(|_| Box::new(// SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::mem::zeroed() })),
        }
    }
}
...
match sig {
                ...
                if device_type != AhciDeviceType::None {
                    ...
                    let clb_physical = virt_to_physical(&*mem.cmd_list as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const _ as u64);
...
unsafe { 
        core::slice::from_raw_parts(identify_buffer.as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u16, 256) 
    };
