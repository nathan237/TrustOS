...
pub struct HbaFis {
    ...
    pub _pad1: [u8; 12],
    pub rfis: FisRegisterD2H,          // 0x40-0x53
    pub _pad2: [u8; 4],
...
pub fn new() -> Self {
        Self {
            ...
            fis: Box::new(// SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::mem::zeroed() }),
            command_tables: core::array::from_fn(|_| Box::new(// SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::mem::zeroed() })),
        }
    }
}
...
/// SATA signatures
const SATA_SIGNAL_ATA: u32 = 0x00000101;
// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SATA_SIGNAL_ATAPI: u32 = 0xEB140101;
// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SATA_SIGNAL_SEMB: u32 = 0xC33C0101;
// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SATA_SIGNAL_PM: u32 = 0x96690101;

/// Port command bits
const HBA_PORT_COMMAND_ST: u32 = 1 << 0;   // Start
const HBA_PORT_COMMAND_FRE: u32 = 1 << 4;  // FIS Receive Enable
const HBA_PORT_COMMAND_FR: u32 = 1 << 14;  // FIS Receive Running
const HBA_PORT_COMMAND_CR: u32 = 1 << 15;  // Command List Running
...
/// ATA Status bits
const ATA_DEVICE_BUSY: u8 = 0x80;
// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const ATA_DEVICE_DRQ: u8 = 0x08;
...
fn stop_command(port: &mut HbaPort) {
    // Clear ST (Stop command engine)
    port.cmd &= !HBA_PORT_COMMAND_ST;
    
    // Clear FRE
    port.cmd &= !HBA_PORT_COMMAND_FRE;
    
    // Wait until FR and CR are cleared
    for _ in 0..1000 {
        if (port.cmd & HBA_PORT_COMMAND_FR) == 0 && (port.cmd & HBA_PORT_COMMAND_CR) == 0 {
            break;
        }
        // Small delay
...
fn start_command(port: &mut HbaPort) {
    // Wait until CR is cleared
    while (port.cmd & HBA_PORT_COMMAND_CR) != 0 {
        core::hint::spin_loop();
    }
...
pub fn init(bar5: u64) -> bool {
    ...
    let abar_physical = (bar5 & !0xF) as u64;
    
        // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const AHCI_MMIO_SIZE: usize = 0x2000;  // 8KB
    
    crate::serial_println!("[AHCI] Mapping MMIO at phys={:#x} size={:#x}", abar_physical, AHCI_MMIO_SIZE);
    
    let abar_virt = // Correspondance de motifs — branchement exhaustif de Rust.
match crate::memory::map_mmio(abar_physical, AHCI_MMIO_SIZE) {
        Ok(virt) => virt,
        Err(e) => {
    ...
    crate::serial_println!("[AHCI] Initializing at ABAR phys={:#x} virt={:#x}", abar_physical, abar_virt);
    
    let hba = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &mut *(abar_virt as *mut HbaMemory) };
...
unsafe { &mut *(hba.ports.as_mut_pointer().add(i)) };
            ...
            if det == 3 && ipm == 1 {
                let sig = port.sig;
                let device_type = // Correspondance de motifs — branchement exhaustif de Rust.
match sig {
                    SATA_SIGNAL_ATA => AhciDeviceType::Sata,
                    SATA_SIGNAL_ATAPI => AhciDeviceType::Satapi,
                    SATA_SIGNAL_SEMB => AhciDeviceType::Semb,
                    SATA_SIGNAL_PM => AhciDeviceType::Pm,
                ...
                if device_type != AhciDeviceType::None {
                    ...
                    let clb_physical = virt_to_physical(&*mem.command_list as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const _ as u64);
                    
                    // Set FIS Base Address
                    let framebuffer_physical = virt_to_physical(&*mem.fis as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const _ as u64);
                    
                    // Verify DMA addresses are within controller capability
                    if !s64a && (clb_physical > 0xFFFF_FFFF || framebuffer_physical > 0xFFFF_FFFF) {
                        crate::serial_println!("[AHCI] WARNING: Port {} DMA buffers above 4GB \
                            but controller lacks S64A! clb={:#x} fb={:#x}", i, clb_physical, framebuffer_physical);
...
unsafe { &mut *(hba.ports.as_mut_pointer().add(port_number as usize)) };
    ...
    let command_table_physical = virt_to_physical(command_table as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const _ as u64);
    ...
    let cfis = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &mut *(command_table.cfis.as_mut_pointer() as *mut FisRegisterH2D) };
    ...
    let mut spin = 0u32;
    while (port.tfd & ((ATA_DEVICE_BUSY | ATA_DEVICE_DRQ) as u32)) != 0 && spin < 1_000_000 {
        spin += 1;
        core::hint::spin_loop();
    }
    if spin >= 1_000_000 {
...
loop {
    ...
    let words = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { 
        core::slice::from_raw_parts(identify_buffer.as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u16, 256) 
    };
