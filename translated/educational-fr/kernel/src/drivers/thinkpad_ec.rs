...
match args.first().copied() {
        None | Some("status") => {
            ...
            if let Some(level) = fan_get_level() {
                let desc = // Correspondance de motifs — branchement exhaustif de Rust.
match level {
                    0x80 => "auto (EC controlled)",
                    0x40 => "DISENGAGED (full speed)",
                    l if l <= 7 => // Correspondance de motifs — branchement exhaustif de Rust.
match l {
                        0 => "0 (off)",
                        1 => "1 (lowest)",
                        2 => "2",
...
match args.first().copied() {
        ...
        Some("set") => {
            ...
            let fid = // Correspondance de motifs — branchement exhaustif de Rust.
match args[1].parse::<u8>() {
            ...
            let vid = // Correspondance de motifs — branchement exhaustif de Rust.
