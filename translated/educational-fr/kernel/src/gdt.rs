...
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const USER_DATA_SELECTOR: u16 = 0x18 | 3; // RPL = 3
/// Segment selector for user code (Ring 3)
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const USER_CODE_SELECTOR: u16 = 0x20 | 3; // RPL = 3
/// TSS selector
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
impl TssEntry {
    ...
    pub fn new(tss_address: u64) -> Self {
        let base = tss_address;
...
unsafe {
        ...
        let ist1_stack = allocator_kernel_stack();
