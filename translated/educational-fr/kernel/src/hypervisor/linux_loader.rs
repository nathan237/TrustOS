...
const BOOT_PARAMS_ADDRESS: u64 = 0x7000;

/// Where the command line is placed
const CMDLINE_ADDRESS: u64 = 0x20000;

/// Maximum command line size
const CMDLINE_MAXIMUM: usize = 2048;

/// Where guest page tables start (PML4)
const PAGE_TABLES_ADDRESS: u64 = 0x70000;

/// Where the guest GDT is placed
const GDT_ADDRESS: u64 = 0x60000;

/// Where the protected-mode kernel is loaded (standard 1MB)
const KERNEL_LOAD_ADDRESS: u64 = 0x100000;

/// Where the initrd is loaded (after kernel, aligned to page)
const INITRD_LOAD_ADDRESS: u64 = 0x1000000; // 16MB — leaves room for kernel

/// Guest stack pointer
const GUEST_STACK_ADDRESS: u64 = 0x80000; // 512KB
...
const LOADED_HIGH: u8 = 0x01;      // Protected-mode code loaded at 0x100000
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CAN_USE_HEAP: u8 = 0x80;     // Set heap_end_ptr

/// Xloadflags bits
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const XLF_KERNEL_64: u16 = 0x01;   // Kernel has 64-bit entry point
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const XLF_CAN_BE_LOADED_ABOVE_4G: u16 = 0x02;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const XLF_EFI_HANDOVER_32: u16 = 0x04;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const XLF_EFI_HANDOVER_64: u16 = 0x08;
...
pub fn load_linux_kernel(
    ...
    let kernel_end = KERNEL_LOAD_ADDRESS as usize + kernel.kernel_data.len();
    if kernel_end > guest_memory.len() {
        return Err(HypervisorError::OutOfMemory);
    }
    guest_memory[KERNEL_LOAD_ADDRESS as usize..kernel_end]
        .copy_from_slice(&kernel.kernel_data);
    crate::serial_println!("[Linux] Kernel loaded at 0x{:X}-0x{:X}", 
                          KERNEL_LOAD_ADDRESS, kernel_end);
    ...
    guest_memory[command_start + cmdline_length] = 0; // null terminate
    crate::serial_println!("[Linux] Command line at 0x{:X}: \"{}\"", 
                          CMDLINE_ADDRESS, config.cmdline);
...
fn setup_boot_params(
    ...
    let bp = BOOT_PARAMS_ADDRESS as usize;
    ...
    write_u32(guest_memory, bp + 0x228, CMDLINE_ADDRESS as u32);

    // ramdisk_image & ramdisk_size
    if config.initrd.is_some() {
        write_u32(guest_memory, bp + 0x218, INITRD_LOAD_ADDRESS as u32);
    ...
    crate::serial_println!("[Linux] boot_params at 0x{:X}, {} e820 entries, cmdline at 0x{:X}",
                          BOOT_PARAMS_ADDRESS, e820_entries, CMDLINE_ADDRESS);
...
fn setup_guest_page_tables(guest_memory: &mut [u8], _memory_size: u64) -> Result<()> {
    ...
    let pdpt_address = PAGE_TABLES_ADDRESS + 0x1000;
    write_u64(guest_memory, pt_base, pdpt_address | 0x3); // Present + Writable

    // PDPT[0..3] → PD0..PD3
    for i in 0..4u64 {
        let pd_address = PAGE_TABLES_ADDRESS + 0x2000 + i * 0x1000;
        write_u64(guest_memory, pt_base + 0x1000 + (i as usize) * 8,
                  pd_address | 0x3); // Present + Writable
    }
...
fn setup_guest_gdt(guest_memory: &mut [u8]) -> Result<()> {
    let gdt_base = GDT_ADDRESS as usize;
    ...
    write_u64(guest_memory, gdt_base + 0x102, GDT_ADDRESS);
...
pub fn configure_vmcs_for_linux(
    ...
    for (sel_field, base_field, limit_field, access_field) in [
        ...
        (fields::GUEST_FILESYSTEM_SELECTOR, fields::GUEST_FILESYSTEM_BASE, fields::GUEST_FILESYSTEM_LIMIT, fields::GUEST_FILESYSTEM_ACCESS_RIGHTS),
...
pub fn create_test_linux_kernel() -> Vec<u8> {
    ...
    let debug_message = b"[TrustVM-Linux] 64-bit kernel entry OK\n";
    for &byte in debug_message {
        image.extend_from_slice(&[0xB0, byte]);
