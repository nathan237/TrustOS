...
const CR_RD_INTERCEPTS: usize = 0x000;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CR_WR_INTERCEPTS: usize = 0x004;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DR_RD_INTERCEPTS: usize = 0x008;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DR_WR_INTERCEPTS: usize = 0x00C;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const EXCEPTION_INTERCEPTS: usize = 0x010;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const INTERCEPT_MISC1: usize = 0x014;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const INTERCEPT_MISC2: usize = 0x018;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PAUSE_FILTER_THRESH: usize = 0x03C;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PAUSE_FILTER_COUNT: usize = 0x03E;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
const TLB_CONTROL: usize = 0x05C;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const V_TPR: usize = 0x060;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const V_INTERRUPT_REQUEST: usize = 0x061;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const V_INTR: usize = 0x062;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const V_INTR_MASKING: usize = 0x063;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const V_INTR_VECTOR: usize = 0x064;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const INTERRUPT_SHADOW: usize = 0x068;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
const EXITINFO2: usize = 0x080;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const EXITINTINFO: usize = 0x088;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const NP_ENABLE: usize = 0x090;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const AVIC_APIC_BAR: usize = 0x098;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GHCB_PA: usize = 0x0A0;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const EVENT_INJ: usize = 0x0A8;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const N_CR3: usize = 0x0B0;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const LBR_CONTROL: usize = 0x0B8;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const VMCB_CLEAN: usize = 0x0C0;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const NEXT_RIP: usize = 0x0C8;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const BYTES_FETCHED: usize = 0x0D0;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GUEST_INST_BYTES: usize = 0x0D1;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const AVIC_APIC_BACKING: usize = 0x0E0;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const AVIC_LOGICAL_TABLE: usize = 0x0F0;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const AVIC_PHYSICAL_TABLE: usize = 0x0F8;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const VMSA_POINTER: usize = 0x108;
}
...
const BASE: usize = 0x400;
    ...
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const ES_SELECTOR: usize = BASE + 0x000;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const ES_ATTRIB: usize = BASE + 0x002;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const ES_LIMIT: usize = BASE + 0x004;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const ES_BASE: usize = BASE + 0x008;
    
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CS_SELECTOR: usize = BASE + 0x010;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CS_ATTRIB: usize = BASE + 0x012;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CS_LIMIT: usize = BASE + 0x014;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CS_BASE: usize = BASE + 0x018;
    
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SS_SELECTOR: usize = BASE + 0x020;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SS_ATTRIB: usize = BASE + 0x022;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SS_LIMIT: usize = BASE + 0x024;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SS_BASE: usize = BASE + 0x028;
    
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DS_SELECTOR: usize = BASE + 0x030;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DS_ATTRIB: usize = BASE + 0x032;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DS_LIMIT: usize = BASE + 0x034;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DS_BASE: usize = BASE + 0x038;
    
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const FILESYSTEM_SELECTOR: usize = BASE + 0x040;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const FILESYSTEM_ATTRIB: usize = BASE + 0x042;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const FILESYSTEM_LIMIT: usize = BASE + 0x044;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const FILESYSTEM_BASE: usize = BASE + 0x048;
    
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GS_SELECTOR: usize = BASE + 0x050;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GS_ATTRIB: usize = BASE + 0x052;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GS_LIMIT: usize = BASE + 0x054;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GS_BASE: usize = BASE + 0x058;
    
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GDTR_SELECTOR: usize = BASE + 0x060;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GDTR_ATTRIB: usize = BASE + 0x062;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GDTR_LIMIT: usize = BASE + 0x064;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GDTR_BASE: usize = BASE + 0x068;
    
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const LDTR_SELECTOR: usize = BASE + 0x070;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const LDTR_ATTRIB: usize = BASE + 0x072;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const LDTR_LIMIT: usize = BASE + 0x074;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const LDTR_BASE: usize = BASE + 0x078;
    
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const IDTR_SELECTOR: usize = BASE + 0x080;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const IDTR_ATTRIB: usize = BASE + 0x082;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const IDTR_LIMIT: usize = BASE + 0x084;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const IDTR_BASE: usize = BASE + 0x088;
    
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TR_SELECTOR: usize = BASE + 0x090;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TR_ATTRIB: usize = BASE + 0x092;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TR_LIMIT: usize = BASE + 0x094;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TR_BASE: usize = BASE + 0x098;
    
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CPL: usize = BASE + 0x0CB;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const EFER: usize = BASE + 0x0D0;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CR4: usize = BASE + 0x148;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CR3: usize = BASE + 0x150;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CR0: usize = BASE + 0x158;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DR7: usize = BASE + 0x160;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DR6: usize = BASE + 0x168;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const RFLAGS: usize = BASE + 0x170;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const RIP: usize = BASE + 0x178;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const RSP: usize = BASE + 0x1D8;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const RAX: usize = BASE + 0x1F8;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const STAR: usize = BASE + 0x200;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const LSTAR: usize = BASE + 0x208;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CSTAR: usize = BASE + 0x210;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SFMASK: usize = BASE + 0x218;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const KERNEL_GS_BASE: usize = BASE + 0x220;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SYSENTER_CS: usize = BASE + 0x228;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SYSENTER_ESP: usize = BASE + 0x230;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SYSENTER_EIP: usize = BASE + 0x238;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CR2: usize = BASE + 0x240;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PAT: usize = BASE + 0x268;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DBGCTL: usize = BASE + 0x270;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const BR_FROM: usize = BASE + 0x278;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const BR_TO: usize = BASE + 0x280;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const LASTEXCPFROM: usize = BASE + 0x288;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const LASTEXCPTO: usize = BASE + 0x290;
}
...
const VINTR: u32 = 1 << 4;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CR0_SEL_WRITE: u32 = 1 << 5;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const IDTR_READ: u32 = 1 << 6;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GDTR_READ: u32 = 1 << 7;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const LDTR_READ: u32 = 1 << 8;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TR_READ: u32 = 1 << 9;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const IDTR_WRITE: u32 = 1 << 10;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GDTR_WRITE: u32 = 1 << 11;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const LDTR_WRITE: u32 = 1 << 12;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TR_WRITE: u32 = 1 << 13;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const RDTSC: u32 = 1 << 14;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const RDPMC: u32 = 1 << 15;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PUSHF: u32 = 1 << 16;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POPF: u32 = 1 << 17;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CPUID: u32 = 1 << 18;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const RSM: u32 = 1 << 19;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const IRET: u32 = 1 << 20;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SWINT: u32 = 1 << 21;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const INVD: u32 = 1 << 22;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PAUSE: u32 = 1 << 23;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const HLT: u32 = 1 << 24;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const INVLPG: u32 = 1 << 25;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const INVLPGA: u32 = 1 << 26;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const IOIO: u32 = 1 << 27;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MSR: u32 = 1 << 28;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TASK_SWITCH: u32 = 1 << 29;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const FERR_FREEZE: u32 = 1 << 30;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SHUTDOWN: u32 = 1 << 31;
}
...
const VMRUN: u32 = 1 << 0;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const VMMCALL: u32 = 1 << 1;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const VMLOAD: u32 = 1 << 2;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const VMSAVE: u32 = 1 << 3;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const STGI: u32 = 1 << 4;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CLGI: u32 = 1 << 5;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SKINIT: u32 = 1 << 6;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const RDTSCP: u32 = 1 << 7;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const ICEBP: u32 = 1 << 8;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const WBINVD: u32 = 1 << 9;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MONITOR: u32 = 1 << 10;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MWAIT: u32 = 1 << 11;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MWAIT_CONDITIONAL: u32 = 1 << 12;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const XSETBV: u32 = 1 << 13;
}

/// VMCB Clean Bits
pub mod clean_bits {
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const INTERCEPTS: u32 = 1 << 0;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const IOPM: u32 = 1 << 1;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const ASID: u32 = 1 << 2;
...
const DO_NOTHING: u32 = 0;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const FLUSH_ALL: u32 = 1;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const FLUSH_THIS_ASID: u32 = 3;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const FLUSH_NON_GLOBAL_THIS_ASID: u32 = 7;
}
...
pub fn write_state(&mut self, offset: usize, value: u64) {
    ...
    pub fn set_fs(&mut self, selector: u16, attrib: u16, limit: u32, base: u64) {
        ...
        self.write_u64(state_offsets::FILESYSTEM_BASE, base);
    }
...
const DPL_3: u16 = 3 << 5;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const S_CODE_DATA: u16 = 1 << 4;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TYPE_CODE_RECEIVE: u16 = 0x0A;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TYPE_DATA_RW: u16 = 0x02;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GRANULARITY: u16 = 1 << 8;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DB: u16 = 1 << 10;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const LONG: u16 = 1 << 9;
