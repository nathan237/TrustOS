...
const VM_CR: u32 = 0xC001_0114;        // VM Control Register
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const VM_HSAVE_PA: u32 = 0xC001_0117;  // Host Save Area Physical Address
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const EFER: u32 = 0xC000_0080;         // Extended Feature Enable Register
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SVM_LOCK: u32 = 0xC001_0118;     // SVM Lock Key
}

/// EFER bits
pub mod efer {
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SVME: u64 = 1 << 12;  // SVM Enable
}

/// VM_CR bits
pub mod vm_cr {
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SVM_DIS: u64 = 1 << 4;       // SVM Disable
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SVM_LOCK: u64 = 1 << 3;      // SVM Lock
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const R_INITIALIZE: u64 = 1 << 1;        // Redirect INIT
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DIS_A20M: u64 = 1 << 0;      // Disable A20 Masking
}
...
match code {
            ...
            _ if code >= 0x40 && code <= 0x5F => // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { 
...
#[inline]
pub // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe fn invlpga(vaddr: u64, asid: u32) {
    core::arch::asm!(
        "invlpga rax, ecx",
        in("rax") vaddr,
        in("ecx") asid,
