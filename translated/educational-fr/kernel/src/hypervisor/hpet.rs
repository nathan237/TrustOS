...
const HPET_PERIOD_FILESYSTEM: u32 = 69_841_279; // ~14.318180 MHz

/// Number of timers we emulate
const NUMBER_TIMERS: usize = 3;

/// Register offsets
mod regs {
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GCAP_ID: u64      = 0x000;  // General Capabilities and ID
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GCONF: u64        = 0x010;  // General Configuration
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GISR: u64         = 0x020;  // General Interrupt Status
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MAIN_COUNTER: u64 = 0x0F0;  // Main Counter Value
    
    // Timer N registers at 0x100 + 0x20*N
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TIMER_BASE: u64   = 0x100;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TIMER_STRIDE: u64 = 0x20;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TN_CONFIGURATION: u64      = 0x00;   // Timer N Config + Capabilities
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TN_COMP: u64      = 0x08;   // Timer N Comparator
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TN_FSB: u64       = 0x10;   // Timer N FSB Route
}
...
pub struct HpetState {
    ...
    pub enabled: bool,
    /// The 3 timers
    pub timers: [HpetTimer; NUMBER_TIMERS],
}

// Implémentation de trait — remplit un contrat comportemental.
impl Default for HpetState {
    fn default() -> Self {
        let mut timers: [HpetTimer; NUMBER_TIMERS] = core::array::from_fn(|_| HpetTimer::default());
...
impl HpetState {
    ...
    pub fn read(&self, offset: u64, size: u8) -> u64 {
        let val64 = // Correspondance de motifs — branchement exhaustif de Rust.
match offset {
            regs::GCAP_ID => {
                ...
                let number_timers_minus_1 = (NUMBER_TIMERS as u64 - 1) << 8;
            ...
            // Timer registers
            off if off >= regs::TIMER_BASE && off < regs::TIMER_BASE + (NUMBER_TIMERS as u64) * regs::TIMER_STRIDE + 0x18 => {
                let timer_off = off - regs::TIMER_BASE;
                let timer_index = (timer_off / regs::TIMER_STRIDE) as usize;
                let register_off = timer_off % regs::TIMER_STRIDE;
                
                if timer_index < NUMBER_TIMERS {
                                        // Correspondance de motifs — branchement exhaustif de Rust.
match register_off {
                        regs::TN_CONFIGURATION => self.timers[timer_index].config,
                        regs::TN_COMP => self.timers[timer_index].comparator,
                        regs::TN_FSB  => self.timers[timer_index].fsb_route,
                        _ => 0,
                    }
                } else {
                    0
...
match register_off {
                        regs::TN_CONFIGURATION => {
                            ...
                            // Bit 14: FSB enable
                            let ro_mask: u64 = self.timers[timer_index].config & 0xFFFF_FFFF_FFFF_8181;
                            let wr_mask: u64 = 0x0000_0000_0000_7E7E;
                            self.timers[timer_index].config = ro_mask | (value & wr_mask);
                        }
                        regs::TN_COMP => {
                            self.timers[timer_index].comparator = value;
                        }
                        regs::TN_FSB => {
                            self.timers[timer_index].fsb_route = value;
                        }
                        _ => {}
                    }
                }
    ...
    pub fn check_timers(&self) -> [(bool, u8); NUMBER_TIMERS] {
        let counter = self.main_counter();
        let mut result = [(false, 0u8); NUMBER_TIMERS];
        
        for i in 0..NUMBER_TIMERS {
            ...
            if counter >= comparator && comparator > 0 {
                let interrupt_request_route = ((config >> 9) & 0x1F) as u8;
                result[i] = (true, interrupt_request_route);
            }
        }
        result
    }
...
mod tests {
    ...
    fn test_hpet_gcap_id() {
        ...
        assert_eq!(period, HPET_PERIOD_FILESYSTEM);
        let number_timers = ((gcap >> 8) & 0x1F) as usize;
        assert_eq!(number_timers, 2); // NUM_TIMERS - 1
