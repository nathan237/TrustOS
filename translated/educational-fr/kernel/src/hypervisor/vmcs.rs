...
impl Vmcs {
    ...
    pub fn new(revision_id: u32) -> Result<Self> {
        ...
        let virt_address = region.as_ref() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
unsafe {
        ...
        let gdt_pointer = gdtr.base as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u64;
...
unsafe {
        ...
        let gdt_base = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::address_of!(gdtr.base).read_unaligned() };
        let idt_base = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
