...
match exit {
            ...
            SvmExitCode::Rdtsc => {
                ...
                let tsc = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
match offset {
                ...
                LAPIC_TIMER_LVT => {
                    ...
                    if self.stats.vmexits < 1000 {
                        ...
                        let mode_str = // Correspondance de motifs — branchement exhaustif de Rust.
match mode {
                            0 => "one-shot",
                            1 => "periodic",
                            2 => "TSC-deadline",
            ...
            // Read
            let value: u32 = // Correspondance de motifs — branchement exhaustif de Rust.
match offset {
...
match self.lapic.dcr & 0xB {
    ...
    fn handle_io(&mut self, info1: u64) {
        ...
        let _size = // Correspondance de motifs — branchement exhaustif de Rust.
match (info1 >> 4) & 0x7 {
            0 => 1, // Byte
            1 => 2, // Word
            2 => 4, // Dword
...
match port {
                ...
                0x2F8..=0x2FF => // Correspondance de motifs — branchement exhaustif de Rust.
match port & 0x7 {
                    5 => 0x60, // LSR: TX empty
                    _ => 0,
                },
...
match msr {
            ...
            // RDMSR
            let value: u64 = // Correspondance de motifs — branchement exhaustif de Rust.
match msr {
    ...
    fn handle_vmmcall(&mut self) -> bool {
        ...
        let (result, should_continue): (i64, bool) = // Correspondance de motifs — branchement exhaustif de Rust.
match function {
            // Exit VM
            0x00 => {
                self.state = SvmVmState::Stopped;
...
mod tests {
    ...
    fn test_lapic_divider_decode() {
        ...
        for &(dcr, expected) in &map {
            let d = // Correspondance de motifs — branchement exhaustif de Rust.
match dcr & 0xB {
                0x0 => 2u64, 0x1 => 4, 0x2 => 8, 0x3 => 16,
