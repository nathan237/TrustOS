...
pub fn allocate() -> Option<u16> {
    ...
    for offset in 0..MAXIMUM_VPID {
        let vpid = ((start as u32 + offset as u32) % (MAXIMUM_VPID as u32)) as u16;
...
unsafe fn invvpid(inv_type: InvvpidType, desc: &InvvpidDescriptor) {
    ...
    core::arch::asm!(
        ...
        in(reg) desc as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const InvvpidDescriptor,
        out(reg_byte) result,
        options(nostack)
    );
