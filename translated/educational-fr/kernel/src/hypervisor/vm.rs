...
pub fn new(id: u64, name: &str, memory_mb: usize) -> Result<Self> {
    ...
    pub fn start(&mut self, entry_point: u64, stack_pointer: u64) -> Result<()> {
        ...
        let exit_handler = vm_exit_stub as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const () as u64;
...
match exit_reason {
            ...
            exit_reason::IO_INSTRUCTION => {
                ...
                let directory = if (exit_qual & 8) == 0 { "OUT" } else { "IN" };
...
const MSR_TSC_AUX: u32 = 0xC000_0103;
        ...
        if is_write {
            ...
            match msr {
                ...
                MSR_FILESYSTEM_BASE | MSR_GS_BASE | MSR_KERNEL_GS_BASE |
...
match function {
            ...
            0x100..=0x1FF => {
                let virtfs_operation = (function - 0x100) as u32;
                let args = [
                    self.guest_regs.rbx,
                    self.guest_regs.rcx,
                    self.guest_regs.rdx,
                ...
                let (result, _data) = super::virtfs::handle_hypercall(self.id, virtfs_operation, &args);
