...
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const KERNEL_OFFSET: u64 = 0x0020_0000;  // 2MB

/// DTB offset from RAM base
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DTB_OFFSET: u64 = 0x0400_0000;     // 64MB

/// Initrd offset from RAM base
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const INITRD_OFFSET: u64 = 0x0500_0000;  // 80MB

/// Default guest RAM size (512MB)
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DEFAULT_RAM_SIZE: u64 = 512 * 1024 * 1024;

/// Maximum kernel size (64MB should be more than enough)
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MAXIMUM_KERNEL_SIZE: usize = 64 * 1024 * 1024;

/// Maximum DTB size (1MB)
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MAXIMUM_DTB_SIZE: usize = 1 * 1024 * 1024;

/// Maximum initrd size (the rest of RAM after initrd offset)
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MAXIMUM_INITRD_SIZE: usize = 400 * 1024 * 1024;
...
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TEXT_OFFSET_FIELD: usize = 0x08;
...
pub fn load_arm64_guest(
    ...
    if kernel_data.len() > MAXIMUM_KERNEL_SIZE {
        return Err(format!("Kernel too large: {}MB (max {}MB)",
            kernel_data.len() / (1024*1024), MAXIMUM_KERNEL_SIZE / (1024*1024)));
    }
    ...
    if let Some(initrd) = initrd_data {
        if initrd.len() > MAXIMUM_INITRD_SIZE {
            return Err(format!("initrd too large: {}MB (max {}MB)",
                initrd.len() / (1024*1024), MAXIMUM_INITRD_SIZE / (1024*1024)));
        }
    }
