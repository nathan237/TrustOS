...
const DATA_ABORT_LOWER: u32  = 0b10_0100;  // 0x24
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const INST_ABORT_LOWER: u32  = 0b10_0000;  // 0x20
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const HVC64: u32             = 0b01_0110;   // 0x16
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SMC64: u32             = 0b01_0111;   // 0x17
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MSR_MRS: u32           = 0b01_1000;   // 0x18
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const WFX: u32              = 0b00_0001;   // 0x01
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SVC64: u32             = 0b01_0101;   // 0x15
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const FP_EXCEPTION: u32            = 0b10_1100;   // 0x2C
}
...
fn handle_data_abort(
    ...
    let register_index = dabt::srt(iss) as usize;

    if is_write {
        // Guest is writing to MMIO — get the value from guest register
        let value = if register_index < 31 { guest_regs[register_index] } else { 0 };
        ...
        if register_index < 31 {
            guest_regs[register_index] = value;
        }
    }
...
fn handle_smc(guest_regs: &mut [u64; 31]) -> TrapAction {
    ...
    mmio_spy::log_smc(mmio_spy::SmcEvent {
        ...
        x2,
        x3,
        smc_type_name:         // Correspondance de motifs — branchement exhaustif de Rust.
match smc_type {
