...
static MMIO_WRITE_INDEX: AtomicUsize = AtomicUsize::new(0);
/// Total MMIO events logged
static MMIO_TOTAL: AtomicU64 = AtomicU64::new(0);

/// Write cursor for SMC events
static SMC_WRITE_INDEX: AtomicUsize = AtomicUsize::new(0);
...
pub fn recent_mmio_events(count: usize) -> alloc::vec::Vec<MmioEvent> {
    ...
    for i in 0..n {
        ...
        let slot = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
pub fn format_mmio_event(event: &MmioEvent) -> alloc::string::String {
    ...
    let size_str = // Correspondance de motifs — branchement exhaustif de Rust.
match event.access_size {
        1 => "B",
        2 => "H",
        4 => "W",
        8 => "D",
