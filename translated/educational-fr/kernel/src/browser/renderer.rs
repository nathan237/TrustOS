    ...
    match tag {
                ...
                if ctx.in_ordered_list {
                    ...
                    if let Some(counter) = ctx.list_counters.last_mut() {
                        *counter += 1;
                        ...
                        let number_x = ctx.x - 20;
                        for (i, c) in number_str.chars().enumerate() {
                            draw_char((number_x + i as i32 * 8) as u32, ctx.y as u32, c, COLOR_TEXT);
                        }
                    }
                } else {
                    // Bullet
        ...
        "img" => {
            ...
            if ctx.y >= clip_y && ctx.y < clip_y + clip_h as i32 {
                ...
                framebuffer::fill_rect(ctx.x as u32, ctx.y as u32, image_w, image_h, 0xFFF0F0F0);
                // Border
                framebuffer::fill_rect(ctx.x as u32, ctx.y as u32, image_w, 1, 0xFFDDDDDD);
                framebuffer::fill_rect(ctx.x as u32, (ctx.y + image_h as i32 - 1) as u32, image_w, 1, 0xFFDDDDDD);
                framebuffer::fill_rect(ctx.x as u32, ctx.y as u32, 1, image_h, 0xFFDDDDDD);
                framebuffer::fill_rect((ctx.x + image_w as i32 - 1) as u32, ctx.y as u32, 1, image_h, 0xFFDDDDDD);
                
                // Draw [IMG] icon in center
                let icon_text = "[IMG]";
                let icon_x = ctx.x + (image_w as i32 / 2) - 20;
                let icon_y = ctx.y + (image_h as i32 / 2) - 12;
                if image_h > 24 {
...
match input_type {
                ...
                "submit" | "button" => {
                    ...
                    if ctx.y >= clip_y && ctx.y < clip_y + clip_h as i32 {
                        framebuffer::fill_rect(ctx.x as u32, ctx.y as u32, button_width, button_height, 0xFFE8E8E8);
                        // Border
                        framebuffer::fill_rect(ctx.x as u32, ctx.y as u32, button_width, 1, 0xFFBBBBBB);
                        framebuffer::fill_rect(ctx.x as u32, (ctx.y + button_height as i32 - 1) as u32, button_width, 1, 0xFFBBBBBB);
                        framebuffer::fill_rect(ctx.x as u32, ctx.y as u32, 1, button_height, 0xFFBBBBBB);
                        framebuffer::fill_rect((ctx.x + button_width as i32 - 1) as u32, ctx.y as u32, 1, button_height, 0xFFBBBBBB);
                        for (i, c) in value.chars().enumerate() {
                            draw_char((ctx.x + 12 + i as i32 * 8) as u32, (ctx.y + 6) as u32, c, COLOR_TEXT);
                        }
                    }
                    ctx.x += button_width as i32 + 8;
                }
                "checkbox" => {
                    if ctx.y >= clip_y && ctx.y < clip_y + clip_h as i32 {
        ...
        "button" => {
            ...
            let button_height = 28u32;
            if ctx.y >= clip_y && ctx.y < clip_y + clip_h as i32 {
                framebuffer::fill_rect(ctx.x as u32, ctx.y as u32, 120, button_height, 0xFFE8E8E8);
                framebuffer::fill_rect(ctx.x as u32, ctx.y as u32, 120, 1, 0xFFBBBBBB);
                framebuffer::fill_rect(ctx.x as u32, (ctx.y + button_height as i32 - 1) as u32, 120, 1, 0xFFBBBBBB);
                framebuffer::fill_rect(ctx.x as u32, ctx.y as u32, 1, button_height, 0xFFBBBBBB);
                framebuffer::fill_rect(119 + ctx.x as u32, ctx.y as u32, 1, button_height, 0xFFBBBBBB);
            }
            ctx.y += 6;
        }
        ...
        "progress" => {
            // HTML5 progress bar
            let maximum_value: f32 = el.attr("max").and_then(|v| v.parse().ok()).unwrap_or(1.0);
            let cur_val: f32 = el.attr("value").and_then(|v| v.parse().ok()).unwrap_or(0.0);
            let pct = (cur_val / maximum_value).min(1.0).max(0.0);
        ...
        "meter" => {
            // HTML5 meter element
            let minimum_value: f32 = el.attr("min").and_then(|v| v.parse().ok()).unwrap_or(0.0);
            let maximum_value: f32 = el.attr("max").and_then(|v| v.parse().ok()).unwrap_or(1.0);
            let cur_val: f32 = el.attr("value").and_then(|v| v.parse().ok()).unwrap_or(0.0);
            let low: f32 = el.attr("low").and_then(|v| v.parse().ok()).unwrap_or(minimum_value);
            let high: f32 = el.attr("high").and_then(|v| v.parse().ok()).unwrap_or(maximum_value);
            let range = maximum_value - minimum_value;
            let pct = if range > 0.0 { ((cur_val - minimum_value) / range).min(1.0).max(0.0) } else { 0.0 };
        ...
        "video" | "audio" | "canvas" | "svg" | "iframe" | "embed" | "object" => {
            ...
            if ctx.y >= clip_y && ctx.y < clip_y + clip_h as i32 {
                ...
                // Icon label
                let label = // Correspondance de motifs — branchement exhaustif de Rust.
match tag {
                    "video" => "[VIDEO]",
                    "audio" => "[AUDIO]",
                    "canvas" => "[CANVAS]",
...
match part {
            ...
            SelectorPart::Tag(_) => {
                if !has_combinator {
                    if let Some(_prev) = last_tag_index {
