...
pub fn draw_char_smooth(
    ...
    for (row_index, &bits) in glyph.iter().enumerate() {
        ...
        let previous_bits = if row_index > 0 { glyph[row_index - 1] } else { 0 };
        ...
        for bit in 0..8u32 {
            ...
            if is_set {
                ...
                let top_set = previous_bits & mask != 0;
                ...
                let tl = bit > 0 && (previous_bits & (mask << 1)) != 0;
                let tr = bit < 7 && (previous_bits & (mask >> 1)) != 0;
