...
const PERSIST_MAXIMUM_SECTORS: u64 = 32768; // 16MB
...
struct FileEntry {
    ...
    data_length: u32,        // Length of file data
...
pub fn save_file(path: &str, data: &[u8]) -> Result<(), &'static str> {
    ...
    if data_start_sector + ((data.len() as u64 + path.len() as u64 + 16) / 512) + 1 
        > PERSIST_START_SECTOR + PERSIST_MAXIMUM_SECTORS {
    ...
    let header_bytes: [u8; 8] = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::mem::transmute(entry_header) };
...
fn restore_all() -> Result<(), &'static str> {
    ...
    while offset + 8 <= all_data.len() && restored < header.entry_count {
        // Read entry header
        let path_length = u16::from_le_bytes([all_data[offset], all_data[offset + 1]]) as usize;
        ...
        if offset + path_length + data_length > all_data.len() {
            break;
        }
        
        // Read path
        let path = // Correspondance de motifs — branchement exhaustif de Rust.
match core::str::from_utf8(&all_data[offset..offset + path_length]) {
            Ok(s) => s,
            Err(_) => {
                offset += path_length + data_length;
                continue;
            }
        };
        offset += path_length;
        ...
        // Read data
        let data = &all_data[offset..offset + data_length];
        offset += data_length;
        ...
        crate::serial_println!("[PERSIST] Restoring: {} ({} bytes)", path, data_length);
...
pub fn status() -> (&'static str, u32, u64) {
    ...
    if crate::drivers::ahci::read_sectors(port, PERSIST_START_SECTOR, 1, &mut header_buffer).is_err() {
        return ("Read error", 0, 0);
    }
    
    if let Ok(header) = parse_header(&header_buffer) {
