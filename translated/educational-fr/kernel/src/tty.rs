...
unsafe { *(argument as *mut u32) = pgid; }
        ...
        TIOCSPGRP => {
            ...
            if argument != 0 && crate::memory::validate_user_pointer(argument, 4, false) {
                let pgid = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { *(argument as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32) };
...
unsafe {
        ...
        TCSETS => {
            ...
            if argument != 0 && crate::memory::validate_user_pointer(argument, 16, false) {
                let (iflag, oflag, cflag, lflag) = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
                    let p = argument as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32;
