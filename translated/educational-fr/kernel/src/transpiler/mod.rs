...
pub fn from_code(code: u8, rex_b: bool, size: u8) -> Self {
        let index = if rex_b { code + 8 } else { code };
                // Correspondance de motifs — branchement exhaustif de Rust.
match size {
            8 => // Correspondance de motifs — branchement exhaustif de Rust.
match index {
            ...
            4 => // Correspondance de motifs — branchement exhaustif de Rust.
match index {
...
pub struct Function {
    ...
    pub syscalls: Vec<SyscallInformation>,
    pub calls: Vec<u64>,
...
pub fn disassemble_all(&mut self) -> Vec<Instruction> {
    ...
    fn next_instruction(&mut self) -> Option<Instruction> {
        ...
        // Decode opcode
        let (mnemonic, operands) = // Correspondance de motifs — branchement exhaustif de Rust.
match b {
            // SYSCALL
            ...
            0xB8..=0xBF => {
                ...
                let reg = Register::from_code(register_code, rex_b, size);
    ...
    fn read_modrm(&mut self, rex_r: bool, rex_b: bool, size: u8) -> Option<Vec<Operand>> {
        ...
        let register_operation = Operand::Register(Register::from_code(reg, rex_r, size));
        let rm_operation = Operand::Register(Register::from_code(rm, rex_b, size));
        
        Some(vec![rm_operation, register_operation])
    }
    ...
    fn read_modrm_with_imm32(&mut self, rex_r: bool, rex_b: bool, size: u8) -> Option<Vec<Operand>> {
        ...
        Some(vec![rm_operation, Operand::Immediate(imm as i64)])
    }
