...
pub fn fast_isqrt(x: f32) -> f32 {
    if x <= 0.0 { return 0.0; }
    let xhalf = 0.5 * x;
    let i = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::mem::transmute::<f32, u32>(x) };
    let i = 0x5f375a86u32.wrapping_sub(i >> 1);
    let y = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::mem::transmute::<u32, f32>(i) };
...
pub fn fast_atan2(y: f32, x: f32) -> f32 {
    let absolute_x = fast_absolute(x);
    let absolute_y = fast_absolute(y);
    let maximum_value = if absolute_x > absolute_y { absolute_x } else { absolute_y };
    let minimum_value = if absolute_x < absolute_y { absolute_x } else { absolute_y };

    if maximum_value < 0.0001 { return 0.0; }

    let a = minimum_value / maximum_value;
    let s = a * a;
    let r = ((-0.0464964749 * s + 0.15931422) * s - 0.327622764) * s * a + a;

    let r = if absolute_y > absolute_x { 1.5707963 - r } else { r };
...
#[inline(always)]
// Fonction publique — appelable depuis d'autres modules.
pub fn fast_absolute(x: f32) -> f32 {
    if x < 0.0 { -x } else { x }
}
