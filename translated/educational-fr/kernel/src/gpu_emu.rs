...
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MAXIMUM_WORKGROUPS: usize = 256;
...
type PixelShaderFn = fn(input: PixelInput) -> PixelOutput;

/// Compute shader function type (generic work item processing)
pub // Alias de type — donne un nouveau nom à un type existant pour la clarté.
type ComputeShaderFn = fn(global_id: u32, local_id: u32, uniforms: *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8) -> u32;
...
impl Send for VirtualGpu {}
// SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe // Implémentation de trait — remplit un contrat comportemental.
impl Sync for VirtualGpu {}
...
const INITIALIZE_BOOL: AtomicBool = AtomicBool::new(false);
        Self {
            ...
            height: 0,
            stride: 0,
            core_busy: [INITIALIZE_BOOL; VIRTUAL_CORES],
...
// SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe // Implémentation de trait — remplit un contrat comportemental.
impl Send for ShaderContext {}
// SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe // Implémentation de trait — remplit un contrat comportemental.
impl Sync for ShaderContext {}
...
const ShaderContext) };
    ...
    for y in start..end {
        ...
        while x + 4 <= width {
            ...
            unsafe {
                let pixels = _mm_loadu_si128(colors.as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const __m128i);
...
const MAXIMUM_ITERATOR: u32 = 64;
    ...
    if iter == MAXIMUM_ITERATOR {
        PixelOutput::from_rgb(0, 0, 0)
    } else {
        let t = iter as f32 / MAXIMUM_ITERATOR as f32;
        let r = (t * 255.0) as u8;
...
fn tunnel_grid_lines(wall_x: f32, z: f32, t: f32) -> f32 {
    ...
    let radial_position = wall_x * radial_divisions;
    let radial_line = fast_absolute(radial_position - (radial_position as i32) as f32 - 0.5);
    ...
    let ring_line = fast_absolute(ring_fract - 0.5);
...
fn parallax_layer(x: u32, y: u32, t: f32, h: u32, speed: f32, brightness: f32, 
    ...
    let intensity = if dist == 0 {
        ...
        brightness * (1.0 - trail_position * trail_position) * 180.0
    };
...
use crate::math::{fast_sqrt, fast_absolute, fast_sin, fast_cos, lerp_u8};
...
pub fn shader_matrix_shapes(input: PixelInput) -> PixelOutput {
    ...
    let cam_z = -3.0;
    let ray_directory_x = u;
    let ray_directory_y = v;
    let ray_directory_z = 1.5;  // focal length
    
    // Normalize ray direction
    let ray_length = fast_sqrt(ray_directory_x * ray_directory_x + ray_directory_y * ray_directory_y + ray_directory_z * ray_directory_z);
    let rd_x = ray_directory_x / ray_length;
    let rd_y = ray_directory_y / ray_length;
    let rd_z = ray_directory_z / ray_length;
    ...
    let edge_x = fast_absolute(hit_position.0) > 0.4;
    let edge_y = fast_absolute(hit_position.1) > 0.4;
    ...
    let (shape_r, shape_g, shape_b) = // Correspondance de motifs — branchement exhaustif de Rust.
match hit_shape {
...
fn sdf_cube(x: f32, y: f32, z: f32, s: f32) -> f32 {
    let dx = fast_absolute(x) - s;
    let dy = fast_absolute(y) - s;
...
fn edge_glow(x: f32, y: f32, z: f32, _t: f32) -> f32 {
    ...
    let ax = fast_absolute(x);
    let ay = fast_absolute(y);
    let az = fast_absolute(z);
    
    // Near edges if two coordinates are close to the cube size
    let on_edge_xy = (fast_absolute(ax - 0.5) < edge_thresh) && (fast_absolute(ay - 0.5) < edge_thresh);
    let on_edge_xz = (fast_absolute(ax - 0.5) < edge_thresh) && (fast_absolute(az - 0.5) < edge_thresh);
    let on_edge_yz = (fast_absolute(ay - 0.5) < edge_thresh) && (fast_absolute(az - 0.5) < edge_thresh);
...
fn matrix_rain_background(x: u32, y: u32, w: f32, h: f32, t: f32) -> (u8, u8, u8) {
    ...
    // Falling position
    let fall_position = (t * fall_speed + phase_offset) % 1.5;
    ...
    let in_trail = row_norm < fall_position && (fall_position - row_norm) < trail_length;
...
pub fn initialize_stride(framebuffer: *mut u32, width: u32, height: u32, stride: u32) {
...
pub fn shader_matrix_rain_3d(input: PixelInput) -> PixelOutput {
    ...
    let number_columns = 32.0;
    let column_angle = (angle + 3.14159) / 6.28318;  // 0 to 1
    let column = (column_angle * number_columns) as u32;
    let in_column_position = (column_angle * number_columns) % 1.0;  // Position within column
    
    // Column seed for random offset
    let column_seed = column.wrapping_mul(48271);
    let column_offset = (column_seed % 1000) as f32 / 1000.0 * 10.0;
    
    // === GLYPH GRID IN EACH COLUMN ===
    let glyph_spacing = 0.15;
    let adjusted_z = z + column_offset;
    ...
    let column_center_dist = fast_absolute(in_column_position - 0.5);
    let glyph_vertical = glyph_local_z > 0.2 && glyph_local_z < 0.8;
    
    // Column width expands outward (perspective)
    let column_width = 0.3 + radius * 0.2;
    let on_column = column_center_dist < column_width;
    
    // Generate glyph pattern
    let glyph_seed = glyph_row.wrapping_mul(31337) ^ column.wrapping_mul(48271);
    let glyph_pattern = matrix3d_glyph(in_column_position, glyph_local_z, glyph_seed);
...
/// Update time
pub fn tick(delta_mouse: u32) {
    VGPU.lock().tick(delta_mouse);
}
...
pub fn shader_cosmic_deform(input: PixelInput) -> PixelOutput {
    ...
    let l = fast_absolute(0.7 - dot_pp);
    ...
    while i <= 6.0 {
        ...
        let diff = fast_absolute(vx - vy) * 0.2;
...
/// Trail length bounds
const MTRAIL_MINIMUM: usize = 10;
// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MTRAIL_MAXIMUM: usize = 45;
...
const fn generator_matrix_color_lut() -> [u32; 256] {
...
static MCOLOR_LUT: [u32; 256] = generator_matrix_color_lut();
...
const fn generator_matrix_glow_lut() -> [u32; 256] {
...
static MGLOW_LUT: [u32; 256] = generator_matrix_glow_lut();
    ...
    pub fn init(&mut self, screen_w: usize, screen_h: usize) {
        ...
        for column in 0..self.number_cols {
            ...
            for di in 0..MDROPS {
                ...
                let speed_minimum = 1 + (255u32.saturating_sub(df)) / 128; // 1..2
                let speed_range = 1 + (255u32.saturating_sub(df)) / 80; // 1..4
                let speed = speed_minimum + (self.rng % speed_range);
            ...
            for di in 0..MDROPS {
                ...
                if drop.counter >= drop.speed {
                    ...
                    if tlen >= 2 && tlen <= 48 {
                        ...
                        for tp in 1..tlen {
                            let cur_index = drop.glyph_at(tp) as u8;
                            if cur_index == previous_index {
                                // Resonance! Lock both positions
                                drop.locked_mask |= (1u64 << (tp - 1)) | (1u64 << tp);
                                drop.locked_glyphs[tp - 1] = cur_index;
                                drop.locked_glyphs[tp] = cur_index;
                            }
                ...
                if drop.y as i32 > maximum_y {
                    ...
                    self.rng = self.rng.wrapping_mul(1103515245).wrapping_add(12345);
                    let speed_minimum = 1 + (255u32.saturating_sub(df)) / 128;
...
struct MatrixRenderContext {
    state: *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
// SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe // Implémentation de trait — remplit un contrat comportemental.
impl Send for MatrixRenderContext {}
// SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe // Implémentation de trait — remplit un contrat comportemental.
impl Sync for MatrixRenderContext {}
...
const MatrixRenderContext) };
    let state = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
    ...
    for column in start..end {
        ...
        for di in 0..MDROPS {
            ...
            for tp in 0..trail_length {
                ...
                let glyph_index = drop.glyph_at(tp);
...
unsafe {
    ...
    let context = MatrixRenderContext {
        state: &*state as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const ShaderMatrixState,
        framebuffer,
