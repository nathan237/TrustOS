...
unsafe {
            let ptr = (self.cq_virt + offset as u64) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CqEntry;
...
impl NvmeController {
    ...
    fn read32(&self, offset: u64) -> u32 {
                // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_volatile((self.bar_virt + offset) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32) }
    }
...
unsafe { core::ptr::write_volatile((self.bar_virt + offset) as *mut u32, value) }
    ...
    fn identify_controller(&mut self) -> Result<(), &'static str> {
        ...
        unsafe {
            let data = buffer_virt as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8;
...
const u8;
            ...
            let nsze = core::ptr::read_unaligned(data as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u64);
...
match self.admin_cmd(cmd) {
            Ok(_) => {
                ...
                let mut ids = Vec::new();
                                // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
                    let list = buffer_virt as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32;
                    for i in 0..1024 {
...
const u32) } as u64;
        let hi = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_volatile((bar_virt + REGISTER_CAPABILITY + 4) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32) } as u64;
        lo | (hi << 32)
    };
...
unsafe { core::ptr::read_volatile((bar_virt + REGISTER_CC) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32) };
    if cc & CC_EN != 0 {
        ...
        // Wait for CSTS.RDY = 0
        for _ in 0..1_000_000u32 {
            let csts = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_volatile((bar_virt + REGISTER_CSTS) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32) };
            if csts & CSTS_RDY == 0 {
                break;
            }
