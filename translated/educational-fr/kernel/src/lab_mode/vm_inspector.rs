...
pub fn new() -> Self {
    ...
    fn parse_hv_event(&mut self, ev: &LabEvent) {
        ...
        if let Some(rip_position) = msg.find("RIP=0x") {
            let rip_str = &msg[rip_position + 6..];
...
pub fn draw(state: &VmInspectorState, x: i32, y: i32, w: u32, h: u32) {
    ...
    for (i, tab) in tabs.iter().enumerate() {
        ...
        crate::framebuffer::fill_rect(transmit as u32, y as u32, tab_w as u32, ch as u32, bg);
        super::draw_lab_text(transmit + 4, y + 2, tab, color);
    }
    // Tab separator
    crate::framebuffer::fill_rect(x as u32, (y + ch) as u32, w, 1, COLUMN_HV);
...
fn draw_overview(state: &VmInspectorState, x: i32, y: i32, w: u32, h: u32) {
    ...
    let maximum_events = ((h as i32 - (cy - y)) / ch).max(0) as usize;
    ...
    for ev in state.events[start..].iter() {
        ...
        let maximum_chars = ((w as i32 - 80) / super::char_w()).max(10) as usize;
        let msg = if ev.message.len() > maximum_chars {
            &ev.message[..maximum_chars]
        } else {
            &ev.message
        };
        
        super::draw_lab_text(x, cy, &time_str, COLUMN_DIM);
        super::draw_lab_text(x + 70, cy, msg, COLUMN_TEXT);
        cy += ch;
    }
}
...
fn draw_exits(state: &VmInspectorState, x: i32, y: i32, w: u32, _h: u32) {
    ...
    let maximum_count = exits.iter().map(|(_, c, _)| *c).max().unwrap_or(1).max(1);
    ...
    for (name, count, color) in &exits {
        ...
        let bar_w = if *count > 0 {
            ((*count as u64 * bar_maximum_w) / maximum_count).max(2)
        } else {
            0
        };
        if bar_w > 0 {
...
fn draw_registers(state: &VmInspectorState, x: i32, y: i32, _w: u32, _h: u32) {
    ...
    for (name, val) in &regs_left {
        let color = if *val != 0 { COLUMN_GREEN } else { COLUMN_DIM };
    ...
    for (name, val) in &regs_right {
        let color = if *val != 0 { COLUMN_CYAN } else { COLUMN_DIM };
