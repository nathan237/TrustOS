...
match b {
        0x00 => COLUMN_DIM,                   // null
        0x09 | 0x0A | 0x0D | 0x20 => COLUMN_CYAN,  // whitespace
        0x01..=0x1F => COLUMN_YELLOW,         // control
        0x20..=0x7E => COLUMN_GREEN,          // printable ASCII
        0x7F => COLUMN_YELLOW,                // DEL
        0xFF => COLUMN_RED,                   // 0xFF
        0x80..=0xFE => COLUMN_PURPLE,         // high bytes
    }
}
...
pub fn draw(state: &HexEditorState, x: i32, y: i32, w: u32, h: u32) {
    ...
    draw_lab_text(x, y, &header, COLUMN_ACCENT);

    // Offset info on right
    let off_information = format!("@{:04X}", state.cursor);
    let off_x = x + w as i32 - (off_information.len() as i32 * cw) - 2;
    draw_lab_text(off_x, y, &off_information, COLUMN_DIM);
    ...
    if state.data.is_empty() {
        draw_lab_text(x + 4, list_y, "Empty — type 'hex <path>' to load a file", COLUMN_DIM);
        return;
    }
    ...
    for row in scroll..end_row {
        ...
        draw_lab_text(x, cy, &off_str, COLUMN_DIM);
        ...
        // Separator bar
        draw_lab_text(ascii_x - 2 * cw, cy, "|", COLUMN_DIM);
    ...
    if legend_y + lh < y + h as i32 {
        ...
        let mut lx = x;
        let items: &[(&str, u32)] = &[
            ("ASCII", COLUMN_GREEN), ("WS", COLUMN_CYAN), ("CTRL", COLUMN_YELLOW),
            ("HIGH", COLUMN_PURPLE), ("NULL", COLUMN_DIM), ("0xFF", COLUMN_RED),
        ];
