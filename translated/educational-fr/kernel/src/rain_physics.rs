...
#[inline]
fn operation_smooth_union(a: f32, b: f32, k: f32) -> f32 {
    let k4 = k * 4.0;
...
pub fn initialize_scene(s: &mut RainSceneState, w: u32, h: u32) {
    let w = w as f32;
    let h = h as f32;
    s.w = w;
    s.h = h;
...
fn tree_sdf(px: f32, py: f32, s: &RainSceneState) -> f32 {
    ...
    let tree = operation_smooth_union(trunk, canopy, 18.0);
    let tree = operation_smooth_union(tree, branches, 12.0);
    operation_smooth_union(tree, roots, 10.0)
}
