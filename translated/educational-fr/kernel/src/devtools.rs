...
pub fn dmesg_read(n: usize) -> Vec<String> {
    ...
    entries[start..]
        .iter()
        .map(|e| {
            let level = // Correspondance de motifs — branchement exhaustif de Rust.
match e.level {
                0 => "TRACE",
                1 => "DEBUG",
                2 => "INFO ",
...
unsafe {
    ...
    // EFER MSR
    #[cfg(target_arch = "x86_64")]
    let efer: u64 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
        let lo: u32;
        let hi: u32;
        core::arch::asm!(
