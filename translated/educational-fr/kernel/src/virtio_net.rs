...
unsafe { alloc::alloc::alloc_zeroed(layout) };
        ...
        let avail_pointer = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { ptr.add(avail_offset) as *mut virtio::VirtqAvail };
        let used_offset = ((avail_offset + 6 + 2 * size as usize) + 4095) & !4095;
        let used_pointer = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
