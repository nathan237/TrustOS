...
unsafe {
        ...
        LStar::write(VirtAddr::new(syscall_entry as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const () as u64));
...
#[inline(never)]
pub // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe fn jump_to_ring3_with_args(entry_point: u64, user_stack: u64, arg1: u64, arg2: u64) -> ! {
...
#[inline(never)]
pub // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe fn execute_ring3_process(entry_point: u64, user_stack: u64) -> i32 {
        // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const USER_CS: u64 = 0x20 | 3;  // 0x23
    const USER_SS: u64 = 0x18 | 3;  // 0x1B
    const USER_RFLAGS: u64 = 0x202; // IF=1, reserved=1
...
pub // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe fn return_from_ring3(exit_code: i32) -> ! {
...
pub // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe fn launch_user_process(entry_point: u64, user_stack: u64) -> i32 {
