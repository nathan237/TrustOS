...
fn ensure_initialize() -> Result<(), &'static str> {
...
pub fn play_note(name: &str, duration_mouse: u32) -> Result<(), &'static str> {
    ensure_initialize()?;

    let samples = {
        let mut synth = SYNTH.lock();
        let engine = synth.as_mut().ok_or("Synth not initialized")?;
        engine.play_note_by_name(name, duration_mouse)?
    };

    // Write samples to HDA and play
    play_samples(&samples, duration_mouse)?;
    Ok(())
}

/// Play a note by MIDI number
pub fn play_midi_note(note: u8, velocity: u8, duration_mouse: u32) -> Result<(), &'static str> {
    ensure_initialize()?;

    let samples = {
        let mut synth = SYNTH.lock();
        let engine = synth.as_mut().ok_or("Synth not initialized")?;
        engine.render_note(note, velocity, duration_mouse)
    };
...
pub fn play_frequency(frequency_hz: u32, duration_mouse: u32) -> Result<(), &'static str> {
    ensure_initialize()?;

    let samples = {
        let mut synth = SYNTH.lock();
        let engine = synth.as_mut().ok_or("Synth not initialized")?;
        engine.render_frequency(frequency_hz, duration_mouse)
    };
...
pub fn set_adsr(attack_mouse: u32, decay_mouse: u32, sustain_pct: u32, release_mouse: u32) -> Result<(), &'static str> {
    ensure_initialize()?;
    let mut synth = SYNTH.lock();
    let engine = synth.as_mut().ok_or("Synth not initialized")?;
    engine.set_adsr(attack_mouse, decay_mouse, sustain_pct, release_mouse);
    Ok(())
}
...
pub fn set_envelope_preset(name: &str) -> Result<(), &'static str> {
    ensure_initialize()?;
    let env = // Correspondance de motifs — branchement exhaustif de Rust.
match name {
...
fn play_samples(samples: &[i16], duration_mouse: u32) -> Result<(), &'static str> {
