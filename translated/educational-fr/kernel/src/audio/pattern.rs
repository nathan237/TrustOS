...
const MAXIMUM_PATTERNS: usize = 16;
...
impl Pattern {
    ...
    pub fn render(&self, engine: &mut SynthEngine) -> Vec<i16> {
        ...
        for (i, step) in self.steps.iter().enumerate() {
            ...
            let buffer_slice = &mut buffer[offset..offset + step_samples * CHANNELS as usize];
            engine.render(buffer_slice, step_samples);
...
pub fn new() -> Self {
    ...
    pub fn add(&mut self, pattern: Pattern) -> Result<usize, &'static str> {
        if self.patterns.len() >= MAXIMUM_PATTERNS {
    ...
    pub fn list(&self) -> String {
        ...
        let mut s = String::new();
        s.push_str(&format!("Patterns ({}/{}):\n", self.patterns.len(), MAXIMUM_PATTERNS));
