...
use crate::process::{self, Pid, ProcessState, ProcessFlags, PID_INITIALIZE};
...
fn print_system_information() {
    ...
    crate::serial_println!("Processes: {}", process_count);
...
pub fn reap_zombies() {
    ...
    for (pid, ppid) in zombies {
        ...
        if ppid == PID_INITIALIZE || process::with_process(ppid, |_| ()).is_none() {
