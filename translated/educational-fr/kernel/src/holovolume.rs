...
pub fn set_shape(&mut self, shape: Shape3D) {
    ...
    pub fn update(&mut self, dt: f32) {
        ...
        let effective_shape = // Correspondance de motifs — branchement exhaustif de Rust.
        ...
        for column in 0..MATRIX_COLS {
            ...
            for row in 0..MATRIX_ROWS {
                ...
                self.intensity_map[Self::index(column, row)] = if minimum_sdf.absolute() < EDGE_THRESHOLD {
                    ...
                    let edge_dist = -minimum_sdf;
                    let maximum_depth = 0.8;
                    let gradient = if edge_dist < maximum_depth {
                        1.0 - (edge_dist / maximum_depth)
                    } else {
                        0.0
                    };
    ...
    pub fn get_u8_intensity_map(&self) -> Vec<u8> {
        ...
        for column in 0..MATRIX_COLS {
            for row in 0..MATRIX_ROWS {
                ...
                result[index] = // Correspondance de motifs — branchement exhaustif de Rust.
