...
use core::panic::PanicInformation;
...
unsafe extern "C" fn kmain() -> ! {
    ...
    #[cfg(target_arch = "aarch64")]
    {
        if let Some(ka_response) = KERNEL_ADDRESS_REQUEST.get_response() {
    ...
    if let Some(kf_response) = KERNEL_FILE_REQUEST.get_response() {
        let file = kf_response.file();
        let ptr = file.address();
...
unsafe { jarvis::pxe_replicator::register_kernel_file(ptr, size); }
    ...
    if let Some(framebuffer_response) = FRAMEBUFFER_REQUEST.get_response() {
        if let Some(framebuffer) = framebuffer_response.framebuffers().next() {
            ...
            serial_println!("Framebuffer: {}x{} @ {:p}", framebuffer.width(), framebuffer.height(), framebuffer.address());
        }
    }
    ...
    if let Some(mmap_response) = MEMORY_MAP_REQUEST.get_response() {
        ...
        let mut total_physical_memory: u64 = 0;
...
match entry.entry_type {
        ...
        memory::set_total_physical_memory(total_physical_memory);
        serial_println!("[MEM] Total physical memory: {} MB", total_physical_memory / 1024 / 1024);

        // Compute dynamic heap size: 25% of detected RAM, clamped [64 MB, 512 MB]
        let dynamic_heap_size = memory::compute_heap_size(total_physical_memory);
        serial_println!("[HEAP] Dynamic size: {} MB (25% of {} MB RAM)", 
            dynamic_heap_size / 1024 / 1024, total_physical_memory / 1024 / 1024);

        // Align up to 4 KiB
        let align_up = |address: u64, align: u64| -> u64 {
            if address % align == 0 { address } else { address + (align - (address % align)) }
        };

        // Find suitable heap region after kernel/modules
        let minimum_heap_base = align_up(core::cmp::maximum(0x100000, kernel_end), 0x1000);
        for entry in mmap_response.entries() {
            if entry.entry_type != limine::memory_map::EntryType::USABLE {
                continue;
            }
            ...
            let heap_start = core::cmp::maximum(region_start, minimum_heap_base);
        ...
        if let Some(heap_physical) = usable_for_heap {
            ...
            if best_length >= memory::HEAP_SIZE_MINIMUM as u64 {
                let heap_start2 = align_up(core::cmp::maximum(best_base, 0x100000), 0x1000);
                let usable_end = best_base.saturating_add(best_length);
                let heap_size2 = core::cmp::minimum(
    ...
    #[cfg(target_arch = "x86_64")]
    {
        ...
        if let Some(rsdp_response) = RSDP_REQUEST.get_response() {
            ...
            if acpi::initialize_direct(rsdp_pointer as u64) {
                if let Some(information) = acpi::get_information() {
                    framebuffer::print_boot_status(&alloc::format!(
                        "ACPI: {} CPUs, {} I/O APICs", 
                        information.cpu_count, information.io_apics.len()
                    ), BootStatus::Ok);
                }
            } else {
    ...
    let is_vbox = acpi::get_information()
        .map(|information| information.oem_id.trim().eq_ignore_ascii_case("VBOX"))
        .unwrap_or(false);
    #[cfg(not(target_arch = "x86_64"))]
    let is_vbox = false;
    if is_vbox {
    ...
    mouse::init();
    let (framebuffer_width, framebuffer_height) = framebuffer::get_dimensions();
    mouse::set_screen_size(framebuffer_width, framebuffer_height);
    ...
    touch::init();
    touch::set_screen_size(framebuffer_width, framebuffer_height);
...
const ENABLE_NETWORK: bool = true;
    ...
    framebuffer::print_boot_status("PCI bus scanning...", BootStatus::Information);
    if ENABLE_PCI {
        pci::init();
    ...
    framebuffer::print_boot_status("Disk subsystem...", BootStatus::Information);
    if ENABLE_DISK {
...
match nvme::init(&nvme_devs[0]) {
                Ok(()) => {
                    if let Some((model, _serial, size, lba_size)) = nvme::get_information() {
    ...
    framebuffer::print_boot_status("VirtIO GPU...", BootStatus::Information);
    ...
    framebuffer::print_boot_status("AMD GPU...", BootStatus::Information);
    drivers::amdgpu::init();
    ...
    framebuffer::print_boot_status("NVIDIA GPU...", BootStatus::Information);
    drivers::nvidia::init();
    ...
    if !is_vbox {
        ...
        let framebuffer_w = framebuffer::FRAMEBUFFER_WIDTH.load(core::sync::atomic::Ordering::SeqCst) as usize;
        let framebuffer_h = framebuffer::FRAMEBUFFER_HEIGHT.load(core::sync::atomic::Ordering::SeqCst) as usize;
        if !framebuffer_address.is_null() && framebuffer_w > 0 && framebuffer_h > 0 {
            let framebuffer_size = framebuffer_w * framebuffer_h * 4;
    ...
    framebuffer::print_boot_status("Network subsystem...", BootStatus::Information);
    if ENABLE_NETWORK {
        network::init();
        if network::is_available() {
            // Display detected platform
            let platform = network::get_platform();
            framebuffer::print_boot_status(&alloc::format!("Platform: {}", platform), BootStatus::Information);
            
            // Auto-probe and load network drivers
            let devs = pci::find_by_class(pci::class::NETWORK);
            if !devs.is_empty() {
                for device in &devs {
                    if drivers::net::probe_device(device) {
                        network::update_mac_from_driver();
                        let driver_name = if device.vendor_id == 0x1AF4 { "virtio-net" } 
                            else if device.vendor_id == 0x8086 { "e1000" }
                            else if device.vendor_id == 0x10EC { "rtl8139" }
                            else { "unknown" };
            ...
            if !drivers::net::has_driver() && !crate::virtio_net::is_initialized() {
                if let Some(device) = devs.iter().find(|d| d.vendor_id == 0x1AF4) {
                    if let Err(e) = crate::virtio_net::init(device) {
...
#[panic_handler]
fn panic(information: &PanicInformation) -> ! {
