...
match p_type {
            1 => {
                ...
                // Track address range
                if p_vaddr < minimum_address {
                    minimum_address = p_vaddr;
                }
                if p_vaddr + p_memsz > maximum_address {
                    maximum_address = p_vaddr + p_memsz;
                }
    ...
    Ok(LoadedBinary {
        entry,
        base: minimum_address,
        end: maximum_address,
        brk_start: (maximum_address + 4095) & !4095, // Page-aligned
        segments,
        interp,
    })
}
