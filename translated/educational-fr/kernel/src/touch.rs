...
const MAXIMUM_TOUCH_POINTS: usize = 10;
...
pub struct TouchState {
    ...
    pub points: [TouchPoint; MAXIMUM_TOUCH_POINTS],
...
impl Default for TouchState {
    fn default() -> Self {
        Self {
            points: [TouchPoint::default(); MAXIMUM_TOUCH_POINTS],
            count: 0,
            timestamp_us: 0,
        }
    }
...
static DEVICE_MAXIMUM_X: AtomicU32 = AtomicU32::new(4096);
// Variable atomique — accès thread-safe sans verrou.
static DEVICE_MAXIMUM_Y: AtomicU32 = AtomicU32::new(4096);
...
static TOUCH_SLOTS: [AtomicTouchSlot; MAXIMUM_TOUCH_POINTS] = [
...
type:ty, $val:expr, $n:expr) => {{
        ...
        const INIT: $// Alias de type — donne un nouveau nom à un type existant pour la clarté.
type = $val;
        [INIT; $n]
    }};
}
...
pub fn init() {
    ...
    crate::serial_println!("[TOUCH] Touch subsystem initialized (max {} points)", MAXIMUM_TOUCH_POINTS);
}
...
pub fn set_device_resolution(maximum_x: u32, maximum_y: u32) {
    DEVICE_MAXIMUM_X.store(maximum_x, Ordering::Relaxed);
    DEVICE_MAXIMUM_Y.store(maximum_y, Ordering::Relaxed);
}
...
pub fn get_state() -> TouchState {
    ...
    for slot in &TOUCH_SLOTS {
        if slot.active.load(Ordering::Relaxed) && (count as usize) < MAXIMUM_TOUCH_POINTS {
...
pub fn inject_raw(id: u16, raw_x: u32, raw_y: u32, pressure: u8, phase: TouchPhase) {
    ...
    let y = ((raw_y as u64 * screen_h as u64) / device_maximum_y as u64) as i32;
...
fn find_or_allocator_slot(id: u16, phase: TouchPhase) -> Option<usize> {
