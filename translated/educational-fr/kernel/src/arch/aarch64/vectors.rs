...
extern "C" fn el1_sync_handler(tf: &TrapFrame) {
    let esr = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { super::cpu::read_esr_el1() };
    let far = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
