...
pub fn mesh_penger() -> Mesh {
    ...
    let belly_receive = 0.16; let belly_ry = 0.28;
    for i in 0..8u32 {
        let angle = i as f32 * 0.7853982; // PI/4 * i
        let x = belly_cx + belly_receive * fast_cos(angle);
...
pub fn mesh_character() -> Mesh {
    ...
    // Color palette (ARGB)
    let column_body   = 0xFF88BBDD; // soft blue-grey torso
    let column_head   = 0xFFAADDFF; // lighter blue head
    let column_limb   = 0xFF7799BB; // darker blue-grey arms/legs
    let column_accent = 0xFFFFCC44; // warm amber joints/details
    let column_foot   = 0xFF6688AA; // dark blue-grey feet
    ...
    for _ in 0..4 { colors.push(column_head); }
    // Face ring
    ...
    for _ in 0..4 { colors.push(column_accent); } // amber crown
    // Jaw → face verticals
    for i in 0..4 { edges.push((head_base + i, face_base + i)); colors.push(column_head); }
    // Face → crown verticals
    for i in 0..4 { edges.push((face_base + i, crown_base + i)); colors.push(column_head); }
    // Crown → top point
    for i in 0..4 { edges.push((crown_base + i, head_top)); colors.push(column_accent); }
    ...
    edges.push((head_base, neck_base)); colors.push(column_accent);
    edges.push((head_base+1, neck_base+1)); colors.push(column_accent);
    edges.push((neck_base, neck_base+1)); colors.push(column_accent);
    ...
    verts.push(V3 { x: -hpw, y: hpy, z:  hpd });
    // Neck → shoulder
    edges.push((neck_base, sh)); colors.push(column_body);
    edges.push((neck_base+1, sh+1)); colors.push(column_body);
    // Shoulder ring
    ...
    for _ in 0..4 { colors.push(column_accent); } // amber belt
    // Hip ring
    ...
    let lua = add_box(&mut verts, &mut edges, &mut colors,
        -0.30, 0.38, 0.0,   0.06, 0.10, 0.06,  column_limb); // upper arm
    let lfa = add_box(&mut verts, &mut edges, &mut colors,
        -0.32, 0.16, 0.0,   0.05, 0.10, 0.05,  column_limb); // forearm
    // Connect shoulder to upper arm (amber joint)
    edges.push((sh, lua + 4)); colors.push(column_accent); // top-front-left of box
    edges.push((sh, lua + 7)); colors.push(column_accent);
    // Connect upper arm bottom to forearm top
    edges.push((lua, lfa + 4)); colors.push(column_accent);
    edges.push((lua + 3, lfa + 7)); colors.push(column_accent);
    // Left hand — small box
    let lhand = add_box(&mut verts, &mut edges, &mut colors,
        -0.33, 0.02, 0.0,   0.04, 0.04, 0.04,  column_accent);
    edges.push((lfa, lhand + 4)); colors.push(column_limb);
    edges.push((lfa + 1, lhand + 5)); colors.push(column_limb);

    // ── RIGHT ARM — upper arm box + forearm box ──
    let rua = add_box(&mut verts, &mut edges, &mut colors,
        0.30, 0.38, 0.0,   0.06, 0.10, 0.06,  column_limb);
    let rfa = add_box(&mut verts, &mut edges, &mut colors,
        0.32, 0.16, 0.0,   0.05, 0.10, 0.05,  column_limb);
    edges.push((sh + 1, rua + 5)); colors.push(column_accent);
    edges.push((sh + 1, rua + 6)); colors.push(column_accent);
    edges.push((rua + 1, rfa + 5)); colors.push(column_accent);
    edges.push((rua + 2, rfa + 6)); colors.push(column_accent);
    let rhand = add_box(&mut verts, &mut edges, &mut colors,
        0.33, 0.02, 0.0,   0.04, 0.04, 0.04,  column_accent);
    edges.push((rfa, rhand + 4)); colors.push(column_limb);
    edges.push((rfa + 1, rhand + 5)); colors.push(column_limb);

    // ── LEFT LEG — thigh box + shin box ──
    let lth = add_box(&mut verts, &mut edges, &mut colors,
        -0.09, -0.18, 0.0,   0.06, 0.12, 0.06,  column_limb);
    let lsh = add_box(&mut verts, &mut edges, &mut colors,
        -0.09, -0.40, 0.0,   0.05, 0.10, 0.05,  column_limb);
    // Hip → thigh
    edges.push((hp, lth + 4)); colors.push(column_accent);
    edges.push((hp + 3, lth + 7)); colors.push(column_accent);
    // Thigh → shin (knee joint)
    edges.push((lth, lsh + 4)); colors.push(column_accent);
    edges.push((lth + 3, lsh + 7)); colors.push(column_accent);
    // Left foot — flat wide box
    let lft = add_box(&mut verts, &mut edges, &mut colors,
        -0.09, -0.53, -0.02,   0.06, 0.03, 0.08,  column_foot);
    edges.push((lsh, lft + 4)); colors.push(column_limb);
    edges.push((lsh + 1, lft + 5)); colors.push(column_limb);
    ...
    let rsh = add_box(&mut verts, &mut edges, &mut colors,
        0.09, -0.40, 0.0,   0.05, 0.10, 0.05,  column_limb);
    edges.push((hp + 1, rth + 5)); colors.push(column_accent);
    edges.push((hp + 2, rth + 6)); colors.push(column_accent);
    edges.push((rth + 1, rsh + 5)); colors.push(column_accent);
    edges.push((rth + 2, rsh + 6)); colors.push(column_accent);
    let rft = add_box(&mut verts, &mut edges, &mut colors,
        0.09, -0.53, -0.02,   0.06, 0.03, 0.08,  column_foot);
    edges.push((rsh, rft + 4)); colors.push(column_limb);
    edges.push((rsh + 1, rft + 5)); colors.push(column_limb);
...
pub fn render_filled_mesh(buffer: &mut [u32], w: usize, h: usize,
    ...
    let faces = // Correspondance de motifs — branchement exhaustif de Rust.
match &mesh.faces {
        Some(f) => f,
        None => return,
    };
    ...
    for face in &visible_faces {
        ...
        let (fr, fg, framebuffer) = // Correspondance de motifs — branchement exhaustif de Rust.
...
unsafe {
    ...
    fn render_wireframe(&self, buffer: &mut [u32], w: usize, h: usize, mesh: &Mesh,
        ...
        for (index, &(a, b)) in mesh.edges.iter().enumerate() {
            ...
            let base_color = // Correspondance de motifs — branchement exhaustif de Rust.
match &mesh.edge_colors {
                Some(ec) if index < ec.len() => ec[index],
                _ => color,
            };
...
match scene {
    ...
    fn render_holo_matrix(&self, buffer: &mut [u32], w: usize, h: usize) {
        ...
        const NUMBER_RINGS: usize = 10;        // radial distribution rings
        const COLS_PER_RING: usize = 18;    // columns per ring
        const NUMBER_COLS: usize = NUMBER_RINGS * COLS_PER_RING; // 180 total
        ...
        {
            let maximum_rad = 40i32;
            let pulse = 0.85 + 0.15 * fast_sin(frame as f32 * 0.03);
            let mr2 = maximum_rad * maximum_rad;
            for ry in -maximum_rad..=maximum_rad {
                let py = cy as i32 + ry;
        ...
        for column_index in 0..NUMBER_COLS {
            let ring = column_index / COLS_PER_RING;
            let slot = column_index % COLS_PER_RING;

            // Deterministic pseudo-random seed per column
            let seed = (column_index as u32).wrapping_mul(2654435761).wrapping_add(374761393);
            ...
            let ring_t = (ring as f32 + 0.3) / NUMBER_RINGS as f32;
            let radius = 0.3 + ring_t * TUBE_RADIUS;

            // Column position in cylinder
            let column_x = radius * fast_cos(theta);
            let column_y_center = radius * fast_sin(theta);

            // Z placement — spread along tunnel, offset by camera movement
            let column_z_base = (seed2 % 10000) as f32 / 10000.0 * Z_DEPTH;
            // Wrap Z modulo Z_DEPTH → infinite tunnel (demoscene distance-table trick)
            let column_z_raw = column_z_base - (cam_z_offset % Z_DEPTH);
            let column_z = ((column_z_raw % Z_DEPTH) + Z_DEPTH) % Z_DEPTH + Z_NEAR;
            ...
            for ci in 0..trail_length {
                ...
                // 3D position
                let vy = column_y_center + local_y;
                let v = V3 { x: column_x, y: vy, z: column_z };
                ...
                let glyph_index = (seed.wrapping_mul(ci as u32 + 1).wrapping_add(frame / 5)) as usize % 16;
        ...
        // Horizontal lines receding to vanishing point + vertical grid lines
        {
            ...
            let number_hz = 12; // horizontal depth lines
            let number_vt = 16; // vertical columns
        ...
        {
            let number_streaks = 30;
            for i in 0..number_streaks {
                let seed = (i as u32).wrapping_mul(2654435761).wrapping_add(frame.wrapping_mul(7));
                let seed2 = seed.wrapping_mul(1664525).wrapping_add(1013904223);
                let angle = (i as f32 / number_streaks as f32) * 6.2831853 + (seed % 1000) as f32 * 0.001;
    ...
    fn render_character_bg(&self, buffer: &mut [u32], w: usize, h: usize) {
        ...
        let number_horiz = 10;
        for i in 0..number_horiz {
            let t = i as f32 / number_horiz as f32;
