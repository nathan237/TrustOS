...
pub fn run_all_tests() -> (usize, usize, String) {
    ...
    fn dummy_builtin(_id: u8, _argc: usize, _argv: *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const i64) -> i64 { 0 }
...
match compile_result {
            Ok(program) => {
                ...
                let execute_result = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
...
match execute_result {
    ...
    for (name, source) in &cross_tests {
        ...
        let native_result = super::native::compile_native(source)
            .and_then(|prog| // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
...
match super::native::compile_native(source) {
        Ok(prog) => {
                        // Correspondance de motifs — branchement exhaustif de Rust.
match             // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
const i64) -> i64 { 0 }
    ...
    // Compile (not timed)
    let prog = // Correspondance de motifs — branchement exhaustif de Rust.
match super::native::compile_native(source) {
        Ok(p) => p,
        Err(_) => return (0, 0),
    };
