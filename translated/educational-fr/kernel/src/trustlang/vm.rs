...
// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const BUILTIN_FRAME_NUMBER: u8 = 27;
// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
match name {
        ...
        "frame_num" => Some(BUILTIN_FRAME_NUMBER),
...
pub fn execute(bytecode: &Bytecode) -> Result<String, String> {
    ...
    let maximum_steps = 500_000_000; // generous limit for graphics programs
    let mut steps = 0;
...
loop {
        ...
        let operation_byte = func.code[frame.ip];
        frame.ip += 1;

        let op = // Correspondance de motifs — branchement exhaustif de Rust.
match Op::from_u8(operation_byte) {
            Some(o) => o,
            None => return Err(format!("unknown opcode: {}", operation_byte)),
        };
...
match op {
            ...
            Op::AddI => { bin_operation_i64(&mut stack, |a, b| a.wrapping_add(b), |a, b| a + b)?; }
            Op::SubI => { bin_operation_i64(&mut stack, |a, b| a.wrapping_sub(b), |a, b| a - b)?; }
            Op::MulI => { bin_operation_i64(&mut stack, |a, b| a.wrapping_mul(b), |a, b| a * b)?; }
            Op::DivI => {
                let b_value = stack.pop().unwrap_or(Value::I64(0));
                let a_value = stack.pop().unwrap_or(Value::I64(0));
                                // Correspondance de motifs — branchement exhaustif de Rust.
match (&a_value, &b_value) {
                    ...
                    _ => {
                        let b = b_value.as_i64()?;
                        let a = a_value.as_i64()?;
...
match v {
            ...
            // Float arithmetic
            Op::AddF => { bin_operation_f64(&mut stack, |a, b| a + b)?; }
            ...
            Op::BitAnd => { bin_operation_i64(&mut stack, |a, b| a & b, |a, b| (a as i64 & b as i64) as f64)?; }
            Op::BitOr => { bin_operation_i64(&mut stack, |a, b| a | b, |a, b| (a as i64 | b as i64) as f64)?; }
            Op::BitXor => { bin_operation_i64(&mut stack, |a, b| a ^ b, |a, b| (a as i64 ^ b as i64) as f64)?; }
            Op::Shl => { bin_operation_i64(&mut stack, |a, b| a << (b & 63), |a, b| ((a as i64) << (b as i64 & 63)) as f64)?; }
            Op::Shr => { bin_operation_i64(&mut stack, |a, b| a >> (b & 63), |a, b| ((a as i64) >> (b as i64 & 63)) as f64)?; }
            // Conversion
            Op::I64toF64 => {
            ...
            Op::JumpIfFalse => {
                let off = read_u16(&func.code, &mut frame.ip) as usize;
                let condition = stack.pop().unwrap_or(Value::Bool(false)).as_bool()?;
            ...
            Op::CallBuiltin => {
                ...
                args.reverse();
                let result = execute_builtin(builtin, &args, &mut output)?;
                stack.push(result);
            }
            Op::Return => {
...
match arr {
            ...
            Op::ArrayLength => {
                ...
                let len = // Correspondance de motifs — branchement exhaustif de Rust.
match &v {
...
fn bin_operation_i64(stack: &mut Vec<Value>, fi: fn(i64, i64) -> i64, ff: fn(f64, f64) -> f64) -> Result<(), String> {
    let b_value = stack.pop().unwrap_or(Value::I64(0));
    let a_value = stack.pop().unwrap_or(Value::I64(0));
...
fn bin_operation_f64(stack: &mut Vec<Value>, f: fn(f64, f64) -> f64) -> Result<(), String> {
...
fn execute_builtin(id: u8, args: &[Value], output: &mut String) -> Result<Value, String> {
        // Correspondance de motifs — branchement exhaustif de Rust.
match id {
        BUILTIN_PRINT => {
            for argument in args { output.push_str(&argument.to_display()); }
            Ok(Value::Void)
        }
        BUILTIN_PRINTLN => {
            for argument in args { output.push_str(&argument.to_display()); }
...
match args.first().unwrap_or(&Value::I64(0)) {
        ...
        BUILTIN_CLEAR_SCREEN => {
            ...
            crate::framebuffer::fill_rect(0, 0, software, sh, color);
            Ok(Value::Void)
        }
        BUILTIN_FILL_RECT => {
...
match v {
        ...
        BUILTIN_FRAME_NUMBER => Ok(Value::I64(crate::trustdaw::live_viz::get_frame_number() as i64)),
        BUILTIN_SIN_F => {
