...
match cmd {
    ...
    fn scroll_up(&mut self) {
        ...
        for y in 0..self.rows as usize - 1 {
            let source_start = (y + 1) * cols;
            let destination_start = y * cols;
            for x in 0..cols {
                self.grid[destination_start + x] = self.grid[source_start + x];
            }
        }
...
unsafe {
    ...
    fn apply_glow(&self, buffer: &mut [u32], width: u32, height: u32) {
        ...
        let mut temporary = buffer.to_vec();
        ...
        buffer.copy_from_slice(&temporary);
    }
