...
match self.scanline {
    ...
    fn render_scanline(&mut self, cart: &Cartridge) {
        ...
        for x in 0..256usize {
            ...
            let (bg_color, bg_palette) = if bg_enabled && (bg_left || x >= 8) {
                ...
                let nt_address = nt_base + (coarse_y + fine_y / 8) * 32 + actual_tile_x;
    ...
    fn get_sprite_pixel(&self, x: u8, y: u8, spr_pattern: u16, spr_h: u8, cart: &Cartridge) -> (u8, u8, bool, bool) {
        for i in 0..self.sprite_count as usize {
            ...
            let flip_h = spr_attribute & 0x40 != 0;
            let flip_v = spr_attribute & 0x80 != 0;
            let priority = spr_attribute & 0x20 != 0; // behind background
            let palette_id = spr_attribute & 3;
            ...
            let (tile, pattern_base) = if spr_h == 16 {
                ...
                let tile_number = spr_tile & 0xFE;
                if row < 8 {
                    (tile_number as u16, bank)
                } else {
                    ((tile_number + 1) as u16, bank)
                }
            } else {
            ...
            let pattern_address = pattern_base + tile * 16 + actual_row;
            let lo = self.ppu_read(pattern_address, cart);
            let hi = self.ppu_read(pattern_address + 8, cart);
