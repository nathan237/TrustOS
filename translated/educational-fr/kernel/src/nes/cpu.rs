...
pub fn new() -> Self {
        ...
        // Fonction publique — appelable depuis d'autres modules.
pub fn reset(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) {
    ...
    fn push8(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus, value: u8) {
        bus.cpu_write(0x0100 | self.sp as u16, value);
        self.sp = self.sp.wrapping_sub(1);
    }
    fn push16(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus, value: u16) {
        self.push8(bus, (value >> 8) as u8);
        self.push8(bus, value as u8);
    }
    fn pull8(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u8 {
        self.sp = self.sp.wrapping_add(1);
        bus.cpu_read(0x0100 | self.sp as u16)
    }
    fn pull16(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u16 {
    ...
    fn read8(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u8 {
        let v = bus.cpu_read(self.pc); self.pc = self.pc.wrapping_add(1); v
    }
    fn read16(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u16 {
    ...
    fn read16_bug(&self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus, address: u16) -> u16 {
    ...
    fn imm(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u8 { self.read8(bus) }
    fn zp_r(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u8 { let a = self.read8(bus) as u16; bus.cpu_read(a) }
    fn zpx_r(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u8 { let a = self.read8(bus).wrapping_add(self.x) as u16; bus.cpu_read(a) }
    fn zpy_r(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u8 { let a = self.read8(bus).wrapping_add(self.y) as u16; bus.cpu_read(a) }
    fn absolute_r(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u8 { let a = self.read16(bus); bus.cpu_read(a) }
    fn abx_r(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> (u8, u32) {
        let base = self.read16(bus); let a = base.wrapping_add(self.x as u16);
        let p = if (base & 0xFF00) != (a & 0xFF00) { 1 } else { 0 };
        (bus.cpu_read(a), p)
    }
    fn aby_r(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> (u8, u32) {
        let base = self.read16(bus); let a = base.wrapping_add(self.y as u16);
        let p = if (base & 0xFF00) != (a & 0xFF00) { 1 } else { 0 };
        (bus.cpu_read(a), p)
    }
    fn izx_r(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u8 {
    ...
    fn izy_r(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> (u8, u32) {
    ...
    fn zp_a(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u16 { self.read8(bus) as u16 }
    fn zpx_a(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u16 { self.read8(bus).wrapping_add(self.x) as u16 }
    fn zpy_a(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u16 { self.read8(bus).wrapping_add(self.y) as u16 }
    fn absolute_a(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u16 { self.read16(bus) }
    fn abx_a(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u16 { let b = self.read16(bus); b.wrapping_add(self.x as u16) }
    fn aby_a(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u16 { let b = self.read16(bus); b.wrapping_add(self.y as u16) }
    fn izx_a(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u16 {
        let z = self.read8(bus).wrapping_add(self.x);
        let lo = bus.cpu_read(z as u16) as u16;
        let hi = bus.cpu_read(z.wrapping_add(1) as u16) as u16;
    ...
    fn izy_a(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u16 {
        let z = self.read8(bus);
        let lo = bus.cpu_read(z as u16) as u16;
        let hi = bus.cpu_read(z.wrapping_add(1) as u16) as u16;
    ...
    fn branch(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus, condition: bool) -> u32 {
    ...
    pub fn step(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl CpuBus) -> u32 {
        ...
        let cycles = // Correspondance de motifs — branchement exhaustif de Rust.
match opcode {
            // ===== LDA =====
