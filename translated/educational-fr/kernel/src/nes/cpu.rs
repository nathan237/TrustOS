...
impl CpuBus) -> u8 { let a = self.read8(bus).wrapping_add(self.y) as u16; bus.cpu_read(a) }
    fn abs_r(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
...
impl CpuBus) -> u16 { self.read8(bus).wrapping_add(self.y) as u16 }
    fn abs_a(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
