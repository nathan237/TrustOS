...
match action.sa_handler {
        ...
        handler => {
            ...
            // Write signal frame
            let frame = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
pub fn sigreturn_restore(
    ...
    let frame = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*(frame_address as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SignalFrame) };
...
fn generate_core_dump(pid: u32, signo: u32) {
    ...
    let context = // Correspondance de motifs — branchement exhaustif de Rust.
