...
pub fn load_bitmap(path: &str) -> Option<WallpaperData> {
...
pub fn load_bitmap_from_bytes(data: &[u8]) -> Option<WallpaperData> {
    parse_bitmap(data)
}

/// Parse BMP data
fn parse_bitmap(data: &[u8]) -> Option<WallpaperData> {
    // Minimum size check
    if data.len() < 54 {
    ...
    let height_u = height_absolute as u32;
...
pub fn create_bitmap(width: u32, height: u32, pixels: &[u32]) -> Vec<u8> {
