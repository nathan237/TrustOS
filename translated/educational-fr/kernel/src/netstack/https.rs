...
match path.rfind('/') {
                    ...
                    alloc::format!("https://{}:{}{}{}", host, port, base_directory, value)
                };
...
fn parse_https_url(url: &str) -> Result<(String, String, u16), HttpsError> {
    ...
    // Split host and port
    let (host, port) = if let Some(colon_position) = host_port.rfind(':') {
        let port_str = &host_port[colon_position + 1..];
        let port = port_str.parse().unwrap_or(443);
        (&host_port[..colon_position], port)
    } else {
        (host_port, 443u16)
    };
...
fn parse_http_response(data: &[u8]) -> Result<HttpsResponse, HttpsError> {
    ...
    for line in headers_str.lines() {
        if let Some(colon_position) = line.find(':') {
            let name = line[..colon_position].trim().to_string();
            let value = line[colon_position + 1..].trim().to_string();
