...
fn parse_url(url: &str) -> Result<(&str, u16, &str), &'static str> {
    ...
    let (host_port, path) = // Correspondance de motifs — branchement exhaustif de Rust.
match url.find('/') {
        Some(i) => (&url[..i], &url[i..]),
        None => (url, "/"),
    };
    
    let (host, port) = // Correspondance de motifs — branchement exhaustif de Rust.
    ...
    if response.status_code >= 300 && response.status_code < 400 {
        if let Some(location) = response.header("Location") {
            ...
            let redirect_url = if location.starts_with("http://") {
                ...
                // Relative URL
                let base_directory = // Correspondance de motifs — branchement exhaustif de Rust.
match path.rfind('/') {
                    Some(i) => &path[..=i],
                    None => "/",
                };
                format!("http://{}:{}{}{}", host, port, base_directory, location)
            };
...
fn contains_end_marker(data: &[u8]) -> bool {
    // Find header end
    let header_end = // Correspondance de motifs — branchement exhaustif de Rust.
match find_header_end(data) {
    ...
    if let Some(cl) = find_content_length(data, header_end) {
        let body_length = data.len() - header_end;
        return body_length >= cl;
    }
