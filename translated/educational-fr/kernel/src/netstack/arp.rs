...
pub fn handle_packet(data: &[u8]) {
    let packet = // Correspondance de motifs — branchement exhaustif de Rust.
...
fn handle_request(packet: &ArpPacket) {
    ...
    let our_ip = // Correspondance de motifs — branchement exhaustif de Rust.
match crate::network::get_interface() {
        Some((_, Some(ip), _)) => ip,
        _ => return,
    };
