...
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const RAMDISK_SECTORS: u64 = 256;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const RAMDISK_SIZE: usize = (RAMDISK_SECTORS as usize) * SECTOR_SIZE;

/// Maximum sectors per read/write
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MAXIMUM_SECTORS: u8 = 128;
...
const MAXIMUM_FILENAME: usize = 32;
// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MAXIMUM_FILES: usize = 10;
// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
pub fn list_files() -> Result<Vec<FileEntry>, &'static str> {
    let sector = read_sector(DIRECTORY_SECTOR)?;
    ...
    let entry_size = 1 + MAXIMUM_FILENAME + 4 + 4 + 1; // flags + name + size + sector + type
    ...
    for i in 0..file_count {
        ...
        while name_end < name_start + MAXIMUM_FILENAME && sector[name_end] != 0 {
            name_end += 1;
        }
        let name = String::from_utf8_lossy(&sector[name_start..name_end]).into_owned();
        
        let size_offset = offset + 1 + MAXIMUM_FILENAME;
...
pub fn create_file(name: &str, data: &[u8]) -> Result<(), &'static str> {
    if name.len() > MAXIMUM_FILENAME - 1 {
        return Err("Filename too long");
    }
    
    // Read current directory
    let mut directory_sector = read_sector(DIRECTORY_SECTOR)?;
    let file_count = directory_sector[0] as usize;
    ...
    // Add directory entry
    let entry_size = 1 + MAXIMUM_FILENAME + 4 + 4 + 1;
    let entry_offset = 1 + file_count * entry_size;
    
    directory_sector[entry_offset] = 1; // Active entry
    
    // Name
    for (i, byte) in name.bytes().enumerate() {
        directory_sector[entry_offset + 1 + i] = byte;
    }
    
    // Size
    let size_bytes = (data.len() as u32).to_le_bytes();
    let size_offset = entry_offset + 1 + MAXIMUM_FILENAME;
    directory_sector[size_offset..size_offset + 4].copy_from_slice(&size_bytes);
    
    // Start sector
    let sector_bytes = (start_sector as u32).to_le_bytes();
    let sector_offset = size_offset + 4;
    directory_sector[sector_offset..sector_offset + 4].copy_from_slice(&sector_bytes);
    
    // Type (0 = file)
    directory_sector[sector_offset + 4] = 0;
    
    // Update file count
    directory_sector[0] = (file_count + 1) as u8;
    
    write_sector(DIRECTORY_SECTOR, &directory_sector)?;
...
pub fn delete_file(name: &str) -> Result<(), &'static str> {
    ...
    for i in 0..file_count {
        ...
        let name_start = offset + 1;
        let mut name_end = name_start;
        while name_end < name_start + MAXIMUM_FILENAME && directory_sector[name_end] != 0 {
            name_end += 1;
        }
        let entry_name = String::from_utf8_lossy(&directory_sector[name_start..name_end]);
        
        if entry_name == name {
            // Mark as deleted
            directory_sector[offset] = 0;
            write_sector(DIRECTORY_SECTOR, &directory_sector)?;
