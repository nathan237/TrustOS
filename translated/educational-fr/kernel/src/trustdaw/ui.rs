...
use super::{PLAYING, RECORDING, PLAYBACK_POSITION, BPM, TICKS_PER_QUARTER, MAXIMUM_TRACKS};
...
const BG: u32 = 0x0D0D1A;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const HEADER_BG: u32 = 0x1A1A2E;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TRANSPORT_BG: u32 = 0x151525;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TRACK_BG: u32 = 0x121222;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TRACK_SELECTED: u32 = 0x1A1A3A;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TRACK_BORDER: u32 = 0x2A2A4A;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TEXT_PRIMARY: u32 = 0xDDDDEE;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TEXT_SECONDARY: u32 = 0x8888AA;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TEXT_DIM: u32 = 0x555577;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PLAY_GREEN: u32 = 0x44DD44;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const STOP_RED: u32 = 0xDD4444;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const REC_RED: u32 = 0xFF2222;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const BUTTON_BG: u32 = 0x2A2A4A;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const BUTTON_ACTIVE: u32 = 0x3A3A6A;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MUTE_COLOR: u32 = 0xFF8800;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SOLO_COLOR: u32 = 0xFFDD00;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
impl DawUI {
        ...
        if let (Some(proj), Some(mix)) = (project.as_ref(), mixer.as_ref()) {
            ...
            if self.show_piano_roll {
                if let Some(track) = proj.tracks.get(self.selected_track) {
                    let playhead = PLAYBACK_POSITION.load(Ordering::Relaxed);
    ...
    fn draw_transport(&self, fb_w: u32, proj: &super::track::Project) {
        ...
        // Track count
        let track_str = format!("Tracks: {}/{}", proj.tracks.len(), MAXIMUM_TRACKS);
    ...
    fn draw_track_list(&self, fb_w: u32, proj: &super::track::Project, mixer: &super::mixer::Mixer) {
        ...
        for (i, track) in proj.tracks.iter().enumerate() {
            ...
            crate::framebuffer::draw_text(&number_str, 8, row_y + 6, colors::TEXT_PRIMARY);
