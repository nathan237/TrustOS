...
pub // Variable atomique — accès thread-safe sans verrou.
static PLAYBACK_POSITION: AtomicU32 = AtomicU32::new(0);
/// Current BPM
pub // Variable atomique — accès thread-safe sans verrou.
...
pub fn init() -> Result<(), &'static str> {
    ...
    *PROJECT.lock() = Some(project);
    ...
    // Create mixer
    let mixer = Mixer::new(MAXIMUM_TRACKS);
    *MIXER.lock() = Some(mixer);
    ...
    crate::serial_println!("[TRUSTDAW] TrustDAW initialized — {} tracks max, {} ticks/quarter",
        MAXIMUM_TRACKS, TICKS_PER_QUARTER);
    Ok(())
}
...
pub fn list_notes(track_index: usize) -> Result<String, &'static str> {
    ...
    s.push_str(&format!("Track {}: \"{}\" — {} notes, {}\n",
        track_index, track.name_str(), track.notes.len(), track.waveform.name()));
