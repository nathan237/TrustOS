...
const MINIMUM_NOTE: u8 = 24;  // C1
/// Highest MIDI note displayed
const MAXIMUM_NOTE: u8 = 96;  // C7
...
const BG: u32 = 0x1A1A2E;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GRID_LINE: u32 = 0x2A2A3E;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GRID_BEAT: u32 = 0x3A3A4E;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const GRID_BAR: u32 = 0x5A5A6E;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const WHITE_KEY_BG: u32 = 0x222236;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const BLACK_KEY_BG: u32 = 0x181828;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const KEY_LABEL_BG: u32 = 0x101020;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const KEY_WHITE: u32 = 0xCCCCDD;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const KEY_BLACK: u32 = 0x333344;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const KEY_TEXT: u32 = 0x888899;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TIMELINE_BG: u32 = 0x151530;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TIMELINE_TEXT: u32 = 0xAAAABB;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const NOTE_BORDER: u32 = 0xFFFFFF;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PLAYHEAD: u32 = 0xFF4444;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SELECTION: u32 = 0x4488FF;
    pub     // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CURSOR: u32 = 0x44FF44;
}
...
impl PianoRoll {
    ...
    fn draw_notes(&self, track: &Track, gx: u32, gy: u32, gw: u32, gh: u32) {
        ...
        for (i, note) in track.notes.iter().enumerate() {
            ...
            if note_pixel_end < gx || note_pixel_start > gx + gw {
                continue;
            }
    ...
    pub fn scroll_up(&mut self) {
        if self.scroll_y < MAXIMUM_NOTE - 12 {
    ...
    pub fn scroll_down(&mut self) {
        if self.scroll_y > MINIMUM_NOTE + 12 {
            self.scroll_y -= 12;
        } else {
            self.scroll_y = MINIMUM_NOTE;
        }
    }
    ...
    pub fn set_grid(&mut self, division: &str) {
        self.grid_snap = // Correspondance de motifs — branchement exhaustif de Rust.
match division {
...
pub fn text_piano_roll(track: &Track, bars: u32) -> String {
    ...
    let tick_per_column = ticks_per_bar / 16;
