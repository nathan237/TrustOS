...
/// Lock protecting CURRENT_USER_SPACE to prevent SMP races during exec.
/// Must be held across the store → Ring 3 → clear sequence.
static EXECUTE_LOCK: spin::Mutex<()> = spin::Mutex::new(());
    ...
    F: FnOnce(&mut AddressSpace) -> R,
{
    ...
    Some(f(// SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &mut *ptr }))
}
...
pub fn execute_path(path: &str, args: &[&str]) -> ExecResult {
    ...
    // Load ELF file
    let elf = // Correspondance de motifs — branchement exhaustif de Rust.
match crate::elf::load_from_path(&resolved) {
        Ok(e) => e,
        Err(e) => {
            ...
            if let Ok(data) = crate::vfs::read_file(&resolved) {
                if let Some((interp, _extra)) = check_shebang(&data) {
                    ...
                    return execute_path(&interp, &new_args[1..]);
                }
            }
    ...
    execute_elf(&elf, args)
}

/// Execute an ELF from bytes
pub fn execute_bytes(data: &[u8], args: &[&str]) -> ExecResult {
    // Load ELF from memory
    let elf = // Correspondance de motifs — branchement exhaustif de Rust.
match crate::elf::load_from_bytes(data) {
        Ok(e) => e,
        Err(e) => {
...
fn execute_elf(elf: &LoadedElf, args: &[&str]) -> ExecResult {
    ...
    let mut address_space = // Correspondance de motifs — branchement exhaustif de Rust.
match AddressSpace::new_with_kernel() {
    ...
    for segment in &elf.segments {
        ...
        for page_index in 0..pages_needed {
            ...
            let physical_page = // Correspondance de motifs — branchement exhaustif de Rust.
match allocator_physical_page() {
            ...
            let is_execute = (segment.flags & 1) != 0;  // PF_X
            let is_write = (segment.flags & 2) != 0; // PF_W
            
            let flags = if is_execute {
                PageFlags::USER_CODE
            } else if is_write {
                PageFlags::USER_DATA
            } else {
            ...
            if copy_start < segment.data.len() {
                ...
                let dest = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
...
unsafe {
    ...
    let mut argument_addrs: Vec<u64> = Vec::new();
...
unsafe {
        ...
        argument_addrs.push(sp);
    }
    argument_addrs.reverse(); // Now arg_addrs[0] = first arg address
...
unsafe { *((physical + hhdm) as *mut u64) = args.len() as u64; }
    ...
    crate::log!("[EXEC] Ready to execute at {:#x}, stack at {:#x} (argc={}, argv={:#x})", 
        elf.entry_point, sp, args.len(), argv_pointer);
    
    // ── Process lifecycle: spawn → run → exit → reap ────────────
    let process_name = args.first().copied().unwrap_or("user");
    let pid = crate::process::spawn(process_name).unwrap_or(0);
    let previous_pid = crate::process::current_pid();
    ...
    // Acquire exec lock to prevent SMP races on CURRENT_USER_SPACE
    let _execute_guard = EXECUTE_LOCK.lock();
...
unsafe {
        ...
        exit_code = crate::userland::execute_ring3_process(elf.entry_point, sp);
    ...
    // Release exec lock (implicit drop of _exec_guard)
    drop(_execute_guard);
    ...
    crate::process::set_current(previous_pid);
...
pub fn execute_test_program() -> ExecResult {
...
match AddressSpace::new_with_kernel() {
    ...
    let code_physical = // Correspondance de motifs — branchement exhaustif de Rust.
match allocator_physical_page() {
        Some(p) => p,
    ...
    let stack_pages = 4;
    let guard_page_address = stack_top - (stack_pages as u64 + 1) * 4096;
    // Guard page at guard_page_addr: left unmapped — access triggers page fault
    
    for i in 0..stack_pages {
        let vaddr = stack_top - (i as u64 + 1) * 4096;
        let physical = // Correspondance de motifs — branchement exhaustif de Rust.
match allocator_physical_page() {
            Some(p) => p,
            None => return ExecResult::MemoryError,
        };
...
unsafe { core::ptr::write_bytes((physical + hhdm) as *mut u8, 0, 4096); }
    ...
    unsafe {
        ...
        let exit_code = crate::userland::execute_ring3_process(code_vaddr, user_stack);
...
pub fn execute_hello_elf() -> ExecResult {
    crate::log!("[EXEC] Running embedded hello world ELF...");
    execute_bytes(HELLO_ELF, &[])
}
...
pub fn execute_memtest() -> ExecResult {
...
fn allocator_physical_page() -> Option<u64> {
    crate::memory::frame::allocator_frame_zeroed()
}
...
pub fn execute_pipe_test() -> ExecResult {
...
pub fn execute_exception_safety_test() -> ExecResult {
...
/// Prints "SIG OK\n" on success, "SIG FAIL\n" on failure.
pub fn execute_signal_test() -> ExecResult {
...
unsafe {
        ...
        crate::process::start_running(pid);
        address_space.activate();
        exit_code = crate::userland::execute_ring3_process(code_vaddr, user_stack);
        CURRENT_USER_SPACE.store(core::ptr::null_mut(), Ordering::Release);
        #[cfg(target_arch = "x86_64")]
        core::arch::asm!("mov cr3, {}", in(reg) kernel_cr3, options(nostack, preserves_flags));
        #[cfg(not(target_arch = "x86_64"))]
...
/// Prints "IO OK\n" on success, "IO FAIL\n" on failure.
pub fn execute_stdio_test() -> ExecResult {
