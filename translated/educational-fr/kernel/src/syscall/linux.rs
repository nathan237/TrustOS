...
match op {
        EPOLL_CONTROLLER_ADD => {
            ...
            let event = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { *(event_pointer as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const EpollEvent) };
