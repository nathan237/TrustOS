...
const CREATOR_SIGNED_PAYLOAD: &str = 
...
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CREATOR_NAME: &str = "Nated0ge";
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CREATOR_GITHUB: &str = "nathan237";
...
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const CREATOR_FINGERPRINT: [u8; 32] = [
...
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const BUILD_TIMESTAMP: &str = env!("TRUSTOS_BUILD_TIME", "unknown");

/// Kernel version
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const KERNEL_VERSION: &str = "0.1.2";
...
static ED25519_BOOT_SIGNAL: Mutex<Option<[u8; 64]>> = Mutex::new(None);
...
pub fn initialize_ed25519(seed: &[u8]) {
    ...
    if let Some(digest) = *digest_lock {
        ...
        *ED25519_BOOT_SIGNAL.lock() = Some(sig);
...
pub fn ed25519_boot_signal_hex() -> Option<String> {
    ED25519_BOOT_SIGNAL.lock().map(|s| crate::ed25519::bytes_to_hex(&s))
}
...
pub fn verify_ed25519_signature() -> Result<bool, &'static str> {
    ...
    let sig = ED25519_BOOT_SIGNAL.lock().ok_or("No Ed25519 signature")?;
...
pub fn ed25519_report() -> Vec<String> {
    ...
    if let Some(hex) = ed25519_boot_signal_hex() {
...
unsafe { &__text_start as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8 as usize };
    let end = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &__text_end as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8 as usize };
    let size = end.saturating_sub(start);
    
    let text_bytes = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
        core::slice::from_raw_parts(start as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8, size)
    };
...
fn hash_rodata_section() -> [u8; 32] {
    let start = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &__rodata_start as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8 as usize };
    let end = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &__rodata_end as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8 as usize };
    let size = end.saturating_sub(start);
    
    let rodata_bytes = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
        core::slice::from_raw_parts(start as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8, size)
    };
