...
// Définition de trait — déclare une interface partagée que les types peuvent implémenter.
pub trait GbBus {
...
pub fn hl(&self) -> u16 { (self.h as u16) << 8 | self.l as u16 }
    ...
    // Fetch
    fn fetch8(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl GbBus) -> u8 {
        let v = bus.read(self.pc); self.pc = self.pc.wrapping_add(1); v
    }
    fn fetch16(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl GbBus) -> u16 {
...
impl GbBus, value: u16) {
    ...
    fn pop16(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl GbBus) -> u16 {
    ...
    fn get_register(&self, r: u8, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl GbBus) -> u8 {
...
match r & 7 {
    ...
    fn set_register(&mut self, r: u8, v: u8, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl GbBus) {
                // Correspondance de motifs — branchement exhaustif de Rust.
match r & 7 {
...
impl GbBus) -> u32 {
    ...
    fn execute(&mut self, op: u8, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl GbBus) -> u32 {
                // Correspondance de motifs — branchement exhaustif de Rust.
match op {
            // ===== 0x00-0x0F =====
    ...
    fn execute_callback(&mut self, bus: &mut // Bloc d'implémentation — définit les méthodes du type ci-dessus.
impl GbBus) -> u32 {
