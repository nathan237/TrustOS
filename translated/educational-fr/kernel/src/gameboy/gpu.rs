...
pub fn read_stat(&self) -> u8 {
    ...
    fn render_bg_scanline(&mut self, ly: usize, offset: usize) {
        ...
        let signed_address = self.lcdc & 0x10 == 0;
        ...
        for x in 0..SCREEN_W {
            ...
            let tile_column = real_x / 8;
            let pixel_column = real_x % 8;

            let map_index = tile_row * 32 + tile_column;
            let tile_id = self.vram[tile_map_area + map_index];

            let tile_address = if signed_address {
            ...
            let row_address = tile_address + pixel_row * 2;
            if row_address + 1 >= self.vram.len() { continue; }

            let lo = self.vram[row_address];
            let hi = self.vram[row_address + 1];
    ...
    fn render_window_scanline(&mut self, ly: usize, offset: usize) {
        ...
        for x in 0..SCREEN_W {
            ...
            let tile_column = win_x as usize / 8;
            let pixel_column = win_x as usize % 8;
            let map_index = tile_row * 32 + tile_column;
            if map_index >= 1024 { continue; }
    ...
    fn render_sprite_scanline(&mut self, ly: usize, offset: usize) {
        ...
        for i in (0..count).rev() {
            ...
            let tile_address = tile as usize * 16 + row as usize * 2;
            if tile_address + 1 >= self.vram.len() { continue; }
            ...
            for px in 0..8i32 {
                ...
                let screen_index = offset + screen_x as usize;

                // Behind BG: only draw if BG pixel is color 0
                if behind_bg {
                    let bg_color = self.framebuffer[screen_index];
                    if bg_color != GB_PALETTE[0] { continue; }
                }

                let palette_color = (palette >> (color_id * 2)) & 3;
                self.framebuffer[screen_index] = GB_PALETTE[palette_color as usize];
            }
        }
    }
    ...
    fn render_bg_scanline_cgb(&mut self, ly: usize, offset: usize) {
        ...
        for x in 0..SCREEN_W {
            ...
            let attrs = self.vram1[tile_map_area + map_index];
            ...
            let row_address = tile_address + actual_row * 2;
            let vram_data = if tile_vram_bank == 1 { &self.vram1 } else { &self.vram };
            if row_address + 1 >= vram_data.len() { continue; }

            let lo = vram_data[row_address];
            let hi = vram_data[row_address + 1];
            let bit = if flip_x { pixel_column } else { 7 - pixel_column };
            let color_id = ((hi >> bit) & 1) << 1 | ((lo >> bit) & 1);
    ...
    fn render_sprite_scanline_cgb(&mut self, ly: usize, offset: usize) {
        ...
        for i in (0..count).rev() {
            ...
            let tile_address = tile as usize * 16 + row as usize * 2;
            let vram_data = if tile_vram_bank == 1 { &self.vram1 } else { &self.vram };
            if tile_address + 1 >= vram_data.len() { continue; }
            ...
            for px in 0..8i32 {
                ...
                let sx_index = screen_x as usize;
                let screen_index = offset + sx_index;
