...
const MAXIMUM_INSTRUCTIONS: u32 = 200_000; // Safety limit

        while frame_cycles < FRAME_MCYCLES {
            safety_counter += 1;
            if safety_counter > MAXIMUM_INSTRUCTIONS {
    ...
    pub fn render(&self, out: &mut [u32], out_w: usize, out_h: usize) {
        ...
        let source_w = gpu::SCREEN_W;
        let source_h = gpu::SCREEN_H;

        for y in 0..out_h {
            let sy = y * source_h / out_h;
            for x in 0..out_w {
                let sx = x * source_w / out_w;
                let si = sy * source_w + sx;
...
match addr {
            ...
            0xFF46 => {
                ...
                for i in 0..160u16 {
                    let byte = // Correspondance de motifs — branchement exhaustif de Rust.
match base + i {
