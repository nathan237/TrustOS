...
impl Certificate {
    ...
    pub fn parse(data: &[u8]) -> Option<Self> {
        ...
        let mut cert_position = 0;
        
        // TBSCertificate SEQUENCE
        let (tag, tbs_data) = parse_element(cert_data, &mut cert_position)?;
        if tag != asn1_tags::SEQUENCE {
            return None;
        }
        
        // Parse TBSCertificate
        let mut tbs_position = 0;
        
        // Version (optional, context-specific [0])
        if tbs_position < tbs_data.len() && tbs_data[tbs_position] == asn1_tags::CONTEXT_0 {
            let _ = parse_element(tbs_data, &mut tbs_position)?;
        }
        ...
        // Issuer
        let (tag, issuer_data) = parse_element(tbs_data, &mut tbs_position)?;
        ...
        // Validity
        let (tag, validity_data) = parse_element(tbs_data, &mut tbs_position)?;
        ...
        // Subject
        let (tag, subject_data) = parse_element(tbs_data, &mut tbs_position)?;
        ...
        let (tag, spki_data) = parse_element(tbs_data, &mut tbs_position)?;
        ...
        let mut san = Vec::new();
        while tbs_position < tbs_data.len() {
            if let Some((tag, ext_wrapper)) = parse_element(tbs_data, &mut tbs_position) {
                if tag == asn1_tags::CONTEXT_3 {
                    // Extensions SEQUENCE
                    let mut ext_position = 0;
                    if let Some((_, extensions)) = parse_element(ext_wrapper, &mut ext_position) {
