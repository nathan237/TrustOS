...
const TLS_VERSION_1_3: u16 = 0x0303; // Legacy version in record layer
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const TLS_VERSION_1_3_DRAFT: u16 = 0x0304;
...
match session.process_record(&record_data) {
        ...
        match recv(&mut recv_buffer) {
            ...
            Ok(n) => {
                ...
                accumulator.extend_from_slice(&recv_buffer[..n]);
            }
