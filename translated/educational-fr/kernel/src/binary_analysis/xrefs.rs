...
fn detect_functions(
    ...
    for (i, (entry, name)) in sorted_entries.iter().enumerate() {
        ...
        let start_index = // Correspondance de motifs — branchement exhaustif de Rust.
match address_to_index.get(entry) {
        ...
        let end_index = if i + 1 < sorted_entries.len() {
            let next_entry = sorted_entries[i + 1].0;
            address_to_index.get(&next_entry).copied().unwrap_or(instructions.len())
        } else {
            instructions.len()
        };
        ...
        let func_instructions = &instructions[start_index..end_index];
        let end_address = func_instructions.last()
...
fn extract_rip_target(operands: &str, inst_address: u64, inst_len: u64) -> Option<u64> {
    ...
    if let Some(rest) = after_rip.strip_prefix('+') {
        ...
        Some(next_address.wrapping_sub(offset))
    } else {
        // [rip] with no offset
        Some(next_address)
    }
