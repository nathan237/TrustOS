...
fn draw_screen(state: &EditorState) {
    ...
    if age < 8000 {
        crate::framebuffer::draw_text(&state.status_msg, 0, message_y, COLOR_YELLOW);
    } else {
    ...
    let cursor_screen_column = state.cursor_col - state.scroll_col + gutter;
...
fn find_next(state: &mut EditorState, query: &str) {
    ...
    let start_column = state.cursor_col + 1;
    ...
    for row in start_row..state.lines.len() {
        ...
        if column_start < state.lines[row].len() {
            if let Some(pos) = state.lines[row][column_start..].find(query) {
                state.cursor_row = row;
                state.cursor_col = column_start + pos;
    ...
    for row in 0..=start_row {
        ...
        if let Some(pos) = state.lines[row][..column_end.min(state.lines[row].len())].find(query) {
            state.cursor_row = row;
