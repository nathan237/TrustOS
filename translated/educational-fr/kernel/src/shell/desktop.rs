...
pub(super) fn launch_desktop_env(initial_window: Option<(&str, crate::desktop::WindowType, i32, i32, u32, u32)>) {
    ...
    // Show tier info
    let tier_name = // Correspondance de motifs — branchement exhaustif de Rust.
...
match args.first().copied() {
        ...
        Some("gate") => {
            ...
            if let Some(subsystem_name) = args.get(1).copied() {
                let subsystem = // Correspondance de motifs — branchement exhaustif de Rust.
...
pub(super) fn command_demo(args: &[&str]) {
    let lang = // Correspondance de motifs — branchement exhaustif de Rust.
match args.first().copied() {
        Some("fr") => "fr",
        _ => "en",
    };
...
pub(super) fn command_cosmic_v2_with_app_timed(initial_app: Option<&str>, timeout_mouse: u64) {
    ...
    let mut active_mode = // Correspondance de motifs — branchement exhaustif de Rust.
match initial_app {
...
match file_type {
                            ...
                            _ if cmd.starts_with("nslookup ") || cmd.starts_with("dig ") => {
                                ...
                                let ip_result = // Correspondance de motifs — branchement exhaustif de Rust.
match host {
                                    "google.com" | "www.google.com" => Some([142, 250, 179, 110]),
                                    "cloudflare.com" | "www.cloudflare.com" => Some([104, 16, 132, 229]),
                                    "github.com" | "www.github.com" => Some([140, 82, 114, 3]),
                ...
                // Dock icon clicks - also reopens window if closed
                    ...
                    for i in 0..8usize {
                        ...
                        if my >= iy && my < iy + icon_size + 16 {
                            active_mode = // Correspondance de motifs — branchement exhaustif de Rust.
match i {
                                0 => AppMode::Files,
                                1 => AppMode::Shell,
                                2 => AppMode::Network,
        ...
        if let Some(menu) = compositor.get_layer_mut(menu_layer) {
            if menu_open {
                ...
                for (i, (name, item)) in menu_items.iter().enumerate() {
                    ...
                    let (color, icon) = // Correspondance de motifs — branchement exhaustif de Rust.
match item {
                        MenuItem::App(_) => {
    ...
    while running {
        ...
        for column in 0..MATRIX_COLS {
            ...
            for i in 0..MATRIX_TRAIL_LENGTH {
                ...
                let char_s = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
match i {
        ...
        if search_length == 0 {
            ...
            let search_display = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
