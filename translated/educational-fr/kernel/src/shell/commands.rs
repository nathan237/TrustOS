...
pub(super) fn command_help(args: &[&str]) {
    if !args.is_empty() {
        command_man(args);
        return;
    }
    ...
    // THINKPAD / HARDWARE CONTROL
    crate::println_color!(COLOR_CYAN, "  THINKPAD / HARDWARE CONTROL");
    ...
    // EASTER EGGS & DEMOS
    crate::println_color!(COLOR_CYAN, "  EASTER EGGS & DEMOS");
    ...
    crate::println_color!(COLOR_BRIGHT_GREEN, "  Total: ~220 commands | Type 'man <cmd>' for detailed usage");
    crate::println!();
}
pub(super) fn command_man(args: &[&str]) {
    if args.is_empty() {
...
pub(super) fn command_ls(args: &[&str]) {
    ...
    if let Some(p) = path {
        ...
        // Correspondance de motifs — branchement exhaustif de Rust.
match crate::ramfs::with_filesystem(|fs| fs.ls(path)) {
        Ok(items) => {
            if items.is_empty() {
                return;
            }
...
match file_type {
                    FileType::Directory => {
                        crate::print_color!(COLOR_CYAN, "{:<width$}", name, width = maximum_name + 2);
                        crate::println_color!(COLOR_DARK_GREEN, " <DIR>");
                    }
                    FileType::File => {
                        crate::print_color!(COLOR_GREEN, "{:<width$}", name, width = maximum_name + 2);
...
pub(super) fn command_ls_vfs(path: &str) {
...
match entry.file_type {
                    VfsFileType::Directory => {
                        crate::print_color!(COLOR_CYAN, "{:<width$}", entry.name, width = maximum_name + 2);
                        crate::println_color!(COLOR_DARK_GREEN, " <DIR>");
                    }
                    VfsFileType::Regular => {
                        crate::print_color!(COLOR_GREEN, "{:<width$}", entry.name, width = maximum_name + 2);
                        crate::println!(" (file)");
                    }
                    _ => {
...
pub(super) fn command_cd(args: &[&str]) {
    let path = args.first().copied().unwrap_or("~");
    
    if let Err(e) = crate::ramfs::with_filesystem(|fs| fs.cd(path)) {
        crate::println_color!(COLOR_RED, "cd: {}: {}", path, e.as_str());
    }
}

pub(super) fn command_pwd() {
    let cwd = crate::ramfs::with_filesystem(|fs| String::from(fs.pwd()));
    crate::println!("{}", cwd);
}

pub(super) fn command_mkdir(args: &[&str]) {
    if args.is_empty() {
    ...
    for path in args {
        if let Err(e) = crate::ramfs::with_filesystem(|fs| fs.mkdir(path)) {
            crate::println_color!(COLOR_RED, "mkdir: {}: {}", path, e.as_str());
        }
    }
}

pub(super) fn command_rmdir(args: &[&str]) {
    if args.is_empty() {
    ...
    for path in args {
        if let Err(e) = crate::ramfs::with_filesystem(|fs| fs.rm(path)) {
            crate::println_color!(COLOR_RED, "rmdir: {}: {}", path, e.as_str());
        }
    }
}

pub(super) fn command_touch(args: &[&str]) {
    if args.is_empty() {
...
match vfs::open(path, flags) {
            ...
            if let Err(e) = crate::ramfs::with_filesystem(|fs| fs.touch(path)) {
...
pub(super) fn command_rm(args: &[&str]) {
    if args.is_empty() {
...
pub(super) fn command_cp(args: &[&str]) {
    if args.len() < 2 {
        crate::println!("Usage: cp <source> <destination>");
        return;
    }
    
    if let Err(e) = crate::ramfs::with_filesystem(|fs| fs.cp(args[0], args[1])) {
        crate::println_color!(COLOR_RED, "cp: {}", e.as_str());
    }
}

pub(super) fn command_mv(args: &[&str]) {
    if args.len() < 2 {
        crate::println!("Usage: mv <source> <destination>");
        return;
    }
    
    if let Err(e) = crate::ramfs::with_filesystem(|fs| fs.mv(args[0], args[1])) {
        crate::println_color!(COLOR_RED, "mv: {}", e.as_str());
    }
}

pub(super) fn command_cat(args: &[&str], redirect: Option<(&str, bool)>, piped: Option<&str>) {
...
match command_cat_vfs(path) {
            ...
            continue;
                ...
                // Correspondance de motifs — branchement exhaustif de Rust.
match crate::ramfs::with_filesystem(|fs| fs.read_file(path).map(|c| c.to_vec())) {
            Ok(content) => {
...
pub(super) fn command_cat_vfs(path: &str) -> Option<alloc::string::String> {
    use crate::vfs::{self, OpenFlags};
    use alloc::string::ToString;
    
    // Open the file
    let fd = // Correspondance de motifs — branchement exhaustif de Rust.
match vfs::open(path, OpenFlags(OpenFlags::O_RDONLY)) {
        Ok(f) => f,
        Err(e) => {
...
loop {
        let bytes_read = // Correspondance de motifs — branchement exhaustif de Rust.
...
pub(super) fn command_head(args: &[&str], piped: Option<&str>) {
    ...
    let content_str = if let Some(input) = piped {
        alloc::string::String::from(input)
                ...
                // Correspondance de motifs — branchement exhaustif de Rust.
match crate::ramfs::with_filesystem(|fs| fs.read_file(args[0]).map(|c| c.to_vec())) {
            Ok(content) => // Correspondance de motifs — branchement exhaustif de Rust.
match core::str::from_utf8(&content) {
...
pub(super) fn command_tail(args: &[&str], piped: Option<&str>) {
...
pub(super) fn command_wc(args: &[&str], piped: Option<&str>) {
...
pub(super) fn command_status(args: &[&str]) {
    if args.is_empty() {
...
pub(super) fn command_tree(args: &[&str]) {
    let path = args.first().copied().unwrap_or("/");
...
fn print_tree_recursive(path: &str, prefix: &str) {
    if let Ok(items) = crate::ramfs::with_filesystem(|fs| fs.ls(Some(path))) {
        let len = items.len();
...
pub(super) fn command_find(args: &[&str]) {
    if args.is_empty() {
...
pub(super) fn command_echo(args: &[&str], redirect: Option<(&str, bool)>) {
    let text = args.join(" ");
...
pub(super) fn command_grep(args: &[&str], piped: Option<&str>) {
    if args.is_empty() {
    ...
    let content = if let Some(input) = piped {
        alloc::string::String::from(input)
                ...
                // Correspondance de motifs — branchement exhaustif de Rust.
match crate::ramfs::with_filesystem(|fs| fs.read_file(args[1]).map(|c| c.to_vec())) {
            Ok(content) => {
                                // Correspondance de motifs — branchement exhaustif de Rust.
match core::str::from_utf8(&content) {
                    Ok(t) => alloc::string::String::from(t),
...
pub(super) fn command_clear() {
    crate::framebuffer::clear();
}

pub(super) fn command_time() {
...
pub(super) fn command_date() {
    let dt = crate::rtc::read_rtc();
    crate::println_color!(COLOR_GREEN, "{}", dt.format());
}

pub(super) fn command_whoami() {
    crate::println!("{}", crate::auth::current_user());
}

pub(super) fn command_hostname() {
...
pub(super) fn command_su(args: &[&str]) {
...
pub(super) fn command_passwd(args: &[&str]) {
    ...
    let mut new_buffer = [0u8; 128];
    let new_length = crate::keyboard::read_line_hidden(&mut new_buffer);
    crate::println!();
    let new_password = core::str::from_utf8(&new_buffer[..new_length]).unwrap_or("").trim();
    
    // Confirm new password
    crate::print!("Retype new password: ");
    let mut confirm_buffer = [0u8; 128];
    let confirm_length = crate::keyboard::read_line_hidden(&mut confirm_buffer);
    crate::println!();
    let confirm = core::str::from_utf8(&confirm_buffer[..confirm_length]).unwrap_or("").trim();
...
pub(super) fn command_adduser(args: &[&str]) {
    if args.is_empty() {
    ...
    let password_length = crate::keyboard::read_line_hidden(&mut password_buffer);
    crate::println!();
    let password = core::str::from_utf8(&password_buffer[..password_length]).unwrap_or("").trim();
...
pub(super) fn command_deluser(args: &[&str]) {
    if args.is_empty() {
...
pub(super) fn command_information() {
...
pub(super) fn command_version() {
    crate::println!("T-RustOs v0.2.0 (Rust + Limine)");
}

pub(super) fn command_uname(args: &[&str]) {
...
pub(super) fn command_df() {
    ...
    for (path, fs_name) in &mounts {
        ...
        let (size, used, avail) = // Correspondance de motifs — branchement exhaustif de Rust.
...
pub(super) fn command_test() {
    ...
    crate::print!("  RAM FS... ");
    let ok = crate::ramfs::with_filesystem(|fs| {
        fs.touch("/tmp/t").ok();
...
pub(super) fn command_restest() {
    ...
    for &(w, h, label) in resolutions {
        ...
        for directory in &snap_dirs {
            desktop.snap_focused_window(*directory);
            if let Some(win) = desktop.windows.iter().find(|w| w.focused) {
                ...
                if right > w + 1 || bottom > h + 1 {
                    ok = false;
                    use core::fmt::Write;
                    let _ = core::fmt::write(&mut detail, format_args!(" SNAP_OOB:{:?}", directory));
                }
            }
        }
...
match crate::exec::execute_hello_elf() {
    ...
    crate::print!("  exec memtest... ");
        // Correspondance de motifs — branchement exhaustif de Rust.
match crate::exec::execute_memtest() {
        crate::exec::ExecResult::Exited(0) => {
            crate::println_color!(COLOR_GREEN, "[OK]");
            passed += 1;
        }
    ...
    let _ = crate::exec::execute_test_program();
    ...
    crate::print!("  exec pipe_test... ");
        // Correspondance de motifs — branchement exhaustif de Rust.
match crate::exec::execute_pipe_test() {
        crate::exec::ExecResult::Exited(0) => {
            crate::println_color!(COLOR_GREEN, "[OK]");
            passed += 1;
        }
...
pub(super) fn command_mesh(args: &[&str]) {
        // Correspondance de motifs — branchement exhaustif de Rust.
match args.first() {
        Some(&"start") => {
        ...
        Some(&"status") | None => {
            ...
            if peers.is_empty() {
                ...
                for (i, peer) in peers.iter().enumerate() {
                    let role_icon = // Correspondance de motifs — branchement exhaustif de Rust.
match peer.role {
...
pub(super) fn command_pxe(args: &[&str]) {
        // Correspondance de motifs — branchement exhaustif de Rust.
match args.first() {
...
// ==================== GUARDIAN COMMAND ====================
pub(super) fn command_guardian(args: &[&str]) {
...
pub(super) fn command_trustpkg(args: &[&str]) {
        // Correspondance de motifs — branchement exhaustif de Rust.
match args.first() {
...
match crate::exec::execute_pipe_test() {
    ...
    crate::print!("  invalid opcode... ");
        // Correspondance de motifs — branchement exhaustif de Rust.
match crate::exec::execute_exception_safety_test() {
    ...
    crate::print!("  signal test... ");
        // Correspondance de motifs — branchement exhaustif de Rust.
match crate::exec::execute_signal_test() {
        crate::exec::ExecResult::Exited(0) => {
            crate::println_color!(COLOR_GREEN, "[OK]");
            passed += 1;
        }
    ...
    crate::print!("  io test... ");
        // Correspondance de motifs — branchement exhaustif de Rust.
match crate::exec::execute_stdio_test() {
        crate::exec::ExecResult::Exited(0) => {
            crate::println_color!(COLOR_GREEN, "[OK]");
            passed += 1;
        }
...
match crate::nvme::read_sectors(test_lba, 1, &mut readback) {
    ...
    crate::println_color!(COLOR_CYAN, "[15/32] TrustLang bytecode VM");
    {
        ...
        let fib_source = r#"fn fibonacci(n: i64) -> i64 {
    if n <= 1 { return n; }
...
fn main() {
    print(to_string(fibonacci(10)));
                ...
                // Correspondance de motifs — branchement exhaustif de Rust.
match crate::trustlang::run(fib_source) {
...
match crate::virtio_blk::read_sectors(0, 1, &mut buf) {
    ...
    crate::println_color!(COLOR_CYAN, "[25/32] TrustScan traceroute + discovery");
    {
        ...
        let hi = crate::netscan::discovery::HostInformation {
            ip: [192, 168, 1, 1],
...
match crate::drivers::hda::parse_wav(&[0u8; 10]) {
        ...
        if frequency_a4 == 440 {
            ...
            crate::println_color!(COLOR_RED, "[FAIL] expected 440, got {}", frequency_a4);
            failed += 1;
        }
        ...
        if frequency_c4 >= 255 && frequency_c4 <= 265 {
            crate::println_color!(COLOR_GREEN, "[OK] freq={}Hz (~261)", frequency_c4);
            passed += 1;
        } else {
            crate::println_color!(COLOR_RED, "[FAIL] expected ~261, got {}", frequency_c4);
            failed += 1;
        }
...
pub(super) fn command_hexdump(args: &[&str]) {
    if args.is_empty() {
...
pub(super) fn command_neofetch() {
    let secs = crate::logger::get_ticks() / 100;
    let (w, h) = crate::framebuffer::get_dimensions();
    let total_memory_mb = crate::memory::total_physical_memory() / 1024 / 1024;
    let memory_stats = crate::memory::stats();
    let heap_used_mb = memory_stats.heap_used / 1024 / 1024;
    let heap_total_mb = (memory_stats.heap_used + memory_stats.heap_free) / 1024 / 1024;
    ...
    crate::println!("{} MB total, {} / {} MB heap", total_memory_mb, heap_used_mb, heap_total_mb);
    crate::print_color!(COLOR_GREEN, r"                      ");
...
pub(super) fn command_cowsay(args: &[&str]) {
...
pub(super) fn command_gpuexec(args: &[&str]) {
...
pub(super) fn command_sdma(args: &[&str]) {
...
match sdma::fill(phys, fill_val, byte_count, 0) {
                Ok(seq) => {
                    ...
                    let ptr = buf as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32;
                    let v0 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_volatile(ptr) };
                    let v1 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_volatile(ptr.add(1)) };
                    let v2 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_volatile(ptr.add(2)) };
                    let v3 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
unsafe { alloc::alloc::alloc_zeroed(layout) } as u64;
            let physical_a = crate::memory::virt_to_physical(buffer_a).unwrap_or(0);
            let physical_b = crate::memory::virt_to_physical(buffer_b).unwrap_or(0);
...
match sdma::copy(physical_a, physical_b, byte_count, 0) {
                Ok(seq) => {
                    ...
                    let dst = buffer_b as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32;
                    let v0 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
unsafe { core::ptr::read_volatile(dst.add(3)) };
                    ...
                    let mut ok = 0u32;
                    for i in 0..(byte_count / 4) {
                        let got = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
pub(super) fn command_neural(args: &[&str]) {
...
pub(super) fn command_gpufw(args: &[&str]) {
        ...
        "status" | "info" => {
            ...
            if let Some(ssid) = wifi::connected_ssid() {
                ...
                if let Some(dbm) = wifi::signal_strength() {
                    let bars = // Correspondance de motifs — branchement exhaustif de Rust.
match dbm {
                        -50..=0 => "████ (excellent)",
                        -60..=-51 => "███░ (good)",
                        -70..=-61 => "██░░ (fair)",
...
match wifi::ensure_started() {
        ...
        "reg" | "csr" => {
            ...
            let offset = // Correspondance de motifs — branchement exhaustif de Rust.
...
match crate::drivers::net::iwl4965::debug_read_csr(offset) {
        ...
        "prph" => {
            ...
            let addr = // Correspondance de motifs — branchement exhaustif de Rust.
...
fn print_scan_results(results: &[crate::drivers::net::wifi::WifiNetwork]) {
    ...
    for net in results {
        let signal_color = // Correspondance de motifs — branchement exhaustif de Rust.
match net.signal_dbm {
        ...
        let sec = // Correspondance de motifs — branchement exhaustif de Rust.
match net.security {
