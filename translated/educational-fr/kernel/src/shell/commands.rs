...
match sdma::fill(physical, fill_value, byte_count, 0) {
                Ok(sequence) => {
                    ...
                    let ptr = buffer as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32;
...
match sdma::copy(physical_a, physical_b, byte_count, 0) {
                Ok(sequence) => {
                    ...
                    let destination = buffer_b as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32;
                    let v0 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
