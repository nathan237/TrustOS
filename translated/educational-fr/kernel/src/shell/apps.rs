...
pub(super) fn command_gterm(args: &[&str]) {
...
pub(super) fn command_wayland(args: &[&str]) {
...
pub(super) fn command_trustlang(args: &[&str]) {
    let subcmd = args.first().copied().unwrap_or("help");

        // Correspondance de motifs — branchement exhaustif de Rust.
match subcmd {
        "run" => {
            let filename = // Correspondance de motifs — branchement exhaustif de Rust.
match args.get(1) {
                Some(f) => *f,
            ...
            let source = // Correspondance de motifs — branchement exhaustif de Rust.
match crate::ramfs::with_filesystem(|fs| fs.read_file(&path).map(|d| d.to_vec())) {
                Ok(data) => // Correspondance de motifs — branchement exhaustif de Rust.
...
match alloc::string::String::from_utf8(data) {
            ...
            fn native_builtin(id: u8, argc: usize, argv: *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const i64) -> i64 {
...
match crate::trustlang::compile_and_run_native(&source, native_builtin) {
        ...
        "bench" => {
            ...
            let vm_source = "fn fib(n: i64) -> i64 { if n <= 1 { return n; } return fib(n - 1) + fib(n - 2); } fn main() { let r = fib(25); println(to_string(r)); }";
            let start = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::arch::x86_64::_rdtsc() };
            let _ = crate::trustlang::run(vm_source);
            let end = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::arch::x86_64::_rdtsc() };
...
fn trustlang_syntax_colors(line: &str) -> alloc::vec::Vec<u32> {
    ...
    let effective_length = comment_ci.unwrap_or(len);
    ...
    while i < effective_length {
        ...
        if chars[i].is_alphabetic() || chars[i] == '_' {
            let start = i;
            while i < effective_length && (chars[i].is_alphanumeric() || chars[i] == '_') {
                i += 1;
            }
            ...
            let mut peek = i;
            while peek < effective_length && chars[peek] == ' ' { peek += 1; }
            let is_fn_call = peek < effective_length && chars[peek] == '(';
...
pub(super) fn command_trustlang_showcase() {
    ...
    let show_code_and_run = |buffer: &mut [u32], w: usize, h: usize,
        ...
        let render_editor = |buffer: &mut [u32], w: usize, h: usize,
            ...
            for vi in 0..(lines_vec.len() - scroll_offset.minimum(lines_vec.len())) {
                ...
                if global_index < chars_shown { global_index += 1; } // \n
            }

            // Scrollbar indicator (when content overflows)
            if lines_vec.len() > maximum_visible {
        ...
        while chars_shown < total_chars {
            ...
            // Advance
            chars_shown += 1;
            crate::cpu::tsc::pit_delay_mouse(delay_mouse);
        }
...
pub(super) fn command_trustos_film() {
    let (software, sh) = crate::framebuffer::get_dimensions();
    let w = software as usize;
    let h = sh as usize;
...
unsafe {
    ...
    // -- Scene 2: Question Marks Rain -- "Do you really know?" --
    {
        ...
        let number_qcols = w / 10;
        let mut qy: alloc::vec::Vec<i32> = (0..number_qcols).map(|i| -((i * 37 % 200) as i32)).collect();
        for frame in 0..total_frames {
            if let Some(k) = crate::keyboard::try_read_key() {
                if k == 0x1B { break; }
                if k == b' ' || k == b'\r' || k == b'\n' { break; }
    ...
    {
        let fpc = 2u32;
        let lines_text: [(&str, u32, usize); 5] = [
        ...
        let total_chars: usize = lines_text.iter().map(|(t,_,_)| t.len()).sum();
        ...
        for frame in 0..total_frames {
            ...
            let chars_shown = (frame / fpc) as usize;
            let total_h: usize = lines_text.iter().map(|(_,_,s)| 16 * s + 12).sum();
            let mut y = if total_h < h { (h - total_h) / 2 } else { 20 };
            let mut counted = 0usize;
            for &(text, color, scale) in lines_text.iter() {
                let tw = text.len() * 8 * scale;
                let sx = if tw < w { (w - tw) / 2 } else { 0 };
    ...
    // -- Scene 6: Bar Chart with Earthquake Shake --
    {
        ...
        let maximum_value = 50_000_000u32;
        let bar_maximum_w = w * 3 / 5;
        let bar_h_pixel = 40usize;
        ...
        for frame in 0..total_frames {
            ...
            for (si, &(prefix, suffix, target, color)) in stats.iter().enumerate() {
                ...
                if target > 0 {
                    ...
                    let number_str = if current >= 1000 {
    ...
    // -- Packet journey animation (on circuit bg) --
    {
        ...
        for pass in 0..2u32 {
            ...
            let packet_color = if pass == 0 { 0xFF00FF88 } else { 0xFF44DDFF };
            ...
            for frame in 0..150u32 {
                ...
                let packet_x = 40 + packet_off + stage_w / 2 - 8;
                let packet_y = lane_y + stage_h + 18;
                // Trail glow
                for trail in 1..6u32 {
            ...
            if frame > type_frames / 2 {
                ...
                for ri in 0..number_rings {
                    ...
                    for py in y_start..y_end {
                        ...
                        let dx_maximum_sq = r2_outer - dy2;
                        let dx_minimum_sq = if r2_inner > dy2 { r2_inner - dy2 } else { 0 };
                        // Integer sqrt approximation
                        let mut dx_maximum = 0usize;
                        while (dx_maximum + 1) * (dx_maximum + 1) <= dx_maximum_sq { dx_maximum += 1; }
                        let mut dx_minimum = 0usize;
...
pub(super) fn command_trustview(args: &[&str]) {
    if args.is_empty() {
...
pub(super) fn command_rv_xlat(args: &[&str]) {
...
pub(super) fn command_rv_disasm(args: &[&str]) {
    let path = args.get(0).copied().unwrap_or("");
...
pub(super) fn command_transpile(args: &[&str]) {
    let subcmd = args.get(0).copied().unwrap_or("help");
...
match crate::ramfs::with_filesystem(|fs| fs.ls(Some("/alpine/bin"))) {
        Ok(e) => e,
        Err(_) => {
...
pub(super) fn create_test_binaries_silent() {
    ...
    for (name, code) in &binaries {
        ...
        let path = alloc::format!("/alpine/bin/{}", name);
        if crate::ramfs::with_filesystem(|fs| { let _ = fs.touch(&path); fs.write_file(&path, &elf) }).is_ok() {
            created += 1;
        }
    }
...
fn transpile_run(path: &str, verbose: bool) {
    ...
    // Read the binary
    let data = // Correspondance de motifs — branchement exhaustif de Rust.
match super::network::read_file_bytes(path) {
        Some(d) => d,
        None => {
    ...
    // Analyze it
    let analysis = // Correspondance de motifs — branchement exhaustif de Rust.
...
pub(super) fn command_video(args: &[&str]) {
    let subcmd = args.first().copied().unwrap_or("help");
...
pub(super) fn command_lab(args: &[&str]) {
        // Correspondance de motifs — branchement exhaustif de Rust.
...
pub(super) fn command_hwscan(args: &[&str]) {
