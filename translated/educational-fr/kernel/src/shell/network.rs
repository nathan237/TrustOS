...
/// Web browser command
pub(super) fn command_browse(args: &[&str]) {
    if args.is_empty() {
...
pub(super) fn command_container(args: &[&str]) {
...
match crate::sandbox::container::container_status(id) {
        ...
        "create" => {
            ...
            let config = // Correspondance de motifs — branchement exhaustif de Rust.
match preset_str {
...
pub(super) fn command_sandbox(args: &[&str]) {
    let subcmd = args.first().copied().unwrap_or("help");
        // Correspondance de motifs — branchement exhaustif de Rust.
match subcmd {
        "open" | "navigate" | "go" => {
            ...
            let url = args[1];
            let preset = // Correspondance de motifs — branchement exhaustif de Rust.
...
match crate::sandbox::navigate(id, url) {
        ...
        "status" | "info" => {
            ...
            let id = crate::sandbox::SandboxId(id_number);
                        // Correspondance de motifs — branchement exhaustif de Rust.
match crate::sandbox::status_string(id) {
                Some(s) => crate::println!("{}", s),
                None => crate::println_color!(COLOR_RED, "Sandbox #{} not found", id_number),
            }
        }
...
match crate::sandbox::destroy(id) {
                    Ok(()) => crate::println_color!(COLOR_GREEN, "Sandbox #{} destroyed.", id_number),
                    Err(e) => crate::println_color!(COLOR_RED, "Error: {:?}", e),
                }
            }
        }
...
match filesystem_command {
                    ...
                    "ls" => {
                        let directory = args.get(3).copied().unwrap_or("/");
                                                // Correspondance de motifs — branchement exhaustif de Rust.
match sb.filesystem.list(directory) {
                            Ok(entries) => {
                                crate::println_color!(COLOR_CYAN, "{}:", directory);
...
match sb.filesystem.delete(path) {
                    ...
                    _ => crate::println!("Unknown fs command: {}", filesystem_command),
                }
            } else {
                crate::println_color!(COLOR_RED, "Sandbox #{} not found", id_number);
            }
        ...
        "js" | "eval" => {
            ...
            let mut jss = crate::sandbox::js_sandbox::JsSandbox::new(
                crate::sandbox::SandboxId(id_number),
...
pub(super) fn read_file_content(path: &str) -> Option<String> {
    ...
    if let Ok(data) = crate::ramfs::with_filesystem(|fs| {
