...
unsafe { core::ptr::read_volatile(virt as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32) };
...
match map_bar0_mmio(dev) {
                Some(virt_base) => {
                    ...
                    let hw_rev = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_volatile((virt_base + 0x028) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32) };
...
match hardware_type { 0 => "4965", 2 => "5300", 4 => "5150", 5 => "5100", 7 => "6000", _ => "unknown" };
                    ...
                    let gp = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_volatile((virt_base + 0x024) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32) };
...
match map_bar0_mmio(dev) {
                Some(base) => {
                    ...
                    let gcap = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_volatile(base as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u16) };
                    let oss = (gcap >> 12) & 0xF;
                    let iss = (gcap >> 8) & 0xF;
                    let bss = (gcap >> 3) & 0x1F;
                    ...
                    let gctl = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_volatile((base + 0x08) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32) };
                    if gctl & 1 != 0 {
                    ...
                    let statests = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_volatile((base + 0x0E) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u16) };
                    if statests & 0x7 != 0 {
...
match dev.vendor_id {
            0x1AF4 => "VirtIO-net",
            0x8086 => // Correspondance de motifs — branchement exhaustif de Rust.
match dev.device_id {
...
match Chain::from_str(args[1]) {
            ...
            let idx: usize = // Correspondance de motifs — branchement exhaustif de Rust.
match args[2].parse() {
                Ok(n) => n,
...
match subcmd {
        "start" | "on" => {
            let ip_str = // Correspondance de motifs — branchement exhaustif de Rust.
match args.get(1) {
                Some(s) => s,
                None => {
