...
match lang {
        ...
        Intent::OpenApp => {
            if let Some(e) = entities.iter().find(|e| e.kind == EntityKind::AppName) {
                ...
                let cmd = // Correspondance de motifs — branchement exhaustif de Rust.
match app {
...
fn explain_topic(topic: &str, lang: Lang) -> Action {
    let explanation = // Correspondance de motifs — branchement exhaustif de Rust.
match topic {
        t if matches_any(t, &["rust", "langage"]) => // Correspondance de motifs — branchement exhaustif de Rust.
match lang {
        ...
        t if matches_any(t, &["tls", "ssl", "https", "crypto"]) => // Correspondance de motifs — branchement exhaustif de Rust.
match lang {
        ...
        t if matches_any(t, &["kernel", "noyau"]) => // Correspondance de motifs — branchement exhaustif de Rust.
match lang {
        ...
        t if matches_any(t, &["smp", "multicore", "multi-core"]) => // Correspondance de motifs — branchement exhaustif de Rust.
match lang {
        ...
        t if matches_any(t, &["trustlang", "language", "langage de prog"]) => // Correspondance de motifs — branchement exhaustif de Rust.
match lang {
        ...
        t if matches_any(t, &["browser", "navigateur", "html"]) => // Correspondance de motifs — branchement exhaustif de Rust.
match lang {
        ...
        t if matches_any(t, &["compositor", "gui", "desktop", "bureau"]) => // Correspondance de motifs — branchement exhaustif de Rust.
match lang {
        ...
        t if matches_any(t, &["jarvis", "ia", "ai", "intelligence"]) => // Correspondance de motifs — branchement exhaustif de Rust.
match lang {
        ...
        _ => // Correspondance de motifs — branchement exhaustif de Rust.
match lang {
...
fn howto_topic(topic: &str, lang: Lang) -> Action {
    let howto = // Correspondance de motifs — branchement exhaustif de Rust.
match topic {
        t if matches_any(t, &["compile", "build", "construire"]) => // Correspondance de motifs — branchement exhaustif de Rust.
match lang {
        ...
        t if matches_any(t, &["file", "fichier", "creer"]) => // Correspondance de motifs — branchement exhaustif de Rust.
match lang {
            Lang::Fr => "Pour creer un fichier:\n  touch /mon_fichier.txt\n  echo 'contenu' > /mon_fichier.txt",
            Lang::En => "To create a file:\n  touch /my_file.txt\n  echo 'content' > /my_file.txt",
        },
        t if matches_any(t, &["network", "reseau", "internet"]) => // Correspondance de motifs — branchement exhaustif de Rust.
match lang {
        ...
        t if matches_any(t, &["theme", "personnaliser", "customize"]) => // Correspondance de motifs — branchement exhaustif de Rust.
match lang {
