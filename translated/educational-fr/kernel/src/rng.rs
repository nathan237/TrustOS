...
static RANDOM_GENERATOR_STATE: AtomicU64 = AtomicU64::new(0);
// Variable atomique — accès thread-safe sans verrou.
...
pub fn init() {
    ...
    let rdrand = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
        let ecx: u32;
    ...
    let rdrand = false;
    #[cfg(target_arch = "x86_64")]
    let rdseed = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
        let ebx_out: u32;
...
pub fn next_u64() -> u64 {
    let mut state = RANDOM_GENERATOR_STATE.load(Ordering::Relaxed);
    if state == 0 {
    ...
    RANDOM_GENERATOR_STATE.store(state, Ordering::Relaxed);
    state
}
...
pub fn has_hardware_random_generator() -> bool {
