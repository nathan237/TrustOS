...
unsafe extern "C" fn android_main(dtb_ptr: u64) -> ! {
    ...
    if dtb_ptr != 0 {
        let valid = FdtHeader::validate(dtb_ptr as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8);
        if valid {
            early_uart_print(b"[ANDROID] DTB valid at 0x");
            early_uart_print_hex(dtb_ptr);
            early_uart_print(b"\r\n");
            DTB_INFORMATION = DtbInfo::from_dtb_pointer(dtb_ptr as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8);
        } else {
...
const u8 as u64;
    ...
    if dtb_ptr != 0 {
        if let Some(parsed) = dtb_parser::parse_dtb(dtb_ptr as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8) {
