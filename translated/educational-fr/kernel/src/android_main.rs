...
unsafe extern "C" fn android_main(dtb_pointer: u64) -> ! {
    ...
    if dtb_pointer != 0 {
        let valid = FdtHeader::validate(dtb_pointer as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8);
        if valid {
            early_uart_print(b"[ANDROID] DTB valid at 0x");
            early_uart_print_hex(dtb_pointer);
            early_uart_print(b"\r\n");
            DTB_INFORMATION = DtbInfo::from_dtb_pointer(dtb_pointer as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8);
        } else {
    ...
    let kernel_end = &__kernel_end as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8 as u64;
    ...
    crate::memory::initialize_android_heap(heap_start, heap_size);
    ...
    if dtb_pointer != 0 {
        if let Some(parsed) = dtb_parser::parse_dtb(dtb_pointer as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8) {
            ...
            android_boot::SOC_INFORMATION = soc;
...
match soc {
            ...
            if let Some(ref sfb) = parsed.simplefb {
                ...
                let bpp = // Correspondance de motifs — branchement exhaustif de Rust.
match sfb.format.as_str() {
    ...
    if android_boot::SOC_INFORMATION == SocFamily::Unknown {
        android_boot::SOC_INFORMATION = SocFamily::QemuVirt;
    }
...
match byte {
                b'\r' | b'\n' => {
                    crate::serial_println!("");
                    let cmd = String::from_utf8_lossy(&command_buffer).into_owned();
                    let cmd = cmd.trim();
                    if !cmd.is_empty() {
