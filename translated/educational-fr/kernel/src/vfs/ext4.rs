...
unsafe { core::ptr::read_unaligned(buf.as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Ext4Inode) })
    }
...
unsafe {
            core::slice::from_raw_parts(
                inode.i_block.as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8,
                60, // 15 * 4 bytes
            )
        };
...
unsafe { core::ptr::read_unaligned(data.as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
const Ext4Extent)
            ...
            for i in 0..entries {
                ...
                let idx = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
                    core::ptr::read_unaligned(data[offset..].as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
unsafe { core::ptr::read_unaligned(sb_buffer.as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
