...
const EXT4_FT_REGISTER_FILE: u8 = 1;
// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const EXT4_FT_DIRECTORY: u8 = 2;
// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
// #[derive] — génère automatiquement les implémentations de traits à la compilation.
#[derive(Clone, Copy)]
struct Ext4GroupDescriptorBase {
...
impl Ext4FilesystemInner {
    ...
    fn read_inode(&self, ino: u32) -> Result<Ext4Inode, ()> {
        ...
        Ok(        // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_unaligned(buffer.as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Ext4Inode) })
    }
...
unsafe {
            core::slice::from_raw_parts(
                inode.i_block.as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8,
                60, // 15 * 4 bytes
            )
        };
...
unsafe { core::ptr::read_unaligned(data.as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Ext4ExtentHeader) };
        ...
        if header.eh_depth == 0 {
            ...
            for i in 0..entries {
                ...
                let extent = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
...
const Ext4Extent)
                ...
                if logical_block >= start && logical_block < start + len {
                    ...
                    return Ok(physical_start + block_in_extent);
                }
            }
            Err(()) // Block not found in extents (sparse file hole)
        } else {
            // Internal node: find the right child
            let mut target_index: Option<(usize, u64)> = None;
            
            for i in 0..entries {
                let offset = 12 + i * 12;
                if offset + 12 > data.len() { break; }
                
                let index = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
                    core::ptr::read_unaligned(data[offset..].as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Ext4ExtentIndex)
            ...
            if let Some((_, child_block)) = target_index {
...
match self.extent_lookup(inode, logical_block) {
    ...
    fn read_directory_entries(&self, ino: u32) -> Result<Vec<(u32, String, u8)>, ()> {
        ...
        while offset < file_size {
            ...
            let physical_block = if (inode.i_flags & EXT4_EXTENTS_FL) != 0 {
...
fn directory_entry_file_type(ft: u8) -> FileType {
        // Correspondance de motifs — branchement exhaustif de Rust.
match ft {
        EXT4_FT_REGISTER_FILE => FileType::Regular,
        EXT4_FT_DIRECTORY => FileType::Directory,
    ...
    fn readdir(&self) -> VfsResult<Vec<DirectoryEntry>> {
        ...
        Ok(entries.into_iterator()
            ...
            .map(|(ino, name, ft)| DirectoryEntry {
                name,
                ino: ino as u64,
                file_type: directory_entry_file_type(ft),
            })
            .collect())
    }
...
impl FileSystem for Ext4Filesystem {
    ...
    fn get_directory(&self, ino: Ino) -> VfsResult<Arc<dyn DirectoryOperations>> {
        ...
        Ok(Arc::new(Ext4DirectoryStandalone {
            ino: ino as u32,
            device: self.inner.lock().device.clone(),
            block_size: self.inner.lock().block_size,
            inode_size: self.inner.lock().inode_size,
...
impl Ext4FileStandalone {
    fn make_inner(&self) -> Ext4FilesystemInner {
        Ext4FilesystemInner {
...
struct Ext4DirectoryStandalone {
    ino: u32,
    device: Arc<dyn BlockDevice>,
    block_size: u32,
    inode_size: u16,
...
pub fn mount(device: Arc<dyn BlockDevice>) -> Result<Arc<Ext4Filesystem>, &'static str> {
    // Read superblock (at byte offset 1024)
    let mut sb_buffer = [0u8; 256]; // Only need first 256 bytes of superblock
    ...
    let sb = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_unaligned(sb_buffer.as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Ext4SuperBlock) };
    ...
    let fs = Arc::new(Ext4Filesystem {
        inner: Mutex::new(Ext4FilesystemInner {
            block_size,
            inode_size,
...
pub fn list_directory(fs: &Ext4Filesystem, path: &str) -> Result<Vec<(String, FileType, u64)>, &'static str> {
    ...
    for (entry_ino, name, ft_byte) in entries {
        ...
        let file_type = directory_entry_file_type(ft_byte);
