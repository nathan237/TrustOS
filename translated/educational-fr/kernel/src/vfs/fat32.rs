...
unsafe { core::ptr::read_unaligned(core::ptr::address_of!(self.bytes_per_sector)) };
        ...
        let reserved = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
unsafe { core::ptr::read_unaligned(core::ptr::address_of!(self.reserved_sectors)) } as u64;
        ...
        let fat_size = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
impl Fat32LfnEntry {
    fn get_chars(&self) -> Vec<char> {
        ...
        let name1 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_unaligned(core::ptr::address_of!(self.name1)) };
        let name2 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_unaligned(core::ptr::address_of!(self.name2)) };
        let name3 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const FAT_EOC_MINIMUM: u32 = 0x0FFFFFF8;  // End of cluster chain
...
unsafe { 
            core::ptr::read_unaligned(boot_buffer.as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Fat32BootSector)
    ...
    fn write_fat_entry(&self, cluster: u32, value: u32) -> VfsResult<()> {
        ...
        let new_value = (value & 0x0FFFFFFF) | 
                        (u32::from_le_bytes([sector_buffer[offset_in_sector], 
                                            sector_buffer[offset_in_sector + 1],
                                            sector_buffer[offset_in_sector + 2], 
                                            sector_buffer[offset_in_sector + 3]]) & 0xF0000000);
...
unsafe { core::ptr::read_unaligned(core::ptr::address_of!(self.bpb.fat_size_32)) };
        let total_sectors_32 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_unaligned(core::ptr::address_of!(self.bpb.total_sectors_32)) };
        ...
        for sector_offset in 0..fat_size {
            ...
            for entry_index in 0..entries_per_sector {
                ...
                let offset = (entry_index * 4) as usize;
                ...
                if value == FAT_FREE {
                    ...
                    self.write_fat_entry(cluster, FAT_EOC_MINIMUM)?;
                    return Ok(cluster);
                }
            }
        }
    ...
    fn create_entry(&self, directory_cluster: u32, name: &str, is_directory: bool) -> VfsResult<(u32, u64)> {
        ...
        let new_cluster = if is_directory {
            ...
            let cluster_size = self.bpb.cluster_size();
            let mut directory_data = vec![0u8; cluster_size];
    ...
    fn delete_entry(&self, directory_cluster: u32, name: &str) -> VfsResult<()> {
        ...
        for &cluster in &chain {
            ...
            for i in 0..entries_per_cluster {
                ...
                let entry = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
                    core::ptr::read_unaligned(data[offset..].as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Fat32DirEntry)
                };
    ...
    fn parse_directory(&self, cluster: u32) -> VfsResult<Vec<(String, Fat32DirEntry, usize)>> {
        ...
        while i + entry_size <= data.len() {
            let entry = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
                core::ptr::read_unaligned(data[i..].as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Fat32DirEntry)
            };
            ...
            if entry.is_long_name() {
                // Parse LFN entry
                let lfn = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
                    core::ptr::read_unaligned(data[i..].as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Fat32LfnEntry)
                };
...
impl FileSystem for Fat32Fs {
    ...
    fn get_file(&self, ino: Ino) -> VfsResult<Arc<dyn FileOperations>> {
        ...
        Ok(Arc::new(Fat32File {
            fs: self as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Fat32Fs,
            ino,
...
struct Fat32File {
    fs: *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Fat32Fs,
    ino: Ino,
    cluster: RwLock<u32>,
    size: RwLock<u64>,
...
impl Send for Fat32File {}
// SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe // Implémentation de trait — remplit un contrat comportemental.
impl Sync for Fat32File {}
...
impl FileOperations for Fat32File {
    fn read(&self, offset: u64, buffer: &mut [u8]) -> VfsResult<usize> {
        ...
        let fs = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*self.fs };
...
// SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe // Implémentation de trait — remplit un contrat comportemental.
impl Send for Fat32Directory {}
// SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe // Implémentation de trait — remplit un contrat comportemental.
impl Sync for Fat32Directory {}

// Implémentation de trait — remplit un contrat comportemental.
impl DirectoryOperations for Fat32Directory {
