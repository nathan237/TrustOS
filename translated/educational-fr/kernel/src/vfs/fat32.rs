...
unsafe { 
            core::ptr::read_unaligned(boot_buffer.as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
    ...
    fn delete_entry(&self, dir_cluster: u32, name: &str) -> VfsResult<()> {
        ...
        for &cluster in &chain {
            ...
            for i in 0..entries_per_cluster {
                ...
                let entry = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
                    core::ptr::read_unaligned(data[offset..].as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Fat32DirEntry)
                };
    ...
    fn parse_directory(&self, cluster: u32) -> VfsResult<Vec<(String, Fat32DirEntry, usize)>> {
        ...
        while i + entry_size <= data.len() {
            let entry = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
                core::ptr::read_unaligned(data[i..].as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Fat32DirEntry)
            };
