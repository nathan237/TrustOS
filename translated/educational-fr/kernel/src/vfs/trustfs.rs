...
match (self.mode >> 12) & 0xF {
    ...
    fn set_type(&mut self, ft: FileType) {
        let type_bits = // Correspondance de motifs — branchement exhaustif de Rust.
match ft {
...
impl TrustFilesystemInner {
    ...
    fn read_inode(&self, ino: Ino) -> VfsResult<DiskInode> {
        ...
        let inode_pointer = buffer[offset..].as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DiskInode;
        let inode = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
unsafe { *inode_pointer = *inode; }
    ...
    fn free_inode_blocks(&self, inode: &DiskInode) -> VfsResult<()> {
        ...
        if inode.indirect != 0 {
            ...
            let ptrs = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*(ind_buffer.as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const [u32; INDIRECT_PTRS]) };
        ...
        if inode.double_indirect != 0 {
            ...
            let l1_ptrs = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*(l1_buffer.as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const [u32; INDIRECT_PTRS]) };
            ...
            for &l2_block in l1_ptrs.iter() {
                if l2_block != 0 {
                    ...
                    let l2_ptrs = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*(l2_buffer.as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const [u32; INDIRECT_PTRS]) };
...
unsafe { &mut *(l2_buffer.as_mut_pointer() as *mut [u32; INDIRECT_PTRS]) };
    ...
    fn readdir(&self, directory_ino: Ino) -> VfsResult<Vec<DirectoryEntry>> {
        ...
        for i in 0..number_entries {
            ...
            let entry_pointer = buffer.as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DiskDirectoryEntry;
            let disk_entry = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*entry_pointer };
    ...
    fn create(&self, parent_ino: Ino, name: &str, file_type: FileType) -> VfsResult<Ino> {
        ...
        let entry_bytes = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
            core::slice::from_raw_parts(
                &entry as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DiskDirectoryEntry as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8,
...
unsafe {
                    core::slice::from_raw_parts(
                        &zero_entry as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DiskDirectoryEntry as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8,
                        entry_size
                    )
...
impl TrustFs {
    ...
    pub fn new(backend: Arc<dyn BlockDevice>, capacity: u64) -> VfsResult<Self> {
        ...
        let sb_pointer = buffer.as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Superblock;
        let existing_sb = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { *sb_pointer };
