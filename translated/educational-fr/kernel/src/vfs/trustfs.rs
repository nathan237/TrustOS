...
impl TrustFilesystemInner {
    ...
    fn read_inode(&self, ino: Ino) -> VfsResult<DiskInode> {
        ...
        let inode_pointer = buf[offset..].as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const DiskInode;
...
unsafe { &*(ind_buffer.as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
unsafe { &*(l1_buffer.as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const [u32; INDIRECT_PTRS]) };
...
unsafe { &*(l2_buffer.as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const [u32; INDIRECT_PTRS]) };
...
unsafe { &mut *(l2_buffer.as_mut_ptr() as *mut [u32; INDIRECT_PTRS]) };
    ...
    fn readdir(&self, dir_ino: Ino) -> VfsResult<Vec<DirectoryEntry>> {
        ...
        for i in 0..number_entries {
            ...
            let entry_ptr = buf.as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
impl TrustFs {
    ...
    pub fn new(backend: Arc<dyn BlockDevice>, capacity: u64) -> VfsResult<Self> {
        ...
        let sb_pointer = buf.as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Superblock;
