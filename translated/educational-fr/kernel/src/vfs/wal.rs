...
impl WriteAheadLog {
    ...
    pub fn commit(
        ...
        let hdr = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
pub fn replay_if_needed(
    ...
    let hdr = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*(hdr_buf.as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const WalHeader) };
