...
const PIT_COMMAND: u16 = 0x43;
    ...
    const MAXIMUM_MOUSE_PER_SHOT: u64 = 50;
...
fn calibrate_from_cpuid() -> Option<u64> {
    let cpuid_15 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::arch::x86_64::__cpuid(0x15) };
    ...
    if cpuid_15.eax != 0 && cpuid_15.ebx != 0 {
        let crystal_frequency = if cpuid_15.ecx != 0 {
            cpuid_15.ecx as u64
        } else {
...
unsafe { core::arch::x86_64::__cpuid(0) };
    if cpuid_0.eax >= 0x16 {
        let cpuid_16 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
fn calibrate_against_pit() -> u64 {
    ...
    let start = read_tsc();
    pit_delay_mouse(200); // 200ms wall-clock (I/O polling is wall-clock accurate)
    let end = read_tsc();
