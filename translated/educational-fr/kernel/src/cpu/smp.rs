...
pub fn send_reschedule_ipi(target_cpu: u32) {
    let apic_id = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { PER_CPU[target_cpu as usize].apic_id };
...
static CPU_READY: [AtomicBool; MAXIMUM_CPUS] = {
...
/// Per-CPU data array
static mut PER_CPU: [PerCpuData; MAXIMUM_CPUS] = {
        // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
pub fn current_cpu_id() -> u32 {
    let cpuid = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::arch::x86_64::__cpuid(1) };
...
unsafe { PER_CPU[i].apic_id == apic_id } {
            return i as u32;
        }
    }
    0 // Default to BSP
...
pub fn init() {
    ...
    BSP_ID.store(// SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { PER_CPU[0].apic_id }, Ordering::Release);
    
    crate::serial_println!("[SMP] BSP initialized (APIC ID: {})", // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { PER_CPU[0].apic_id });
}
...
impl SmpInformation {
    ...
    pub fn detect() -> Self {
        ...
        if let Some(acpi_information) = crate::acpi::get_information() {
            let mut ap_ids = Vec::new();
            for lapic in &acpi_information.local_apics {
...
pub fn print_status() {
    ...
    for i in 0..cpu_count().min(MAXIMUM_CPUS as u32) as usize {
        ...
        let work = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { PER_CPU[i].work_completed };
        crate::println!("║ CPU {:2}: {} APIC {:3}  Work: {:8}  ║", 
            i, ready, // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { PER_CPU[i].apic_id }, work);
    }
    crate::println!("╚══════════════════════════════════════╝");
}
...
pub fn get_stats() -> (u32, u32, u64) {
    ...
    for i in 0..total as usize {
        total_work += // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { PER_CPU[i].work_completed };
    }
...
/// Per-CPU work queues
static WORK_QUEUES: [Mutex<WorkItem>; MAXIMUM_CPUS] = {
        // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
/// Signal work pending
static WORK_PENDING: [AtomicBool; MAXIMUM_CPUS] = {
        // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const INIT: AtomicBool = AtomicBool::new(false);
    [INIT; MAXIMUM_CPUS]
};
...
const MAXIMUM_WAIT_SPINS: u32 = 1_000_000;
...
pub fn parallel_for(total_items: usize, func: WorkFn, data: *mut u8) {
    ...
    if dispatched > 0 {
        ...
        while WORK_COMPLETION.load(Ordering::Acquire) < expected {
            ...
            if spin_count > MAXIMUM_WAIT_SPINS {
...
unsafe extern "C" fn ap_entry(smp_info: &limine::smp::Cpu) -> ! {
    ...
    crate::gdt::initialize_ap(processor_id as u32);
