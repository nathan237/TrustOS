...
static CPU_INITIALIZE: AtomicBool = AtomicBool::new(false);
...
impl CpuCapabilities {
    ...
    pub fn detect() -> Self {
        ...
        let cpuid_0 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::arch::x86_64::__cpuid(0) };
        let maximum_basic_leaf = cpuid_0.eax;
        ...
        caps.vendor = // Correspondance de motifs — branchement exhaustif de Rust.
match &vendor_str {
        ...
        if maximum_basic_leaf >= 1 {
            let cpuid_1 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::arch::x86_64::__cpuid(1) };
        ...
        if maximum_basic_leaf >= 7 {
            let cpuid_7 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::arch::x86_64::__cpuid_count(7, 0) };
        ...
        // Extended CPUID
        let cpuid_ext_maximum = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::arch::x86_64::__cpuid(0x80000000) };
        let maximum_ext_leaf = cpuid_ext_maximum.eax;
        
        // CPUID 0x80000001: Extended features
        if maximum_ext_leaf >= 0x80000001 {
            let cpuid_ext1 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::arch::x86_64::__cpuid(0x80000001) };
        ...
        if maximum_ext_leaf >= 0x80000007 {
            let cpuid_ext7 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::arch::x86_64::__cpuid(0x80000007) };
            caps.tsc_invariant = (cpuid_ext7.edx & (1 << 8)) != 0;
        }
        
        // Brand string (CPUID 0x80000002-4)
        if maximum_ext_leaf >= 0x80000004 {
            for i in 0..3 {
                let cpuid_brand = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::arch::x86_64::__cpuid(0x80000002 + i) };
...
pub fn init() {
    if CPU_INITIALIZE.swap(true, Ordering::SeqCst) {
...
pub fn capabilities() -> Option<&'static CpuCapabilities> {
    if !CPU_INITIALIZE.load(Ordering::Relaxed) {
        return None;
    }
