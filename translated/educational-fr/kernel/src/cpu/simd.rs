...
const u8, len: usize) -> bool {
    let mut i = 0;
    while i + 8 <= len {
        let va = (a.add(i) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u64).read_unaligned();
        let vb = (b.add(i) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u64).read_unaligned();
        if va != vb {
            return false;
        }
