...
pub fn spawn_kernel(name: &str, entry: fn(u64) -> i32, argument: u64) -> Tid {
    ...
    let thread = Thread::new_kernel(pid, name, entry as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
pub fn schedule() {
    ...
    let next_tid = // Boucle infinie — tourne jusqu'à un `break` explicite.
loop {
...
fn context_switch(from: Tid, to: Tid) {
    ...
    let to_context_pointer: *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const ThreadContext;
    let to_kernel_stack: u64;
    
    {
        let mut threads = // Correspondance de motifs — branchement exhaustif de Rust.
match THREADS.try_write() {
            Some(t) => t,
            None => return, // lock contended, skip context switch
        };
        
        let from_thread = // Correspondance de motifs — branchement exhaustif de Rust.
match threads.get_mut(&from) {
            Some(t) => t as *mut Thread,
            None => return,
        };
        
        let to_thread = // Correspondance de motifs — branchement exhaustif de Rust.
match threads.get(&to) {
            Some(t) => t,
            None => return,
        };
        
        from_context_pointer = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &mut (*from_thread).context as *mut ThreadContext };
        to_context_pointer = &to_thread.context as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const ThreadContext;
...
#[// SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe(naked)]
extern "C" fn switch_context(from: *mut ThreadContext, to: *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const ThreadContext) {
    core::arch::naked_asm!(
...
#[// SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe(naked)]
extern "C" fn switch_context(_from: *mut ThreadContext, _to: *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const ThreadContext) {
    core::arch::naked_asm!(
