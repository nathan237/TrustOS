...
pub fn load_from_bytes(data: &[u8]) -> ElfResult<LoadedElf> {
    ...
    for i in 0..ph_count {
        ...
        let phdr = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*(data[offset..].as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Elf64Phdr) };
    ...
    if let Some((dyn_off, dyn_sz)) = dynamic_phdr {
        ...
        if dynamic_information.rela_count > 0 && (dynamic_information.rela_offset as usize) < data.len() {
            ...
            for i in 0..dynamic_information.rela_count {
                ...
                let rela = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*(data[off..].as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Elf64Rela) };
...
fn parse_dynamic(data: &[u8], start: usize, end: usize, _base: u64, information: &mut DynamicInfo) {
    ...
    while off + entry_size <= end {
        let dyn_entry = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*(data[off..].as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const Elf64Dyn) };
                // Correspondance de motifs — branchement exhaustif de Rust.
