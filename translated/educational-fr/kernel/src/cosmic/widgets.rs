...
impl Widget for Label {
    fn size(&self) -> Size {
        let char_w = // Correspondance de motifs — branchement exhaustif de Rust.
match self.size {
            LabelSize::Small => 6.0,
        ...
        let height = // Correspondance de motifs — branchement exhaustif de Rust.
match self.size {
...
match self.direction {
    ...
    fn draw(&self, renderer: &mut CosmicRenderer, bounds: Rect) {
        ...
        for child in &self.children {
            let s = child.size();
            let child_bounds = // Correspondance de motifs — branchement exhaustif de Rust.
match self.direction {
                Direction::Vertical => {
                    let b = Rect::new(
