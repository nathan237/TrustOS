...
const u32, count: usize) {
    ...
    while remaining >= 16 {
        let v0 = _mm_loadu_si128(source_pointer as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const __m128i);
        let v1 = _mm_loadu_si128(source_pointer.add(4) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const __m128i);
        let v2 = _mm_loadu_si128(source_pointer.add(8) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const __m128i);
        let v3 = _mm_loadu_si128(source_pointer.add(12) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const __m128i);
    ...
    while remaining >= 4 {
        let v = _mm_loadu_si128(source_pointer as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const __m128i);
...
unsafe fn blend_fill_row_sse2(dst: *mut u32, count: usize, color: u32, alpha: u32) {
    ...
    // Process 4 pixels at a time
    while remaining >= 4 {
        let d = _mm_loadu_si128(ptr as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const __m128i);
...
fn has_avx2() -> bool {
    ...
    let result = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
...
const u32, count: usize) {
    ...
    // Process 32 pixels per unrolled iteration
    while remaining >= 32 {
        let a = _mm256_loadu_si256(s as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const __m256i);
        let b = _mm256_loadu_si256(s.add(8) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const __m256i);
        let c = _mm256_loadu_si256(s.add(16) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const __m256i);
        let e = _mm256_loadu_si256(s.add(24) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const __m256i);
    ...
    // 8 pixels at a time
    while remaining >= 8 {
        _mm256_storeu_si256(d as *mut __m256i, _mm256_loadu_si256(s as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const __m256i));
        d = d.add(8);
        s = s.add(8);
        remaining -= 8;
