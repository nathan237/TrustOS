...
match self.bg_pattern {
    ...
    fn draw_window_controls(&self, x: f32, y: f32, focused: bool, z: f32) {
        ...
        let minimum_color = if focused { 0xFF3A3A30 } else { 0xFF202010 };
        self.draw_circle(x + spacing, y + 8.0, radius, minimum_color, z);
        
        // Maximize button (green)
        let maximum_color = if focused { 0xFF2A3A2F } else { 0xFF10201A };
        self.draw_circle(x + spacing * 2.0, y + 8.0, radius, maximum_color, z);
    }
...
pub fn initialize_compositor(width: u32, height: u32) {
