...
unsafe {
            core::slice::from_raw_parts(
                self as *const Self as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8,
...
unsafe fn validate(ptr: *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8) -> bool {
        if ptr.is_null() {
            return false;
        }
        let magic = u32::from_be((ptr as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u32).read_unaligned());
        magic == FDT_MAGIC
    }

    /// Read header from a physical address
    pub     // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe fn from_pointer(ptr: *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
impl DtbInfo {
    ...
    pub     // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe fn from_dtb_pointer(ptr: *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
pub static mut SOC_INFORMATION: SocFamily = SocFamily::Unknown;

/// Get the detected SoC family
pub fn soc_family() -> SocFamily {
        // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { SOC_INFORMATION }
}
