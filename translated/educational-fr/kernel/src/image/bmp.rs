...
pub fn load_bitmap(path: &str) -> Option<Image> {
...
match crate::vfs::read_file(path) {
    ...
    load_bitmap_from_bytes(&data)
}

/// Load BMP from raw bytes
pub fn load_bitmap_from_bytes(data: &[u8]) -> Option<Image> {
    // Minimum size check
    if data.len() < 54 {
...
pub fn create_bitmap_from_pixels(width: u32, height: u32, pixels: &[u32]) -> Vec<u8> {
