...
pub fn init(usable_regions: &[PhysicalRegion], heap_physical: u64, heap_size: u64) {
    ...
    let minimum_physical = // Correspondance de motifs — branchement exhaustif de Rust.
match usable_regions.iter().map(|r| r.base).min() {
        Some(v) => v,
        None => { crate::serial_println!("[FRAME] BUG: no min in non-empty regions"); return; }
    };
    let maximum_physical = // Correspondance de motifs — branchement exhaustif de Rust.
...
unsafe { core::slice::from_raw_parts((phys + hhdm) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8, 4096) };
