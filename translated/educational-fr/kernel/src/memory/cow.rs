...
unsafe {
            core::ptr::copy_nonoverlapping(
                (old_physical + hhdm) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8,
...
pub fn clone_cow(parent_cr3: u64) -> Option<AddressSpace> {
    ...
    let parent_pml4 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*((parent_cr3 + hhdm) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PageTable) };
