...
impl AddressSpace {
    ...
    pub fn new() -> Option<Self> {
        ...
        let pml4_virt = &*pml4 as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
unsafe {
        ...
        let current_pml4 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { 
            &*(current_pml4_virt as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PageTable) 
        };
        
        // Get our PML4
        let our_pml4_virt = self.pml4_physical + self.hhdm_offset;
        let our_pml4 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { 
    ...
    pub fn map_page(&mut self, virt: u64, physical: u64, flags: PageFlags) -> Option<()> {
        ...
        // Get or create PDPT
        let pdpt_physical = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { self.ensure_table_at(&mut (*pml4).entries[pml4_index])? };
        let pdpt = (pdpt_physical + self.hhdm_offset) as *mut PageTable;
        
        // Get or create PD
        let pd_physical = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { self.ensure_table_at(&mut (*pdpt).entries[pdpt_index])? };
        let pd = (pd_physical + self.hhdm_offset) as *mut PageTable;
        
        // Get or create PT
        let pt_physical = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
unsafe {
    ...
    fn ensure_table_at(&mut self, entry: &mut PageTableEntry) -> Option<u64> {
        if entry.is_present() {
            ...
            let table_virt = &*new_table as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PageTable as u64;
...
unsafe {
    ...
    pub fn translate(&self, virt: u64) -> Option<u64> {
        ...
        let pml4 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*((self.pml4_physical + self.hhdm_offset) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PageTable) };
        if !pml4.entries[pml4_index].is_present() { return None; }
        
        let pdpt = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*((pml4.entries[pml4_index].physical_address() + self.hhdm_offset) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PageTable) };
        if !pdpt.entries[pdpt_index].is_present() { return None; }
        
        let pd = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*((pdpt.entries[pdpt_index].physical_address() + self.hhdm_offset) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PageTable) };
        if !pd.entries[pd_index].is_present() { return None; }
        
        let pt = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*((pd.entries[pd_index].physical_address() + self.hhdm_offset) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PageTable) };
...
unsafe fn activate(&self) {
    ...
    pub fn is_accessible(&self, virt: u64, required_flags: PageFlags) -> bool {
        ...
        let pml4_virt = self.pml4_physical + self.hhdm_offset;
        let pml4 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*(pml4_virt as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PageTable) };
        ...
        let pdpt_virt = pml4.entries[pml4_index].physical_address() + self.hhdm_offset;
        let pdpt = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*(pdpt_virt as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PageTable) };
        ...
        let pd_virt = pdpt.entries[pdpt_index].physical_address() + self.hhdm_offset;
        let pd = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*(pd_virt as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PageTable) };
        ...
        let pt_virt = pd.entries[pd_index].physical_address() + self.hhdm_offset;
        let pt = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*(pt_virt as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PageTable) };
...
impl AddressSpace {
    ...
    pub fn release_user_frames(&self) -> usize {
        ...
        let pml4 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*((self.pml4_physical + hhdm) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PageTable) };
...
const PageTable) };
                ...
                for pd_index in 0..ENTRIES_PER_TABLE {
                    ...
                    let pt = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*((pd.entries[pd_index].physical_address() + hhdm) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PageTable) };
