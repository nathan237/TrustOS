...
unsafe { &*((pd.entries[pd_index].phys_addr() + hhdm) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PageTable) };
...
unsafe fn activate(&self) {
    ...
    pub fn is_accessible(&self, virt: u64, required_flags: PageFlags) -> bool {
        let entry = // Correspondance de motifs — branchement exhaustif de Rust.
...
impl AddressSpace {
    ...
    pub fn release_user_frames(&self) -> usize {
        ...
        let pml4 = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*((self.pml4_phys + hhdm) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PageTable) };
