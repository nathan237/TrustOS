...
const MAXIMUM_SWAP_SLOTS: usize = 65536;
...
pub fn handle_swap_fault(cr3: u64, fault_address: u64) -> bool {
    let virt_page = fault_address & !0xFFF;
    let key = (cr3, virt_page);
    
    let mut state = SWAP.lock();
    let slot = // Correspondance de motifs — branchement exhaustif de Rust.
match state.swap_map.get(&key) {
        Some(&s) => s,
        None => return false,
    };
    ...
    let new_physical = // Correspondance de motifs — branchement exhaustif de Rust.
match crate::memory::frame::allocator_frame_zeroed() {
    ...
    read_swap_slot(&state, slot, new_physical);
...
fn write_swap_slot(_state: &SwapState, slot: SwapSlot, physical_address: u64) {
    ...
    let source = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::slice::from_raw_parts((physical_address + hhdm) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
fn read_swap_slot(_state: &SwapState, slot: SwapSlot, physical_address: u64) {
    let hhdm = crate::memory::hhdm_offset();
    let destination = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
unsafe { &mut *((pt_physical + hhdm) as *mut [u64; 512]) };
    ...
    pt[pt_index] = ((slot as u64) << 1) | (1u64 << 62);
