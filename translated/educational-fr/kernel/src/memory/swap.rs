...
fn write_swap_slot(_state: &SwapState, slot: SwapSlot, phys_addr: u64) {
    ...
    let src = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::slice::from_raw_parts((phys_addr + hhdm) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
