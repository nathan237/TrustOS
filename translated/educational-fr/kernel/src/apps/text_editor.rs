            ...
            c if c >= 0x20 && c < 0x7F => {
                ...
                if self.cursor_line < self.lines.len() {
                    ...
                    if self.cursor_column <= self.lines[self.cursor_line].len() {
                        ...
                        // Auto-close brackets
                        let close = // Correspondance de motifs — branchement exhaustif de Rust.
match character {
                            '{' => Some('}'),
                            '(' => Some(')'),
                            '[' => Some(']'),
    ...
    fn toggle_comment(&mut self) {
        self.save_undo_state();
        let comment_prefix = // Correspondance de motifs — branchement exhaustif de Rust.
match self.language {
    ...
    pub fn update_matching_bracket(&mut self) {
        ...
        let (target, forward) = // Correspondance de motifs — branchement exhaustif de Rust.
match character {
            b'(' => (b')', true),
            b')' => (b'(', false),
            b'{' => (b'}', true),
...
pub fn render_editor(
    ...
    let lang_prefix = // Correspondance de motifs — branchement exhaustif de Rust.
