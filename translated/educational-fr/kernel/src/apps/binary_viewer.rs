...
match key {
            // Tab: cycle panels
            '\t' => {
                self.active_panel = // Correspondance de motifs — branchement exhaustif de Rust.
...
pub fn draw_binary_viewer(
    ...
    let headers = [
        ...
        (information_x, information_w, "Info / Xrefs", ActivePanel::Info),
    ];
    ...
    for (px, pw, label, panel) in &headers {
        ...
        draw_text_fn(*px + 4, content_y + 3, label, COLUMN_HEADER);
    }

    // ── Panel Separators ──
    for px in &[hex_x, disasm_x, information_x] {
        crate::framebuffer::fill_rect(*px as u32, (content_y + header_h) as u32, 1, content_h as u32, COLUMN_SEPARATOR);
    }
    ...
    // ── Info Panel ──
    draw_info_panel(state, information_x + 2, panel_y, information_w - 4, visible_lines, line_h, draw_text_fn);
    ...
    let address_str = format!("0x{:016X} ", state.current_addr);
...
match item {
        ...
        let maximum_chars = (w / 8).max(4) as usize;
...
fn draw_hex_panel(
    ...
    for line in 0..visible {
        ...
        for &b in chunk {
            ...
            let col = if b >= 0x20 && b < 0x7F { COLUMN_ASCII } else { 0xFF484F58 };
...
fn draw_disasm_panel(
    ...
    for (vi, idx) in (start..end).enumerate() {
        ...
        let maximum_operation_chars = ((w - (cx - x)) / 8).max(1) as usize;
