...
#[derive(Debug, Clone)]
// Structure publique — visible à l'extérieur de ce module.
pub struct LocalApicNmiInformation {
    /// ACPI Processor UID (0xFF = all processors)
...
/// Parse MADT table
pub fn parse(madt_virt: u64) -> Option<(u64, Vec<LocalApic>, Vec<IoApic>, Vec<IntSourceOverride>, Vec<LocalApicNmiInformation>)> {
    let header = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { &*(madt_virt as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const SdtHeader) };
    ...
    let madt_header = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { 
        &*((madt_virt + madt_header_offset as u64) as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MadtHeader) 
    };
    
    let mut local_apic_address = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { 
...
unsafe { &*(offset as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const MadtEntryHeader) };
...
unsafe { core::ptr::read_unaligned(core::ptr::address_of!(entry.io_apic_address)) };
                    let gsi_base = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
const IntSourceOverrideEntry) };
                    let gsi = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
const LocalApicAddrOverrideEntry) };
                    local_apic_address = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { 
...
unsafe { &*(offset as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const X2ApicEntry) };
                    let x2apic_id = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_unaligned(core::ptr::address_of!(entry.x2apic_id)) };
                    let flags = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_unaligned(core::ptr::address_of!(entry.flags)) };
                    let uid = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
