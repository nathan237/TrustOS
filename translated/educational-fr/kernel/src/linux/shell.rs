...
match command {
        ...
        "mkdir" => command_mkdir(args),
        "touch" => command_touch(args),
        "rm" => command_rm(args),
        "cp" => command_cp(args),
        "mv" => command_mv(args),
        "clear" => command_clear(),
        "uname" => command_uname(args),
        "whoami" => crate::println!("root"),
        "id" => crate::println!("uid=0(root) gid=0(root) groups=0(root)"),
        "hostname" => command_hostname(args),
        "date" => command_date(),
        "uptime" => command_uptime(),
        "free" => command_free(args),
        "df" => command_df(args),
        "ps" => command_ps(args),
        "top" => command_top(),
        "dmesg" => command_dmesg(),
        "mount" => command_mount(),
        "env" | "printenv" => command_env(),
        "export" => command_export(args),
        "head" => command_head(args),
        "tail" => command_tail(args),
        "wc" => command_wc(args),
        "grep" => command_grep(args),
        "find" => command_find(args),
        "which" => command_which(args),
        ...
        "apk" => command_apk(args),
        "wget" | "curl" => command_wget(args),
        "ping" => command_ping(args),
        "ifconfig" | "ip" => command_ifconfig(args),
        "netstat" | "ss" => command_netstat(),
...
fn command_ls(args: &[&str]) {
    ...
    let linux_path = if target.starts_with('/') {
        ...
        // Correspondance de motifs — branchement exhaustif de Rust.
match rootfs::list_directory(&linux_path.replace("/linux", "").replace("//", "/")) {
        Ok(entries) => {
            if long_format {
...
fn command_export(args: &[&str]) {
    if args.is_empty() {
        command_env();
    } else {
        // Just acknowledge, we don't actually store env vars yet
        for argument in args {
            crate::println!("export {}", argument);
        }
    }
}

fn command_head(args: &[&str]) {
    let lines = 10;
...
fn command_test(_args: &[&str]) {
    // Basic test command, always succeeds for now
}

fn command_apk(args: &[&str]) {
    if args.is_empty() {
