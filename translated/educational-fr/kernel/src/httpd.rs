...
fn page_files(path: &str) -> String {
    let filesystem_path = if path == "/files" || path == "/files/" {
        "/"
    } else {
        &path[6..] // strip "/files"
    };
    ...
    let entries = crate::ramfs::with_filesystem(|fs| {
        fs.ls(Some(filesystem_path)).unwrap_or_default()
    });

    let mut rows = String::new();
    for (name, ftype, size) in &entries {
        let icon = if *ftype == crate::ramfs::FileType::Directory { "d" } else { "-" };
        let link = format!("/files{}{}{}", filesystem_path,
            if filesystem_path.ends_with('/') { "" } else { "/" }, name);
...
<table>{}</table>
</body></html>"#, filesystem_path, filesystem_path, rows);
