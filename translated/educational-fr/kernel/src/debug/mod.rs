...
const MAXIMUM_CHECKPOINTS: usize = 32;

/// Boot checkpoint log (fixed array + count — no heap needed)
struct CheckpointLog {
    entries: [(u64, u8, &'static str); MAXIMUM_CHECKPOINTS],
    count: usize,
}
...
impl CheckpointLog {
    const fn new() -> Self {
        Self {
            entries: [(0, 0, ""); MAXIMUM_CHECKPOINTS],
            count: 0,
        }
    }
    fn push(&mut self, entry: (u64, u8, &'static str)) {
        if self.count < MAXIMUM_CHECKPOINTS {
...
const POST_SERIAL_INITIALIZE:     u8 = 0x10;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POST_FRAMEBUFFER:     u8 = 0x11;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POST_GDT:             u8 = 0x20;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POST_IDT:             u8 = 0x21;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POST_CPU_DETECT:      u8 = 0x22;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POST_ACPI:            u8 = 0x30;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POST_APIC:            u8 = 0x31;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POST_SMP:             u8 = 0x32;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POST_MEMORY:          u8 = 0x40;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POST_HEAP:            u8 = 0x41;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POST_PAGING:          u8 = 0x42;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POST_PCI:             u8 = 0x50;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POST_DISK:            u8 = 0x51;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POST_NETWORK:         u8 = 0x60;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POST_VFS:             u8 = 0x70;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POST_PROCESS:         u8 = 0x80;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POST_SHELL_READY:     u8 = 0xAA;
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const POST_PANIC:           u8 = 0xFF;
...
pub fn beep_warn()  { beep(500, 200); busy_wait_mouse(100); beep(500, 200); } // Double low = warning
...
unsafe {
    ...
    for _ in 0..maximum_frames {
        ...
        let frame_pointer = rbp as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u64;
        
        // Read return address (at rbp+8) and previous rbp (at rbp)
        let return_value_address = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_volatile(frame_pointer.add(1)) };
        let previous_rbp = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::ptr::read_volatile(frame_pointer) };
        
        if return_value_address == 0 {
            break;
        }
        
        frames.push((return_value_address, rbp));
...
pub fn format_backtrace(maximum_frames: usize) -> Vec<String> {
    ...
    if frames.is_empty() {
        ...
        for (i, (return_value_address, rbp)) in frames.iter().enumerate() {
            lines.push(format!("  #{:>2}: 0x{:016x}  (rbp=0x{:016x})", i, return_value_address, rbp));
        }
    }
...
unsafe { core::arch::asm!("mov {}, rsp", out(reg) rsp); }
        ...
        let stack_pointer = rsp as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8;
        for row in 0..16 {
            let offset = row * 16;
