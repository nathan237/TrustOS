...
loop {
        {
            let mut reg = REGISTRY.write();
            let &(pipe_id, is_write) = // Correspondance de motifs — branchement exhaustif de Rust.
match reg.fd_map.get(&fd) {
            ...
            let pipe = // Correspondance de motifs — branchement exhaustif de Rust.
match reg.pipes.get_mut(&pipe_id) {
                Some(p) => p,
                None => return -9,
            };
            if !pipe.read_open {
...
match reg.fd_map.get(&fd) {
    ...
    let pipe = // Correspondance de motifs — branchement exhaustif de Rust.
match reg.pipes.get(&pipe_id) {
        Some(p) => p,
        None => return (false, false, false),
    };
    if is_write {
        // Write end: writable if buffer has space, hangup if read end closed
        let has_space = pipe.data.len() < PIPE_BUFFER_SIZE;
