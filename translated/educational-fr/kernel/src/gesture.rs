...
use crate::touch::{self, TouchEvent, TouchPhase, TouchPoint, MAXIMUM_TOUCH_POINTS};
...
const TAP_MAXIMUM_DURATION_US: u64 = 200_000;

/// Maximum displacement for a tap (10 pixels)
const TAP_MAXIMUM_DISPLACEMENT: i32 = 10;
...
const DOUBLE_TAP_MAXIMUM_DISTANCE: i32 = 30;
...
pub struct GestureRecognizer {
    ...
    fingers: [FingerTracker; MAXIMUM_TOUCH_POINTS],
...
impl GestureRecognizer {
    ...
    pub const fn new(screen_width: i32, screen_height: i32) -> Self {
        Self {
            ...
            fingers: [FingerTracker::new(); MAXIMUM_TOUCH_POINTS],
            finger_count: 0,
            screen_width,
            screen_height,
            last_tap_x: 0,
...
match point.phase {
    ...
    pub fn check_long_press(&mut self) -> Option<GestureEvent> {
            ...
            && !self.long_press_fired
        {
            ...
            let (fx, fy, start_t, maximum_display) = // Correspondance de motifs — branchement exhaustif de Rust.
...
match self.state {
    ...
    fn on_finger_up(&mut self, point: &TouchPoint) -> Option<GestureEvent> {
        ...
        let slot = // Correspondance de motifs — branchement exhaustif de Rust.
match slot {
            Some(s) => s,
            None => {
                self.reset();
        ...
        let finger = self.fingers[slot];
        let gesture = // Correspondance de motifs — branchement exhaustif de Rust.
match self.state {
            RecogState::Tracking if self.finger_count == 1 => {
    ...
    fn classify_single_finger_lift(&mut self, finger: &FingerTracker, now_us: u64) -> Option<GestureEvent> {
        ...
        if duration < TAP_MAXIMUM_DURATION_US && displacement < TAP_MAXIMUM_DISPLACEMENT {
            ...
            if since_last_tap < DOUBLE_TAP_WINDOW_US && tap_dist < DOUBLE_TAP_MAXIMUM_DISTANCE {
...
// Implémentation de trait — remplit un contrat comportemental.
impl<'a> Iterator for GestureIterator<'a> {
