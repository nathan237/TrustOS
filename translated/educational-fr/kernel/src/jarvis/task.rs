...
match rng.next() % 3 {
    ...
    let (a, b, expr, result) = // Correspondance de motifs — branchement exhaustif de Rust.
match op {
        MathOp::Add => {
...
pub fn handle_task_execute(payload: &[u8]) -> (super::rpc::Status, Vec<u8>) {
    let expr = // Correspondance de motifs — branchement exhaustif de Rust.
match parse_task_payload(payload) {
    ...
    let answer = // Correspondance de motifs — branchement exhaustif de Rust.
