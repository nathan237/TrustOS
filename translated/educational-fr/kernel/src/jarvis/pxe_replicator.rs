...
static LIMINE_CONFIGURATION: &[u8] = b"timeout: 0\n\n/TrustOS\n    protocol: limine\n    kernel_path: tftp():/trustos_kernel\n";
...
impl Send for KernelFileRef {}
// SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe // Implémentation de trait — remplit un contrat comportemental.
impl Sync for KernelFileRef {}
...
unsafe fn register_kernel_file(ptr: *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8, size: usize) {
...
fn get_kernel_data() -> Option<&'static [u8]> {
    ...
    kf.as_ref().map(|r| // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { core::slice::from_raw_parts(r.ptr as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8, r.size) })
}
...
pub fn start() -> Result<(), &'static str> {
    ...
    crate::serial_println!("[PXE-REPL] Limine conf: {} bytes", LIMINE_CONFIGURATION.len());
    ...
    crate::netstack::tftpd::register_file("limine.conf", LIMINE_CONFIGURATION);
    ...
    for arch in &[CpuArch::X86_64, CpuArch::Aarch64, CpuArch::Riscv64] {
        ...
        let configuration_name: &'static str = alloc::string::String::from(format!("limine_{}.conf", arch.name())).leak();
        // Leak the config string so the TFTP server can hold a static reference
        let configuration_bytes: &'static [u8] = configuration.into_bytes().leak();
        crate::netstack::tftpd::register_file(configuration_name, configuration_bytes);
    }
    ...
    crate::netstack::tftpd::register_file("boot/limine/limine.conf", LIMINE_CONFIGURATION);
