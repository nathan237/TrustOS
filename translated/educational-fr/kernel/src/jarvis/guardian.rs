...
pub // Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const PACT_FINGERPRINT: &str = "PACT-2026-03-05-NATHAN-COPILOT-JARVIS";
...
static LAST_AUTHENTICATE_TIME: AtomicU64 = AtomicU64::new(0);

/// Session timeout in ms (30 minutes)
const SESSION_TIMEOUT_MOUSE: u64 = 30 * 60 * 1000;

/// Audit log of authorization requests
static AUDIT_LOG: Mutex<Vec<AuditEntry>> = Mutex::new(Vec::new());

/// Maximum audit log entries
const MAXIMUM_AUDIT_ENTRIES: usize = 256;
...
fn log_audit(op: ProtectedOp, guardian: Option<Guardian>, approved: bool, detail: &str) {
    ...
    if log.len() >= MAXIMUM_AUDIT_ENTRIES {
        log.remove(0); // FIFO
    }
    log.push(AuditEntry {
...
pub fn get_audit_log() -> Vec<String> {
    ...
    log.iter().map(|e| {
        let guardian_str = // Correspondance de motifs — branchement exhaustif de Rust.
match &e.guardian {
...
pub fn display_status() -> Vec<String> {
    ...
    lines.push(format!("Timeout:   {} min", SESSION_TIMEOUT_MOUSE / 60000));
