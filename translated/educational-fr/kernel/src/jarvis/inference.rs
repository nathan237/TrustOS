...
pub fn new() -> Self {
        KVCache {
            k: (0..N_LAYERS).map(|_| Vec::with_capacity(MAXIMUM_SEQUENCE * D_MODEL)).collect(),
            v: (0..N_LAYERS).map(|_| Vec::with_capacity(MAXIMUM_SEQUENCE * D_MODEL)).collect(),
            len: 0,
        }
    }
...
pub fn new() -> Self {
    ...
    pub fn predict_next_token(&mut self, model: &TransformerWeights, context: &[u8]) -> u8 {
        self.kv_cache.clear();
        for &token in context.iter().take(MAXIMUM_SEQUENCE) {
            self.forward_one(model, token);
        }
        // Greedy: take argmax
    ...
    fn sample_token_with_penalty(&mut self, recent_tokens: &[u8]) -> u8 {
        let temporary = self.config.temperature;
