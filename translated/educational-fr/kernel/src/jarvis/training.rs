...
//! simple but functional for our 4.4M parameter model.
//!
//! Training modes:
...
//! - Our model is compact (4.4M params vs. billions)
...
const MAXIMUM_TRAIN_SEQUENCE: usize = 64;

/// Fraction of parameters to update per step (1.0 = all, 0.01 = 1%)
const PARAMETER_SAMPLE_RATE: f32 = 0.01;
...
pub fn train_step(model: &mut TransformerWeights, tokens: &[u8], learning_rate: f32) -> f32 {
    let tokens = if tokens.len() > MAXIMUM_TRAIN_SEQUENCE { &tokens[..MAXIMUM_TRAIN_SEQUENCE] } else { tokens };
...
match component {
        ...
        1 => {
            let layer_index = (step / 6) as usize % N_LAYERS;
            train_weight_slice(model, WeightTarget::LayerWq(layer_index), tokens, learning_rate);
        }
        2 => {
            let layer_index = (step / 6) as usize % N_LAYERS;
            train_weight_slice(model, WeightTarget::LayerWk(layer_index), tokens, learning_rate);
        }
        3 => {
            let layer_index = (step / 6) as usize % N_LAYERS;
            train_weight_slice(model, WeightTarget::LayerWo(layer_index), tokens, learning_rate);
        }
        4 => {
            let layer_index = (step / 6) as usize % N_LAYERS;
            train_weight_slice(model, WeightTarget::LayerWgate(layer_index), tokens, learning_rate);
        }
        5 => {
...
fn train_weight_slice(model: &mut TransformerWeights, target: WeightTarget, tokens: &[u8], lr: f32) {
    ...
    let (ptr, n) = // Correspondance de motifs — branchement exhaustif de Rust.
match target {
