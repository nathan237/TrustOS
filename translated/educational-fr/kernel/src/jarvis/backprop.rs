...
//!   4 layers, d_model=256, n_heads=4, d_ff=1024, vocab=256, max_seq=256
//!
...
pub fn forward_backward(model: &TransformerWeights, tokens: &[u8]) -> (f32, ModelGrads) {
    ...
    let mut all_acts: Vec<PositionActivations> = Vec::with_capacity(sequence_length);
    ...
    for t in 0..sequence_length {
        ...
        for l in 0..N_LAYERS {
            ...
            for h in 0..N_HEADS {
                let ho = h * D_K;
                let mut scores = vec![0.0f32; n_position];
                for p in 0..n_position {
                    let mut s = 0.0f32;
                    for d in 0..D_K {
        ...
        all_acts.push(PositionActivations {
            x: x.clone(),
    ...
    for t in 0..n_targets {
        ...
        for l in (0..N_LAYERS).rev() {
            ...
            for h in 0..N_HEADS {
                ...
                let mut d_wts = vec![0.0f32; n_position];
                for p in 0..n_position {
                    let mut s = 0.0f32;
                    for d in 0..D_K {
                ...
                let dot: f32 = (0..n_position).map(|p| d_wts[p] * wts[p]).sum();
                let mut d_scores = vec![0.0f32; n_position];
                for p in 0..n_position {
    ...
    let average_loss = total_loss / n_targets as f32;
    (average_loss, grads)
}
