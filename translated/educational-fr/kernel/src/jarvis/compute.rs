...
static GPU_OPERATIONS: AtomicU64 = AtomicU64::new(0);
// Variable atomique — accès thread-safe sans verrou.
static CPU_OPERATIONS: AtomicU64 = AtomicU64::new(0);
// Variable atomique — accès thread-safe sans verrou.
...
match active_backend() {
        ...
        Backend::CpuSimd => {
            ...
            CPU_OPERATIONS.fetch_add(1, Ordering::Relaxed);
        }
    }
}
...
pub fn summary() -> String {
    ...
    let gpu_operations = GPU_OPERATIONS.load(Ordering::Relaxed);
    let cpu_operations = CPU_OPERATIONS.load(Ordering::Relaxed);
    alloc::format!("Backend: {}, GPU ops: {}, CPU ops: {}", backend, gpu_operations, cpu_operations)
}
...
pub fn info_lines() -> Vec<String> {
    ...
    lines.push(alloc::format!("  GPU ops:  {}", GPU_OPERATIONS.load(Ordering::Relaxed)));
