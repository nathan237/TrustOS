...
pub fn init() {
    ...
    if MICRO_BIN.len() >= 4 && MICRO_BIN.len() % 4 == 0 {
        ...
        let floats: &[f32] = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
            core::slice::from_raw_parts(MICRO_BIN.as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
match load_full_brain() {
            Ok(bytes) => {
                ...
                let _ = crate::ramfs::with_filesystem(|fs| fs.rm(WEIGHTS_PATH));
...
pub fn load_full_brain() -> Result<usize, &'static str> {
    ...
    let floats: &[f32] = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
        core::slice::from_raw_parts(data.as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const f32, float_count)
    };
...
pub fn cache_brain_to_ramfs() -> Result<usize, &'static str> {
    ...
    let bytes: &[u8] = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
        core::slice::from_raw_parts(floats.as_pointer() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8, byte_count)
    };
    crate::ramfs::with_filesystem(|fs| {
        let _ = fs.mkdir(WEIGHTS_DIRECTORY);
    });
    crate::ramfs::with_filesystem(|fs| {
...
pub fn generate(prompt: &str, maximum_tokens: usize) -> String {
    if !is_ready() {
...
match model_guard.as_ref() {
            Some(m) => m,
            None => return micro_generate(prompt, maximum_tokens),
        };

        let mut engine_guard = ENGINE.lock();
        let engine = // Correspondance de motifs — branchement exhaustif de Rust.
match engine_guard.as_mut() {
        ...
        let generated = engine.generate(model, &tokens, maximum_tokens);
        GENERATION_COUNT.fetch_add(1, Ordering::Relaxed);
        return tokenizer::decode(&generated);
    }

    // Fallback: micro sentinel
    micro_generate(prompt, maximum_tokens)
}

/// Generate using the micro sentinel (small model, always available)
fn micro_generate(prompt: &str, maximum_tokens: usize) -> String {
    let micro_guard = MICRO_MODEL.lock();
    let micro = // Correspondance de motifs — branchement exhaustif de Rust.
match micro_guard.as_ref() {
        Some(m) => m,
...
pub fn propagation_status() -> Vec<String> {
    ...
    lines.push(format!("║ Fed Rounds:  {}  Interval: {}ms{}",
        ...
        federated::current_interval_mouse(),
        "              ║"));
...
const WEIGHTS_DIRECTORY: &str = "/jarvis";
...
const u8, byte_count)
    ...
    crate::ramfs::with_filesystem(|fs| {
        let _ = fs.mkdir(WEIGHTS_DIRECTORY); // Ignore AlreadyExists
    });
...
pub fn has_saved_weights() -> bool {
    crate::ramfs::with_filesystem(|fs| fs.exists(WEIGHTS_PATH))
}
...
pub fn pretrain(epochs: usize, lr: f32) -> (usize, f32, u64) {
    ...
    let lr_minimum = lr * 0.1; // decay to 10% of peak LR
    let mut step = 0u64;
    ...
    let hardware_sequences = if crate::jarvis_hw::probe::is_scanned() {
    ...
    if let (Some(adam), Some(model)) = (opt_guard.as_mut(), model_guard.as_mut()) {
        for epoch in 0..epochs {
            for (phase_index, phase) in corpus::CORPUS.iter().enumerate() {
                ...
                for &text in *phase {
                    ...
                    let current_lr = optimizer::cosine_lr(
                        step, total_steps as u64, warmup_steps, lr, lr_minimum
                    );
                    adam.lr = current_lr;
            ...
            if !hardware_sequences.is_empty() {
                let mut hardware_loss = 0.0f32;
                let mut hardware_count = 0u32;
                let mut accum_grads = backprop::ModelGrads::new();
                let mut accum_count = 0usize;

                for text in &hardware_sequences {
                    let tokens = tokenizer::encode(text);
                    if tokens.len() < 2 { step += 1; continue; }
                ...
                let average = if hardware_count > 0 { hardware_loss / hardware_count as f32 } else { 0.0 };
                crate::serial_println!("[JARVIS] Epoch {}/{} Phase HW (Hardware Context) — {} seqs, avg loss={:.3}",
                    epoch + 1, epochs, hardware_count, average);
            }
        }
    } else {
    ...
    let average_loss = if loss_count > 0 { total_loss / loss_count as f32 } else { f32::MAX };

    crate::serial_println!("[JARVIS] Pre-training done: {} steps, avg loss={:.3}, {} ms",
        step, average_loss, elapsed);

    (step as usize, average_loss, elapsed)
}
...
pub fn pretrain_phase(phase: usize, epochs: usize, lr: f32) -> (usize, f32, u64) {
    if !is_ready() { init(); }
    if !is_ready() { return (0, f32::MAX, 0); }
    if phase >= corpus::number_phases() { return (0, f32::MAX, 0); }
    ...
    for epoch in 0..epochs {
        for &text in sequences {
            let current_lr = optimizer::cosine_lr(step, total_seqs as u64, warmup, lr, lr_minimum);
        ...
        if epochs > 1 {
            let average = if loss_count > 0 { total_loss / loss_count as f32 } else { 0.0 };
            crate::serial_println!("[JARVIS]   Epoch {}/{}: avg loss={:.3}", epoch + 1, epochs, average);
        }
    }
    ...
    (total_steps, average_loss, elapsed)
}
...
match model_guard.as_ref() {
    ...
    let average = if count > 0 { total_loss / count as f32 } else { f32::MAX };
    average
}
...
pub fn mesh_status() -> String {
    ...
    let mesh_information = mesh::status_summary();
    let consensus_information = consensus::status();
    let fed_information = federated::stats();
    let (rpc_served, rpc_made, rpc_running) = rpc::get_stats();

    format!("{}\nConsensus: {}\nFederated: {}\nRPC: served={} made={} running={}",
        mesh_information, consensus_information, fed_information,
