...
unsafe {
            core::slice::from_raw_parts(MICRO_BIN.as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
...
pub fn load_full_brain() -> Result<usize, &'static str> {
    ...
    let floats: &[f32] = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
        core::slice::from_raw_parts(data.as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const f32, float_count)
    };
...
unsafe {
        core::slice::from_raw_parts(floats.as_ptr() as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8, byte_count)
    };
