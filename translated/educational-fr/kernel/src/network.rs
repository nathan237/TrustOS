...
const MAXIMUM_PACKET_SIZE: usize = 1518;
...
/// Transmit queue
static TRANSMIT_QUEUE: Mutex<VecDeque<Vec<u8>>> = Mutex::new(VecDeque::new());
...
/// Detected NIC info
static NIC_INFORMATION: Mutex<Option<NicInformation>> = Mutex::new(None);
...
pub fn get_nic_information() -> Option<NicInformation> {
    NIC_INFORMATION.lock().clone()
}
...
pub fn detect_platform() -> Platform {
    ...
    let (hv_maximum, signal_bytes) = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe {
        let a: u32;
        let b: u32;
        let c: u32;
...
pub fn init() {
    ...
    *PLATFORM.lock() = platform;
    ...
    let default_dns = // Correspondance de motifs — branchement exhaustif de Rust.
match platform {
...
pub fn send_packet(data: &[u8]) -> Result<(), &'static str> {
    ...
    let mut transmit = TRANSMIT_QUEUE.lock();
    transmit.push_back(data.to_vec());
