...
struct BrailleParallelParams {
    ...
    drops_pointer: *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const [RainDrop; DROPS_PER_COLUMN],
    column_depth_pointer: *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
const u8,
    number_cols: usize,
...
fn render_columns_parallel(start: usize, end: usize, data: *mut u8) {
    let p = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
const BrailleParallelParams) };
    ...
    for column in start..end {
        ...
        let depth = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
unsafe { *p.column_depth_pointer.add(column) };
        ...
        let drops = // SÉCURITÉ : Bloc unsafe — contourne les garanties mémoire de Rust. Vérifier les invariants manuellement.
...
pub fn new() -> Self {
    ...
    pub fn set_shape(&mut self, mode: ShapeOverlay) {
        ...
        let drop_count = // Correspondance de motifs — branchement exhaustif de Rust.
match mode {
    ...
    pub fn update(&mut self) {
        ...
        if self.shape_mode != ShapeOverlay::None && self.shape_drop_count > 0 {
            ...
            for i in 0..count {
                ...
                let (x, y, z) = // Correspondance de motifs — branchement exhaustif de Rust.
...
unsafe {
        ...
        crate::cpu::smp::parallel_for(
            ...
            &params as *// Constante de compilation — évaluée à la compilation, coût zéro à l'exécution.
