...
pub const CG_: u8 = 0x80;
pub const EC_: u8 = 0x40;
pub const IS_: u8 = 0x20;
pub const AZ_: u8 = 0x10;
...
impl Cpu {
    ...
    fn zf(&self) -> bool { self.f & CG_ != 0 }
    fn nf(&self) -> bool { self.f & EC_ != 0 }
    fn hf(&self) -> bool { self.f & IS_ != 0 }
    fn cf(&self) -> bool { self.f & AZ_ != 0 }
    fn set_flags(&mut self, z: bool, ae: bool, h: bool, c: bool) {
        self.f = (if z { CG_ } else { 0 })
               | (if ae { EC_ } else { 0 })
               | (if h { IS_ } else { 0 })
               | (if c { AZ_ } else { 0 });
    }
    ...
    fn fetch8(&mut self, bus: &mut impl Eg) -> u8 {
        let v = bus.read(self.pc); self.pc = self.pc.wrapping_add(1); v
    }
    fn fetch16(&mut self, bus: &mut impl Eg) -> u16 {
    ...
    fn get_reg(&self, r: u8, bus: &mut impl Eg) -> u8 {
        match r & 7 {
    ...
    fn set_reg(&mut self, r: u8, v: u8, bus: &mut impl Eg) {
        match r & 7 {
    ...
    fn cb_rl(&mut self, v: u8) -> u8 { let bnx = if self.cf() { 1u8 } else { 0 }; let c = v >> 7; let r = (v << 1) | bnx; self.set_flags(r == 0, false, false, c != 0); r }
    fn cb_rr(&mut self, v: u8) -> u8 { let bnx = if self.cf() { 1u8 } else { 0 }; let c = v & 1; let r = (v >> 1) | (bnx << 7); self.set_flags(r == 0, false, false, c != 0); r }
    ...
    fn inc8(&mut self, v: u8) -> u8 {
        ...
        self.f = (if r == 0 { CG_ } else { 0 }) | (if (v & 0xF) + 1 > 0xF { IS_ } else { 0 }) | (self.f & AZ_);
        r
    }
    fn dec8(&mut self, v: u8) -> u8 {
        let r = v.wrapping_sub(1);
        self.f = (if r == 0 { CG_ } else { 0 }) | EC_ | (if v & 0xF == 0 { IS_ } else { 0 }) | (self.f & AZ_);
        r
    }
    fn add_hl(&mut self, v: u16) {
        let hl = self.hl(); let r = hl.wrapping_add(v);
        self.f = (self.f & CG_) | (if (hl & 0xFFF) + (v & 0xFFF) > 0xFFF { IS_ } else { 0 }) | (if hl as u32 + v as u32 > 0xFFFF { AZ_ } else { 0 });
        self.set_hl(r);
    }
    fn daa(&mut self) {
        ...
        self.a = a as u8;
        self.f = (if self.a == 0 { CG_ } else { 0 }) | (self.f & EC_) | (if c { AZ_ } else { 0 });
    }

    
    
    
    pub fn step(&mut self, bus: &mut impl Eg) -> u32 {
        
        if self.ime_next { self.ime = true; self.ime_next = false; }

        
        if self.ime || self.halted {
            let drt = bus.read(0xFFFF);
            let iflag = bus.read(0xFF0F);
            let pending = drt & iflag & 0x1F;
            if pending != 0 {
                self.halted = false;
                if self.ime {
                    self.ime = false;
                    for bf in 0u16..5 {
                        if pending & (1 << bf) != 0 {
                            bus.write(0xFF0F, iflag & !(1 << bf as u8));
    ...
    fn execute(&mut self, op: u8, bus: &mut impl Eg) -> u32 {
        match op {
            ...
            0x17 => { let bnx = if self.cf() { 1u8 } else { 0 }; let c = self.a >> 7; self.a = (self.a << 1) | bnx; self.set_flags(false, false, false, c != 0); 1 }
            ...
            0x1F => { let bnx = if self.cf() { 1u8 } else { 0 }; let c = self.a & 1; self.a = (self.a >> 1) | (bnx << 7); self.set_flags(false, false, false, c != 0); 1 }
            ...
            0x2F => { self.a = !self.a; self.f = (self.f & (CG_ | AZ_)) | EC_ | IS_; 1 }
            ...
            0x3F => { let c = !self.cf(); self.f = (self.f & CG_) | if c { AZ_ } else { 0 }; 1 }
            ...
            0xE0 => { let ae = self.fetch8(bus); bus.write(0xFF00 | ae as u16, self.a); 3 }
            ...
            0xF0 => { let ae = self.fetch8(bus); self.a = bus.read(0xFF00 | ae as u16); 3 }
    ...
    fn execute_cb(&mut self, bus: &mut impl Eg) -> u32 {
        ...
        match cb {
            0x00..=0x07 => { let arz = self.cb_rlc(v); self.set_reg(r, arz, bus); }
            0x08..=0x0F => { let arz = self.cb_rrc(v); self.set_reg(r, arz, bus); }
            0x10..=0x17 => { let arz = self.cb_rl(v); self.set_reg(r, arz, bus); }
            0x18..=0x1F => { let arz = self.cb_rr(v); self.set_reg(r, arz, bus); }
            0x20..=0x27 => { let arz = self.cb_sla(v); self.set_reg(r, arz, bus); }
            0x28..=0x2F => { let arz = self.cb_sra(v); self.set_reg(r, arz, bus); }
            0x30..=0x37 => { let arz = self.cb_swap(v); self.set_reg(r, arz, bus); }
            0x38..=0x3F => { let arz = self.cb_srl(v); self.set_reg(r, arz, bus); }
            0x40..=0x7F => {
                let bf = (cb >> 3) & 7;
                let z = (v >> bf) & 1 == 0;
                self.f = (if z { CG_ } else { 0 }) | IS_ | (self.f & AZ_);
                return if clm { 3 } else { 2 };
            }
            0x80..=0xBF => {
                let bf = (cb >> 3) & 7;
                self.set_reg(r, v & !(1 << bf), bus);
            }
            0xC0..=0xFF => {
                let bf = (cb >> 3) & 7;
                self.set_reg(r, v | (1 << bf), bus);
